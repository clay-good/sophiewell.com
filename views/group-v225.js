...
export const renderers = {
  'nugent-score'(root) {
    note(root, 'Nugent score (Nugent 1991): Gram-stain morphotype scoring (0-10). 0-3 normal, 4-6 intermediate, 7-10 bacterial vaginosis.');
  ...
  'amsel-criteria'(root) {
    note(root, 'Amsel criteria (Amsel 1983): thin gray-white discharge, pH > 4.5, positive whiff test, clue cells > 20%. >= 3 of 4 = bacterial vaginosis.');
  ...
  'ferriman-gallwey'(root) {
    note(root, 'Modified Ferriman-Gallwey (1961/1981): terminal-hair grades 0-4 over 9 areas (0-36). >= 8 indicates hirsutism.');
  ...
  'pbac-hmb'(root) {
    note(root, 'PBAC (Higham 1990): weighted tally of pads, tampons, and clots over the cycle. > 100 corresponds to > 80 mL (heavy menstrual bleeding).');
  ...
  'thompson-hie'(root) {
    note(root, 'Thompson score (Thompson 1997): 9 signs of neonatal HIE summed. 0-10 mild, 11-14 moderate, >= 15 severe.');
  ...
  'menopause-rating-scale'(root) {
    note(root, 'Menopause Rating Scale (Heinemann 2004): 11 items each 0-4 (0-44). 0-4 none/little, 5-8 mild, 9-16 moderate, >= 17 severe.');
  ...
  'kupperman-index'(root) {
    note(root, 'Blatt-Kupperman index (Kupperman 1953): 11 weighted symptoms rated 0-3 (to 51). Band cutoffs vary by variant.');
