...
export function aomCriteria(input = {}) {
  ...
  // spec-v1194: the bulging was defaulted to 'none', the zero row of its own
  // picklist, so a drum nobody had looked at read as a drum looked at and found
  // flat -- and the tile answered "Criteria not met" for acute otitis media.
  // Bulging is the finding the diagnosis turns on: two of the three routes are
  // written around it.
  const bulgingStated = typeof o.bulging === 'string' && o.bulging.trim() !== '';
  const bulging = bulgingStated ? o.bulging : 'none';
  ...
  // Rule 13: MET is monotone in the bulging -- a diagnosis made on drainage, or on
  // a drum already recorded as bulging, holds whatever else is found. NOT met is
  // the reading that needs the drum to have been looked at. Effusion is the other
  // exception: without objective fluid the guideline forbids the diagnosis
  // whatever the drum looks like, so that answer stands on its own.
  const bulgingOpen = !diagnosed && !bulgingStated && effusion;

  let notMetReason = null;
  if (bulgingOpen) {
    notMetReason = 'The drum was not graded. Bulging is what two of the three criteria turn on -- moderate to severe bulging is diagnostic on its own, and mild bulging is diagnostic with recent pain or intense redness -- so this is not yet a "criteria not met".';
  } else if (!diagnosed) {
    if (!effusion) {
  ...
  const state = diagnosed
    ...
    : bulgingOpen
      ? 'the drum was not graded, and bulging is what two of the three criteria turn on'
  ...
  return {
    ...
    scopeNote,
    abnormal: diagnosed,
    bandLabel: diagnosed ? (severe ? 'Criteria met, severe' : 'Criteria met') : bulgingOpen ? 'Not yet assessable' : 'Criteria not met',
