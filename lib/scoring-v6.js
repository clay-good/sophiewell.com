...
// spec-v930: `?? 0` fires for null and undefined and NOT for an empty string, so a points
// picker nobody touched arrived as '' and threw, while the same picker left off the call
// entirely scored 0 and produced a band. A blank picker and an absent one are both "nothing
// selected", which on a points scale is zero.
function pts(v) {
  return v === null || v === undefined || (typeof v !== 'number' && String(v).trim() === '') ? 0 : v;
}
...
// spec-v1401 Part B: CMQCC's Terms of Use (cmqcc.org, read 2026-09-19) bar reproducing, modifying, or
// creating "derivative works based on" its content except for a user's own internal use, so this tile
// does not reproduce the CMQCC hemorrhage stages, and its note says so for a California reader.
export function qblPph(input = {}) {
  // spec-v930: default parameters fire only when the argument is ABSENT. A blank volume arrived
  // as '' and threw; the same field left off entirely defaulted to 0. Nothing weighed is zero.
  const measuredMl = pts(input.measuredMl);
  const padGrams = pts(input.padGrams);
  const dryTareGrams = pts(input.dryTareGrams);
  const vaginal = input.vaginal === undefined ? true : input.vaginal;
  const unstable = input.unstable === undefined ? false : input.unstable;
  const riskFactors = pts(input.riskFactors);
  ...
  return {
    ...
    tier,
    note: '1 g = 1 mL weighed-pad convention; dry-pad/irrigation tare subtracted. Decision support, not a transfusion order. It does not reproduce the CMQCC hemorrhage stages, because CMQCC\'s terms of use bar derivative works of its toolkit, so the stages on a California hospital\'s poster may read differently.',
  };
}
...
export function burchWartofsky(p) {
  ...
  const temp = num('temp', pts(p.temp), { min: 0, max: 30 });
  const cns = num('cns', pts(p.cns), { min: 0, max: 30 });
  const gi = num('gi', pts(p.gi), { min: 0, max: 20 });
  const hr = num('hr', pts(p.hr), { min: 0, max: 25 });
  const chf = num('chf', pts(p.chf), { min: 0, max: 15 });
...
export function ariscat(p) {
  ...
  const age = num('agePts', pts(p.agePts), { min: 0, max: 16 });       // 0 / 3 / 16
  const spo2 = num('spo2Pts', pts(p.spo2Pts), { min: 0, max: 24 });    // 0 / 8 / 24
  const incision = num('incisionPts', pts(p.incisionPts), { min: 0, max: 24 }); // 0 / 15 / 24
  const duration = num('durationPts', pts(p.durationPts), { min: 0, max: 23 }); // 0 / 16 / 23
