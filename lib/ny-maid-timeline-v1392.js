...
//
// No expiry of an unfilled prescription appears in 2899-f, so none is printed. The Department's
// regulations were proposed, not final, when read.
//
...
export function nyMaidTimeline(input = {}) {
  ...
  if (o.dieSooner === 'yes') {
    return { ...base, abnormal: false, bandLabel: 'Fill once all requirements are affirmed', band: 'The attending has medically confirmed the patient may die within five days, so the prescription may be filled once the attending affirms every other requirement is met (2899-f(3)).' };
  }
