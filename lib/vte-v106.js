...
export function fourPeps({
  ...
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: PEPS_NOTE };
  const terms = [];
  let total = 0;
...
export function genevaOriginal({
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: GENEVA_NOTE };
  const terms = [];
  let total = 0;
  const add = (label, value) => { total += value; terms.push({ label, value }); };
  ...
  // spec-v1064: a heart rate above 100 adds a point and a blank one adds nothing,
  // so a total scored without it is a LOWER BOUND. That is safe while the score
  // has already left the low band, and unsafe at the boundary: a 4 scored without
  // the pulse is a 5 -- intermediate probability -- if the pulse is above 100, and
  // "low clinical probability" is the sentence a pulmonary embolism gets ruled out
  // on. So the low band is withheld until the rate is entered, and every other
  // band says what it was scored from.
  const hrMissing = hr == null;
  const band = total <= 4 ? { key: 'low', prev: 10 } : total <= 8 ? { key: 'intermediate', prev: 38 } : { key: 'high', prev: 81 };
  if (hrMissing && band.key === 'low') {
    return {
      valid: false,
      band: `Enter a heart rate. The score so far is ${total}, and a rate above 100 adds the point that would take it out of the low-probability band, so this cannot yet read as low clinical probability.`,
      note: GENEVA_NOTE,
    };
  }
  const partial = hrMissing ? ' Scored without a heart rate, which can only raise it.' : '';
  return {
    valid: true, total, probability: band.key, prevalence: band.prev, terms, hrMissing,
    band: `Original Geneva score ${total}: ${band.key} clinical probability (~${band.prev}% PE prevalence).${partial}`,
