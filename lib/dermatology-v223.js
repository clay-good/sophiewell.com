...
const GLASGOW7_NOTE = 'Weighted Glasgow 7-point checklist (MacKie RM, BMJ 1990;301(6759):1005-1006; NICE NG12 weighting): major features (change in size, irregular shape/border, irregular colour) 2 points each; minor features (largest diameter >= 7 mm, inflammation, oozing/crusting, change in sensation) 1 point each. A score >= 3 prompts referral for a suspicious pigmented lesion. A screening checklist, not a diagnosis.';
export function glasgow7(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  let s = 0; const p = [];
  const add = (c, pts, l) => { if (bool(o[c])) { s += pts; p.push(`${l} (+${pts})`); } };
  add('size', 2, 'change in size'); add('shape', 2, 'irregular shape/border'); add('color', 2, 'irregular colour');
