...
// spec-v1143: on the time basis both times defaulted to 0 and only the TOTAL had
// to be positive, so one blank produced "the NPP performed 20 of 20 total minutes
// (more than half)" -- a statement that the physician spent none, about a
// physician nobody had timed (rule 11). Both are required on the time basis; a
// typed 0 is still an answer (rule 1).
export function splitShared({ basis, physicianTime, nppTime, mdmBy = '' }) {
  ...
  if (b === 'time') {
    const missing = ['physicianTime', 'nppTime'].filter((k) => {
      const v = k === 'physicianTime' ? physicianTime : nppTime;
      return v === null || v === undefined || v === '';
    });
    if (missing.length) {
      throw new RangeError(`enter both the physician and the NPP minutes -- the rule turns on which is more than half of the total, and a blank one is not a time of zero (missing: ${missing.join(', ')})`);
    }
...
export function therapyUnits({ totalMinutes, rule, perServiceMinutes }) {
  ...
  if (!Array.isArray(perServiceMinutes) || perServiceMinutes.length === 0) {
    throw new TypeError('Rule of Eights needs per service minutes: a non-empty array of per-service minute totals');
  }
  let total = 0;
  let units = 0;
  const services = [];
...
// spec-v1143: `medicalDirection = 'aa'` defaulted to the single highest-paying
// row of the table -- personally performed, 100% -- so a call that omitted the
// modifier was priced at twice what a medically-directed case pays, and the
// reading then ASSERTED it: "100% for AA". The concurrency modifier is on every
// anesthesia claim line; the tile's own notice already said "Enter the ASA base
// units, the time, the CF, AND the medical-direction modifier", which is rule 23
// -- the message lists it, and the tool answered without it. Required now.
export function anesthesiaUnits({ baseUnits, timeMinutes, modifyingUnits = 0, conversionFactor, medicalDirection }) {
  if (medicalDirection === null || medicalDirection === undefined || medicalDirection === '') {
    throw new TypeError(`choose the medical-direction modifier (${MEDICAL_DIRECTION_KEYS.join(', ')}): it sets the concurrency percentage, and AA and QK differ by half the payment`);
  }
