...
export function mswat(input = {}) {
  ...
  for (const category of MSWAT_CATEGORIES) {
    ...
    // category examined and found absent does, and mSWAT is a weighted sum of
    // non-negative terms -- so a partial assessment can only under-state it.
    if (!fieldEntered(o[category.key])) missingCategories.push(categoryLabel(category, erythrodermic).toLowerCase());
    rows.push({
      key: category.key,
  ...
  return {
    ...
    total: rounded,
    max: MSWAT_MAX,
    footing: regionFooting(MSWAT_CATEGORIES.length - missingCategories.length, MSWAT_CATEGORIES.length, missingCategories, { unit: 'lesion categories', singular: 'lesion category', zeroReads: 'exactly as a category examined and found absent does' }),
    erythrodermic,
    totalBsa,
    categories: rows,
