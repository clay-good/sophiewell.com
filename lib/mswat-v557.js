...
const EXCLUSIVE_TEXT = `The three categories are mutually exclusive per unit of skin: each square centimetre is counted ONCE, so the three percentages together cannot exceed ${MAX_TOTAL_BSA}.`;
...
export function mswat(input = {}) {
  ...
  if (totalBsa > MAX_TOTAL_BSA + 1e-9) {
    return { valid: false, message: `The three areas total ${totalBsa} percent of body surface area, which exceeds ${MAX_TOTAL_BSA}. Each square centimetre of skin is counted once, in one category only, so these are not independent measurements.` };
  }
