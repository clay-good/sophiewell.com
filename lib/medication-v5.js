...
export function digoxin({ crCl, ageYears = null, indication = 'hf', levelNgMl = null, hoursPostDose = null }) {
  ...
  if (levelNgMl != null) {
    ...
    // spec-v69: the subtherapeutic ("below") threshold must match the
    // indication's own lower target bound, not the HF bound for both. HF floor
    // 0.5; AF rate-control floor 0.8 — the 0.8-2.0 ng/mL range this function and
    // the renderer label (views/group-v8.js) already commit to. Hardcoding 0.5
    // for both made a rate-control level of 0.6-0.7 render "within 0.8-2.0",
    // flatly contradicting the target string printed one line above.
    const targetLow = indication === 'hf' ? 0.5 : 0.8;
    ...
    else if (levelNgMl < targetLow) levelInterp = `Level ${r2(levelNgMl)} ng/mL: below ${target}.`;
