...
const OTHER_QUESTIONS_NOTE = 'The filtration rate is not the only question about gadolinium. Retention, pregnancy and prior reactions to a contrast agent are separate matters, and this does not address them.';
const SCOPE_NOTE = 'This applies a published grouping to an agent and a filtration rate already known. It does not choose an agent, and it does not authorize a scan.';
const oneOf = (list, v, fallback) => (known(list, v) ? v : fallback);

export function gadoliniumNsf(input = {}) {
  const o = input && typeof input === 'object' ? input : {};

  // spec-v1193: the kidney function was read through `oneOf(RENAL_STATES,
  // o.renalState, 'normal')`, and the first row of that table is "stable kidney
  // function at an eGFR of 30 or above". So a renal state nobody had entered read
  // as a kidney measured and found normal, and the tile answered:
  //
  //   No heightened concern -- with stable kidney function at an eGFR of 30 or
  //   above, no group carries a heightened concern for nephrogenic systemic
  //   fibrosis.
  //
  // The renal state is the whole question here. Nephrogenic systemic fibrosis is
  // a risk OF impaired clearance, so defaulting it to normal does not soften the
  // answer, it deletes it -- and prints a filtration rate nobody measured as the
  // reason.
  if (!known(RENAL_STATES, o.renalState)) {
    return {
      valid: false,
      message: 'Choose the kidney function. A blank is not stable function at an eGFR of 30 or above -- read that way it clears every agent group, and impaired clearance is the entire reason this question is asked.',
      otherQuestionsNote: OTHER_QUESTIONS_NOTE,
      scopeNote: SCOPE_NOTE,
      note: GAD_NOTE,
    };
  }

  // The agent group keeps its fallback: "Not known which agent will be used" is a
  // real row of the table and the honest reading of a blank, and it is the
  // CAUTIOUS end rather than the reassuring one.
  const group = oneOf(AGENT_GROUPS, o.agentGroup, 'unknown');
  const renal = o.renalState;
  ...
  const otherQuestionsNote = OTHER_QUESTIONS_NOTE;
