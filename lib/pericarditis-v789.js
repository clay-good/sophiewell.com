...
export function pericarditis(input = {}) {
  ...
  // spec-v1465: a blank course is not "acute (new onset)". The course never changes the count, so
  // a blank one is simply not printed.
  const courseBlank = o.course === undefined || o.course === null || String(o.course).trim() === '';
  const course = courseBlank ? null : String(o.course).trim();
  if (!courseBlank && !Object.prototype.hasOwnProperty.call(COURSES, course)) {
  ...
  return {
    ...
    supporting: support,
    course,
    courseLabel: courseBlank ? 'not entered' : COURSES[course],
    diagnostic,
    abnormal: diagnostic,
    bandLabel: `Pericarditis criteria ${met.length} of 4`,
    band: diagnostic
      ? `Pericarditis criteria ${met.length} of 4 — criteria met${courseBlank ? '. No course was entered, so the episode is not classified as acute, incessant, recurrent or chronic' : `, ${COURSES[course]}`}.`
