...
export function feno(input = {}) {
  ...
  // spec-v1463: a blank age group is not the adult one. The cutpoints are age-specific, so a blank
  // is asked for when the two groups read the number differently, and disclosed when they agree.
  const groupGiven = AGE_GROUPS.some((i) => i.value === o.ageGroup);
  ...
  const read = (lo, hi) => (ppb < lo ? 'low' : ppb > hi ? 'high' : 'intermediate');
  if (!groupGiven && read(ADULT_LOW, ADULT_HIGH) !== read(CHILD_LOW, CHILD_HIGH)) {
    return {
      valid: false,
      message: `Choose the age group: ${ppb} ppb reads ${read(ADULT_LOW, ADULT_HIGH)} at twelve and over but ${read(CHILD_LOW, CHILD_HIGH)} under twelve, so the cutpoints decide the reading.`,
    };
  }
  ...
  }[band] + (groupGiven ? '' : ' No age group was entered; both age groups read this value the same.');
  ...
  const ageNote = !groupGiven ? `The cutpoints differ by age (${ADULT_LOW} and ${ADULT_HIGH} ppb at twelve and over, ${CHILD_LOW} and ${CHILD_HIGH} under twelve); at ${ppb} ppb both read ${band}.` : `Read against the ${group === 'child' ? `under-${CHILD_AGE_YEARS}` : `${CHILD_AGE_YEARS}-and-over`} cutpoints of ${low} and ${high} ppb. The cutpoints differ by age: at ${ppb} ppb the other age group would read ${(() => {
  ...
  return {
    valid: true,
    fenoPpb: ppb,
    ageGroup: groupGiven ? group : null,
    cutpoints: { low, high },
    action,
    notAsthmaNote,
    intermediateNote,
    ...
    // spec-v1068: `band` is assigned twice in this object and the second wins, so
    // the short key ('low' / 'high' / 'intermediate') never left the function.
    // The sentence is what the house convention puts in `band` -- `bandLabel`
    // above carries the short form -- so the behaviour is right and the dead
    // first assignment has been removed rather than the live second one.
    band: action,
