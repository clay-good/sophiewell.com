...
export function icansGrade(input = {}) {
  ...
  // spec-v1095: a blank ICE reads as iceG 0, which is the grade for an ICE of
  // TEN -- a normal cognitive exam. The band then asserted the measurement it
  // never had: "No ICANS (ICE 10 and no consciousness, seizure, motor, or
  // raised-ICP findings)". Printing a specific score nobody took is worse than
  // a silent zero, and it is printed for a patient under CAR-T neurotoxicity
  // monitoring, where the whole point of the ICE is to catch the change early.
  let iceG = 0;
  const raw = typeof o.ice === 'string' ? o.ice.trim() : o.ice;
  const iceEntered = raw !== '' && raw !== undefined && raw !== null;
  if (iceEntered) {
  ...
  let band;
  if (grade === 0 && !iceEntered) {
    // The other four domains are all negative, so the grade rests entirely on a
    // score that was never taken. An ICE of 0-2 is grade 3 on its own.
    band = 'ICANS cannot be excluded: the ICE score was not entered. '
      + 'No consciousness, seizure, motor, or raised-ICP finding was recorded, so the grade rests entirely on the ICE, '
      + 'and an ICE of 2 or less is grade 3 on its own. Enter the ICE score.';
  } else if (grade === 0) {
    ...
    if (!iceEntered && grade < 3) {
      // The grade stands -- it came from a domain that was assessed -- but the
      // ICE can only raise it, to a maximum of 3 from that domain alone.
      band += ' The ICE score was not entered, and it can only raise this grade: an ICE of 2 or less is grade 3.';
    }
  }
  // spec-v1462: a domain left out is not a domain examined. The grade from the entered domains
  // stands (each can only raise it), but the band must not describe findings nobody entered.
  const has = (table, v) => Object.prototype.hasOwnProperty.call(table, v);
  const unentered = [
    !has(LOC, o.loc) && 'level of consciousness',
    !has(SEIZURE, o.seizure) && 'seizure',
    !has(ICP, o.icp) && 'raised ICP',
  ].filter(Boolean);
  if (unentered.length && grade < 4 && iceEntered) {
    const which = unentered.join(', ').replace(/, ([^,]*)$/, ' or $1');
    const can = unentered.length === 1 ? 'it can only' : 'any of them can only';
    if (grade === 0) band = `No ICANS from what was entered (ICE 10). No ${which} finding was entered; ${can} raise the grade.`;
    else band += ` No ${which} finding was entered; ${can} raise this grade.`;
  }
  ...
  return {
    valid: true,
    grade,
    // spec-v1095: `grade: 0` on its own reads as "no ICANS" to anything that
    // takes the number and not the sentence, so the gap is a field too.
    incomplete: !iceEntered,
    severe: grade >= 3,
    abnormal: grade >= 3,
