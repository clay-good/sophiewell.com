...
// spec-v1403: a blank sex read as male, so a woman's 20 kg grip was "low strength" against the
// male cutoff of 27 instead of normal against 16. The page always sends a sex; an agent may not.
// The tool's own missing-input messages still come first, and then a blank sex is asked.
export function ewgsop2(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  if (o.sex === 'male' || o.sex === 'female') return ewgsop2For(o);
  const r = ewgsop2For({ ...o, sex: 'male' });
  if (r.valid === false) return r;
  return { valid: false, message: 'Choose the sex. The EWGSOP2 cutoffs differ by sex: grip below 27 kg in men and 16 kg in women.' };
}

function ewgsop2For(o) {
  const sex = o.sex;
