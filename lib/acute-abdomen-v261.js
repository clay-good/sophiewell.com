...
export function ripasa(input = {}) {
  ...
  let total = 0;
  const fired = [];
  // Demographics — each always contributes one of two weights. spec-v1477: a blank one is not the
  // higher weight; it is 0.5 or 1, so the score is a range until it is entered.
  const unentered = [];
  let spread = 0;
  const demo = (v, low, high, lowText, highText, name) => {
    if (v === low) { total += 0.5; fired.push(lowText); } else if (v === high) { total += 1; fired.push(highText); } else { total += 0.5; spread += 0.5; unentered.push(name); }
  };
  demo(o.gender, 'female', 'male', 'female (+0.5)', 'male (+1)', 'the gender');
  demo(o.ageBand, 'gt40', 'le40', 'age > 40 (+0.5)', 'age <= 40 (+1)', 'the age band');
  demo(o.duration, 'gt48', 'lt48', 'symptom duration > 48h (+0.5)', 'symptom duration < 48h (+1)', 'the symptom duration');
  // Boolean items.
  const boxes = [
  ...
  const which = unentered.join(', ').replace(/, ([^,]*)$/, ' and $1');
  if (unentered.length && ripasaBand(total) !== ripasaBand(total + spread)) {
    return { valid: false,
      message: `Choose ${which}: without ${unentered.length === 1 ? 'it' : 'them'} the score is between ${total} and ${total + spread}, which spans ${ripasaBand(total)} and ${ripasaBand(total + spread)}.`,
      note: RIPASA_NOTE };
  }
  const label = ripasaBand(total);
  const cutoffText = total >= 7.5 ? ' (>= 7.5 diagnostic cutoff)' : '';
  if (unentered.length) {
    const hi = total + spread;
    return { valid: true, score: null, scoreRange: [total, hi], abnormal: total >= 7.5, bandLabel: `RIPASA ${total} to ${hi}`,
      band: `RIPASA ${total} to ${hi} of 16 — ${label}${cutoffText}. No value was entered for ${which}; the score is in this band either way.`,
...
export function pulp(input = {}) {
  ...
  for (const [on, label, pts] of boxes) { if (on === true) { total += pts; fired.push(label); } }
  // spec-v1115: the seven items above are checkboxes, so an unticked one is a
  // real "no" (rule 4). The ASA class is not -- it is a graded select, and this
  // lookup fell through to 0, which is ASA 1: a healthy patient. The detail line
  // said it out loud, "no risk factors positive (ASA 1)", about a class nobody
  // had assigned. The score is a sum of non-negative points, so what is entered
  // is a floor; high risk (>= 8) rules in and is untouched (rule 13).
  const asaKey = Number(o.asa);
  const asaKnown = Object.prototype.hasOwnProperty.call(PULP_ASA, asaKey);
  const asaPts = asaKnown ? PULP_ASA[asaKey] : 0;
  if (asaPts > 0) { total += asaPts; fired.push(`ASA ${asaKey} (+${asaPts})`); }
  const high = total >= 8;
  const floored = !asaKnown && !high;
  const label = high ? 'high risk (> 25% 30-day mortality per source)' : 'low risk (< 25% 30-day mortality per source)';
  return { valid: true, score: total, abnormal: high, asaStated: asaKnown, floorOnly: floored,
    bandLabel: floored ? `PULP at least ${total}` : `PULP ${total}`,
    band: floored
      ? `PULP at least ${total} of 18 on what was entered. The ASA class is not stated and carries `
        + 'up to 7 points, so the low-risk reading cannot be given without it.'
      : `PULP ${total} of 18 — ${label}.`,
    detail: `Contributing: ${fired.length ? fired.join(', ') : 'no risk factors positive'}.`
      + `${asaKnown ? '' : ' The ASA class is not stated.'}`,
    note: PULP_NOTE };
}
...
export function emergencySurgeryScore(input = {}) {
  ...
  // spec-v1128: the boxes above are checkboxes and an unticked one is a real "no"
  // (rule 4). The transfer source and the WBC band are graded SELECTS, and each
  // fell through to its zero level -- not transferred, WBC normal. Both carry
  // points, so what has been entered is a FLOOR and the figure is understated.
  const stated = (v) => v !== undefined && v !== null && String(v).trim() !== '';
  const unstated = [
    !stated(o.transfer) && 'the transfer source (adds 1)',
    !stated(o.wbc) && 'the white-cell band (adds up to 2)',
  ].filter(Boolean);
  const label = essBand(total);
  return { valid: true, score: total, abnormal: total >= 11, unstated,
    floorOnly: unstated.length > 0,
    bandLabel: unstated.length ? `ESS at least ${total}` : `ESS ${total}`,
    band: unstated.length
      ? `ESS at least ${total} of 29 on what was entered: ${unstated.join(' and ')} `
        + `${unstated.length === 1 ? 'is' : 'are'} not stated, and each can only raise the score, `
        + 'so the mortality band cannot be read off this total yet.'
      : `ESS ${total} of 29 — ${label}.`,
