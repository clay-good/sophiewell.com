...
export function blood4hWindow(input = {}) {
  ...
  if (rate === null) {
    band = `No pump rate was entered, so this is the slowest that finishes in time: at least ${minRate} mL/h to run ${vol} mL in the ${hm(left)} left of the 4-hour window.`;
  } else {
    finishes = need <= left;
    if (finishes) {
  ...
  if (!started) {
    steps.push((elapsedIn === null ? 'No spiking time was entered, so this assumes the full 4 hours are left. ' : '')
      + 'The clock starts when the bag is entered (spiked). Start slowly and watch closely at first: the Circular calls for a slow initial rate and gives no number, so the first-minutes rate is your facility\'s policy.');
  }
