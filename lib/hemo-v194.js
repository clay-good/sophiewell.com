...
// spec-v1226: see the note in lib/nephrology-v226.js. `pos` and `inRange` below
// return `null` for a blank field, for a non-number AND for a value outside
// their own bounds, and every caller reads `null` as absent -- so a PA systolic
// pressure of 3000 mmHg was answered with *"Enter the PA systolic pressure
// (mmHg)"*. `gradeFault` skips a blank, so the `missing` list still names every
// empty field at once and only the out-of-range value is called out. The bounds
// and the labels are the callers' own.
...
export function papi(input = {}) {
  ...
  const fault = gradeFault([
    ['PA systolic pressure (mmHg)', o.pasp, null, 300],
    ['PA diastolic pressure (mmHg)', o.padp, null, 200],
    ['right atrial pressure (mmHg)', o.rap, null, 100],
  ]);
...
export function pressureGradients(input = {}) {
  ...
  const fault = gradeFault([
    ['mean PA pressure (mmHg)', o.mpap, null, 200],
    ['PA diastolic pressure (mmHg)', o.padp, null, 200],
    ['mean PCWP (mmHg)', o.pcwp, null, 100],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function teiIndex(input = {}) {
  ...
  const fault = gradeFault([
    ['isovolumic contraction time (ms)', o.ivct, 0, 2000],
    ['isovolumic relaxation time (ms)', o.ivrt, 0, 2000],
    ['ejection time (ms)', o.et, null, 2000],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function shuntFraction(input = {}) {
  ...
  const fault = gradeFault([
    ['hemoglobin (g/dL)', o.hb, null, 30],
    ['alveolar/end-capillary O₂ tension (mmHg)', o.pAO2, null, 800],
    ['arterial SaO₂ (%)', o.sao2, 1, 100],
    ['arterial PaO₂ (mmHg)', o.pao2, null, 800],
    ['mixed-venous SvO₂ (%)', o.svo2, 1, 100],
    ['mixed-venous PvO₂ (mmHg)', o.pvo2, null, 800],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
