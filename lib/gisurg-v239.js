...
export function bonaciniCds(input = {}) {
  ...
  const fault = gradeFault([
    ['Platelet count (10^3/uL)', o.platelet, 1, 1000],
    ['ALT / AST ratio', o.altAstRatio, 0.05, 20],
    ['INR', o.inr, 0.5, 15],
  ]);
...
export function guci(input = {}) {
  ...
  const plt = fin(o.platelet, 1, 1000);
  const fault = gradeFault([
    ['AST (U/L)', o.ast, 1, 5000],
    ['AST upper limit of normal (U/L)', o.astUln, 5, 100],
    ['INR', o.inr, 0.5, 15],
    ['Platelet count (10^9/L)', o.platelet, 1, 1000],
  ]);
  if (fault) return { valid: false, message: fault };
