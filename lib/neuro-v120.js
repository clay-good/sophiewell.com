...
export function hints(input = {}) {
  ...
  const impulseCentral = o.headImpulse === 'normal';
  const nystagmusCentral = o.nystagmus === 'changing';
  const skewCentral = o.skew === 'present';
  const hearingLoss = onFlag(o.hearingLoss);
  ...
  // One central feature is enough to call the pattern central, but a peripheral reading needs all
  // three steps reassuring: a step left blank is not a reassuring step.
  const unrated = [];
  if (o.headImpulse !== 'abnormal') unrated.push('the head-impulse test');
  if (o.nystagmus !== 'fixed') unrated.push('the nystagmus direction');
  if (o.skew !== 'absent') unrated.push('the test of skew');
  if (unrated.length) {
    return {
      valid: false,
      message: `Choose ${unrated.join(', ').replace(/, ([^,]*)$/, ' and $1')}: no central feature was entered, and a peripheral pattern needs all three steps reassuring.`,
    };
  }
  return {
