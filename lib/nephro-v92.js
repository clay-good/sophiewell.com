...
export function mehranCin({
  ...
  if (missing.length && total <= 5) {
    return {
      ...
      band: `Enter the rest of the Mehran score to band the risk. Missing: ${missing.map((k) => MEHRAN_MEASUREMENTS[k][0]).join(' and ')}. `
        + `The ${missing.length === 1 ? 'unentered value' : 'two unentered values'} can only add points, `
        + 'so a partial score cannot place this in the low-risk band.',
      note: MEHRAN_NOTE,
    };
  }
  const partial = missing.length
    ? ` Scored from ${2 - missing.length} of 2 measurements; the rest can only raise it.`
    : '';
  let bandKey;
