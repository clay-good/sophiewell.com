...
export function ckdStaging({ egfr, uacr, aCategory } = {}) {
  ...
  // spec-v1406: the eGFR envelope lib/bounds.js declares; an impossible value computed.
  const egfrFault = envelopeFault([['eGFR', e]]);
  if (egfrFault) return { ...egfrFault, note: 'KDIGO stages CKD on two axes: GFR category (G1-G5) and albuminuria category (A1-A3).' };
...
export function ktvUrr({ preBun, postBun, ufVolume, dialysisTime, postWeight } = {}) {
  ...
  // spec-v1406: the BUN envelope lib/bounds.js declares; an impossible value computed.
  const bunFault = envelopeFault([['bun', pre], ['bun', post]]);
  ...
  const weightFault = w == null ? null : boundsAdvisory('weightKg', w);
  if (weightFault) return { valid: false, band: weightFault, message: weightFault };
...
export function mehranCin({
  ...
  // spec-v1406: the eGFR envelope lib/bounds.js declares; an impossible value scored.
  const egfrFault = envelopeFault([['eGFR', e]]);
  if (egfrFault) return { ...egfrFault, note: MEHRAN_NOTE };
