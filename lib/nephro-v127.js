...
export function kfre(input = {}) {
  ...
  // spec-v1210: the linear predictor saturates, so an impossible input does not
  // produce an obviously broken number -- it produces a CLEAN one, in the
  // reassuring direction. An age of 9999 (or an eGFR of 9999) reported
  // "0% 2-year and 0% 5-year probability of treated kidney failure" where the
  // same patient at 60 reads 2.6% and 7.9%.
  //
  // Three of the four bounds are already in lib/bounds.js. The ACR ceiling is the
  // assay's: urine albumin-to-creatinine is reported to about 30,000 mg/g at the
  // top of the nephrotic range, so 100,000 is frankly impossible without naming
  // a clinical limit (spec-v1189).
  //
  // After the missing-value branch, never inside it (spec-v1207).
  const kfreFault = gradeFault([
    ['the age in years', age, BOUNDS.ageYears.min, BOUNDS.ageYears.max],
    ['the eGFR in mL/min/1.73m2', egfr, BOUNDS.eGFR.min, BOUNDS.eGFR.max],
    ['the urine albumin-to-creatinine ratio in mg/g', acrMgG, 0, 100000],
  ]);
  if (kfreFault) return { valid: false, message: kfreFault };
...
export function rifleAki(input = {}) {
  ...
  // impossible BASELINE makes a real current value look unremarkable and the
  // whole staging vanishes -- a baseline of 250 mg/dL turned "RIFLE class
  // Failure" into "no criteria met" and "AKIN stage 3" into "no AKI criteria
  // met". The reassuring direction, on the staging that drives nephrotoxin holds
  // and renal-replacement timing.
  //
  // The envelope is lib/bounds.js's. gradeFault skips a blank, so the arm stays
  // optional: a tile scored from urine output alone is untouched.
  const crFault = gradeFault([
    ['the baseline creatinine in mg/dL', baseline, BOUNDS.scr.min, BOUNDS.scr.max],
    ['the current creatinine in mg/dL', current, BOUNDS.scr.min, BOUNDS.scr.max],
  ]);
  if (crFault) return { valid: false, message: crFault };
  const crClass = rifleCrClass(baseline, current);
  // spec-v1209: the urine-output category is a 0-3 selection and `clampInt` read
  // anything above 3 as Failure -- the worst RIFLE/AKIN class -- without a word.
  // The class table's own range; nothing clinical decided here.
  const uoFault = gradeFault([['the urine-output category', o.uoClass, 0, 3]]);
  if (uoFault) return { valid: false, message: uoFault };
...
function akinCrStage(baseline, current, rrt) {
  if (rrt) return 3;
  // spec-v1063: null means "the pair is incomplete", not "no rise". Returning 0
  // here made a missing current creatinine read as a normal one.
...
export function akinAki(input = {}) {
  ...
  // spec-v1063: the creatinine criterion needs BOTH values, and the stage is the
  // worse of two arms. An arm nobody could assess must not be scored as a normal
  // one -- with a baseline entered and the current creatinine still blank, this
  // returned "AKIN: no AKI criteria met", which is the one thing an incomplete
  // score may never say (spec-v1006).
  const crAssessed = crStage !== null;
  if (!crAssessed && uoStage === 0) {
    return { valid: false, message: 'Enter both a baseline and a current creatinine, mark RRT, or select the urine-output category. A single creatinine cannot rule acute kidney injury in or out.' };
  }
  const crEff = crAssessed ? crStage : 0;
  const stage = Math.max(crEff, uoStage);
  let governing;
  if (rrt && crEff >= uoStage) governing = 'renal replacement therapy';
  else governing = crEff >= uoStage ? (crEff > 0 ? 'creatinine' : 'neither') : 'urine output';
  const partial = crAssessed ? '' : ' Scored from the urine-output criterion alone; the creatinine criterion needs both a baseline and a current value, and can only raise the stage.';
  return {
    valid: true, stage,
    abnormal: stage >= 1,
    crAssessed,
    band: (stage === 0 ? 'AKIN: no AKI criteria met.' : `AKIN stage ${stage} (governed by ${governing}).`) + partial,
    note: AKIN_NOTE,
  };
}
...
export function ufrDialysis(input = {}) {
  ...
  // DENOMINATOR drives it toward zero and the tool reports "at or below the
  // 13 mL/kg/hr threshold" -- reassuring, from a session of 9,999 hours or a
  // patient of 9,999 kg. The weight envelope is lib/bounds.js's; the session
  // length and the volume removed are bounded by what a dialysis session is
  // (a long run is 8 hours; 10 L in one session is already extreme).
  const ufrFault = gradeFault([
    ['the ultrafiltration volume in liters', volumeL, 0, 20],
    ['the session length in hours', hours, 0, 24],
    ['the post-dialysis weight in kg', weight, BOUNDS.weightKg.min, BOUNDS.weightKg.max],
  ]);
  if (ufrFault) return { valid: false, message: ufrFault };
