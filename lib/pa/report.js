...
const REMEDIATION_BY_PREFIX = {
  ...
  'R-PA-SURG-': 'Review the rule-specific surgical note and attach only the evidence required for that procedure and workflow.',
...
function buildJsonReportInner(bundle, findings, opts) {
  ...
  return {
    ...
    auditTrail: {
      ...
      disabledRules: findings
        .filter((f) => f.status === 'disabled')
        .map((f) => ({ ruleId: f.ruleId, reason: f.note })),
