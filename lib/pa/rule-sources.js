...
export const CORE_RULE_SOURCES = Object.freeze({
  ...
  'R-PA-016': ['nppes-npi'], //            provider NPI checksum-valid
  'R-PA-019': ['nppes-npi'], //            second NPI for separate servicing entity
...
// enrollment check. Every other R-PA-CMS-* rule is an NCD/LCD
// coverage-policy rule (cms-ncd-lcd).
const CMS_IOM_RULES = new Set([
  'R-PA-CMS-002', // Standard Written Order present + signed (IOM Pub 100-08 ch.5)
  'R-PA-CMS-003', // SWO required elements complete (IOM Pub 100-08 ch.5)
  'R-PA-CMS-004', // proof of delivery retained (IOM Pub 100-08 ch.4 §4.7.3.1)
  'R-PA-CMS-009', // supplier Medicare enrollment record (program integrity)
]);

const SPECIALTY_RULE_SOURCES = Object.freeze({
  'R-PA-SURG-001': ['cms-tja-lcd'],
  'R-PA-SURG-002': ['cms-tja-lcd'],
  'R-PA-SURG-003': ['asa-preanesthesia'],
  'R-PA-SURG-004': ['asa-physical-status'],
  'R-PA-SURG-005': ['cms-hospital-surgery'],
  'R-PA-BH-001': ['apa-dsm'],
  'R-PA-BH-002': [],
  'R-PA-BH-003': ['asam-criteria'],
  'R-PA-BH-004': ['joint-commission-suicide-screening'],
  'R-PA-BH-005': ['samhsa-mat-act'],
  'R-PA-GEN-001': ['nci-hereditary-cancer-testing'],
  'R-PA-GEN-002': ['nci-hereditary-cancer-testing'],
  'R-PA-GEN-003': ['nci-hereditary-cancer-testing'],
  'R-PA-GEN-004': [],
  'R-PA-GEN-005': ['nhgri-gina'],
});

const OVERLAY_RULE_SOURCE_OVERRIDES = Object.freeze({
  'R-PA-AETNA-008': [],
  'R-PA-AETNA-014': [],
  'R-PA-AETNA-016': [],
});
...
const OVERLAY_PREFIX_SOURCES = [
  ...
  ['R-PA-HCSC-', 'hcsc-precert'], //     commercial overlay (wave 52-12), §4.5.12
  ['R-PA-HIGHMARK-', 'highmark-precert'], // commercial overlay (wave 52-13), §4.5.13
  ['R-PA-FLBLUE-', 'floridablue-precert'], // commercial overlay (wave 52-14), §4.5.14
  ['R-PA-BCBSM-', 'bcbsm-precert'], //     commercial overlay (wave 52-15), §4.5.15
  ['R-PA-BSCA-', 'blueshieldca-precert'], // commercial overlay (wave 52-16), §4.5.16
  ['R-PA-IBX-', 'ibx-precert'], //         commercial overlay (wave 52-17), §4.5.17
  ['R-PA-CAREFIRST-', 'carefirst-precert'], // commercial overlay (wave 52-18), §4.5.18
  ['R-PA-BCBSNC-', 'bcbsnc-precert'], //   commercial overlay (wave 52-19), §4.5.19
  ['R-PA-HORIZON-', 'horizon-precert'], // commercial overlay (wave 52-20), §4.5.20
  ['R-PA-BCBST-', 'bcbst-precert'], //     commercial overlay (wave 52-21), §4.5.21
  ['R-PA-BCBSMA-', 'bcbsma-precert'], //   commercial overlay (wave 52-22), §4.5.22
  ['R-PA-BCBSAL-', 'bcbsal-precert'], //   commercial overlay (wave 52-23), §4.5.23
  ['R-PA-BCBSSC-', 'bcbssc-precert'], //   commercial overlay (wave 52-24), §4.5.24
  ['R-PA-ARKBCBS-', 'arkbcbs-precert'], // commercial overlay (wave 52-25), §4.5.25
  ['R-PA-BLUEKC-', 'bluekc-precert'], //   commercial overlay (wave 52-26), §4.5.26
  ['R-PA-BCBSMN-', 'bcbsmn-precert'], //   commercial overlay (wave 52-27), §4.5.27
  ['R-PA-BCBSLA-', 'bcbsla-precert'], //   commercial overlay (wave 52-28), §4.5.28
  ['R-PA-HMSA-', 'hmsa-precert'], //       commercial overlay (wave 52-29), §4.5.29
  ['R-PA-MCAL-', 'medi-cal-precert'], //   per-state Medicaid overlay (wave 52-30), §4.5.30
  ['R-PA-MCNY-', 'ny-medicaid-precert'], // per-state Medicaid overlay (wave 52-31), §4.5.31
  ['R-PA-MCTX-', 'tx-medicaid-precert'], // per-state Medicaid overlay (wave 52-32), §4.5.32
  ['R-PA-MCFL-', 'fl-medicaid-precert'], // per-state Medicaid overlay (wave 52-33), §4.5.33
  ['R-PA-MCOH-', 'oh-medicaid-precert'], // per-state Medicaid overlay (wave 52-34), §4.5.34
  ['R-PA-MCIL-', 'il-medicaid-precert'], // per-state Medicaid overlay (wave 52-35), §4.5.35
  ['R-PA-MCWA-', 'wa-medicaid-precert'], // per-state Medicaid overlay (wave 52-36), §4.5.36
  ['R-PA-MCGA-', 'ga-medicaid-precert'], // per-state Medicaid overlay (wave 52-37), §4.5.37
  ['R-PA-MCNC-', 'nc-medicaid-precert'], // per-state Medicaid overlay (wave 52-38), §4.5.38
  ['R-PA-MCPA-', 'pa-medicaid-precert'], // per-state Medicaid overlay (wave 52-40), §4.5.40
  ['R-PA-MCMI-', 'mi-medicaid-precert'], // per-state Medicaid overlay (wave 52-41), §4.5.41
  ['R-PA-MCNJ-', 'nj-medicaid-precert'], // per-state Medicaid overlay (wave 52-42), §4.5.42
  ['R-PA-MCAZ-', 'az-medicaid-precert'], // per-state Medicaid overlay (wave 52-43), §4.5.43
  ['R-PA-MCIN-', 'in-medicaid-precert'], // per-state Medicaid overlay (wave 52-44), §4.5.44
  ['R-PA-OPD-', 'cms-opd-pa-list'],      // CMS Hospital OPD Prior Authorization list (wave 52-45), §4.5.2.1
];
...
export function ruleSourceIds(id) {
  ...
  if (Object.prototype.hasOwnProperty.call(SPECIALTY_RULE_SOURCES, ruleId)) {
    return SPECIALTY_RULE_SOURCES[ruleId].slice();
  }
  if (Object.prototype.hasOwnProperty.call(OVERLAY_RULE_SOURCE_OVERRIDES, ruleId)) {
    return OVERLAY_RULE_SOURCE_OVERRIDES[ruleId].slice();
  }
