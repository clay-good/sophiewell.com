...
// spec-v1170: `Date.UTC(2026, 12, 45)` rolls over to 2027-02-14 rather than
// failing, so every caller below had to be trusted not to hand it an impossible
// day. It is checked here instead, against the one calendar rule in lib/num.js;
// an unreal date is `null`, which is what every caller already handles.
function utc(y, m, d) {
  return isRealYmd(y, m + 1, d) ? new Date(Date.UTC(y, m, d)) : null;
}
