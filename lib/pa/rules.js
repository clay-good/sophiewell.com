...
export const STARTER_RULES = [
  ...
  },
  {
    ...
    severity: 'flag',
    citation: 'spec-v52 §4.5.1 R-PA-005. Default retro-auth window is 90 days; payer overlays override. Dates more than 5 years in the past are excluded as likely DOB / lookback-context, not service dates (a heuristic that lands cleaner once the classifier in v52-1g+ tags dates by role).',
    check(bundle) {
      const today = todayUtc();
      const MAX_LOOKBACK_DAYS = 5 * 365;
      let oldest = null;
      for (const d of bundle.documents) {
        for (const raw of d.extract.dates) {
          ...
          if (age === null || age < 0 || age > MAX_LOOKBACK_DAYS) continue;
          if (oldest === null || dt.getTime() < oldest.getTime()) oldest = dt;
        }
      }
      if (oldest === null) return { pass: true, evidence: 'No plausible service date in the past 5 years; rule vacuously satisfied (R-PA-004 covers presence).' };
      const age = diffDays(today, oldest);
      if (age !== null && age > RETRO_AUTH_WINDOW_DAYS) {
        return { pass: false, note: 'Oldest plausible service date is ' + age + ' days in the past, beyond the default ' + RETRO_AUTH_WINDOW_DAYS + '-day retro window.' };
      }
      return { pass: true, evidence: 'Oldest plausible service date is ' + age + ' days in the past.' };
    },
  },
  {
    id: 'R-PA-006',
    ...
    severity: 'block',
    citation: 'spec-v52 §4.5.1 R-PA-006. Hard ceiling, not payer-specific.',
    check(bundle) {
      const today = todayUtc();
      let furthest = null;
      for (const d of bundle.documents) {
      ...
      if (furthest === null) return { pass: true, evidence: 'No parsable service date; cannot evaluate (handled by R-PA-004).' };
      const ahead = diffDays(furthest, today);
      if (ahead !== null && ahead > FUTURE_AUTH_WINDOW_DAYS) {
        return { pass: false, note: 'Furthest service date is ' + ahead + ' days in the future, beyond the ' + FUTURE_AUTH_WINDOW_DAYS + '-day ceiling.' };
      }
      return { pass: true, evidence: 'Furthest service date is ' + ahead + ' days in the future.' };
    },
  },
  {
    id: 'R-PA-015',
