...
//   - 'medicaid-ny'               (New York State Medicaid; the second per-state
//                                  Medicaid overlay, wave 52-31)
//   - 'medicaid-tx'               (Texas Medicaid / TMHP; the third per-state
//                                  Medicaid overlay, wave 52-32)
//   - 'medicaid-fl'               (Florida Medicaid / AHCA; the fourth per-state
//                                  Medicaid overlay, wave 52-33)
//   - 'medicaid-oh'               (Ohio Medicaid; the fifth per-state Medicaid
//                                  overlay, wave 52-34)
//   - 'medicaid-il'               (Illinois Medicaid / HFS; the sixth per-state
//                                  Medicaid overlay, wave 52-35)
//   - 'medicaid-wa'               (Washington Apple Health / Medicaid; the
//                                  seventh per-state Medicaid overlay, wave 52-36)
//   - 'medicaid-ga'               (Georgia Medicaid / GAMMIS; the eighth
//                                  per-state Medicaid overlay, wave 52-37)
//   - 'medicaid-nc'               (North Carolina Medicaid / NCTracks; the ninth
//                                  per-state Medicaid overlay, wave 52-38)
//   - 'medicaid-pa'               (Pennsylvania Medicaid / Medical Assistance /
//                                  PROMISe; the tenth per-state Medicaid overlay,
//                                  wave 52-40)
//   - 'medicaid-mi'               (Michigan Medicaid / Healthy Michigan Plan /
//                                  CHAMPS; the eleventh per-state Medicaid overlay,
//                                  wave 52-41)
//   - 'medicaid-nj'               (New Jersey Medicaid / NJ FamilyCare / NJMMIS;
//                                  the twelfth per-state Medicaid overlay,
//                                  wave 52-42)
//   - 'medicaid-az'               (Arizona Medicaid / AHCCCS; the thirteenth
//                                  per-state Medicaid overlay, wave 52-43)
//   - 'medicaid-in'               (Indiana Medicaid / Healthy Indiana Plan /
//                                  IHCP; the fourteenth per-state Medicaid
//                                  overlay, wave 52-44)
...
const RULES = [
  ...
  {
    // New York State Medicaid -- the second per-state Medicaid overlay (spec-v52
    // §4.5.31, wave 52-31). Detected before the generic 'medicaid' bucket. The
    // 'emedny' / 'epaces' anchors are New York's Medicaid system / portal; an
  ...
  {
    // Texas Medicaid -- the third per-state Medicaid overlay (spec-v52 §4.5.32,
    // wave 52-32). Detected before the generic 'medicaid' bucket. Texas Medicaid
    // PA and claims run through the Texas Medicaid & Healthcare Partnership
    // (TMHP); an explicit "Medicare Advantage" string still wins the MA bucket
    // earlier.
    bucket: 'medicaid-tx',
    anchors: [
      'texas medicaid',
  ...
  {
    // Florida Medicaid -- the fourth per-state Medicaid overlay (spec-v52
    // §4.5.33, wave 52-33). Detected before the generic 'medicaid' bucket.
    // Florida runs Medicaid through the Agency for Health Care Administration
    // (AHCA) and the Statewide Medicaid Managed Care (SMMC) program; FMMIS is the
    // fiscal-agent portal. Distinct from the 'florida-blue' commercial bucket
    // (that matches 'florida blue' / 'guidewell'); an explicit "Medicare
  ...
  {
    // Ohio Medicaid -- the fifth per-state Medicaid overlay (spec-v52 §4.5.34,
    // wave 52-34). Detected before the generic 'medicaid' bucket. Ohio routes PA
    // through the Ohio Department of Medicaid and its Provider Network Management
    // (PNM) module; an explicit "Medicare Advantage" string still wins the MA
    // bucket earlier.
    bucket: 'medicaid-oh',
    anchors: [
      'ohio medicaid',
  ...
  {
    // Illinois Medicaid -- the sixth per-state Medicaid overlay (spec-v52
    // §4.5.35, wave 52-35). Detected before the generic 'medicaid' bucket. Run
    // by the Illinois Department of Healthcare and Family Services (HFS), with
    // IMPACT / MEDI as the provider portals. Distinct from the 'hcsc' commercial
    // bucket (which matches 'blue cross [and] blue shield of illinois'); an
    // explicit "Medicare Advantage" string still wins the MA bucket earlier.
    bucket: 'medicaid-il',
    anchors: [
      'illinois medicaid',
  ...
  {
    // Washington Apple Health (Medicaid) -- the seventh per-state Medicaid
    // overlay (spec-v52 §4.5.36, wave 52-36). Detected before the generic
    // 'medicaid' bucket. Washington brands Medicaid as "Apple Health"; the Health
    // Care Authority (HCA) runs it and ProviderOne is the provider portal. An
    // explicit "Medicare Advantage" string still wins the MA bucket earlier.
    bucket: 'medicaid-wa',
    anchors: [
  ...
  {
    // Georgia Medicaid -- the eighth per-state Medicaid overlay (spec-v52
    // §4.5.37, wave 52-37). Detected before the generic 'medicaid' bucket. Run by
    // the Georgia Department of Community Health; GAMMIS is the provider portal.
  ...
  {
    // North Carolina Medicaid -- the ninth per-state Medicaid overlay (spec-v52
    // §4.5.38, wave 52-38). Detected before the generic 'medicaid' bucket. NC
    // Medicaid (NC DHHS) uses NCTracks as the provider portal. Distinct from the
    // 'bcbsnc' commercial bucket (Blue Cross NC); an explicit "Medicare
    // Advantage" string still wins the MA bucket earlier.
    bucket: 'medicaid-nc',
    anchors: [
  ...
  {
    // Pennsylvania Medicaid -- the tenth per-state Medicaid overlay (spec-v52
    // §4.5.40, wave 52-40). Detected before the generic 'medicaid' bucket. PA
    // brands its Medicaid program "Medical Assistance" (DHS); the fee-for-service
    // provider portal is PROMISe and the managed-care program is HealthChoices.
    // Distinct from the same-state commercial Blues licensees ('highmark',
    // 'ibx'), whose own brand anchors are matched later in the commercial range;
    // an explicit "Medicare Advantage" string still wins the MA bucket earlier.
    bucket: 'medicaid-pa',
    anchors: [
  ...
  {
    // Michigan Medicaid -- the eleventh per-state Medicaid overlay (spec-v52
    // §4.5.41, wave 52-41). Detected before the generic 'medicaid' bucket.
    // Michigan is administered by MDHHS; the provider portal is CHAMPS and the
    // Medicaid-expansion program is branded the Healthy Michigan Plan. Distinct
    // from the same-state commercial Blues licensee 'bcbsm' (Blue Cross Blue
    // Shield of Michigan), whose own brand anchors are matched later in the
    // commercial range; an explicit "Medicare Advantage" string still wins the
    // MA bucket earlier.
    bucket: 'medicaid-mi',
    anchors: [
      'michigan medicaid',
  ...
  {
    // New Jersey Medicaid -- the twelfth per-state Medicaid overlay (spec-v52
    // §4.5.42, wave 52-42). Detected before the generic 'medicaid' bucket. New
    // Jersey administers Medicaid + CHIP through DMAHS (NJ DHS) under the brand
    // NJ FamilyCare; the fiscal-agent provider portal is NJMMIS (the New Jersey
    // Medicaid Management Information System). Distinct from the same-state
    // commercial Blues licensee 'horizon' (Horizon Blue Cross Blue Shield of New
    // Jersey, §4.5.20), whose own brand anchors are matched later in the
    // commercial range; an explicit "Medicare Advantage" string still wins the
    // MA bucket earlier.
    bucket: 'medicaid-nj',
    anchors: [
  ...
  {
    // Arizona Medicaid -- the thirteenth per-state Medicaid overlay (spec-v52
    // §4.5.43, wave 52-43). Detected before the generic 'medicaid' bucket.
    // Arizona's single state Medicaid agency is AHCCCS (the Arizona Health Care
    // Cost Containment System, pronounced "access"); its managed-care program is
    // AHCCCS Complete Care and the provider portal is AHCCCS Online. The 'ahcccs'
    // brand is unambiguous, so it anchors directly. No commercial Blues licensee
    // for Arizona is modeled yet, so there is no same-state disambiguation to
    // make here; an explicit "Medicare Advantage" string still wins the MA
    // bucket earlier.
    bucket: 'medicaid-az',
    anchors: [
      'arizona medicaid',
  ...
  {
    // Indiana Medicaid -- the fourteenth per-state Medicaid overlay (spec-v52
    // §4.5.44, wave 52-44). Detected before the generic 'medicaid' bucket.
    // Indiana administers Medicaid through FSSA / OMPP under the umbrella
    // "Indiana Health Coverage Programs" (IHCP); its expansion program is the
    // Healthy Indiana Plan (HIP) and the claims system is CoreMMIS (providers use
    // the IHCP Provider Healthcare Portal). Distinct from the same-state Blues
    // licensee 'anthem' (Anthem Blue Cross Blue Shield, HQ'd in Indianapolis,
    // §4.5.9), whose own brand anchors are matched later in the commercial range;
    // an explicit "Medicare Advantage" string still wins the MA bucket earlier.
    //
    // Anchor note: the bare token "hip" is deliberately NOT an anchor (it would
    // false-match "hip replacement" / "hip pain" in an orthopedic packet), and
    // neither is "in medicaid" (it would match "enrolled in medicaid"). Only the
    // unambiguous program / brand / portal names anchor.
    bucket: 'medicaid-in',
    anchors: [
      'indiana medicaid',
