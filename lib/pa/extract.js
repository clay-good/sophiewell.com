...
// spec-v59 §3.3 U-1: a hard ceiling on how many matches an uncapped matchAll
// loop appends to the report. A multi-MB repeated-date/POS document would
// otherwise build a multi-million-element array that lands in the report. The
// cap is far above any real packet (a true PA packet has a handful of dates),
// so byte-stable golden fixtures are unaffected; only hostile input is bounded.
const EXTRACT_MATCH_CAP = 200;
...
export function extractPosCodes(text) {
  ...
  for (const m of String(text || '').matchAll(re)) {
    out.add(m[1]);
    if (out.size >= EXTRACT_MATCH_CAP) break;
  }
  return [...out];
}
...
export function extractDates(text) {
  ...
  for (const m of String(text || '').matchAll(re)) {
    out.push(m[1]);
    if (out.length >= EXTRACT_MATCH_CAP) break;
  }
  return out;
}
...
export function extractPatientName(text) {
  // "Patient: Jane Doe" / "Name: Jane Doe" / "Name: Doe, Jane Q" -- capture
  // letters, spaces, and a single "Last, First" comma up to end of line.
  // spec-v59 §3.2 R-5: the comma is included so a "Last, First" name is
  // captured whole (it previously stopped at the comma, capturing only the
  // surname and under-seeding the literal-PHI redaction set).
