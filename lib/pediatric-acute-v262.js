...
export function labScore(input = {}) {
  ...
  let total = 0;
  const fired = [];
  // spec-v1115: `|| LABSCORE_CRP.lt40` and `|| LABSCORE_PCT.lt05` are explicit
  // fallbacks to the lowest band of each biomarker -- and these two biomarkers
  // are the whole point of the score. An unmeasured CRP or procalcitonin was
  // read as a reassuring one, and the detail line said "all biomarkers in the
  // low band" about labs nobody had drawn, under "low risk of serious bacterial
  // infection" in a febrile child.
  //
  // The urine dipstick is a checkbox and an unticked one is a real "no"
  // (rule 4). The two biomarkers are graded selects. The score is a
  // non-negative sum, so high risk (>= 3) rules in and is untouched (rule 13).
  const unmeasured = [];
  const known = (map, v) => Object.prototype.hasOwnProperty.call(map, String(v));
  if (known(LABSCORE_CRP, o.crp)) {
    const crp = LABSCORE_CRP[String(o.crp)];
    total += crp[0]; if (crp[0] > 0) fired.push(crp[1]);
  } else unmeasured.push('a CRP');
  if (known(LABSCORE_PCT, o.pct)) {
    const pct = LABSCORE_PCT[String(o.pct)];
    total += pct[0]; if (pct[0] > 0) fired.push(pct[1]);
  } else unmeasured.push('a procalcitonin');
  if (o.urinePositive === true) { total += 1; fired.push('urine dipstick positive (+1)'); }
  const high = total >= 3;
  const floored = unmeasured.length > 0 && !high;
  return { valid: true, score: total, abnormal: high, unmeasured, floorOnly: floored,
    bandLabel: floored ? `Lab-score at least ${total}` : `Lab-score ${total}`,
    band: floored
      ? `Lab-score at least ${total} of 9 on what was measured. ${unmeasured.join(' and ').replace(/^a/, 'A')} `
        + `${unmeasured.length === 1 ? 'is' : 'are'} not entered, and each can only raise it, so `
        + 'the low-risk reading cannot be given yet.'
      : `Lab-score ${total} of 9 — ${high ? 'high risk of serious bacterial infection (>= 3)' : 'low risk of serious bacterial infection (< 3)'}.`,
    detail: `Contributing: ${fired.length ? fired.join(', ') : 'nothing in a raised band among those entered'}.`
      + `${unmeasured.length ? ` Not entered: ${unmeasured.join(', ')}.` : ''}`,
    note: LABSCORE_NOTE };
}
