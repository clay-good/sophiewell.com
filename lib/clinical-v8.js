...
export function norepinephrineEquivalent(input = {}) {
  // spec-v930: default parameters fire only when the argument is ABSENT, so a blank infusion
  // field arrived as '' and threw while the same field left off entirely defaulted to 0. A drug
  // nobody entered is a drug that is not running.
  const zero = (v) => (v === null || v === undefined || (typeof v !== 'number' && String(v).trim() === '') ? 0 : v);
  const norepinephrine = zero(input.norepinephrine);
  const epinephrine = zero(input.epinephrine);
  const dopamine = zero(input.dopamine);
  const phenylephrine = zero(input.phenylephrine);
  const vasopressin = zero(input.vasopressin);
  const angiotensin2 = zero(input.angiotensin2);
...
export function anticoagReversalDose({ weightKg, inr, agent }) {
  ...
  // spec-v1155: the INR is read ONLY on the warfarin path, where it chooses the
  // 4F-PCC band (25, 35 or 50 units/kg). It was required of every agent, and a
  // blank one reaching the warfarin path as 0 produced "INR <2: 4F-PCC dosing not
  // defined by the label band" about a coagulation study nobody had run. Required
  // where it decides and optional where it is not read (rule 25).
  const inrStated = inr !== null && inr !== undefined && inr !== '' && Number.isFinite(Number(inr));
  if (a === 'warfarin' && !inrStated) {
    throw new RangeError('enter the INR: the 4F-PCC dose band is chosen by it (25, 35 or 50 units/kg), and a blank one is not an INR of zero');
  }
  if (inrStated) num('inr', inr, { min: 0, max: 30 });
