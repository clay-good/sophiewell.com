...
// spec-v930: a blank field and an absent field mean the same thing. Number('') is 0, so without
// this an empty form reads as a form full of zeros -- koff-bladder-capacity answered "60 mL for
// age 0 years" and snappe-ii scored its worst band from an unfilled page.
const isBlank = (v) => v === null || v === undefined || (typeof v !== 'number' && String(v).trim() === '');
const fin = (v) => {
...
// Clamp an integer sub-score into [lo, hi]; a blank or non-finite value -> lo. spec-v930: the
// comment said "blank" and the code only handled non-finite, so an empty string reached the
// clamp as a 0 instead.
const band = (v, lo, hi) => {
  if (isBlank(v)) return lo;
  const n = typeof v === 'number' ? v : Number(v);
...
// spec-v1478: a blank baseline incidence is not 0.5 per 1,000. The intercept moves the risk enough
// to change the management band (a culture at 0.5, none at 0.3), so every listed incidence is run;
// the tool asks when they disagree and says which it used when they agree.
const eosManagement = (r) => (r.posteriorRisk >= 3 ? 3 : r.posteriorRisk >= 1 ? 2 : r.priorRisk >= 1 ? 1 : 0);
export function eosCalculator(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  if (EOS_INTERCEPT[o.incidence] !== undefined) return eosAt(o, EOS_INTERCEPT[o.incidence]);
  const runs = Object.keys(EOS_INTERCEPT).map((k) => eosAt(o, EOS_INTERCEPT[k]));
  if (runs.some((r) => !r.valid)) return runs.find((r) => !r.valid);
  if (new Set(runs.map(eosManagement)).size > 1) {
    return { valid: false, message: 'Choose the baseline EOS incidence for your population (0.3 to 0.6 per 1,000 live births): on these inputs it changes the management band.' };
  }
  const r = eosAt(o, EOS_INTERCEPT['0.5']);
  return { ...r, incidenceStated: false, band: `${r.band} No baseline incidence was entered; 0.5 per 1,000 was used, and every listed incidence (0.3 to 0.6) gives this same management.` };
}

function eosAt(o, intercept) {
  ...
  // spec-v1165: `gbsPos` and `gbsUnk` were both 0 for an ABSENT status, which is
  // the NEGATIVE coefficient -- the most favourable of the three, and a maternal
  // culture result nobody had reported. The Kaiser model carries its own category
  // for exactly this state, so an unstated status takes the UNKNOWN coefficient
  // rather than silently claiming a negative culture, and the reading says which
  // it used (the spec-v1133 model).
  const GBS_KNOWN = ['positive', 'negative', 'unknown'];
  const gbsStated = GBS_KNOWN.includes(o.gbs);
  const gbsPos = o.gbs === 'positive' ? 1 : 0;
  const gbsUnk = (o.gbs === 'unknown' || !gbsStated) ? 1 : 0;
  // The antibiotic categories have no "not reported" level -- the reference level
  // IS "none, or any antibiotic under 2 h before delivery" -- so an absent value
  // is named rather than re-mapped.
  const abxStated = ['none', 'tx1', 'tx2'].includes(o.abx);
  const bx = intercept
    + 0.8680 * tempF
    - 6.9325 * ga
    + 0.0877 * ga * ga
  ...
  return {
    ...
    band: eosBand(priorPer1000, postPer1000, postStr, priorStr)
      + (gbsStated ? '' : ' The maternal GBS status was not stated, so the model\'s UNKNOWN-status coefficient was used, not a negative culture.')
      + (abxStated ? '' : ' Intrapartum antibiotics were not stated, and the model has no category for that -- this reads as none given, which is its reference level.'),
    note: EOS_NOTE,
  };
}
...
export function snappeII(input = {}) {
  ...
  // spec-v1016: the same lower-bound rule as spec-v1006/v1007. Every SNAPPE-II
  // item adds points or leaves them alone, so a partial total is a floor -- safe
  // to call an infant sick, never safe to call one well. The tile says plainly
  // that "items left blank score their normal (0-point) band", which is a fair
  // design for a partly charted neonate, and it still meant an EMPTY form
  // answered "SNAPPE-II 0/162: lower illness severity". Nothing had been
  // measured. Below the high-severity threshold the reading now waits for at
  // least one of the seven measurements, and says how many it had.
  // Presence is read from what was ENTERED, not from the derived P/F ratio: a
  // PaO2 with an FiO2 of 0 has no ratio (the divide is guarded) but the blood
  // gas was still measured.
  const MEASURED = {
    'mean blood pressure': bp, 'temperature': temp,
    'PaO2': fin(o.pao2), 'FiO2': fin(o.fio2),
    'pH': ph, 'urine output': urine, 'birth weight': bw, '5-minute Apgar': apgar,
  };
  const present = Object.keys(MEASURED).filter((k) => MEASURED[k] != null);
  if (!present.length && score < 40) {
    return {
      valid: false, score: null, abnormal: false,
      band: 'Enter at least one SNAPPE-II measurement -- blood pressure, temperature, PaO2 with FiO2, pH, urine output, birth weight or the 5-minute Apgar. '
        + 'Every item can only add points, so an unmeasured infant cannot be scored as a well one.',
      note: SNAPPE_NOTE,
    };
  }
  // spec-v1234: the pH item bands at >= 7.20 / 7.10 / below, open at the top, so
  // a pH of 80 scored the 0-point band -- the reassuring end of an illness-
  // severity score. BOUNDS.pH is the one envelope this tile's nine variables map
  // to in their own unit; the blood pressure here is a MEAN and the temperature
  // a neonatal low, both of which the table's entries would misjudge.
  const snFault = envelopeFault([['pH', ph]]);
  if (snFault) return { ...snFault, score: null, abnormal: false, note: SNAPPE_NOTE };
  const partial = present.length < 8
    ? ` Scored from ${present.length} of 8 measurements; the rest can only raise it.`
    : '';
  return {
    valid: true, score,
    abnormal: score >= 40,
    scoredFrom: present.length,
    band: `SNAPPE-II ${score}/162: ${sev} illness severity -- higher totals carry a higher neonatal mortality risk.${partial}`,
    note: SNAPPE_NOTE,
  };
}
...
const KOFF_NOTE = 'Koff expected bladder capacity (Koff SA, Urology 1983): expected bladder capacity in milliliters = (age in years + 2) x 30. It is the standard estimate for children roughly 1 to 12 years; under about 1 year other weight-based formulas are used, and in older children the estimate plateaus near the adult capacity. It reports an expected capacity for reference; the clinical interpretation stays with the clinician.';
