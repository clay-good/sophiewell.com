...
function ratio(value, uln) {
  // spec-v1040: `Number('')` and `Number(null)` are both 0, and 0 is finite -- so
  // a lab that had not come back read as a lab of zero, its ratio came out at 0,
  // and the tile reported "One lab criterion is not met: the bilirubin threshold.
  // The rule is not reached." A criterion nobody measured is not a criterion that
  // failed, and here the two were written in the same sentence.
  if (value === null || value === undefined || String(value).trim() === '') return null;
  if (uln === null || uln === undefined || String(uln).trim() === '') return null;
...
// spec-v1234: a bilirubin here is used as a RATIO to its own upper limit of
// normal, and a ratio hides the size of both numbers -- 600 mg/dL over a ULN of
// 1.2 is a 500x ratio, which is over every threshold in the rule in exactly the
// way 3x is. Envelope and wording from BOUNDS + boundsAdvisory, checked after
// the tile's own missing-value branch (spec-v1207). BOTH ends of the ratio are
// checked, because both are a total bilirubin in mg/dL.
...
export function hysLaw(input = {}) {
  ...
  const biliFault = envelopeFault([['bilirubin', o.bilirubin], ['bilirubin', o.bilirubinUln]]);
  if (biliFault) return biliFault;
