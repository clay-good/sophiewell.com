...
export function startBack(input = {}) {
  ...
  //
  // That single point only matters in one place, and the fix is scoped to it: a
  // total of exactly 3 is low risk, and 4 is not, so an unstated bother is the
  // difference between low and medium at that one total. Everywhere else the
  // point cannot cross a boundary, and refusing there would break the ordinary
  // case to catch a defect that is not present (rule 12).
  const botherStated = !(o.bother === undefined || o.bother === null || o.bother === '');
  const botherCouldCross = !botherStated && total === 3;
  ...
  return {
    ...
    band: botherCouldCross
      ? `STarT Back at least ${total} of 9, psychosocial subscore ${sub} of 5. Item 9, how `
        + 'bothersome the back pain has been, is not answered and scores 1 for "very much" or '
        + '"extremely" — which at this total is the difference between low and medium risk.'
