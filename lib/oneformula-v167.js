...
// spec-v1227: `pos` above returns `null` for a blank field, for a non-number AND
// for a value past its own `max`, and every caller reads `null` as absent -- so a
// reader who typed an impossible number was told to enter the number they had
// just typed, and retyping it produced the same sentence. `gradeFault`
// (lib/num.js, spec-v1209) SKIPS a blank, so each function's own `missing` list
// still names every empty field at once and only the out-of-range value is
// called out. The bounds and the labels are the caller's own; nothing clinical
// is decided here and no bound moves. See docs/spec-v1226.md for the shape.
...
export function meanAirwayPressure(input = {}) {
  ...
  const fault = gradeFault([
    ['peak inspiratory pressure PIP (cmH₂O)', o.pip, null, 120],
    ['inspiratory time Ti (s)', o.ti, null, 30],
    ['expiratory time Te (s)', o.te, null, 60],
  ]);
  ...
  // The I:E ratio display divides by Ti; a valid-but-denormalized Ti (e.g.
  // 1e-308) overflows te/ti to Infinity, which would leak into the band string
  // (the numeric-field guard only inspects number fields). Guard the display.
  const ieEst = te / ti;
  const ieDisplay = Number.isFinite(ieEst) ? r1(ieEst) : '—';
  return {
    valid: true,
    maw,
    ieRatio: ratio,
    bandLabel: 'Mean airway pressure',
    band: `Mean airway pressure ${maw} cmH₂O (PIP ${pip}, PEEP ${peep}, I:E ≈ 1:${ieDisplay}).`,
...
export function cerebroplacentalRatio(input = {}) {
  ...
  const fault = gradeFault([
    ['MCA pulsatility index', o.mca, null, 10],
    ['umbilical-artery pulsatility index', o.ua, null, 10],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function toeBrachialIndex(input = {}) {
  ...
  const fault = gradeFault([
    ['toe systolic pressure (mmHg)', o.toe, null, 400],
    ['brachial systolic pressure (mmHg)', o.brachial, null, 400],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
