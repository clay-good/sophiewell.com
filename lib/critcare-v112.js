...
export function sicScore(input = {}) {
  ...
  // spec-v1205: `lactateClearance` in this module has had the envelope guard
  // since spec-v1181 and these two never got it, which is what
  // `scripts/probe-unguarded-sibling.mjs` prints the module for.
  //
  // The platelet item is the one that matters, and it fails in the REASSURING
  // direction. This field is x10^9/L; a US report prints the same count as
  // 20,000/uL. Entered that way, a severe thrombocytopenia of 20 reads as a
  // normal count and scores 0 instead of 2 -- the SIC total falls below the
  // threshold and the coagulopathy the score exists to catch is scored away.
  // Same units confusion spec-v1180 fixed for `sokal-cml`.
  for (const [key, v] of [['platelets', plt], ['inr', inr]]) {
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, note: SIC_NOTE };
  }
  const pltPts = plt >= 150 ? 0 : plt >= 100 ? 1 : 2;
  const inrPts = inr <= 1.2 ? 0 : inr <= 1.4 ? 1 : 2;
  // spec-v1209: the SOFA total is defined on 0-24. The clamp that was here had
  // no ceiling, so a SOFA of 9999 scored the same 2 points a SOFA of 2 does and
  // a negative one scored 0 -- neither said anything.
  const sofaFault = gradeFault([['SOFA score', sofa, 0, 24]]);
  if (sofaFault) return { valid: false, band: sofaFault, note: SIC_NOTE };
  const sofaPts = sofa >= 2 ? 2 : sofa;
...
export function cpisVap(input = {}) {
  ...
  // spec-v1205: the leukocyte count here is per mm^3, not BOUNDS.wbc's x10^9/L.
  // spec-v1255 converts that same count envelope explicitly: 200 x10^9/L is
  // 200,000/mm^3. Applying the unconverted 200 ceiling would refuse legitimate
  // CPIS values, while leaving this field open silently accepts unit mistakes.
  const tempFault = boundsAdvisory('temperature', temp);
  if (tempFault) return { valid: false, band: tempFault, note: CPIS_NOTE };
  const wbcFault = gradeFault([['Leukocyte count (per mm^3)', wbc, 0, 200000]]);
  if (wbcFault) return { valid: false, band: wbcFault, note: CPIS_NOTE };
  ...
  // spec-v1119: the temperature and leukocyte count are required, and the four
  // remaining components are graded selects that fell through to 0 -- no
  // secretions, oxygenation not low, a clear radiograph, a negative culture. The
  // guard's own message says "then select the remaining CPIS components", and
  // did not require them, so an unfilled form answered "CPIS 6/12: VAP less
  // likely by the CPIS threshold" about a chest nobody had looked at.
  //
  // The score is a non-negative sum, so VAP-likely (> 6) rules in from a subset
  // (rule 13) and only the reassuring reading waits.
  const stated = (v) => v !== undefined && v !== null && v !== '';
  const unstated = [
    !stated(input.secretions) && 'the tracheal secretions',
    !stated(input.oxygenation) && 'the oxygenation',
    !stated(input.cxr) && 'the chest radiograph',
    !stated(input.culture) && 'the tracheal-aspirate culture',
  ].filter(Boolean);
  const vap = total > 6;
  const floored = unstated.length > 0 && !vap;
  return {
    valid: true, total, vap, unstated, floorOnly: floored,
    abnormal: vap,
    ...
    band: vap
      ...
      : (floored
        ? `CPIS at least ${total}/12 on what was entered: ${unstated.join(', ')} `
          + `${unstated.length === 1 ? 'is' : 'are'} not stated, and each can only add points, so `
          + 'the VAP-less-likely reading cannot be given yet.'
...
export function lactateClearance(input = {}) {
  ...
  // invented here.
  for (const v of [initial, repeat]) {
    const fault = boundsAdvisory('lactate', v);
    if (fault) return { valid: false, band: fault, note: LAC_NOTE };
  }
...
const MRC_LABELS = {
  shoulderL: 'left shoulder abduction', shoulderR: 'right shoulder abduction',
  elbowL: 'left elbow flexion', elbowR: 'right elbow flexion',
  wristL: 'left wrist extension', wristR: 'right wrist extension',
  hipL: 'left hip flexion', hipR: 'right hip flexion',
  kneeL: 'left knee extension', kneeR: 'right knee extension',
  ankleL: 'left ankle dorsiflexion', ankleR: 'right ankle dorsiflexion',
};
...
export function mrcSumScore(input = {}) {
  ...
  // spec-v1209: `clampInt(v, 0, 5)` used to sit in the loop above, and it turned
  // an impossible grade into a legitimate one SILENTLY -- twelve grades of 500
  // clamped to 5 each, scored 60 of 60, and read "ICU-acquired weakness is not
  // met". The MRC scale is defined only on 0-5, so this decides nothing clinical.
  //
  // After the missing-value branch, never inside it (spec-v1207): the 12-group
  // sentence above is better than any per-field one, which is why this is
  // `gradeFault` and not `inputFault`.
  // The label a reader sees, not the key a developer wrote: "shoulderL" is not a
  // muscle group anybody has heard of (spec-v1017 / spec-v1015, the same mistake
  // in `boundsAdvisory` and `lib/num.js`).
  const fault = gradeFault(MRC_GROUPS.map((k) => [MRC_LABELS[k], fin(input[k]), 0, 5]));
  if (fault) return { valid: false, band: fault, note: MRC_NOTE };
  for (const k of MRC_GROUPS) total += fin(input[k]);
