...
export function nafldFibrosis({ age, bmi, ifgDm, ast, alt, platelets, albumin } = {}) {
  ...
  // spec-v1406: the age and BMI envelopes lib/bounds.js declares; an impossible value computed.
  for (const [key, v] of [['ageYears', a], ['bmi', b]]) {
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, message: fault, note: 'Envelope from lib/bounds.js; the score was not computed.' };
  }
  // spec-v1174: platelets carry a negative coefficient here, so the x1000 unit
  // confusion (a US report reads 200,000/uL for what this field wants as 200)
  // drove the score to -2596.66 and "excludes advanced fibrosis". Ceiling from
  // BOUNDS.platelets in lib/bounds.js.
  // spec-v1178: albumin carries a negative coefficient here too, so the g/L
  // figure (ten times the g/dL one) drove the score down and read as "excludes
  // advanced fibrosis" -- the same shape as the platelet guard below.
  if (alb > BOUNDS.albumin.max) {
    return {
      valid: false,
      band: `A serum albumin of ${alb} g/dL is above ~${BOUNDS.albumin.max}, beyond recorded extremes. An albumin reported in g/L is ten times the g/dL figure, so ${alb} g/L is entered as ${alb / 10}.`,
      note: 'The NAFLD Fibrosis Score is a fixed linear equation in age, BMI, the AST/ALT ratio, platelets, albumin and a glucose-status indicator (Angulo 2007).',
    };
  }
...
export function glasgowImrie(inputs = {}) {
  ...
  // spec-v1232: each item is a threshold comparison, so past the threshold the
  // size of the number stops mattering -- a WBC of 2000 x10^9/L met "> 15" the
  // same way 16 does, and the tile read "predicts severe pancreatitis; 8 of 8
  // items assessed", an eight-of-eight completeness claim over an impossible lab.
  //
  // ONLY TWO OF THE EIGHT ARE CHECKED, and the reason is spec-v1205's rule: an
  // envelope is a claim about a quantity IN A UNIT. Imrie states calcium in
  // mmol/L, albumin in g/L and glucose in mmol/L; BOUNDS holds all three in
  // mg/dL and g/dL, so applying them here would refuse every legitimate value.
  // Urea and LDH have no envelope at all. WBC (x10^9/L is the same number as
  // x10^3/uL) and PaO2 (mmHg) are the two that match.
  // spec-v1406: age (years) matches BOUNDS.ageYears too; an age of 1300 met "> 55".
  for (const [key, k] of [['wbc', 'wbc'], ['paO2', 'pao2'], ['ageYears', 'age']]) {
    const v = fin(inputs[k]);
    if (v == null) continue;
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, message: fault, note: 'Envelope from lib/bounds.js; the item was not scored.' };
  }
...
export function trueloveWitts({ stools, bleeding, temp, heartRate, hemoglobin, esr } = {}) {
  ...
  // spec-v1120: spec-v1066 guarded the four systemic MEASUREMENTS and left the
  // rectal-bleeding select beside them alone -- the fifth time this programme
  // has found "one tile, two gaps, one guarded" (spec-v1101). Severe needs six
  // bloody stools AND a systemic criterion, so an unstated bleeding makes severe
  // unreachable just as surely as four unmeasured labs did, and the tile graded
  // MODERATE for it. The guard's own message already asks for it: "Enter the
  // number of stools per day AND WHETHER RECTAL BLEEDING IS PRESENT."
  const bleedingStated = bleeding === 'present' || bleeding === 'absent'
    || bleeding === 'yes' || bleeding === 'no' || bleeding === true || bleeding === false;
  const bloody = bleeding === 'present' || bleeding === 'yes' || bleeding === true;
  const t = fin(temp);
  const hr = fin(heartRate);
  const hgb = fin(hemoglobin);
  const e = fin(esr);
  // spec-v1232: the three systemic measurements with an envelope in their own
  // unit -- temperature in C, heart rate in bpm, haemoglobin in g/dL. A
  // temperature of 450 C met "> 37.8" and supplied the one systemic criterion
  // that turns six bloody stools into SEVERE. The ESR has no envelope.
  for (const [key, v] of [['temperature', t], ['hr', hr], ['hemoglobin', hgb]]) {
    if (v == null) continue;
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, message: fault, note: 'Envelope from lib/bounds.js; severity was not graded.' };
  }
  const systemic = [];
  ...
  // spec-v1066: a systemic criterion nobody measured is not a criterion that is
  // absent. One of the four is all that separates severe colitis from moderate,
  // so with six bloody stools entered and all four systemic values blank this
  // said "No systemic toxicity criterion met" and graded MODERATE -- a rule-out
  // built on four labs nobody had taken, ahead of an admission decision.
  const unmeasured = [];
  if (t == null) unmeasured.push('a temperature');
  if (hr == null) unmeasured.push('a heart rate');
  if (hgb == null) unmeasured.push('a hemoglobin');
  if (e == null) unmeasured.push('an ESR');
  const outstanding = [...unmeasured];
  if (!bleedingStated) outstanding.push('whether there is rectal bleeding');
  const unmeasuredText = outstanding.length
    ? ` Not entered: ${outstanding.join(', ')} -- severe needs six bloody stools and a systemic criterion, so this cannot yet rule severe colitis out.`
    : '';
  const sysText = systemic.length
    ? `Systemic criteria met: ${systemic.join(', ')}.`
    : (unmeasured.length === 4
      ? 'No systemic value has been entered.'
      : 'No systemic toxicity criterion met among those entered.');
  return {
    valid: true,
    bandKey,
    severe: bandKey === 'severe',
    systemic,
    unmeasuredSystemic: unmeasured,
    bleedingStated,
    outstanding,
    band: `Truelove & Witts: ${BAND[bandKey]} ${sysText}${bandKey === 'severe' ? '' : unmeasuredText}`,
...
export function harveyBradshaw({ wellbeing, pain, stools, mass, complications } = {}) {
  ...
  // spec-v1200: the stool count was the one subscore whose clamp was invisible.
  // Four of the five reported it and `st` was left out, so a count of 2,000,000
  // was silently capped at 1e6 and the reading carried no parenthetical at all --
  // while a wellbeing of 999 did. Guard the set, not the four that were written.
  const clamped = wb.clamped || pn.clamped || ms.clamped || cx.clamped || st.clamped;
  return {
    valid: true,
    total,
    bandKey,
