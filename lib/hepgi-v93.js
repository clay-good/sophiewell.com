...
export function trueloveWitts({ stools, bleeding, temp, heartRate, hemoglobin, esr } = {}) {
  ...
  const unmeasuredText = unmeasured.length
    ? ` Not entered: ${unmeasured.join(', ')} -- any one of these can meet a systemic criterion, so this cannot yet rule severe colitis out.`
    : '';
