...
export function indomethacinHeadacheIchd3(input = {}) {
  ...
  // spec-v1076: three of the four numbers here gate a criterion with
  // `x !== null && ...`, so leaving one blank made the criterion unmet and the
  // tile said "Neither indomethacin-responsive criteria set is met on these
  // entries" -- indistinguishable from a patient whose attacks genuinely run
  // too long or too few. Only 3.2 uses the attack count, duration and daily
  // frequency; only 3.4 uses the months, so each is outstanding just for the
  // set that asks for it.
  const notEnteredPh = [];
  if (attacks === null) notEnteredPh.push('the number of attacks');
  if (duration === null) notEnteredPh.push('the attack duration');
  if (perDay === null) notEnteredPh.push('the attacks per day');
  const notEnteredHc = months === null ? ['how many months the headache has been continuous'] : [];
  const notEntered = [...new Set([...notEnteredPh, ...notEnteredHc])];
  ...
  return {
    ...
    band: met
      ...
      : notEntered.length
        ? `Not assessed: enter ${notEntered.join(', ')} — until then neither indomethacin-responsive set can be ruled out.`
