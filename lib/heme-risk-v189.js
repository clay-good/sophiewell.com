...
export function impedeVte(input = {}) {
  ...
  // spec-v1127: both graded selects fell through to 'none', and this score is
  // NOT monotone -- rule 10, checked rather than assumed, and the check paid.
  // Dexamethasone adds (low +2, high +4); thromboprophylaxis SUBTRACTS (aspirin
  // -3, therapeutic anticoagulation -4). So an unstated dexamethasone leaves the
  // score too LOW and an unstated prophylaxis leaves it too HIGH, and a partial
  // form gives a RANGE rather than a bound in either direction -- the `ces-d`
  // shape from spec-v1108, and the second instance of it in this programme.
  const dexStated = Object.prototype.hasOwnProperty.call(DEX_PTS, String(o.dexamethasone));
  const dex = dexStated ? String(o.dexamethasone) : 'none';
  if (dex !== 'none') { score += DEX_PTS[dex]; parts.push(DEX_LABEL[dex]); }
  const prophStated = Object.prototype.hasOwnProperty.call(PROPH_PTS, String(o.thromboprophylaxis));
  const proph = prophStated ? String(o.thromboprophylaxis) : 'none';
  if (proph !== 'none') { score += PROPH_PTS[proph]; parts.push(PROPH_LABEL[proph]); }

  const unstated = [
    !dexStated && 'the dexamethasone dose (adds up to 4)',
    !prophStated && 'the thromboprophylaxis (subtracts up to 4)',
  ].filter(Boolean);
  const low = score + (prophStated ? 0 : -4);
  const high = score + (dexStated ? 0 : 4);
  const bandOf = (n) => (n <= 3 ? 'low' : n <= 7 ? 'intermediate' : 'high');
  const risk = bandOf(score);
  // The band stands only where the whole range sits inside it.
  const settled = !unstated.length || bandOf(low) === bandOf(high);
  return {
    valid: true,
    score,
    risk: settled ? risk : null,
    unstated,
    low,
    high,
    rangeOnly: !settled,
    abnormal: settled ? score >= 4 : low >= 4,
    bandLabel: settled ? `IMPEDE ${cap(risk)} risk` : `IMPEDE ${low} to ${high}`,
    band: settled
      ? `IMPEDE VTE ${score} — ${risk} VTE risk.`
      : `IMPEDE VTE between ${low} and ${high} on what was entered: ${unstated.join(' and ')} `
        + `${unstated.length === 1 ? 'is' : 'are'} not stated. One raises the score and the other `
        + 'lowers it, so this is a range rather than a floor, and it spans more than one risk band.',
