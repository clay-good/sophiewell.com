...
export function cfs({ level }) {
  const lv = Math.max(1, Math.min(9, Math.round(Number(level))));
  const descriptors = {
    1: 'Very fit',
    2: 'Well',
    3: 'Managing well',
...
export function rass({ level }) {
  const lv = Math.max(-5, Math.min(4, Math.round(Number(level))));
  const descriptors = {
...
export function hacor({ hr, ph, gcs, pao2, fio2, rr }) {
  ...
  const pf = fio > 0 ? (pao / fio) : 0;
  const parts = {
    hr: hacorHrPts(Number(hr) || 0),
    ph: hacorPhPts(Number(ph) || 7.4),
    gcs: hacorGcsPts(Number(gcs) || 15),
    pf: hacorPfRatioPts(pf),
    rr: hacorRrPts(Number(rr) || 0),
  };
...
export function lisMurray({ quadrants, pao2, fio2, peep, complianceMlPerCmH2O }) {
  const fio = Number(fio2);
  const pao = Number(pao2);
  const pf = fio > 0 ? (pao / fio) : 300;
  const parts = {
    quadrants: lisQuadrantPts(quadrants),
    pf: lisPfPts(pf),
    peep: lisPeepPts(Number(peep) || 0),
    compliance: lisCompliancePts(Number(complianceMlPerCmH2O) || 80),
  };
...
export function bps({ facial, upperLimb, ventilatorCompliance }) {
  const f = Math.max(1, Math.min(4, Math.round(Number(facial) || 1)));
  const u = Math.max(1, Math.min(4, Math.round(Number(upperLimb) || 1)));
  const v = Math.max(1, Math.min(4, Math.round(Number(ventilatorCompliance) || 1)));
  const score = f + u + v;
  return {
    score,
...
export function mgap({ mechanismBlunt, gcs, ageLt60, sbp }) {
  const g = Number(gcs);
  const s = Number(sbp);
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new Error('mgap: GCS must be 3-15');
  if (!Number.isFinite(s) || s < 0) throw new Error('mgap: SBP must be a non-negative number');
...
export function gap({ gcs, ageLt60, sbp }) {
  const g = Number(gcs);
  const s = Number(sbp);
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new Error('gap: GCS must be 3-15');
  if (!Number.isFinite(s) || s < 0) throw new Error('gap: SBP must be a non-negative number');
...
export function big({ baseDeficit, inr, gcs }) {
  ...
  const i = Number(inr);
  const g = Number(gcs);
  if (!Number.isFinite(bd)) throw new Error('big: base deficit must be a number');
  if (!Number.isFinite(i) || i < 0) throw new Error('big: INR must be a non-negative number');
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new Error('big: GCS must be 3-15');
...
export function braden({ sensory, moisture, activity, mobility, nutrition, friction }) {
  ...
  for (const [k, x] of Object.entries(v)) {
    if (!Number.isFinite(x)) throw new Error(`braden: ${k} must be a number`);
    const max = k === 'friction' ? 3 : 4;
    if (x < 1 || x > max) throw new Error(`braden: ${k} must be 1-${max}`);
  }
...
export function ichScore({ gcs, age, ichVolumeMl, infratentorial, ivh }) {
  ...
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new Error('ich-score: GCS must be 3-15');
  if (!Number.isFinite(a) || a < 0) throw new Error('ich-score: age must be a non-negative number');
  if (!Number.isFinite(v) || v < 0) throw new Error('ich-score: ICH volume must be a non-negative number');
  let gcsPts = 0;
  if (g >= 3 && g <= 4) gcsPts = 2;
...
export function huntHessWfns({ huntHess, gcs, focalMotorDeficit }) {
  ...
  const g = Number(gcs);
  if (!Number.isInteger(hh) || hh < 1 || hh > 5) throw new Error('hunt-hess-wfns: Hunt-Hess must be 1-5');
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new Error('hunt-hess-wfns: GCS must be 3-15');
  let wfns;
  if (g === 15) wfns = 1;
...
export function mnihss(answers) {
  ...
  for (const it of MNIHSS_ITEMS) {
    ...
    if (!Number.isFinite(v) || v < 0 || v > it.max) {
      throw new Error(`mnihss: ${it.id} must be 0-${it.max}`);
    }
    total += v;
  }
  let severity;
...
export function bloodCompat({ recipient, product }) {
  if (!ABO_RH.has(recipient)) throw new Error(`blood-compat: recipient must be one of ${[...ABO_RH].join(', ')}`);
  ...
  throw new Error('blood-compat: product must be one of prbc / ffp / platelets / cryo');
}
...
export function insulinCorrection({
  ...
  if (!Number.isFinite(bg) || bg < 0) throw new Error('insulin-correction: currentBG required (mg/dL)');
  if (!Number.isFinite(target) || target <= 0) throw new Error('insulin-correction: targetBG required (mg/dL)');
  ...
  if (!Number.isFinite(computedIsf) || computedIsf <= 0) {
    ...
    if (!Number.isFinite(tdd) || tdd <= 0) {
      throw new Error('insulin-correction: provide isf or totalDailyDose');
    }
  ...
  return {
    isf: round1(computedIsf),
    isfDerivedFromTdd: derivedFromTdd,
    correctionUnits: round1(correction),
    mealUnits: round1(meal),
    totalUnits: round1(total),
    text: `Correction ${round1(correction)} U + meal ${round1(meal)} U = ${round1(total)} U total (ISF ${round1(computedIsf)} per ADA 2024).`,
  };
}
...
export function electrolyteReplacement({ electrolyte, level, route, renalImpaired }) {
  ...
  if (!Number.isFinite(l) || l < 0) throw new Error('electrolyte-replacement: level required');
  const r = String(route || 'iv').toLowerCase();
  if (r !== 'iv' && r !== 'po') throw new Error('electrolyte-replacement: route must be iv or po');
  ...
  throw new Error('electrolyte-replacement: electrolyte must be one of k / mg / phos');
}
...
export function crrtDose({
  ...
  if (!Number.isFinite(w) || w <= 0) throw new Error('crrt-dose: weightKg required');
  if (!Number.isFinite(r) || r < 0) throw new Error('crrt-dose: effluentRateMlPerHr required');
  const round1 = (n) => Math.round(n * 10) / 10;
  const dose = r / w;
  const banners = [];
  ...
  if (Number.isFinite(sysCa) && sysCa > 0 && (sysCa < 1.1 || sysCa > 1.2)) {
    banners.push(`Systemic ionised Ca ${round1(sysCa)} mmol/L outside 1.1-1.2 mmol/L target per Davenport 2009: titrate Ca replacement.`);
  }
  const postCa = Number(postFilterIonisedCa);
  if (Number.isFinite(postCa) && postCa > 0 && (postCa < 0.25 || postCa > 0.35)) {
    banners.push(`Post-filter ionised Ca ${round1(postCa)} mmol/L outside 0.25-0.35 mmol/L target per Davenport 2009: titrate citrate flow.`);
  }
  const total = Number(totalCa);
  let ratio;
  if (Number.isFinite(total) && Number.isFinite(sysCa) && sysCa > 0) {
    ratio = total / sysCa;
    if (ratio >= 2.5) banners.push(`Total/ionised Ca ratio ${round1(ratio)} >= 2.5: citrate accumulation suspected per Davenport 2009.`);
  }
  const uf = Number(ultrafiltrationMlPerHr);
  return {
    effluentDoseMlPerKgPerHr: round1(dose),
    modality: modality || 'CVVH',
    ultrafiltrationMlPerHr: Number.isFinite(uf) ? uf : 0,
    totalIonisedRatio: ratio === undefined ? null : round1(ratio),
    banners,
    text: `Delivered effluent dose ${round1(dose)} mL/kg/h (KDIGO 2012 target 20-25 mL/kg/h).`,
  };
}
...
export function ecmoTitration({
  ...
  if (!Number.isFinite(w) || w <= 0) throw new Error('ecmo-titration: weightKg required');
  if (!Number.isFinite(sweep) || sweep < 0) throw new Error('ecmo-titration: currentSweepLpm required');
  if (!Number.isFinite(flow) || flow <= 0) throw new Error('ecmo-titration: currentFlowLpm required');
  const round1 = (n) => Math.round(n * 10) / 10;
  const mod = String(modality || 'VV').toUpperCase();
  if (mod !== 'VV' && mod !== 'VA') throw new Error('ecmo-titration: modality must be VV or VA');
  ...
  if (Number.isFinite(paco2) && paco2 > 0) {
    suggestedSweep = round1(sweep * (paco2 / target));
  }
  const hbN = Number(hb);
...
export function ewsEscalation({ news2Total, vitalsTimestamp, singleParam3 }) {
  ...
  if (!Number.isFinite(total) || total < 0) throw new Error('ews-escalation: news2Total required');
  let nextHours;
  let banner;
  if (total >= 7) {
    nextHours = 0;
...
export function restraintTimer({ type, ageYears, orderTimestamp }) {
  ...
  const age = Number(ageYears);
  if (!Number.isFinite(age) || age < 0) throw new Error('restraint-timer: ageYears required');
  const ts = orderTimestamp ? new Date(orderTimestamp).getTime() : NaN;
  if (!Number.isFinite(ts)) throw new Error('restraint-timer: valid orderTimestamp required');
  let renewalHours;
  let reassessMin;
  let faceToFaceMin;
  const banners = [];
  if (t === 'violent' || t === 'violent-self-destructive') {
    ...
    throw new Error('restraint-timer: type must be violent or non-violent');
  }
...
export function sepsisBundleClock({
  ...
  if (!Number.isFinite(t0ms)) throw new Error('sepsis-bundle-clock: t0 required (ISO timestamp)');
...
export function codeBlueClock({
  ...
  if (!Number.isFinite(start)) throw new Error('code-blue-clock: codeStartTimestamp required');
...
export function mtpTracker({ prbcUnits, ffpUnits, plateletUnits, cryoUnits }) {
  const prbc = Math.max(0, Math.trunc(Number(prbcUnits) || 0));
  const ffp = Math.max(0, Math.trunc(Number(ffpUnits) || 0));
  const plt = Math.max(0, Math.trunc(Number(plateletUnits) || 0));
  const cryo = Math.max(0, Math.trunc(Number(cryoUnits) || 0));
  const banners = [];
...
export function deviceDayCounter({ device, insertionTimestamp, criteriaMet, asOf }) {
  ...
  if (d !== 'foley' && d !== 'central-line') {
    throw new Error('device-day-counter: device must be foley or central-line');
  }
  const ins = new Date(insertionTimestamp).getTime();
  if (!Number.isFinite(ins)) throw new Error('device-day-counter: insertionTimestamp required');
  const nowMs = asOf ? new Date(asOf).getTime() : Date.now();
...
export function bristolGirth({
  ...
  if (!Number.isInteger(b) || b < 1 || b > 7) throw new Error('bristol-girth: bristolType must be 1-7');
...
export function ventSbtPeep({
  ...
  if (Number.isFinite(lookF) && lookF > 0 && lookF <= 1.0) {
    ...
    if (armNorm !== 'low' && armNorm !== 'high') throw new Error('vent-sbt-peep: ardsArm must be low or high');
    arm = armNorm;
    ...
    throw new Error('vent-sbt-peep: lookupFiO2 must be a fraction 0-1');
  }
  const banners = [];
...
export function hypothermiaRewarm({ coreTempC, state, ecprExclusion = false, potassium }) {
  ...
  if (!Number.isFinite(t)) throw new Error('hypothermia-rewarm: coreTempC must be a number');
  if (t < 0 || t > 38) throw new Error('hypothermia-rewarm: coreTempC must be 0-38');
  if (!HYPOTHERMIA_STATE.has(state)) {
    throw new Error(`hypothermia-rewarm: state must be one of ${[...HYPOTHERMIA_STATE].join(', ')}`);
  }
  let k = null;
  if (potassium !== undefined && potassium !== null && potassium !== '') {
    k = Number(potassium);
    if (!Number.isFinite(k)) throw new Error('hypothermia-rewarm: potassium must be a number when provided');
    if (k < 0 || k > 30) throw new Error('hypothermia-rewarm: potassium must be 0-30 mmol/L when provided');
  }
  let stage;
...
export function heatstrokeDecision({ coreTempC, cns, sweating = true, setting = 'field' }) {
  const t = Number(coreTempC);
  if (!Number.isFinite(t)) throw new Error('heatstroke-decision: coreTempC must be a number');
  if (t < 35 || t > 47) throw new Error('heatstroke-decision: coreTempC must be 35-47');
  if (!HEAT_CNS.has(cns)) throw new Error(`heatstroke-decision: cns must be one of ${[...HEAT_CNS].join(', ')}`);
  if (!HEAT_SETTING.has(setting)) throw new Error(`heatstroke-decision: setting must be field or hospital`);
...
function checkOrdinal(name, key, value, max) {
  ...
  if (!Number.isInteger(v)) throw new Error(`${name}: ${key} must be an integer`);
  if (v < 0 || v > max) throw new Error(`${name}: ${key} must be 0-${max}`);
  return v;
}
...
function checkSigned(name, key, value, max) {
  const v = Number(value);
  if (!Number.isInteger(v)) throw new Error(`${name}: ${key} must be an integer`);
  if (v < -max || v > max) throw new Error(`${name}: ${key} must be ${-max}..${max}`);
  return v;
}
export function npass({ crying, behavior, facial, extremities, vitals, gestationalAgeWeeks }) {
  ...
  if (!Number.isFinite(ga) || ga < 20 || ga > 44) {
    throw new Error('npass: gestationalAgeWeeks must be 20-44');
  }
...
function checkOrdinalRange(name, key, value, min, max) {
  const v = Number(value);
  if (!Number.isInteger(v)) throw new Error(`${name}: ${key} must be an integer`);
  if (v < min || v > max) throw new Error(`${name}: ${key} must be ${min}-${max}`);
  return v;
}
...
export function wat1({
  ...
  if (!Number.isFinite(rmRaw) || rmRaw < 0) {
    throw new Error('wat-1: recoveryMinutes must be a non-negative number');
  }
  let recoveryPoints;
...
export function sbs({ level }) {
  ...
  if (v > 2) throw new Error('sbs: level must be -3..+2');
...
export function meows(input) {
  ...
  if (rr < 0 || spo2 < 0 || spo2 > 100 || sbp < 0 || dbp < 0 || hr < 0) {
    throw new Error('meows: vitals out of plausible range');
  }
  if (neuro !== 'A' && neuro !== 'V' && neuro !== 'P' && neuro !== 'U') {
    throw new Error('meows: neuro must be one of A, V, P, U');
  }
  if (!Number.isInteger(pain) || pain < 0 || pain > 3) {
    throw new Error('meows: pain must be integer 0-3');
  }
...
function cpssCheckBinary(name, key, v) {
  if (!Number.isInteger(v) || (v !== 0 && v !== 1)) {
    throw new Error(`${name}: ${key} must be integer 0 or 1`);
  }
}
...
function lamsCheck(name, key, v, max) {
  if (!Number.isInteger(v) || v < 0 || v > max) {
    throw new Error(`${name}: ${key} must be integer 0-${max}`);
  }
}
...
function cssrsBool(name, key, v) {
  if (typeof v !== 'boolean') throw new Error(`${name}: ${key} must be boolean`);
}
export function cssrs({
  ...
  if (behaviorPast3Months && !behaviorLifetime) {
    throw new Error('cssrs: behaviorPast3Months cannot be true if behaviorLifetime is false');
  }
  const parts = {
    wishDead, thoughtsKilling, thoughtsMethods,
    someIntent, planIntent, behaviorLifetime, behaviorPast3Months,
...
export function barthel(input) {
  ...
  for (const [key, allowed] of Object.entries(BARTHEL_ITEMS)) {
    ...
    if (!Number.isInteger(v) || !allowed.includes(v)) {
      throw new Error(`barthel: ${key} must be one of ${allowed.join(', ')}`);
    }
    parts[key] = v;
    score += v;
  }
...
export function lawtonIadl(input) {
  ...
  for (const key of LAWTON_ITEMS) {
    const v = input[key];
    if (!Number.isInteger(v) || (v !== 0 && v !== 1)) {
      throw new Error(`lawtonIadl: ${key} must be integer 0 (needs help) or 1 (independent)`);
    }
    parts[key] = v;
    score += v;
  }
...
export function katzAdl(input) {
  ...
  for (const key of KATZ_ITEMS) {
    const v = input[key];
    if (!Number.isInteger(v) || (v !== 0 && v !== 1)) {
      throw new Error(`katzAdl: ${key} must be integer 0 (dependent) or 1 (independent)`);
    }
    parts[key] = v;
    score += v;
  }
...
function fourScoreCheck(name, key, v) {
  if (!Number.isInteger(v) || v < 0 || v > 4) {
    throw new Error(`${name}: ${key} must be integer 0-4`);
  }
}
...
function gussCheckDeglut(name, key, v) {
  if (!Number.isInteger(v) || v < 0 || v > 2) {
    throw new Error(`${name}: ${key} must be integer 0-2`);
  }
}
