...
import { r1, localTimestamp, parseStamp, stampOut, measured, gradeFault } from './num.js';
...
export function grace({ age, heartRate, sbp, creatinineMgDl,
  ...
  // spec-v930: every banded value below is an if/else chain with no missing-value guard, so an
  // absent input fell through to the WORST band and a blank one -- Number('') is 0 -- took the
  // best. Neither is a score. Five values are needed and the rest are flags.
  const need = { age, heartRate, sbp, creatinineMgDl, killipClass };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return { valid: false, score: null, band: 'Enter age, heart rate, systolic BP, creatinine and Killip class to score.', missing };
  }
  // spec-v1231: every band below is an if/else chain that is open at one end,
  // so a value past the envelope simply lands in the outermost band -- and on
  // three of these four tiles the outermost band is the REASSURING one. Envelope
  // and wording from BOUNDS + boundsAdvisory; the check runs after the
  // missing-value branch above (spec-v1207).
  // grace: an SBP of 3000 mmHg fell into the lowest-points BP band and the tile
  // read "Low (in-hospital mortality < 1%)".
  for (const [key, v] of [['ageYears', age], ['hr', heartRate], ['sbp', sbp], ['scr', creatinineMgDl]]) {
    const fault = boundsAdvisory(key, Number(v));
    if (fault) return { valid: false, score: null, band: fault, missing: [] };
  }
  let pts = 0;
  // Age points (banded)
  if      (age < 30)  pts += 0;
...
export function geneva({ age65, priorVte, recentSurgery, activeMalignancy,
  ...
  // spec-v1029: the heart rate is worth up to 5 of the 25 points and a blank one
  // read as 0 beats a minute scores none of them, so a form with no pulse in it
  // reported "Low (~8%)". Points already banked can only rise, so the high band
  // still stands on its own; the low one waits for the measurement.
  const haveHr = hr !== null && hr !== undefined && hr !== '' && Number.isFinite(Number(hr));
  const genevaBandOf = (t) => (t <= 3 ? 0 : t <= 10 ? 1 : 2);
  if (!haveHr && genevaBandOf(s) !== genevaBandOf(s + 5)) {
    return {
      score: null,
      points: s,
      incomplete: true,
      band: `Revised Geneva is at least ${s} from the criteria ticked. `
        + 'Enter the heart rate: it is worth up to 5 points, so the probability group cannot be read yet.',
    };
  }
  return {
    score: s,
...
export function psi({ age, sex, nursingHome, neoplasm, liverDisease,
  ...
  let pts = 0;
  // spec-v931: `age` arrives as a string from the MCP surface, and `0 + '70'` is the STRING
  // '070'. Every `pts +=` after it concatenated instead of adding, so an agent asking for a
  // 70-year-old man with a respiratory rate of 30 got score '07020' and "Class V (admit)" --
  // where the arithmetic gives 90 and Class III. The browser was unaffected because its reader
  // already returns a Number; nothing on the page could have shown it.
  // And converting it exposed the other half: Number('') is 0, so a blank age would have scored
  // as a newborn where an absent one gave NaN. PSI has no meaning without an age -- it IS the
  // largest term -- so a missing one refuses rather than scoring. spec-v930's invariant holds:
  // blank and absent reach the same place.
  if (isBlank(age) || !Number.isFinite(Number(age))) {
    return { score: null, band: 'Enter the patient age to score. Age is the largest single term in the PSI.' };
  }
  const ageNum = Number(age);
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age scored as points.
  const ageFault = boundsAdvisory('ageYears', ageNum);
  if (ageFault) return { valid: false, score: null, band: ageFault, message: ageFault };
  const agePoints = sex === 'F' ? Math.max(0, ageNum - 10) : ageNum;
  pts += agePoints;
  ...
  // Fine 1997 Step 1: a patient aged <= 50 with no risk factor beyond age
  // (no comorbidity, no exam abnormality, and no abnormal entered lab) is Risk
  // Class I (0.1% 30-day mortality). Class I is therefore "no points other than
  // the age contribution" -- `pts === agePoints`. The prior `pts === 0` check
  // was unreachable, since age itself always adds points, so every genuinely
  // low-risk young patient was mislabeled Class II. Routing any entered risk
  // factor to point scoring (II-V) keeps the rule from ever under-triaging.
  let cls;
  if (ageNum <= 50 && pts === agePoints) cls = 'I';
...
export function sofa({ respiration, coagulation, liver, cardiovascular, cns, renal }) {
  // spec-v1122: `Number(n) || 0` read an unrated organ system as a system graded
  // 0 -- no dysfunction. All six are graded selects that opened on "0", so the
  // tile answered "SOFA 0 - Low (~10% mortality)" for a patient whose six organ
  // systems nobody had graded, beside a qSOFA whose three inputs ARE checkboxes
  // and were right all along (rule 4).
  //
  // The score is a sum of non-negative grades, so what has been graded is a
  // FLOOR: the higher bands rule in from a subset (rule 13) and only the "Low"
  // reading has to wait.
  const v = (n) => Math.max(0, Math.min(4, Number(n) || 0));
  const rated = (n) => n !== undefined && n !== null && n !== '' && Number.isFinite(Number(n));
  const systems = [
    [respiration, 'respiration'], [coagulation, 'coagulation'], [liver, 'liver'],
    [cardiovascular, 'cardiovascular'], [cns, 'CNS'], [renal, 'renal'],
  ];
  const ungraded = systems.filter(([n]) => !rated(n)).map(([, label]) => label);
  const s = systems.reduce((acc, [n]) => acc + (rated(n) ? v(n) : 0), 0);
  const label = band(s, [
  ...
  const floored = ungraded.length > 0 && s <= 6;
  return {
    score: s,
    ungraded,
    floorOnly: floored,
    band: floored
      ? `at least ${s} of 24 on what was graded -- ${ungraded.length} of the 6 organ systems `
        + `(${ungraded.join(', ')}) ${ungraded.length === 1 ? 'is' : 'are'} not graded, and each `
        + 'can only add points, so the low-mortality band cannot be given yet'
      : label,
  };
}
...
// spec-v1229: the clamp is not the guard, again -- the shape spec-v1224 named on
// the cardiovascular engines, here on the liver ones.
//
...
// 120 to 125 is what the score IS. It is not a plausibility check, and a sodium
// of 2000 mEq/L was clamped to 137 and reported as a transplant-priority score.
//
...
// liver there is.
//
// A throw is the surfacing here because it is what this file's neighbours
// already do (`maddreyDf` and `lille` below) and what both surfaces read: the
// renderer's safe() wrapper prints `err.message`, and `computeCalculator`
// reports `valid: false`. The sentence is boundsAdvisory's; no clinical number
// and no operational bound is decided here.
function envelopeThrow(rows) {
  for (const [key, v] of rows) {
    if (v === null || v === undefined || v === '') continue;
    const n = Number(v);
    if (!Number.isFinite(n)) continue;
    const fault = boundsAdvisory(key, n);
    if (fault) throw new RangeError(fault);
  }
}

export function meld30({ bilirubin, inr, creatinine, sodium, albumin, sex,
  hadDialysisTwiceLastWeek }) {
  envelopeThrow([['bilirubin', bilirubin], ['scr', creatinine], ['sodium', sodium], ['albumin', albumin], ['inr', inr]]);
...
export function childPugh({ bilirubin, albumin, inr, ascites, encephalopathy }) {
  envelopeThrow([['bilirubin', bilirubin], ['albumin', albumin], ['inr', inr]]);
...
export function ranson({ admission, fortyEightHour }) {
  ...
  return {
    score: total,
    band: band(total, [
      // spec-v1048: was '<1% mortality', which disagreed with this tile's own
      // interpretation table -- printed on the same page under "How this is
      // calculated" -- and with the three bands below it, all of which carry the
      // figures Ranson 1974 is cited for (15%, 40%, 100%). The reader saw "<1%"
      // above "Roughly 2% mortality in acute pancreatitis".
      { min: 0, max: 2, label: '~2% mortality' },
...
export function mcisaac({ tonsillarExudate, tenderAnteriorAdenopathy,
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age picked the modifier.
  const ageFault = isBlank(ageYears) ? null : boundsAdvisory('ageYears', Number(ageYears));
  if (ageFault) return { valid: false, score: null, ageModifier: null, band: ageFault, message: ageFault };
...
export function bishop({ dilation, effacement, station, consistency, position }) {
  // spec-v930: the three measurements are compared with bare ternaries and no guard, so an
  // ABSENT set fell through to the top band ("Favorable") while a BLANK set -- Number('') is 0 --
  // took the bottom one ("Unfavorable"). An induction decision should not land anywhere on an
  // unfilled form.
  const need = { dilation, effacement, station };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return { score: null, band: 'Enter cervical dilation, effacement and station to score.', missing };
  }
...
export const CAGE_CONFIG = {
  ...
  // Matches META.cage.example, which the /tools/cage/ page states as the
  // result: "Score 2 of 4 = Positive." An all-No prefill scored 0, so the page
  // and the tool disagreed, and an example that answers nothing shows the
  // reader nothing.
  exampleAnswers: [1, 1, 0, 0],
};
...
export const EPDS_CONFIG = {
  id: 'epds',
  // Neutral domain labels for the 10 items (over the past 7 days). The EPDS's verbatim
  // first-person item wording is proprietary (Royal College of Psychiatrists); these are
  // short topic cues, not the questionnaire's statements. Items 1-2 stay positively framed
  // because they are reverse-scored (scoring is positional, not text-derived).
  items: [
    ...
    'Unhappiness affecting sleep',
    'Feeling sad or miserable',
    'Unhappiness leading to crying',
...
export function miniCog({ wordsRecalled, clockNormal }) {
  ...
  // spec-v1036: the recall is a test that was administered or was not. Read as
  // zero it produced "Positive screen - further evaluation indicated" from a form
  // nobody had filled in -- a dementia screen reported positive on a patient who
  // had not been asked to recall anything. One word recalled would have made the
  // same form read negative.
  if (isBlank(wordsRecalled) || !Number.isFinite(Number(wordsRecalled))) {
    return {
      score: null,
      incomplete: true,
      band: 'Enter the number of words recalled (0-3): the clock draw alone scores '
        + `${c} of 5, and the recall decides which side of the cutoff this lands on.`,
    };
  }
  const w = Math.max(0, Math.min(3, Number(wordsRecalled)));
  const score = w + c;
...
export function ciwaAr({
  ...
  // spec-v1028: CIWA-Ar is ten clinician-rated observations, and every one adds
  // points or leaves them alone -- so a partial total is a LOWER BOUND, and the
  // rule of spec-v1006 applies to a scale that gates medication. An unrated item
  // used to score 0, and the tile's inputs opened AT 0, so a form nobody had
  // filled in read "CIWA-Ar: 0 -- Mild withdrawal (<8): supportive care", which
  // is the sentence a symptom-triggered protocol uses to WITHHOLD a
  // benzodiazepine. Above 8 the score already argues for treatment and stands on
  // what was rated; below it, the reassuring reading waits for the rest.
  const items = { nausea, tremor, sweats, anxiety, agitation, tactile, auditory, visual, headache, orientation };
  const unrated = Object.keys(items).filter((k) => isBlank(items[k]));
  if (unrated.length && score < 8) {
    return {
      score,
      rated: 10 - unrated.length,
      band: `CIWA-Ar is at least ${score} from ${10 - unrated.length} of 10 items. `
        + `Rate the remaining ${unrated.length}: each can only add points, so a partial score cannot support the mild-withdrawal reading.`,
      incomplete: true,
    };
  }
  const partial = unrated.length ? ` Scored from ${10 - unrated.length} of 10 items; the rest can only raise it.` : '';
  return {
    score,
    rated: 10 - unrated.length,
    band: band(score, [
...
export function cows({
  pulse, sweating, restlessness, pupil, jointAches,
  runnyNose, gi, tremor, yawning, anxiety, gooseflesh,
} = {}) {
  ...
  // spec-v1028: the same rule as ciwaAr above, on the scale that times a
  // buprenorphine induction. Eleven observations, each adding points or leaving
  // them alone, so a partial total is a lower bound -- and an unrated form used
  // to read "COWS: 0 -- No active withdrawal", which is the reading that says
  // not to start. From 5 up the score already says withdrawal is present and
  // stands on what was rated.
  const items = { pulse, sweating, restlessness, pupil, jointAches, runnyNose, gi, tremor, yawning, anxiety, gooseflesh };
  const unrated = Object.keys(items).filter((k) => isBlank(items[k]));
  if (unrated.length && score < 5) {
    return {
      score,
      rated: 11 - unrated.length,
      incomplete: true,
      band: `COWS is at least ${score} from ${11 - unrated.length} of 11 items. `
        + `Rate the remaining ${unrated.length}: each can only add points, so a partial score cannot support the no-withdrawal reading.`,
    };
  }
  const partial = unrated.length ? ` Scored from ${11 - unrated.length} of 11 items; the rest can only raise it.` : '';
  return {
    score,
    rated: 11 - unrated.length,
    band: band(score, [
...
export function ascvdPce({ age, sex, race, totalChol, hdl, sbp, treatedSbp,
  smoker, diabetes }) {
  // Guard non-finite numeric inputs FIRST: `age < 40 || age > 79` is false when
  // age is NaN, so without this a blank field would fall through and render
  // "NaN%". Returns the same {score:null, band} shape the renderer already
  // handles, showing guidance instead of NaN.
  if (![age, totalChol, hdl, sbp].every(Number.isFinite)) {
    return { valid: false, score: null, band: 'Enter age, total cholesterol, HDL, and systolic BP.' };
  }
  if (age < 40 || age > 79) {
    return { valid: false, score: null, band: 'PCE valid for ages 40-79 only.' };
  }
  // spec-v1224: the two 10-year risk engines here answered from an impossible
  // systolic BP for the same reason the five in lib/cvrisk-v103.js did -- the
  // model is monotone in ln(SBP) and nothing stopped it. An SBP of 3000 mmHg
  // read "High (>=20%)", a band that reads as an assessment of the patient.
  // Envelope and wording from BOUNDS + boundsAdvisory; no clinical number
  // invented here, and it runs after the missing-value branch above.
  const sbpFault = boundsAdvisory('sbp', sbp);
  if (sbpFault) return { valid: false, score: null, band: sbpFault };
...
export function prevent10yr({ age, sex, totalChol, hdl, sbp, treatedSbp,
  ...
  // Guard non-finite numeric inputs FIRST (see ascvdPce): a blank field would
  // otherwise pass the age-range check (NaN comparisons are false) and render
  // "NaN%". Returns the renderer-handled {score:null, band} guidance shape.
  if (![age, totalChol, hdl, sbp, bmi, egfr].every(Number.isFinite)) {
    return { valid: false, score: null, band: 'Enter age, total cholesterol, HDL, systolic BP, BMI, and eGFR.' };
  }
  if (age < 30 || age > 79) {
    return { valid: false, score: null, band: 'PREVENT valid for ages 30-79 only.' };
  }
  // spec-v1224: see ascvdPce above. PREVENT splits SBP at 110 into two linear
  // terms and neither saturates, so the risk simply climbs with the entered
  // pressure; 3000 mmHg reported "High (>=20%)".
  const sbpFault = boundsAdvisory('sbp', sbp);
  if (sbpFault) return { valid: false, score: null, band: sbpFault };
  // spec-v1406: the bmi and eGFR envelopes lib/bounds.js declares; an impossible value computed.
  const bodyFault = boundsAdvisory('bmi', bmi) || boundsAdvisory('eGFR', egfr);
  if (bodyFault) return { valid: false, score: null, band: bodyFault, message: bodyFault };
  if (sex !== 'M' && sex !== 'F') throw new RangeError('sex must be "M" or "F"');
...
export function news2({ rr, spo2, scale2, onO2, sbp, pulse, acvpu, temp }) {
  // spec-v930: every band helper below is an if/else chain with no missing-value guard, so a set
  // of observations nobody has taken scored the worst value in each and returned "High (>=7):
  // emergency assessment by critical-care team". An empty chart is not a deteriorating patient.
  const need = { rr, spo2, sbp, pulse, temp };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return { score: null, band: 'Enter respiratory rate, oxygen saturation, systolic BP, pulse and temperature to score.', missing };
  }
  // spec-v1199: spec-v1181 gave `mews` directly below this the envelope guard and
  // left NEWS2 without it, though the comment above says the two share the defect
  // it was written for. Every NEWS2 band saturates at its extreme, so a pulse of
  // 3000 scores the same 3 points a survivable tachycardia does. Envelope and
  // wording from BOUNDS + boundsAdvisory; no clinical number invented here.
  for (const [key, v] of [['rr', rr], ['sbp', sbp], ['hr', pulse], ['temperature', temp]]) {
    const fault = boundsAdvisory(key, Number(v));
    if (fault) return { valid: false, score: null, band: fault, missing: [] };
  }
...
export function mews({ sbp, pulse, rr, temp, avpu }) {
  // spec-v930: same as news2 -- unguarded band chains, so an empty observation set returned
  // ">=5: increased risk of death, ICU admission".
  const need = { sbp, pulse, rr, temp };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return { score: null, band: 'Enter systolic BP, pulse, respiratory rate and temperature to score.', missing };
  }
  // spec-v1181: the last of spec-v1175's reassuring rows. Envelope and wording
  // from BOUNDS + boundsAdvisory in lib/bounds.js; no clinical number invented.
  // Every MEWS band saturates at its extreme, so an impossible observation scores
  // the same 2 points a survivable one does and the total lands in "low risk".
  for (const [key, v] of [['sbp', sbp], ['hr', pulse], ['rr', rr], ['temperature', temp]]) {
    const fault = boundsAdvisory(key, Number(v));
    // spec-v1199: `valid: false` as well as the sentence. mcp/tools.js treats a
    // library result as an answer unless it is null or carries `valid: false`, so
    // this refusal reached the browser and reached an AGENT as a successful
    // computation whose score happened to be null. The view keys off `score`, not
    // `valid`, so nothing changes on the page.
    if (fault) return { valid: false, score: null, band: fault, missing: [] };
  }
...
export function pesi({ age, sex, cancer, heartFailure, chronicLungDisease,
  ...
  // spec-v1029: the score IS the age in years plus these points, so a blank age
  // is not a patient of zero -- it is the largest term of the sum missing. Read
  // as 0 the tile answered "Class I (very low risk)", which is the sentence that
  // sends a pulmonary embolism home.
  let s = 0;
  if (sex === 'M') s += 10;
  if (cancer) s += 30;
  ...
  if (sao2Lt90) s += 20;
  const ageNum = Number(age);
  const haveAge = age !== null && age !== undefined && age !== '' && Number.isFinite(ageNum);
  const classOf = (t) => (t <= 65 ? 'I' : t <= 85 ? 'II' : t <= 105 ? 'III' : t <= 125 ? 'IV' : 'V');
  if (!haveAge) {
    const floor = classOf(s);
    return {
      score: null,
      points: s,
      class: null,
      incomplete: true,
      band: `PESI is the age in years plus ${s} point${s === 1 ? '' : 's'} from the criteria above. `
        + 'Enter the age: without it there is no class.'
        + (floor === 'I' ? '' : ` The criteria alone already reach Class ${floor}, and the age can only raise it.`),
    };
  }
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age scored as points.
  const ageFault = boundsAdvisory('ageYears', ageNum);
  if (ageFault) return { valid: false, score: null, class: null, band: ageFault, message: ageFault };
  s += ageNum;
  let cls, band;
...
export function oakland({ age, sex, priorLgibAdmission, dreBlood,
  hr, sbp, hgbGdl }) {
  // spec-v930: the four banded values have no missing-value guard, so an absent set scored the
  // safe-discharge band and a blank set scored against it. A discharge decision is not something
  // to reach from an unfilled form.
  const need = { age, hr, sbp, hgbGdl };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return { valid: false, score: null, parts: null, band: 'Enter age, heart rate, systolic BP and hemoglobin to score.', missing };
  }
  // spec-v1231: see grace above. `oaklandHgb` is open at the top -- a hemoglobin
  // of 250 g/dL scores 0 points, the same as a healthy one -- so an impossible
  // value read "Oakland <=8: safe for outpatient management (95% probability of
  // safe discharge)". A discharge decision from a number no patient has.
  for (const [key, v] of [['ageYears', age], ['hr', hr], ['sbp', sbp], ['hemoglobin', hgbGdl]]) {
    const fault = boundsAdvisory(key, Number(v));
    if (fault) return { valid: false, score: null, parts: null, band: fault, missing: [] };
  }
  const parts = {
...
export function maddreyDf({ patientPtSec, controlPtSec, bilirubinMgDl }) {
  ...
  if (!(pt > 0)) throw new RangeError('patient pt sec must be positive');
  if (!(ctrl > 0)) throw new RangeError('control pt sec must be positive');
  if (!(bili >= 0)) throw new RangeError('bilirubin mg dl must be non-negative');
  envelopeThrow([['bilirubin', bili]]);
...
export function lille({ ageYears, albuminGDl, creatinineMgDl,
  ...
  if (!(age > 0)) throw new RangeError('age years must be positive');
  if (!(albGDl > 0)) throw new RangeError('albumin GDl must be positive');
  if (!(cr > 0)) throw new RangeError('creatinine mg dl must be positive');
  if (!(b0 >= 0)) throw new RangeError('bilirubin Day0 mg dl must be non-negative');
  if (!(b7 >= 0)) throw new RangeError('bilirubin Day7 mg dl must be non-negative');
  if (!(pt > 0)) throw new RangeError('pt sec must be positive');
  envelopeThrow([['ageYears', age], ['albumin', albGDl], ['scr', cr], ['bilirubin', b0], ['bilirubin', b7]]);
...
export function pecarnHead({ ageYears, gcs15, palpableSkullFx, basalSkullFxSigns,
  ...
  // spec-v1029: the age does not add points here -- it picks the rule. Read as 0
  // a blank age silently ran the under-2 branch and, with nothing else entered,
  // printed "Very low risk. CT not recommended." on a head-injured child nobody
  // had described. Both branches are evaluated below and the answer stands only
  // where they agree.
  const haveAge = ageYears !== null && ageYears !== undefined && ageYears !== ''
  ...
  const intermediateUnder2Pre = (Number(locSec) >= 5) || severeMechanism
    || occipitalParietalTemporalHematoma || notActingNormally;
  const intermediateOver2Pre = !!locSec || vomiting || severeMechanism || severeHeadache;
  if (!haveAge) {
    const tierOf = (hi, mid) => (hi ? 'high' : mid ? 'intermediate' : 'very-low');
    const under2 = tierOf(highRiskUnder2, intermediateUnder2Pre);
    const over2 = tierOf(highRiskOver2, intermediateOver2Pre);
    if (under2 !== over2) {
      return {
        tier: null,
        ciTbiRiskPct: null,
        incomplete: true,
        band: 'PECARN has a separate rule for children under 2 and for 2 and over, and on these '
          + `findings they disagree: under 2 this is ${under2.replace('-', ' ')} risk, at 2 and over `
          + `it is ${over2.replace('-', ' ')} risk. Enter the age to choose the rule.`,
      };
    }
  }
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age picked a rule.
  const ageFault = haveAge ? boundsAdvisory('ageYears', Number(ageYears)) : null;
  if (ageFault) return { valid: false, tier: null, ciTbiRiskPct: null, band: ageFault, message: ageFault };
  if (highRisk) return {
    tier: 'high',
    ciTbiRiskPct: 4.4,
  ...
  const intermediate = isUnder2 ? intermediateUnder2Pre : intermediateOver2Pre;
  ...
  if (!haveAge) {
    // Both branches agree it is very low risk, but the two rules quote different
    // figures, so the reading is given without one rather than picking a rule.
    return {
      tier: 'very-low',
      ciTbiRiskPct: null,
      band: 'Very low risk on both PECARN rules per Kuppermann 2009: CT not recommended. '
        + 'Enter the age for the quoted ciTBI rate (<0.02% under 2, <0.05% at 2 and over).',
    };
  }
  return {
    tier: 'very-low',
...
export function hospitalScore({ hgbLt12, oncologyDischarge, sodiumLt135,
  ...
  const haveAdmissions = priorAdmissions12mo !== null && priorAdmissions12mo !== undefined
    && priorAdmissions12mo !== '' && Number.isFinite(Number(priorAdmissions12mo));
  const parts = {
    hgbLt12: hgbLt12 ? 1 : 0,
  ...
  const score = parts.hgbLt12 + parts.oncologyDischarge + parts.sodiumLt135
    ...
    + parts.losGe5;
  // spec-v1029: prior admissions add up to 5 points and never remove any. With a
  // blank count read as none, an untouched form reported "low risk" -- the reading
  // a discharge planner uses to skip the transitional-care intervention.
  const hospitalBandOf = (t) => (t <= 4 ? 0 : t <= 6 ? 1 : 2);
  if (!haveAdmissions && hospitalBandOf(score) !== hospitalBandOf(score + 5)) {
    return {
      score: null,
      parts,
      incomplete: true,
      band: `HOSPITAL is at least ${score} from the rest of the form. `
        + 'Enter the number of admissions in the past 12 months: it adds up to 5 more points, '
        + 'so a low-risk reading cannot stand on the fields entered.',
    };
  }
  let band;
...
export function lace({ losDays, acuteAdmission, charlsonScore, edVisits6mo }) {
  ...
  // spec-v1020: three of LACE's four terms are counts somebody has to look up --
  // the length of stay, the Charlson index and the emergency visits in six
  // months -- and Number('') is 0, so a length of stay alone answered "LACE 1:
  // low risk of 30-day death or unplanned readmission". Every term only adds, so
  // the moderate and high bands stand on what is entered; the low one waits for
  // the counts.
  const laceMissing = [];
  if (isBlank(losDays)) laceMissing.push('the length of stay');
  if (isBlank(charlsonScore)) laceMissing.push('the Charlson comorbidity index');
  if (isBlank(edVisits6mo)) laceMissing.push('the emergency visits in the prior 6 months');
  if (score <= 4 && laceMissing.length) {
    return {
      score,
      band: `LACE is at least ${score} so far. Enter ${laceMissing.join(', ')}: `
        + 'each can only add points, so the low-risk reading cannot be given without them.',
      parts,
      awaiting: laceMissing.length,
    };
  }
...
export function charlson({ items = {}, ageYears = null } = {}) {
  const W = {
  ...
  const age = Number(ageYears);
  const haveAge = ageYears !== null && ageYears !== undefined && ageYears !== '' && Number.isFinite(age);
  // spec-v1029: the age adjustment adds up to 4 points and never removes any, so
  // a comorbidity count alone is a LOWER bound. That is safe to rule in -- five
  // points is the worst band whatever the age -- and never safe to rule out: with
  // the age read as 0 an untouched form said "10-year survival ~98%".
  const charlsonBandOf = (t) => (t === 0 ? 0 : t <= 2 ? 1 : t <= 4 ? 2 : 3);
  // The age adds at most 4 points, so ask only when those 4 could move the band.
  if (!haveAge && charlsonBandOf(comorbidity) !== charlsonBandOf(comorbidity + 4)) {
    return {
      score: null,
      comorbidity,
      ageAdj: null,
      incomplete: true,
      band: `Charlson is at least ${comorbidity} from the comorbidities ticked. `
        + 'Enter the age: it adds up to 4 more points, so the survival estimate cannot be read yet.',
    };
  }
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age earned the age points.
  const ageFault = haveAge ? boundsAdvisory('ageYears', age) : null;
  if (ageFault) return { valid: false, score: null, comorbidity, ageAdj: null, band: ageFault, message: ageFault };
  let ageAdj = 0;
  if (haveAge && age >= 50) ageAdj = Math.min(4, Math.floor((age - 40) / 10));
...
export function cfs({ level }) {
  ...
  // spec-v930: Number('') is 0, which is finite, so a blank level slipped past this refusal and
  // was clamped up to 1 -- an unanswered form reported "Very fit: not frail".
  const n = isBlank(level) ? NaN : Math.round(Number(level));
...
export function rass({ level }) {
  ...
  // spec-v930: a blank field is not a level either. Number('') is 0, and RASS 0 is a real
  // answer -- "light-sedation target band" -- so an unanswered form used to report one.
  const n = isBlank(level) ? NaN : Math.round(Number(level));
...
export function sasRiker({ level }) {
  // spec-v930: an unanswered level used to fall through to the default of 4, so a blank form
  // reported "calm and cooperative; goal sedation". A missing observation is not a calm patient.
  if (isBlank(level) || !Number.isFinite(Number(level))) {
    return { level: null, descriptor: null, band: 'Enter a SAS level (1-7).' };
  }
  const n = Number(level);
  const lv = Math.max(1, Math.min(7, Math.round(n)));
  const descriptors = {
    1: 'Unarousable.',
...
const NUTRIC_LABELS = {
  ageYears: 'age', apache2: 'APACHE II', sofa: 'SOFA',
  comorbidities: 'comorbidity count', daysHospitalToIcu: 'days from hospital to ICU',
};
// nutricMissing(given) -> [label]. IL-6 is deliberately not required: it is the
// one input mNUTRIC drops, and Heyland 2011 reports the score with and without it.
function nutricMissing(given) {
  return Object.keys(NUTRIC_LABELS)
    .filter((k) => isBlank(given[k]) || !Number.isFinite(Number(given[k])))
    .map((k) => NUTRIC_LABELS[k]);
}
...
export function nutric({ ageYears, apache2, sofa, comorbidities,
  ...
  // BOUND -- it can rule a patient IN to high risk and can never rule them out.
  // These readers used `Number(x) || 0`, which turns a missing value into a
  // best-case one, so an empty form answered "low nutritional risk" and so did a
  // form with nothing but an age in it. Below the cutoff the score is withheld
  // until the required inputs are present; at or above it the reading stands.
  const missing = nutricMissing({ ageYears, apache2, sofa, comorbidities, daysHospitalToIcu });
  if (missing.length && score < 6) {
    return {
      score: null, highRisk: null, parts: null,
      band: `Enter the NUTRIC inputs to score. Missing: ${missing.join(', ')}. `
        + 'Each unentered value can only add points, so a partial total cannot be read as reassurance.',
    };
  }
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = isBlank(ageYears) ? null : boundsAdvisory('ageYears', Number(ageYears));
  if (ageFault) return { valid: false, score: null, highRisk: null, parts: null, band: ageFault, message: ageFault };
  const partial = missing.length ? ` Scored without ${missing.join(', ')}; the rest can only raise it.` : '';
  // spec-v1151: `nutricMissing` deliberately leaves IL-6 out, because IL-6 is
  // rarely available and the modified NUTRIC exists for exactly that -- but the
  // IL-6 term still adds a point at >= 400 pg/mL, so a total of 5 without it is
  // ONE POINT SHORT of the cutoff and cannot be read as reassurance. It was:
  // "NUTRIC 5 of 10: low nutritional risk", where an IL-6 of 500 makes it 6 and
  // high risk. Rule 3, and the mNUTRIC cut-off is >= 5 in any case -- the same
  // patient is already high risk on the form that omits IL-6 by design.
  const il6Absent = isBlank(il6Pg) || !Number.isFinite(Number(il6Pg));
  if (il6Absent && score === 5) {
    return {
      score,
      highRisk: null,
      parts,
      band: `NUTRIC at least ${score} of 10: the IL-6 is not entered, and it adds a point at >= 400 pg/mL -- `
        + 'one point short of the >=6 cutoff, so this cannot yet read as low nutritional risk. '
        + 'The modified NUTRIC (mNUTRIC), which omits IL-6 by design, calls 5 HIGH risk (Rahman 2016 cutoff >=5).'
        + partial,
    };
  }
  return {
    ...
    highRisk: score >= 6,
    band: score >= 6
      ? `NUTRIC ${score} of 10: high nutritional risk per Heyland 2011 (cutoff >=6); benefits most from aggressive nutrition therapy.${partial}`
      : `NUTRIC ${score} of 10: low nutritional risk per Heyland 2011 (cutoff >=6).${il6Absent ? ' The IL-6 is not entered; it adds at most one point, which would not reach the >=6 cutoff from here.' : ''}`,
    parts,
  };
}
...
export function mnutric({ ageYears, apache2, sofa, comorbidities,
  ...
  // spec-v1006: the same lower-bound rule as nutric() above.
  const missing = nutricMissing({ ageYears, apache2, sofa, comorbidities, daysHospitalToIcu });
  if (missing.length && score < 5) {
    return {
      score: null, highRisk: null, parts: null,
      band: `Enter the mNUTRIC inputs to score. Missing: ${missing.join(', ')}. `
        + 'Each unentered value can only add points, so a partial total cannot be read as reassurance.',
    };
  }
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  ...
  return {
    ...
    highRisk: score >= 5,
    band: score >= 5
      ? `mNUTRIC ${score} of 9: high nutritional risk per Rahman 2016 (cutoff >=5).${partial}`
...
export function mustNutrition({ bmi, weightLossPct, acuteDiseaseNoIntakeGt5d }) {
  // spec-v930: the guards below are right, but Number('') is 0 and 0 is finite -- so a blank BMI
  // scored as a BMI under 18.5 and an unfilled form reported "high malnutrition risk; refer to
  // dietitian".
  const b = isBlank(bmi) ? NaN : Number(bmi);
  const w = isBlank(weightLossPct) ? NaN : Number(weightLossPct);
  ...
  // spec-v1038: spec-v930 stopped a blank BMI from SCORING 2, and left it able to
  // score 0. Each of the three components adds points or leaves them alone, so a
  // partial total is a lower bound: it can carry the referral, never the
  // reassurance. Read with the weight loss entered and the BMI blank, this said
  // "MUST 0: low malnutrition risk".
  const unmeasured = [];
  if (!Number.isFinite(b)) unmeasured.push('a BMI');
  if (!Number.isFinite(w)) unmeasured.push('an unplanned weight-loss percentage');
  if (unmeasured.length && score < 2) {
    return {
      score: null,
      partial: score,
      incomplete: true,
      band: `MUST is at least ${score} from what has been entered. Enter ${unmeasured.join(' and ')}: `
        + `${unmeasured.length === 1 ? 'it can' : 'each can'} only add points, so a low-risk reading `
        + `cannot stand without ${unmeasured.length === 1 ? 'it' : 'them'}.`,
    };
  }
  // spec-v1406: the bmi envelope lib/bounds.js declares; an impossible BMI scored as a normal one.
  const bmiFault = boundsAdvisory('bmi', b);
  if (bmiFault) return { valid: false, score: null, band: bmiFault, message: bmiFault };
  let band;
...
export function hacor({ hr, ph, gcs, pao2, fio2, rr }) {
  ...
  // spec-v1038: `Number(null)` is 0, and 0 is finite. The guard below was written
  // for `Number('')` -> NaN, but the renderer was changed to pass null for a blank
  // field, so a missing heart rate arrived as a heart rate of zero and scored the
  // best band. Same trap as `Number('')`, one value along.
  const raw = { hr, ph, gcs, pao2, fio2, rr };
  const vals = {};
  for (const [k, v] of Object.entries(raw)) vals[k] = isBlank(v) ? NaN : Number(v);
  const missing = Object.values(vals).some((v) => !Number.isFinite(v));
  if (missing || vals.fio2 <= 0) {
    return { valid: false, score: null, pfRatio: null, parts: null,
  ...
  // and wording from BOUNDS + boundsAdvisory; the check runs after the
  // missing-value branch above (spec-v1207).
  // hacor: a pH of 80 and a PaO2 of 7000 each scored zero points and the tile
  // read "HACOR 0: not in the Duan 2017 high-risk band".
  for (const [key, k] of [['hr', 'hr'], ['pH', 'ph'], ['gcs', 'gcs'], ['paO2', 'pao2'], ['fio2', 'fio2'], ['rr', 'rr']]) {
    const fault = boundsAdvisory(key, vals[k]);
    if (fault) return { valid: false, score: null, pfRatio: null, parts: null, band: fault };
  }
...
export function lisMurray({ quadrants, pao2, fio2, peep, complianceMlPerCmH2O }) {
  ...
  // spec-v1041: and the same `Number(null)` trap as hacor (spec-v1040), in the
  // guard directly above it. The renderer passes null for a blank field, 0 is
  // finite, so "refuse from an empty instrument" refused nothing and the tile
  // answered "Murray LIS 0: no lung injury" with the chest film unscored.
  const raw = { quadrants, pao2, fio2, peep, compliance: complianceMlPerCmH2O };
  const vals = {};
  for (const [k, v] of Object.entries(raw)) vals[k] = isBlank(v) ? NaN : Number(v);
  const missing = Object.values(vals).some((v) => !Number.isFinite(v));
  if (missing || vals.fio2 <= 0) {
  ...
  // and wording from BOUNDS + boundsAdvisory; the check runs after the
  // missing-value branch above (spec-v1207).
  // lis-murray: a PaO2 of 7000 mmHg gave a P/F far above 300 and the tile read
  // "Murray LIS 0: no lung injury".
  for (const [key, k] of [['paO2', 'pao2'], ['fio2', 'fio2']]) {
    const fault = boundsAdvisory(key, vals[k]);
    if (fault) return { valid: false, score: null, pfRatio: null, parts: null, band: fault };
  }
  const pf = vals.pao2 / vals.fio2;
...
function smartCopOxLow({ pao2, spo2, pfRatio, age }) {
  // spec-v930: a blank oxygenation value is not a low one. Number('') is 0, which is finite and
  // below every threshold here, so an unfilled form scored the 2-point oxygenation criterion.
  const p = isBlank(pao2) ? NaN : Number(pao2);
  const s = isBlank(spo2) ? NaN : Number(spo2);
  const pf = isBlank(pfRatio) ? NaN : Number(pfRatio);
  if (age <= 50) {
...
export function smartCop({ ageYears, sbpLt90, multilobar, albuminLt35,
  ...
  // spec-v1037: the age is not a term of this score -- it chooses the THRESHOLDS
  // the respiratory rate and the oxygenation are measured against (>50 or not).
  // Read as 0 it silently applied the under-50 cut-offs and, with the rest of a
  // worked example in place, answered "SMART-COP 0: low risk" -- a prediction
  // about needing vasopressors, made against the wrong thresholds. The monotone
  // argument does not rescue this one: a missing age can move the score either
  // way, so there is no band until it is entered.
  if (isBlank(ageYears) || !Number.isFinite(Number(ageYears))) {
    return {
      score: null,
      incomplete: true,
      parts: null,
      band: 'Enter the age: SMART-COP measures the respiratory rate and the oxygenation against '
        + 'different thresholds above and below 50, so neither can be scored without it.',
    };
  }
  const age = Number(ageYears);
  const ageFault = boundsAdvisory('ageYears', age);
  if (ageFault) {
    return { valid: false, score: null, parts: null, band: ageFault };
  }
  const oxygenFault = gradeFault([
    ['PaO2 (mmHg)', pao2, 10, 700],
    ['SpO2 (%)', spo2, 0, 100],
    // spec-v1411: the ceiling critcare-severity already applies to the same ratio; with
    // none, a P/F of 999999 read exactly as a normal one.
    ['PaO2/FiO2 ratio', pfRatio, 0, 1000],
  ]);
  if (oxygenFault) {
    return { valid: false, score: null, parts: null, band: oxygenFault };
  }
  const parts = {
    sbp: sbpLt90 ? 2 : 0,
  ...
  // spec-v1020: the rule of spec-v1006/v1007, on the likelier form -- not an
  // empty page but a page with a couple of values in it. SMART-COP's checkboxes
  // are criteria and an unticked one is a real "no", but two of its terms are
  // MEASUREMENTS: the age-adjusted respiratory rate, and the oxygenation trio
  // (PaO2, SpO2 or P/F, any one of which serves). Blank, they scored nothing,
  // and an age alone answered "SMART-COP 0: low risk" -- a prediction about
  // needing vasopressors or ventilation, from a patient nobody had examined.
  // The score is monotone, so the two alarming bands stand as soon as the
  // entered values reach them; only "low risk" waits.
  const rrMeasured = !isBlank(rr) && Number(rr) > 0;
  const oxMeasured = !isBlank(pao2) || !isBlank(spo2) || !isBlank(pfRatio);
  const missing = [];
  if (!rrMeasured) missing.push('a respiratory rate');
  if (!oxMeasured) missing.push('an oxygenation value (PaO2, SpO2 or P/F)');
  ...
  if (score >= 5) band = `SMART-COP ${score}: high risk of needing intensive respiratory or vasopressor support per Charles 2008 (cutoff >=5).`;
    ...
    return {
      score,
      band: `SMART-COP is at least ${score} so far. Enter ${missing.join(' and ')}: `
        + 'each can only add points, so the low-risk reading cannot be given without them.',
      parts,
      awaiting: missing.length,
    };
  } else band = `SMART-COP ${score}: low risk per Charles 2008.`;
  const partial = missing.length ? ` Scored without ${missing.join(' or ')}; either can only raise it.` : '';
  return { score, band: band + partial, parts };
}
...
function modsRespiratory(pf) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(pf) ? NaN : Number(pf);
...
function modsRenal(cr) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(cr) ? NaN : Number(cr);
  if (!Number.isFinite(v)) return 0;
  if (v <= 1.1) return 0;
  if (v <= 2.3) return 1;
  if (v <= 4.0) return 2;
...
function modsHepatic(bili) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(bili) ? NaN : Number(bili);
  if (!Number.isFinite(v)) return 0;
  if (v <= 1.2) return 0;
  if (v <= 3.5) return 1;
  if (v <= 7.0) return 2;
...
function modsCardiovascular(par) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(par) ? NaN : Number(par);
  if (!Number.isFinite(v)) return 0;
  if (v <= 10.0) return 0;
  if (v <= 15.0) return 1;
  if (v <= 20.0) return 2;
...
function modsHematologic(plt) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(plt) ? NaN : Number(plt);
  if (!Number.isFinite(v)) return 0;
  if (v > 120) return 0;
  if (v >= 81) return 1;
  if (v >= 51) return 2;
...
function modsNeurologic(gcs) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const v = isBlank(gcs) ? NaN : Number(gcs);
  if (!Number.isFinite(v)) return 0;
  if (v >= 15) return 0;
  if (v >= 13) return 1;
  if (v >= 10) return 2;
...
export function mods({ pfRatio, creatinineMgDl, bilirubinMgDl, par,
  plateletsK, gcs }) {
  // spec-v1199: every organ band below saturates at its worst step, so a
  // creatinine of 250 mg/dL scores the same 4 points a survivable one does and
  // the total reads as a mortality estimate. Checked on the values that ARE
  // present; the ones that are not are the missing-value branch further down,
  // and this must not pre-empt it (`pos()`-style null-for-out-of-range is the
  // failure `scripts/probe-envelope-unbounded.mjs` keeps its own section for).
  for (const [key, v] of [['scr', creatinineMgDl], ['bilirubin', bilirubinMgDl],
    ['platelets', plateletsK], ['gcs', gcs]]) {
    if (isBlank(v) || !Number.isFinite(Number(v))) continue;
    const fault = boundsAdvisory(key, Number(v));
    if (fault) return { valid: false, score: null, parts: null, band: fault };
  }
  const parts = {
  ...
  // spec-v1006: every MODS organ system scores zero or more, so a partial total is
  // a LOWER BOUND -- safe to rule IN, never safe to rule out. It used to rule out:
  // an empty form answered "MODS 0 of 24: ICU mortality 0%", and a creatinine of 5
  // on its own answered "MODS 3 of 24: ICU mortality ~1-2%" while five organ
  // systems went unmeasured. A missing measurement is not a normal organ. Below the
  // first mortality step the score is withheld until all six are present; at or
  // above it the reading stands, because the unmeasured systems can only add.
  const LABELS = {
    pfRatio: 'P/F ratio', creatinineMgDl: 'creatinine', bilirubinMgDl: 'bilirubin',
    par: 'pressure-adjusted heart rate', plateletsK: 'platelets', gcs: 'GCS',
  };
  const given = { pfRatio, creatinineMgDl, bilirubinMgDl, par, plateletsK, gcs };
  const missing = Object.keys(LABELS)
    .filter((k) => isBlank(given[k]) || !Number.isFinite(Number(given[k])));
  if (missing.length && score <= 4) {
    return {
      score: null,
      parts: null,
      band: `Enter all six MODS organ systems to score. Missing: ${missing.map((k) => LABELS[k]).join(', ')}. `
        + `The ${missing.length} unmeasured system${missing.length === 1 ? '' : 's'} can only add points, `
        + 'so a partial total cannot be read as a mortality estimate.',
    };
  }
  ...
  const partial = missing.length
    ? ` Scored from ${6 - missing.length} of 6 organ systems; the rest can only raise it.`
    : '';
  return {
    score,
    parts,
    partialFrom: missing.length ? 6 - missing.length : null,
    band: `MODS ${score} of 24: ${mortality}${partial}`,
  };
}
...
// spec-v1086: eight situations, and an unrated one is not "would never doze".
//
// Every item scores 0-3 and epworthClamp reads anything non-finite as 0, so the
// eight sliders resting at 0 read "Epworth 0 of 24: normal daytime sleepiness"
// for somebody who had answered nothing. Summing only the rated situations is
// the same error in slower motion: the total is a floor, and the reassuring band
// is the low one, so a partial can only understate the sleepiness.
const EPWORTH_LABELS = {
  reading: 'reading', tv: 'watching television', publicPlace: 'inactive in a public place',
  carPassenger: 'car passenger', lyingDown: 'lying down to rest', sittingTalking: 'talking while seated',
  afterLunch: 'seated quietly after lunch', carTraffic: 'in a car stopped in traffic',
};
export function epworth({ reading, tv, publicPlace, carPassenger,
  lyingDown, sittingTalking, afterLunch, carTraffic }) {
  const given = { reading, tv, publicPlace, carPassenger, lyingDown, sittingTalking, afterLunch, carTraffic };
  const unrated = Object.entries(given)
    .filter(([, v]) => v == null || (typeof v !== 'number' && String(v).trim() === ''))
    .map(([k]) => EPWORTH_LABELS[k]);
  if (unrated.length) {
    return {
      valid: false, score: null, band: null, parts: {}, unrated,
      itemsScored: 8 - unrated.length, itemsTotal: 8,
      text: `Epworth not scored: rate ${unrated.join(', ')}. All eight situations are rated 0-3; the total is a floor, so a partial one can only understate the sleepiness.`,
    };
  }
  const parts = {
  ...
  return { valid: true, score, parts, band, unrated: [], itemsScored: 8, itemsTotal: 8 };
}
...
export function bps({ facial, upperLimb, ventilatorCompliance }) {
  ...
  // spec-v930: Number('') is 0, which is finite, so a blank item slipped past this refusal and
  // was clamped to 1 ("relaxed") -- the exact default this guard exists to prevent.
  const nf = isBlank(facial) ? NaN : Number(facial);
  const nu = isBlank(upperLimb) ? NaN : Number(upperLimb);
  const nv = isBlank(ventilatorCompliance) ? NaN : Number(ventilatorCompliance);
...
// spec-v1082: every domain has to be assessed before this says "eligible".
//
// whiteSongClamp returns 0 for anything non-finite, and the tile rendered seven
// sliders parked at 2, so an untouched form read "White-Song 14 of 14:
// fast-track eligible" -- a post-anaesthesia DISCHARGE decision, for a patient
// nobody had looked at. Summing only the rated domains is no better: it
// understates the total, and the rule also fails a patient on any single domain
// below 1, so a partial can manufacture an ineligibility as easily as an
// eligibility.
const WHITE_SONG_LABELS = {
  loc: 'level of consciousness', physicalActivity: 'physical activity',
  hemodynamicStability: 'hemodynamic stability', respiratoryStability: 'respiratory stability',
  oxygenSaturation: 'oxygen saturation', postoperativePain: 'postoperative pain',
  postoperativeEmesis: 'postoperative emetic symptoms',
};
export function whiteSong({ loc, physicalActivity, hemodynamicStability,
  ...
  postoperativeEmesis }) {
  const given = { loc, physicalActivity, hemodynamicStability, respiratoryStability, oxygenSaturation, postoperativePain, postoperativeEmesis };
  const unrated = Object.entries(given)
    .filter(([, v]) => v == null || (typeof v !== 'number' && String(v).trim() === ''))
    .map(([k]) => WHITE_SONG_LABELS[k]);
  if (unrated.length) {
    return {
      valid: false, score: null, fastTrackEligible: null, anyDomainLt1: null,
      unrated, domainsScored: 7 - unrated.length, domainsTotal: 7, parts: {},
      band: `White-Song not scored: rate ${unrated.join(', ')}. Fast-track eligibility needs all seven domains -- the rule fails a patient on any single domain below 1, so a partial score can invent an eligibility or an ineligibility.`,
    };
  }
  const parts = {
  ...
  return { valid: true, score, parts, fastTrackEligible, anyDomainLt1, unrated: [], domainsScored: 7, domainsTotal: 7, band };
}
...
function mississippiClass(plateletNadir) {
  // spec-v930: a blank field is missing, not a value. Number('') is 0, which is finite.
  const p = isBlank(plateletNadir) ? NaN : Number(plateletNadir);
...
export function carpenterCoustan({ fasting, oneHour, twoHour, threeHour }) {
  const given = { fasting, oneHour, twoHour, threeHour };
  const present = (v) => !isBlank(v) && Number.isFinite(Number(v));
  const flags = {
    fasting: present(fasting) && Number(fasting) >= CARPENTER_COUSTAN_CUTOFFS.fasting,
    oneHour: present(oneHour) && Number(oneHour) >= CARPENTER_COUSTAN_CUTOFFS.oneHour,
    twoHour: present(twoHour) && Number(twoHour) >= CARPENTER_COUSTAN_CUTOFFS.twoHour,
    threeHour: present(threeHour) && Number(threeHour) >= CARPENTER_COUSTAN_CUTOFFS.threeHour,
  };
  const exceeded = Object.values(flags).filter(Boolean).length;
  const gdm = exceeded >= 2;
  const igt = exceeded === 1;

  // spec-v1228: every draw is compared against a cutoff and nothing looked at
  // the number itself, so a glucose of 20000 mg/dL was one value over cutoff and
  // the test read "impaired glucose tolerance per Carpenter 1982". A cutoff
  // comparison is monotone: past the cutoff, the size of the number stops
  // mattering, which is exactly where an entry error hides. Envelope and wording
  // from BOUNDS + boundsAdvisory; the check runs after `present`, so a draw that
  // was never taken is still reported as missing rather than as out of range.
  for (const k of Object.keys(given)) {
    if (!present(given[k])) continue;
    const fault = boundsAdvisory('glucose', Number(given[k]));
    if (fault) return { valid: false, exceeded: null, gdm: null, igt: null, flags, cutoffs: CARPENTER_COUSTAN_CUTOFFS, band: fault };
  }

  // spec-v1006: a missing draw is not a normal draw. An empty form, and a form
  // with one normal value in it, both used to answer "0 of 4 values exceed
  // cutoffs -> not diagnostic of GDM". Two abnormal values diagnose, so an
  // incomplete OGTT can DIAGNOSE and can never EXCLUDE: once two are over, the
  // unentered draws cannot take that back, but "not diagnostic" needs all four.
  const LABELS = { fasting: 'fasting', oneHour: '1-hour', twoHour: '2-hour', threeHour: '3-hour' };
  const missing = Object.keys(LABELS).filter((k) => !present(given[k]));
  if (missing.length && !gdm) {
    return {
      valid: false,
      exceeded: null, gdm: null, igt: null, flags,
      cutoffs: CARPENTER_COUSTAN_CUTOFFS,
      band: `Enter all four Carpenter-Coustan draws to read the test. Missing: ${missing.map((k) => LABELS[k]).join(', ')}. `
        + 'An incomplete 3-hour OGTT can diagnose gestational diabetes once two values are over cutoff, '
        + 'but it cannot rule it out.',
    };
  }
  ...
  if (missing.length) band += ` Read from ${4 - missing.length} of 4 draws.`;
  return {
...
export function iadpsg({ fasting, oneHour, twoHour }) {
  const given = { fasting, oneHour, twoHour };
  const present = (v) => !isBlank(v) && Number.isFinite(Number(v));
  const flags = {
    fasting: present(fasting) && Number(fasting) >= IADPSG_CUTOFFS.fasting,
    oneHour: present(oneHour) && Number(oneHour) >= IADPSG_CUTOFFS.oneHour,
    twoHour: present(twoHour) && Number(twoHour) >= IADPSG_CUTOFFS.twoHour,
  };
  const exceeded = Object.values(flags).filter(Boolean).length;
  const gdm = exceeded >= 1;

  // spec-v1228: see carpenterCoustan above. One value over cutoff diagnoses
  // here, so an impossible draw did not merely score -- it DIAGNOSED gestational
  // diabetes on its own.
  for (const k of Object.keys(given)) {
    if (!present(given[k])) continue;
    const fault = boundsAdvisory('glucose', Number(given[k]));
    if (fault) return { valid: false, exceeded: null, gdm: null, flags, cutoffs: IADPSG_CUTOFFS, band: fault };
  }

  // spec-v1006: as for Carpenter-Coustan. One abnormal value diagnoses, so a
  // single draw over cutoff stands on its own; "not diagnostic" needs all three.
  const LABELS = { fasting: 'fasting', oneHour: '1-hour', twoHour: '2-hour' };
  const missing = Object.keys(LABELS).filter((k) => !present(given[k]));
  if (missing.length && !gdm) {
    return {
      valid: false,
      exceeded: null, gdm: null, flags,
      cutoffs: IADPSG_CUTOFFS,
      band: `Enter all three IADPSG draws to read the test. Missing: ${missing.map((k) => LABELS[k]).join(', ')}. `
        + 'A single value over cutoff diagnoses gestational diabetes, so an incomplete 75-g OGTT can '
        + 'diagnose it but cannot rule it out.',
    };
  }
...
export function mgap({ mechanismBlunt, gcs, ageLt60, sbp }) {
  const g = Number(gcs);
  // spec-v1154: the same shape as gap() above -- a blank systolic scored the
  // hypotensive band, 0 of a possible 5 points, and MGAP also runs higher-is-better.
  const sbpStated = !isBlank(sbp) && Number.isFinite(Number(sbp));
  const s = sbpStated ? Number(sbp) : 0;
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new RangeError('mgap: GCS must be 3-15');
  if (sbpStated && s < 0) throw new RangeError('mgap: SBP must be a non-negative number');
  ...
  const riskOf = (v) => (v < 18 ? 'high' : (v <= 22 ? 'moderate' : 'low'));
  const risk = riskOf(score);
  if (!sbpStated) {
    const best = riskOf(score + 5);
    // spec-v1154 / rule 25, as in gap() above.
    if (best === risk) {
      return {
        score, sbpStated: false,
        parts: { mechanism: mechanismPts, gcs: g, age: agePts, sbp: null },
        risk,
        band: `${risk} risk per Sartorius 2010 (<18 high; 18-22 moderate; 23-29 low). The systolic BP is `
          + `not entered and is worth 0 to 5 points, and every value it could take leaves the band at ${risk}.`,
      };
    }
    return {
      score, sbpStated: false,
      parts: { mechanism: mechanismPts, gcs: g, age: agePts, sbp: null },
      risk: null,
      band: `The systolic BP is not entered and it is worth 0 to 5 points, so ${risk} risk is the WORST `
        + `case here -- a systolic above 120 would make it ${score + 5} and ${best} risk. Enter the systolic `
        + 'BP to read the band (Sartorius 2010: <18 high; 18-22 moderate; 23-29 low).',
    };
  }
  return {
    score,
...
export function gap({ gcs, ageLt60, sbp }) {
  const g = Number(gcs);
  // spec-v1154: the SBP guard rejects a negative and allows 0, so a blank systolic
  // read as 0 and scored the HYPOTENSIVE band -- 0 of a possible 6 points. GAP runs
  // the other way from most scores (higher is better), so that did not soften the
  // reading, it invented an alarm: 24 -> 18, and "low risk" -> "moderate risk", on
  // a blood pressure nobody had taken. Rule 6: an alarm from nothing is not the
  // safe direction either. The total is a floor and the risk shown is the worst
  // case, so it says so instead of stating one.
  const sbpStated = !isBlank(sbp) && Number.isFinite(Number(sbp));
  const s = sbpStated ? Number(sbp) : 0;
  if (!Number.isFinite(g) || g < 3 || g > 15) throw new RangeError('gap: GCS must be 3-15');
  if (sbpStated && s < 0) throw new RangeError('gap: SBP must be a non-negative number');
  ...
  const riskOf = (v) => (v <= 10 ? 'high' : (v <= 18 ? 'moderate' : 'low'));
  const risk = riskOf(score);
  if (!sbpStated) {
    const best = riskOf(score + 6);
    // spec-v1154 / rule 25: when every value the missing field could take lands in
    // the SAME band, the reading stands and says so rather than hedging.
    if (best === risk) {
      return {
        score, sbpStated: false,
        parts: { gcs: g, age: agePts, sbp: null },
        risk,
        band: `${risk} risk per Kondo 2011 (<=10 high; 11-18 moderate; 19-24 low). The systolic BP is not `
          + `entered and is worth 0 to 6 points, and every value it could take leaves the band at ${risk}.`,
      };
    }
    return {
      score, sbpStated: false,
      parts: { gcs: g, age: agePts, sbp: null },
      risk: null,
      band: `The systolic BP is not entered and it is worth 0 to 6 points, so ${risk} risk is the WORST `
        + `case here -- a systolic above 120 would make it ${score + 6} and ${best} risk. Enter the systolic `
        + 'BP to read the band (Kondo 2011: <=10 high; 11-18 moderate; 19-24 low).',
    };
  }
  return {
    score,
...
export function big({ baseDeficit, inr, gcs }) {
  // spec-v1041: the base deficit is the whole first term. Read as 0 -- a blood
  // gas nobody had drawn -- the tile answered "BIG 2.5: <16, below the Borgman
  // 2011 high-mortality threshold", which is a triage reading. The score rises
  // with every term, so what is entered is a lower bound: it can carry the
  // high-mortality reading, never the reassuring one.
  //
  // spec-v1146: that guard covered ONE of the three terms. A blank INR
  // contributes 2.5 x 0 and a blank GCS contributes 15 - 0 = 15, and with the
  // base deficit present the function went straight past this branch and printed
  // "BIG 0.0: <16 -> below the Borgman 2011 high-mortality threshold" from an INR
  // nobody had run. Every term is checked now, and the floor is built from
  // whichever ones are actually there.
  const terms = [
    ['the base deficit', baseDeficit, (v) => v],
    ['the INR', inr, (v) => 2.5 * v],
    ['the GCS', gcs, (v) => 15 - v],
  ];
  const stated = terms.filter(([, v]) => !isBlank(v) && Number.isFinite(Number(v)));
  const missing = terms.filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)));
  if (missing.length) {
    const floor = stated.reduce((acc, [, v, part]) => acc + part(Number(v)), 0);
    const names = missing.map(([label]) => label);
    const phrase = names.length === 1 ? names[0]
      : `${names.slice(0, -1).join(', ')} and ${names[names.length - 1]}`;
    const from = stated.length
      ? ` from ${stated.map(([label]) => label).join(' and ')}`
      : '';
    if (stated.length && floor >= 16) {
      return {
        score: null, parts: null, highMortalityRisk: true, incomplete: true,
        band: `BIG is at least ${floor.toFixed(1)}${from} alone, which is already >=16: `
          + `high predicted mortality per Borgman 2011. Enter ${phrase} for the total.`,
      };
    }
    return {
      score: null, parts: null, highMortalityRisk: null, incomplete: true,
      band: `BIG is at least ${floor.toFixed(1)}${from}. Enter ${phrase}: `
        + 'each is added to that total, so the below-threshold reading cannot stand without it.',
    };
  }
  ...
  // spec-v1406: the inr envelope lib/bounds.js declares; an impossible INR computed.
  const inrFault = boundsAdvisory('inr', i);
  if (inrFault) throw new RangeError(inrFault);
...
// risk, so a sum over some of the six understates the total and reads as more
// risk than the patient has -- an alarm from nothing, which spec-v1036 says is
// not the safe direction either. Reporting it the other way round, as the tile
// did with sliders parked at their maxima, gave "Braden 23: not at risk" for a
// patient nobody had assessed.
//
// There is no honest partial band on an inverted scale of this shape, so it
// asks for the subscales it has not been given instead of banding.
const BRADEN_LABELS = {
...
export function braden({ sensory, moisture, activity, mobility, nutrition, friction }) {
  const raw = { sensory, moisture, activity, mobility, nutrition, friction };
  const missing = [];
  const v = {};
  for (const [k, x] of Object.entries(raw)) {
    if (x == null || (typeof x !== 'number' && String(x).trim() === '')) { missing.push(BRADEN_LABELS[k]); continue; }
    v[k] = Number(x);
  }
  if (missing.length) {
    return {
      ...
      band: null,
      missing,
      itemsScored: 6 - missing.length,
      itemsTotal: 6,
      text: `Braden not scored: rate ${missing.join(', ')}. All six subscales are needed -- a higher total means LESS risk, so a partial sum reads as more risk than the patient has, and parking the unrated ones at their best reads as none.`,
    };
  }
...
export function ichScore({ gcs, age, ichVolumeMl, infratentorial, ivh }) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = boundsAdvisory('ageYears', a);
...
export function huntHessWfns({ huntHess, gcs, focalMotorDeficit }) {
  ...
  // WFNS IV is GCS 7-12 regardless of motor deficit (the deficit only splits
  // grades II/III at GCS 13-14, handled above), so no ternary here.
  else if (g >= 7) wfns = 4;
  else wfns = 5;
  return {
    huntHess: hh,
...
export function mnihss(answers) {
  ...
  for (const it of MNIHSS_ITEMS) {
    ...
    // spec-v1078: an unrated item is absent, not a zero -- the same reading its
    // full-scale sibling has had since spec-v1007. `Number(x || 0)` collapsed
    // the two, so an unexamined patient scored 0 and was told "no stroke
    // symptoms".
    if (raw == null || (typeof raw !== 'number' && String(raw).trim() === '')) continue;
    const v = Number(raw);
  ...
  const complete = scored === MNIHSS_ITEMS.length;
  let severity;
  ...
  // Every item adds points or leaves them alone, so a partial total is a lower
  // bound: every band above zero stays true, and exactly one reading does not.
  const unscoredExam = !complete && total === 0;
  if (unscoredExam) severity = 'not scored';
  const shortfall = complete ? '' : ` Scored from ${scored} of ${MNIHSS_ITEMS.length} items; each unrated item can only raise the total, so treat this as a floor.`;
  const band = unscoredExam
    ? `mNIHSS not scored: ${MNIHSS_ITEMS.length - scored} of ${MNIHSS_ITEMS.length} items unscored -- an unscored exam is not a normal exam. Rate the remaining items to score it per Meyer 2002.`
    : `mNIHSS ${total} of 31: ${severity} per Meyer 2002.${shortfall}`;
  return {
    total,
    severity,
    itemsScored: scored,
    itemsTotal: MNIHSS_ITEMS.length,
    complete,
    band,
  };
}
...
// meaning less risk -- and PUSH is a WOUND score that runs the other way, three
// items where 0 is a closed wound. The tile rendered all eight as sliders, five
// parked at 4 and three at 0, so an untouched form read "Norton 20 of 20 (low
// risk); PUSH 0 of 17" -- a patient at no pressure-injury risk with a healed
// wound, before anyone had looked at either.
//
// They are independent: a nurse scores Norton on any patient and PUSH only when
// there is a wound to measure. So this is spec-v1045's panel shape rather than
// an all-or-nothing refusal -- each half reports if its own items were rated,
// and is asked for if they were not. Neither half discloses a partial, because
// on both a partial sum reads the reassuring way: Norton's downward (fewer
// points than the patient has) and PUSH's downward too (a smaller wound).
const NORTON_LABELS = {
  physicalCondition: 'physical condition', mentalCondition: 'mental condition',
  activity: 'activity', mobility: 'mobility', incontinence: 'incontinence',
};
const PUSH_LABELS = {
  lengthWidthBand: 'length x width band', exudate: 'exudate amount', tissueType: 'tissue type',
};
const unratedOf = (given, labels) => Object.entries(given)
  .filter(([, v]) => v == null || (typeof v !== 'number' && String(v).trim() === ''))
  .map(([k]) => labels[k]);
...
export function nortonPush({
  ...
  const nortonGiven = { physicalCondition, mentalCondition, activity, mobility, incontinence };
  const pushGiven = { lengthWidthBand, exudate, tissueType };
  const nortonUnrated = unratedOf(nortonGiven, NORTON_LABELS);
  const pushUnrated = unratedOf(pushGiven, PUSH_LABELS);

  let norton = null; let nortonTotal = null; let nortonBand = null;
  if (!nortonUnrated.length) {
    norton = {
    ...
    nortonTotal = norton.physicalCondition + norton.mentalCondition + norton.activity + norton.mobility + norton.incontinence;
    if (nortonTotal >= 19) nortonBand = 'low risk';
    else if (nortonTotal >= 15) nortonBand = 'medium risk';
    else nortonBand = 'at risk';
  }

  let push = null; let pushTotal = null;
  if (!pushUnrated.length) {
    ...
    push = { lengthWidthBand: lw, exudate: ex, tissueType: tt };
    pushTotal = lw + ex + tt;
  }

  const said = [];
  said.push(nortonTotal === null
    ? `Norton not scored: rate ${nortonUnrated.join(', ')} (a higher total means LESS risk, so a partial sum reads as more risk than the patient has)`
    : `Norton ${nortonTotal} of 20 (${nortonBand}; <=14 at risk per Norton 1962)`);
  said.push(pushTotal === null
    ? `PUSH not scored: measure ${pushUnrated.join(', ')} (an unmeasured parameter left at 0 reads as a closed wound)`
    : `PUSH ${pushTotal} of 17 per NPIAP 2005 (declining total indicates healing)`);

  return {
    valid: nortonTotal !== null || pushTotal !== null,
    norton,
    nortonTotal,
    nortonBand,
    nortonUnrated,
...
// spec-v1083: two scales on one tile, and 0 is a finding on both.
//
// VIP 0 is "no signs of phlebitis" and INS grade 0 is "no symptoms" -- readings
// about a cannula site somebody looked at. The tile rendered both as sliders
// resting at 0, so an untouched form asserted a clean line. They are scored
// independently (a site can be graded for phlebitis without an infiltration
// grade and the other way round), so each reports if it was graded and is asked
// for if it was not -- spec-v1045's panel shape.
const absent = (x) => x == null || (typeof x !== 'number' && String(x).trim() === '');

export function vipExtravasation({ vip, insGrade, vesicant }) {
  const vipUnrated = absent(vip);
  const insUnrated = absent(insGrade);
  const v = vipUnrated ? null : Math.max(0, Math.min(5, Math.trunc(Number(vip) || 0)));
  const i = insUnrated ? null : Math.max(0, Math.min(4, Math.trunc(Number(insGrade) || 0)));
  const vesicantFlag = Boolean(vesicant);
  const banners = [];
  if (v !== null && v >= 3) banners.push('VIP >=3: remove cannula and resite per Jackson 1998 / INS 2021.');
  if (i !== null && i >= 3) banners.push('INS grade >=3: stop infusion, remove cannula immediately, photograph, escalate per INS 2021 sec 38.');
  if (i === 4 && vesicantFlag) banners.push('Grade 4 vesicant extravasation: antidote decision per INS 2021 Table 38-3.');
  const said = [
    // "not scored" deliberately: it is the phrase the shared asking vocabulary
    // already carries (test/lib/asking-language.js). Wording a refusal to fit
    // the vocabulary is the right way round -- widening the vocabulary for one
    // tile excuses every tile that phrases a rule-out the same way (spec-v1076).
    vipUnrated ? 'VIP not scored: grade the site 0-5 (0 means a site looked at and found clean)' : `VIP ${v} of 5`,
    insUnrated ? 'INS infiltration / extravasation not scored: grade the site 0-4' : `INS infiltration / extravasation grade ${i} of 4`,
  ];
  return {
    valid: !vipUnrated || !insUnrated,
    vip: v,
    insGrade: i,
    vipUnrated,
    insUnrated,
    vesicant: vesicantFlag,
    vipLabel: v === null ? null : VIP_LABELS[v],
    insLabel: i === null ? null : INS_LABELS[i],
    banners,
    text: `${said[0]}; ${said[1]}${vesicantFlag ? ' (vesicant flagged)' : ''}.`,
  };
}
...
export function insulinCorrection({
  ...
  if (!Number.isFinite(bg) || bg < 0) throw new TypeError('insulin-correction: current BG required (mg/dL)');
  if (!Number.isFinite(target) || target <= 0) throw new TypeError('insulin-correction: target BG required (mg/dL)');
  ...
  if (!Number.isFinite(computedIsf) || computedIsf <= 0) {
    ...
    if (!Number.isFinite(tdd) || tdd <= 0) {
      throw new TypeError('insulin-correction: provide isf or total daily dose');
    }
  ...
  const carbsEntered = !isBlank(carbs);
  const c = carbsEntered ? Number(carbs) : null;
  if (carbsEntered && (!Number.isFinite(c) || c < 0)) {
    throw new TypeError('insulin-correction: carbs must be zero or more');
  }
  let meal = 0;
  if (c > 0) {
    const r = Number(icr);
    if (!Number.isFinite(r) || r <= 0) {
      throw new TypeError('insulin-correction: provide a positive insulin-to-carb ratio when carbs are entered');
    }
    meal = c / r;
  }
  const total = correction + meal;
  const mealBelowDisplayResolution = meal > 0 && r1(meal) === 0;
  const mealNote = mealBelowDisplayResolution
    ? 'The entered carbohydrate coverage is greater than 0 U but below the 0.1 U display precision.'
    : null;
  return {
    isf: r1(computedIsf),
...
export function electrolyteReplacement({ electrolyte, level, route, renalImpaired }) {
  ...
  if (e === 'phos' || e === 'phosphate' || e === 'phosphorus' || e === 'po4') {
    ...
    banners.push('Recheck phos and ionized Ca 6 h after infusion per Brown 2006.');
...
export function crrtDose({
  ...
  if (!Number.isFinite(w) || w <= 0) throw new TypeError('crrt-dose: weight kg required');
  // spec-v1404: the weight envelope lib/bounds.js declares; a 5000 kg weight computed.
  { const f = boundsAdvisory('weightKg', w); if (f) throw new RangeError(f); }
  if (!Number.isFinite(r) || r < 0) throw new TypeError('crrt-dose: effluent rate ml per hr required');
  const dose = r / w;
  const banners = [];
  ...
  if (Number.isFinite(sysCa) && sysCa > 0 && (sysCa < 1.1 || sysCa > 1.2)) {
    banners.push(`Systemic ionized Ca ${r1(sysCa)} mmol/L outside 1.1-1.2 mmol/L target per Davenport 2009: titrate Ca replacement.`);
  }
  const postCa = Number(postFilterIonisedCa);
  if (Number.isFinite(postCa) && postCa > 0 && (postCa < 0.25 || postCa > 0.35)) {
    banners.push(`Post-filter ionized Ca ${r1(postCa)} mmol/L outside 0.25-0.35 mmol/L target per Davenport 2009: titrate citrate flow.`);
  }
  const total = Number(totalCa);
  let ratio;
  if (Number.isFinite(total) && Number.isFinite(sysCa) && sysCa > 0) {
    ratio = total / sysCa;
    if (ratio >= 2.5) banners.push(`Total/ionized Ca ratio ${r1(ratio)} >= 2.5: citrate accumulation suspected per Davenport 2009.`);
  }
  // spec-v1213: `Number(null)` is 0, so an ultrafiltration nobody prescribed was
  // published to the agent surface as a rate of 0 mL/h. Absent is now absent.
  const uf = measured(ultrafiltrationMlPerHr);
  return {
    effluentDoseMlPerKgPerHr: r1(dose),
    modality: modality || 'CVVH',
    ultrafiltrationMlPerHr: uf,
...
export function ecmoTitration({
  ...
  if (!Number.isFinite(w) || w <= 0) throw new TypeError('ecmo-titration: weight kg required');
  // spec-v1404: the weight envelope lib/bounds.js declares; a 5000 kg weight computed.
  { const f = boundsAdvisory('weightKg', w); if (f) throw new RangeError(f); }
  if (!Number.isFinite(sweep) || sweep < 0) throw new TypeError('ecmo-titration: current sweep lpm required');
  if (!Number.isFinite(flow) || flow <= 0) throw new TypeError('ecmo-titration: current flow lpm required');
  const mod = String(modality || 'VV').toUpperCase();
  if (mod !== 'VV' && mod !== 'VA') throw new TypeError('ecmo-titration: modality must be VV or VA');
  // spec-v1153: when an input was absent, the "suggested" setting was silently the
  // CURRENT one echoed back -- the headline read "Sweep 5 L/min / Flow 4 L/min"
  // whether the tile had titrated anything or not. A recommendation that is the
  // reader's own input, presented as a recommendation, is the worst kind of silent
  // default (rule 21): there is nothing on screen to distinguish it from a
  // computed one. Each half now says when it did not titrate, and why.
  const targetStated = Number(targetPaCO2) > 0;
  const target = targetStated ? Number(targetPaCO2) : 40;
  const paco2 = Number(currentPaCO2);
  const paco2Stated = Number.isFinite(paco2) && paco2 > 0;
  ...
  const oxygenationStated = Number.isFinite(hbN) && hbN > 0 && Number.isFinite(saO2) && saO2 > 0;
  if (oxygenationStated) {
  ...
  const banners = [];
  if (!paco2Stated) {
    banners.push(`The current PaCO2 is not entered, so the sweep has NOT been titrated: ${sweep} L/min is the setting you gave, not a suggestion.`);
  } else if (!targetStated) {
    banners.push(`Titrated to the default target PaCO2 of ${target} mmHg, because no target was entered.`);
  }
  if (!oxygenationStated) {
    const which = [
      Number.isFinite(hbN) && hbN > 0 ? null : 'the hemoglobin',
      Number.isFinite(saO2) && saO2 > 0 ? null : 'the saturation',
    ].filter(Boolean).join(' and ');
    banners.push(`DO2i needs the hemoglobin and the saturation, and ${which} ${which.includes(' and ') ? 'are' : 'is'} not entered -- so oxygen delivery has not been checked and ${flow} L/min is the pump flow you gave, not a suggestion.`);
  }
  ...
  return {
    ...
    sweepTitrated: paco2Stated,
    targetPaCO2Stated: targetStated,
    do2iMlPerKgPerMin: do2i === undefined ? null : do2i,
    suggestedFlowLpm: suggestedFlow,
    flowTitrated: oxygenationStated,
    banners,
...
export function ewsEscalation({ news2Total, vitalsTimestamp, singleParam3 }) {
  ...
  if (!Number.isFinite(total) || total < 0) throw new TypeError('ews-escalation: news2 total required');
  let nextHours;
  let banner;
  if (total >= 7) {
    nextHours = 0;
  ...
  // spec-v1172: strict, and answered in the frame it was given (see parseStamp).
  // The timestamp is optional, so a BLANK one just means no due time -- but a
  // timestamp that was entered and could not be read is a gap, not an absence,
  // and dropping it silently leaves the interval on screen with no clock.
  let nextDueIso = null;
  if (vitalsTimestamp) {
    const p = parseStamp(vitalsTimestamp);
    if (!p) throw new TypeError('Enter the time the vitals were taken as a date and a time (YYYY-MM-DDThh:mm), or leave it blank for the interval alone.');
    nextDueIso = stampOut(new Date(p.date.getTime() + nextHours * 3600_000), p.zoned);
  }
  return {
    news2Total: total,
    nextHours,
...
// spec-v1401 Part B: an optional New York OMH line. Source: 14 NYCRR 526.4(e)(5)(ii) and (7) (Cornell
// LII copy, amended effective June 4, 2014; read 2026-09-19). In an OMH-operated or licensed
// psychiatric setting each order for restraint or seclusion is no more than 4 hours for adults, 1 hour
// for ages 9 to 17, and 30 minutes under 9; manual restraint orders are 30 minutes at any age. If
// mechanical restraint or seclusion passes 2 hours (adults), 1 hour (9-17), or 30 minutes (under 9) and
// more is expected, the medical director or director of psychiatry is notified and consulted. An RN,
// NP, or PA assesses the patient at least every 30 minutes. These are stricter than the CMS intervals
// for minors, so both are shown; with the line off, nothing changes.
export function restraintTimer({ type, ageYears, orderTimestamp, nyOmh }) {
  const t = String(type || '').toLowerCase();
  const age = Number(ageYears);
  if (!Number.isFinite(age) || age < 0) throw new TypeError('restraint-timer: age years required');
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age picked a renewal cadence.
  const ageFault = boundsAdvisory('ageYears', age);
  if (ageFault) throw new RangeError(ageFault);
  // spec-v1170: `new Date('3/14/2026')` is a timestamp, not an error, so an
  // order written as a bare date came back with a face-to-face deadline of
  // 06:00 UTC -- a time of day nobody entered, on a clock counted in hours. The
  // browser's control can only send the `datetime-local` shape; this requires
  // the same of every caller (rule 18).
  const stamp = orderTimestamp ? parseStamp(orderTimestamp) : null;
  if (!stamp) throw new TypeError('Enter the time the restraint was ordered, as a date and a time (YYYY-MM-DDThh:mm).');
  const ts = stamp.date.getTime();
  const { zoned } = stamp;
  let renewalHours;
  let reassessMin;
  let faceToFaceMin;
  ...
  if (t === 'violent' || t === 'violent-self-destructive') {
    ...
    if (nyOmh === 'yes') {
      const nyMin = age >= 18 ? 240 : age >= 9 ? 60 : 30;
      const consultMin = age >= 18 ? 120 : age >= 9 ? 60 : 30;
      ny = { orderMin: nyMin, consultMin };
      banners.push(`New York OMH (14 NYCRR 526.4): each order lasts no more than ${nyMin >= 60 ? `${nyMin / 60} h` : `${nyMin} min`} at this age (manual restraint 30 min at any age); an RN, NP, or PA assesses at least every 30 min; past ${consultMin >= 60 ? `${consultMin / 60} h` : `${consultMin} min`} of mechanical restraint or seclusion, notify and consult the medical director or director of psychiatry.`);
    }
    ...
    if (nyOmh === 'yes') banners.push('The New York OMH limits (14 NYCRR 526.4) govern restraint and seclusion used for a psychiatric emergency; they are not applied to a non-violent medical-surgical restraint here.');
  } else {
    throw new TypeError('restraint-timer: type must be violent or non-violent');
  }
  const isoAfterMin = (m) => stampOut(new Date(ts + m * 60_000), zoned);
  return {
    type: t === 'violent-self-destructive' ? 'violent' : (t === 'medical-surgical' ? 'non-violent' : t),
    ageYears: age,
    orderIso: stampOut(new Date(ts), zoned),
    ...
    nyOrderExpiresIso: ny ? isoAfterMin(ny.orderMin) : null,
    nyAssessIso: ny ? isoAfterMin(30) : null,
    nyConsultIso: ny ? isoAfterMin(ny.consultMin) : null,
    banners,
  };
}
...
// spec-v1401 Part B: an optional New York line. Source: 10 NYCRR 405.4(a)(4)-(8) (Cornell LII copy,
// read 2026-09-19). Hospitals adopt evidence-based sepsis protocols, with adult and pediatric
// components, covering (i) screening and early recognition, (ii) identifying who is treated and who is
// excluded, (iii) hemodynamic support with monitoring, endpoints, and timeframe GOALS, (iv) for
// children, fluid resuscitation with explicit timeframes for access and delivery, and (v) finding the
// source and early antibiotics with timeframe goals. The regulation sets no clock of its own; the
// hospital's protocol does. Its reporting definitions: sepsis is a confirmed or suspected infection with
// two SIRS criteria; severe sepsis adds organ dysfunction.
const NY_SEPSIS_LINE = 'New York (10 NYCRR 405.4, Rory\'s Regulations): each hospital adopts sepsis protocols with adult and pediatric components -- screening and early recognition, who is treated and who is excluded, hemodynamic support with timeframe goals, pediatric fluid resuscitation with explicit timeframes, and source control with early antibiotics. The regulation sets no clock itself; your hospital\'s protocol does. The timers above are the Surviving Sepsis bundle, not New York law.';

export function sepsisBundleClock({
  t0, lactateValue, lactateTime, repeatLactateValue, repeatLactateTime,
  cultureTime, antibioticTime, fluidStartTime, vasoTime, nyState,
}) {
  const t0Stamp = parseStamp(t0);
  if (!t0Stamp) throw new TypeError('Enter the sepsis time-zero as a date and a time (YYYY-MM-DDThh:mm).');
  const t0ms = t0Stamp.date.getTime();
  const { zoned } = t0Stamp;
  function check(label, ts, dueMin) {
    const dueIso = stampOut(new Date(t0ms + dueMin * 60_000), zoned);
    if (!ts) return { label, status: 'pending', dueIso };
    const p = parseStamp(ts);
    if (!p) return { label, status: 'pending', dueIso };
    const minutes = Math.round((p.date.getTime() - t0ms) / 60_000);
  ...
  return {
    t0Iso: stampOut(new Date(t0ms), zoned),
    items,
    lactateClearancePct,
    banners,
    nyNote: nyState === 'NY' ? NY_SEPSIS_LINE : null,
  };
}
...
export function codeBlueClock({
  ...
  const startStamp = parseStamp(codeStartTimestamp);
  if (!startStamp) throw new TypeError('Enter the time the code started, as a date and a time (YYYY-MM-DDThh:mm).');
  const start = startStamp.date.getTime();
  const { zoned } = startStamp;
  const nowMs = asOf ? (parseStamp(asOf)?.date.getTime() ?? NaN) : Date.now();
  const minutesFromStart = Math.max(0, Math.round((nowMs - start) / 6000) / 10);
  const lastRhythmStamp = lastRhythmCheck ? parseStamp(lastRhythmCheck) : null;
  const lastRhythm = lastRhythmStamp ? lastRhythmStamp.date.getTime() : start;
  const nextRhythmIso = Number.isFinite(lastRhythm)
    ? stampOut(new Date(lastRhythm + 2 * 60_000), zoned)
    : null;
  const lastEpiStamp = lastEpi ? parseStamp(lastEpi) : null;
  const nextEpiIso = lastEpiStamp
    ? stampOut(new Date(lastEpiStamp.date.getTime() + 4 * 60_000), zoned)
    : null;
  // spec-v1018: the next-rhythm and next-epinephrine times are arithmetic on the
  // timestamps entered and stand whenever the page is opened. The elapsed code
  // time is measured from NOW, and it runs away: the worked example pins a start
  // of 2026-05-19, so months later the tile reported "Code time: 154215.8 min",
  // which is 107 days of CPR. A resuscitation lasts minutes; past a day the
  // number is not a reading, it is a start time that needs checking.
  const CODE_RUNAWAY_MIN = 24 * 60;
  const startRunaway = minutesFromStart > CODE_RUNAWAY_MIN;
  return {
    minutesFromStart,
    startRunaway,
    startRunawayNote: startRunaway
      ? `The code start entered is ${Math.round(minutesFromStart / 1440)} days ago. Enter the time this code started; the interval targets below are unaffected.`
      : null,
...
export function deviceDayCounter({ device, insertionTimestamp, criteriaMet, asOf }) {
  ...
  const insStamp = parseStamp(insertionTimestamp);
  if (!insStamp) throw new TypeError('Enter the time the device was inserted, as a date and a time (YYYY-MM-DDThh:mm).');
  const ins = insStamp.date.getTime();
  const { zoned } = insStamp;
  const nowMs = asOf ? (parseStamp(asOf)?.date.getTime() ?? NaN) : Date.now();
  ...
  // spec-v1173: the neighbour got this in spec-v1018 and this tile did not. The
  // device-day count is measured from NOW, so a pinned insertion time runs away:
  // the worked example reads "Device-days: 117 d 0 h" beside "remove Foley
  // today", which is not a scenario anyone wrote.
  //
  ...
  // implausibility and sets no threshold the sources do not carry. It says where
  // the number came from, which is the one thing the reader cannot see: the
  // count moves every day while the insertion time on screen does not.
  const DWELL_LONG_DAYS = 30;
  const dwellFromNow = !asOf;
  const dwellNote = dwellFromNow && deviceDays > DWELL_LONG_DAYS
    ? `Counted to now: the insertion time entered is ${deviceDays} days ago. Check it if that is not what you meant -- the count moves with the clock, the insertion time does not.`
    : null;
  const banners = [];
  if (removeToday) {
  ...
  return {
    device: d,
    insertionIso: stampOut(new Date(ins), zoned),
    deviceDays,
    deviceHours,
...
const BRISTOL_LABELS = {
  ...
  6: 'Fluffy mushy stool (mild diarrhea)',
  7: 'Liquid with no solid pieces (severe diarrhea)',
};
export function bristolGirth({
  bristolType, girthT0Cm, girthT1Cm, t0Timestamp, t1Timestamp,
}) {
  const b = Math.trunc(Number(bristolType));
  if (!Number.isInteger(b) || b < 1 || b > 7) throw new RangeError('bristol-girth: bristol type must be 1-7');
  const bristolLabel = BRISTOL_LABELS[b];
  let category;
  if (b <= 2) category = 'constipation';
  else if (b <= 4) category = 'normal';
  else if (b === 5) category = 'soft';
  else category = 'diarrhea';
  let girthDeltaCm = null;
  let intervalHours = null;
  let deltaPerHourCm = null;
  // spec-v1213: `Number(null)` and `Number('')` are both 0 and 0 is finite, so
  // this pair of guards passed for a girth nobody measured -- 95 cm at T1 with T0
  // blank read as a 95 cm rise and raised both ACS banners. `measured()` is the
  // read that keeps blank distinguishable from a typed zero.
  const g0 = measured(girthT0Cm);
  const g1 = measured(girthT1Cm);
  if (g0 !== null && g1 !== null && t0Timestamp && t1Timestamp) {
    // spec-v1170: same rule as restraintTimer above. A loosely parsed timestamp
    // gives a wrong interval, and the interval is the denominator of the cm/h
    // rate that raises the abdominal-compartment-syndrome banner.
    const a0 = localTimestamp(t0Timestamp);
    const a1 = localTimestamp(t1Timestamp);
    const dt = a0 && a1 ? a1.getTime() - a0.getTime() : NaN;
...
export function ventSbtPeep({
  ...
  // spec-v1038: a blank P/F used to fail its own check and be reported as a
  // FAILED criterion -- "SBT not ready: PaO2/FiO2 >=150" -- which reads as a
  // measured reason to keep a patient on the ventilator rather than a
  // measurement nobody had. Not-met and not-measured are now different answers.
  const num3 = (v) => (v === null || v === undefined || v === '' ? NaN : Number(v));
  const measured = { 'PaO2/FiO2 >=150': num3(pao2FiO2), 'PEEP <=8': num3(peep), 'FiO2 <=0.5': num3(fio2) };
  const unmeasured = Object.entries(measured).filter(([, v]) => !Number.isFinite(v)).map(([k]) => k);
  const sbtChecks = {
    'PaO2/FiO2 >=150':              Number.isFinite(measured['PaO2/FiO2 >=150']) && measured['PaO2/FiO2 >=150'] >= 150,
    'PEEP <=8':                     Number.isFinite(measured['PEEP <=8']) && measured['PEEP <=8'] <= 8,
    'FiO2 <=0.5':                   Number.isFinite(measured['FiO2 <=0.5']) && measured['FiO2 <=0.5'] <= 0.5,
    'No or minimal vasopressors':   !vasopressors,
    'Awake / cooperative':          Boolean(awakeCooperative),
  };
  const failed = Object.entries(sbtChecks)
    .filter(([k, ok]) => !ok && !unmeasured.includes(k)).map(([k]) => k);
  const sbtReady = failed.length === 0 && unmeasured.length === 0;
  ...
  if (Number.isFinite(lookF) && lookF > 0 && lookF <= 1.0) {
    ...
    if (armNorm !== 'low' && armNorm !== 'high') throw new TypeError('vent-sbt-peep: ards arm must be low or high');
    arm = armNorm;
    ...
    throw new RangeError('vent-sbt-peep: lookup fi O2 must be a fraction 0-1');
  }
  const banners = [];
  ...
  else if (failed.length) banners.push(`SBT not ready (Boles 2007): ${failed.join('; ')}.`);
  if (unmeasured.length && !failed.length) {
    banners.push(`Readiness not assessed: enter ${unmeasured.join('; ')}. `
      + 'The criteria entered so far are met, but a value nobody measured is not a criterion that failed.');
  } else if (unmeasured.length) {
    banners.push(`Also not measured: ${unmeasured.join('; ')}.`);
  }
...
export function hypothermiaRewarm({ coreTempC, state, ecprExclusion = false, potassium }) {
  ...
  if (!Number.isFinite(t)) throw new TypeError('hypothermia-rewarm: core temp C must be a number');
  if (t < 0 || t > 38) throw new RangeError('hypothermia-rewarm: core temp C must be 0-38');
...
export function heatstrokeDecision({ coreTempC, cns, sweating = true, setting = 'field' }) {
  const t = Number(coreTempC);
  if (!Number.isFinite(t)) throw new TypeError('heatstroke-decision: core temp C must be a number');
  if (t < 35 || t > 47) throw new RangeError('heatstroke-decision: core temp C must be 35-47');
...
// 7-10 severe.
// spec-v930: a blank field and an absent field mean the same thing. Number('') is 0, which is
// an integer, so without this an unanswered item scored as a zero while a missing one threw.
function isBlank(value) {
  return value === null || value === undefined || String(value).trim() === '';
}
function checkOrdinal(name, key, value, max) {
  if (isBlank(value)) throw new TypeError(`${name}: ${key} must be an integer`);
...
// spec-v1082: zero is the middle of this scale, not the absence of an answer.
//
// N-PASS runs -2 to +2 per item: negative is sedation, positive is pain, and 0
// is "neither". The tile rendered five sliders sitting at 0, so a neonate nobody
// had assessed read "pain 0 (no significant pain); sedation 0 (no sedation)" --
// a reading about a patient who cannot report either.
const NPASS_LABELS = {
  crying: 'crying / irritability', behavior: 'behavior / state', facial: 'facial expression',
  extremities: 'extremities / tone', vitals: 'vital signs',
};
export function npass({ crying, behavior, facial, extremities, vitals, gestationalAgeWeeks }) {
  const given = { crying, behavior, facial, extremities, vitals };
  const unrated = Object.entries(given)
    .filter(([, v]) => v == null || (typeof v !== 'number' && String(v).trim() === ''))
    .map(([k]) => NPASS_LABELS[k]);
  if (unrated.length) {
    return {
      valid: false, painScore: null, sedationScore: null, painBand: null, sedationBand: null,
      unrated, itemsScored: 5 - unrated.length, itemsTotal: 5,
      text: `N-PASS not scored: rate ${unrated.join(', ')}. Each item runs -2 to +2 with 0 meaning neither pain nor sedation, so an unrated item left at 0 reads as an assessment that found nothing.`,
    };
  }
  ...
  if (!Number.isFinite(ga) || ga < 20 || ga > 44) {
    throw new RangeError('npass: gestational age weeks must be 20-44');
  }
...
export function wat1({
  ...
  // spec-v1047: WAT-1 is the paediatric sibling of CIWA-Ar and COWS, and it had
  // the defect those two lost in spec-v1028: every item rendered parked at 0, so
  // the tile opened on "WAT-1 0 of 12: no significant withdrawal" -- the reading
  // that defers a wean adjustment -- for a child nobody had assessed. Its items
  // were SLIDERS, which cannot be blank, so the fix is a control change as well
  // as a guard; see the closing section of docs/spec-v1029.md.
  //
  // The scale is monotone: each item adds a point or none. So a partial total is
  // a lower bound, safe to carry the withdrawal reading (>= 3) and never the
  // reassuring one.
  const items = {
    looseStools, vomiting, fever, sbsStatePositive, tremor, sweating,
    uncoordinatedMovement, yawnSneeze, startleToTouch, increasedMuscleTone,
  };
  const unrated = Object.keys(items).filter((k) => isBlank(items[k]));
  const rated = 10 - unrated.length;
  const recoveryBlank = isBlank(recoveryMinutes);
  if (unrated.length || recoveryBlank) {
    const partial = Object.values(items)
      .filter((v) => !isBlank(v))
      .reduce((sum, v) => sum + checkOrdinal('wat-1', 'item', v, 1), 0);
    if (partial < 3) {
      const outstanding = unrated.length + (recoveryBlank ? 1 : 0);
      return {
        score: null,
        partial,
        rated,
        ...
        withdrawal: null,
        band: 'not scored',
        text: `WAT-1 is at least ${partial} from ${rated} of 10 items`
          + `${recoveryBlank ? ' (the recovery time is not entered either)' : ''}. `
          + `Rate the remaining ${outstanding}: each can only add points, so a partial score `
          + 'cannot support the no-withdrawal reading per Franck 2008 (>=3 indicates withdrawal).',
      };
    }
  }
  const ord = (key, v) => (isBlank(v) ? 0 : checkOrdinal('wat-1', key, v, 1));
  const ls = ord('looseStools', looseStools);
  const v = ord('vomiting', vomiting);
  const f = ord('fever', fever);
  const sbsP = ord('sbsStatePositive', sbsStatePositive);
  const tr = ord('tremor', tremor);
  const sw = ord('sweating', sweating);
  const um = ord('uncoordinatedMovement', uncoordinatedMovement);
  const ys = ord('yawnSneeze', yawnSneeze);
  const st = ord('startleToTouch', startleToTouch);
  const mt = ord('increasedMuscleTone', increasedMuscleTone);
  const rmRaw = isBlank(recoveryMinutes) ? 0 : Number(recoveryMinutes);
  if (!Number.isFinite(rmRaw) || rmRaw < 0) {
    throw new RangeError('wat-1: recovery minutes must be a non-negative number');
  }
  let recoveryPoints;
  ...
  const footing = (unrated.length || recoveryBlank)
    ? ` Scored from ${rated} of 10 items${recoveryBlank ? ' without the recovery time' : ''}; the rest can only raise it.`
    : '';
  ...
  return {
    ...
    withdrawal,
    band,
    text: `WAT-1 ${score} of 12: ${band} per Franck 2008 (>=3 indicates iatrogenic opioid/benzodiazepine withdrawal).${footing}`,
  };
}
...
const MEOWS_LABELS = {
  ...
  sbp: 'systolic BP', dbp: 'diastolic BP', hr: 'heart rate', pain: 'the pain score',
};
export function meows(input) {
  const { rr, spo2, temp, sbp, dbp, hr, neuro, pain } = input;
  // spec-v1036: the same guard NEWS2 got in spec-v930, for the same reason.
  // Every band helper below is an if/else chain with no missing-value branch, so
  // a set of observations nobody had taken scored 0 for each and came back "MEOWS:
  // trigger (5 red, 0 yellow). Activate the obstetric MEOWS response" -- a
  // rapid-response call invented from an empty chart. An alarm from nothing is
  // not the safe direction; it is a different wrong answer.
  // spec-v1086: the pain score is a MEOWS observation like the other six, and it
  // was left out of this set because its control was a slider that could not be
  // blank. With the slider gone it can be, so it joins them: an unrecorded pain
  // score is not a pain score of nought.
  const need = { rr, spo2, temp, sbp, dbp, hr, pain };
  const missing = Object.entries(need)
    .filter(([, v]) => isBlank(v) || !Number.isFinite(Number(v)))
    .map(([k]) => k);
  if (missing.length) {
    return {
      flags: {}, redCount: 0, yellowCount: 0, trigger: null, band: 'not scored',
      missing,
      // spec-v1086: name the ones actually outstanding rather than the whole set.
      // A fixed sentence listing six observations was already slightly wrong
      // when only one was missing, and became wronger when the pain score joined
      // them and went unmentioned.
      text: `Enter ${missing.map((k) => MEOWS_LABELS[k] || k).join(', ')}: `
        + 'MEOWS triggers on the observations themselves, so a missing one cannot be scored either way.',
    };
  }
  // spec-v1200: this function's own comment above is the argument -- "An alarm
  // from nothing is not the safe direction; it is a different wrong answer" --
  // and it was applied to the empty chart and not to the impossible observation.
  // Every MEOWS band saturates, so a systolic of 3000 flags one red the same way
  // a survivable extreme does, and the tile answered "MEOWS: trigger (1 red, 0
  // yellow). Activate the obstetric MEOWS response".
  //
  // Returned in the same shape as the missing branch above, so the page reads it
  // the way it already reads that one, and with `valid: false` so the refusal
  // reaches an agent as a refusal rather than as a successful call.
  for (const [key, v] of [['rr', rr], ['sbp', sbp], ['dbp', dbp], ['hr', hr], ['temperature', temp]]) {
    const fault = boundsAdvisory(key, Number(v));
    if (fault) {
      return {
        valid: false, flags: {}, redCount: 0, yellowCount: 0, trigger: null,
        band: 'not scored', missing: [], text: fault,
      };
    }
  }
...
export function cssrs({
  ...
  if (behaviorPast3Months && !behaviorLifetime) {
    throw new TypeError('cssrs: behavior Past3 months cannot be true if behavior lifetime is false');
  }
  const parts = {
    wishDead, thoughtsKilling, thoughtsMethods,
    someIntent, planIntent, behaviorLifetime, behaviorPast3Months,
...
// spec-v1081: an unrated item is not a dependency, and it is not independence.
//
// Both indices count the items the patient is INDEPENDENT in, so a sum over some
// of them understates independence and reads as more impairment than the patient
// has -- an alarm from nothing (spec-v1036). Rendering the unrated ones as
// sliders parked at 1, which is what the tile did, reads as the opposite: "full
// independence" for somebody nobody assessed. Neither is a reading the form has
// earned, so it asks.
export function lawtonIadl(input) {
  ...
  for (const key of LAWTON_ITEMS) {
    const v = input[key];
    if (v == null || (typeof v !== 'number' && String(v).trim() === '')) { unrated.push(key); continue; }
    if (!Number.isInteger(v) || (v !== 0 && v !== 1)) {
  ...
  if (unrated.length) {
    return {
      valid: false, score: null, band: null, parts, unrated,
      itemsScored: LAWTON_ITEMS.length - unrated.length, itemsTotal: LAWTON_ITEMS.length,
      text: `Lawton IADL not scored: rate ${unrated.join(', ')}. The index counts the activities the patient manages alone, so a partial sum reads as more impairment than they have, and leaving the rest at "independent" reads as none.`,
    };
  }
  let band;
  ...
  return { valid: true, score, parts, band, unrated: [], itemsScored: LAWTON_ITEMS.length, itemsTotal: LAWTON_ITEMS.length, text };
}
...
export function katzAdl(input) {
  ...
  if (unrated.length) {
    return {
      valid: false, score: null, band: null, parts, unrated,
      itemsScored: KATZ_ITEMS.length - unrated.length, itemsTotal: KATZ_ITEMS.length,
      text: `Katz ADL not scored: rate ${unrated.join(', ')}. The index counts the activities the patient manages alone, so a partial sum reads as more impairment than they have, and leaving the rest at "independent" reads as none.`,
    };
  }
  let band;
  ...
  return { valid: true, score, parts, band, unrated: [], itemsScored: KATZ_ITEMS.length, itemsTotal: KATZ_ITEMS.length, text };
}
...
//
// GUSS stops by design: semisolid is only trialled if the indirect swallow
// scores 5, liquid only if semisolid does, solid only if liquid does. A stopped
// protocol is a COMPLETE assessment with a low total, and the tile must keep
// saying so -- refusing every partial screen would refuse most real ones.
//
// What it must not do is what it did. Seventeen sliders, each parked at its best
// value, meant an untouched form read "GUSS 20 of 20 (slight / no dysphagia).
// Normal diet, normal liquids; no further investigation" -- a swallow screen
// clearing a stroke patient for a normal diet before a spoonful of water.
//
// So each consistency is one of three things: SCORED, GATED (the stage before it
// did not reach 5, so the protocol correctly stopped), or OUTSTANDING (the gate
// opened and nobody recorded the trial). A total is only a GUSS when nothing is
// outstanding.
const GUSS_STAGE1 = ['vigilance', 'coughClear', 'salivaSwallow', 'salivaNoDrool', 'salivaNoVoiceChange'];
const GUSS_STAGE1_LABELS = {
  vigilance: 'vigilance', coughClear: 'voluntary cough / throat clearing',
  salivaSwallow: 'saliva swallow', salivaNoDrool: 'drooling', salivaNoVoiceChange: 'voice change',
};
const gussAbsent = (x) => x == null || (typeof x !== 'number' && String(x).trim() === '');
const gussSubtestRated = (prefix, input) => ['Swallow', 'NoCough', 'NoDrool', 'NoVoiceChange']
  .every((suffix) => !gussAbsent(input[`${prefix}${suffix}`]));

export function guss(input) {
  const stage1Unrated = GUSS_STAGE1.filter((k) => gussAbsent(input[k])).map((k) => GUSS_STAGE1_LABELS[k]);
  if (stage1Unrated.length) {
    return {
      valid: false, score: null, stage1: null, band: null,
      semisolid: null, liquid: null, solid: null, gated: [], pending: [], outstanding: ['the indirect swallow'],
      stage1Unrated,
      text: `GUSS not scored: rate ${stage1Unrated.join(', ')}. The indirect swallow comes first and gates the rest -- no consistency is trialled until it scores 5 of 5.`,
    };
  }
  for (const k of GUSS_STAGE1) gussCheckBinary('guss', k, input[k]);
  ...
  const pending = [];
  let total = stage1;
  let semisolidEff = null;
  let liquidEff = null;
  let solidEff = null;
  // Walk the stages in order, carrying whether the gate in front of us opened.
  //
  // Two ways to stop, and they are not the same thing. A stage that SCORED
  // below 5 gates the ones after it -- Trapl's protocol says not to trial them,
  // so "not performed" is the finished, correct state. A stage nobody recorded
  // leaves the ones after it merely PENDING: the protocol has not said anything
  // about them yet, and calling them gated would credit the instrument with a
  // decision it did not make.
  let open = stage1 >= 5;
  let interrupted = false;
  for (const [prefix, name] of [['semisolid', 'semisolid'], ['liquid', 'liquid'], ['solid', 'solid']]) {
    if (interrupted) { pending.push(name); continue; }
    if (!open) { gated.push(name); continue; }
    if (!gussSubtestRated(prefix, input)) { outstanding.push(name); interrupted = true; continue; }
    const scored = gussSubtest(prefix, input);
    total += scored;
    if (prefix === 'semisolid') semisolidEff = scored;
    else if (prefix === 'liquid') liquidEff = scored;
    else solidEff = scored;
    open = scored >= 5;
  }
  if (outstanding.length) {
    return {
      valid: false, score: null, stage1, band: null,
      semisolid: semisolidEff, liquid: liquidEff, solid: solidEff,
      gated, pending, outstanding, stage1Unrated: [],
      text: `GUSS not scored: the ${outstanding[0]} trial is outstanding. The stage before it scored 5 of 5, so the protocol continues -- give the ${outstanding[0]} trial and score it. A total that stops here is not a GUSS, and the diet recommendation depends on it.`,
    };
  }
  let band;
