...
import { r1, localTimestamp } from './num.js';
...
export function restraintTimer({ type, ageYears, orderTimestamp }) {
  ...
  const dt = orderTimestamp ? localTimestamp(orderTimestamp) : null;
  if (!dt) throw new TypeError('Enter the time the restraint was ordered, as a date and a time (YYYY-MM-DDThh:mm).');
  const ts = dt.getTime();
  let renewalHours;
  let reassessMin;
  let faceToFaceMin;
...
export function codeBlueClock({
  ...
  if (!Number.isFinite(start)) throw new TypeError('code-blue-clock: code start timestamp required');
...
export function deviceDayCounter({ device, insertionTimestamp, criteriaMet, asOf }) {
  ...
  if (!Number.isFinite(ins)) throw new TypeError('device-day-counter: insertion timestamp required');
  const nowMs = asOf ? new Date(asOf).getTime() : Date.now();
