...
export function grace({ age, heartRate, sbp, creatinineMgDl,
  ...
  const missing = Object.entries(need)
    ...
    .map(([k]) => k);
  if (missing.length) {
    return { score: null, band: 'Enter age, heart rate, systolic BP, creatinine and Killip class to score.', missing };
  }
  let pts = 0;
  // Age points (banded)
  if      (age < 30)  pts += 0;
...
export function oakland({ age, sex, priorLgibAdmission, dreBlood,
  ...
  const missing = Object.entries(need)
    ...
    .map(([k]) => k);
  if (missing.length) {
    return { score: null, parts: null, band: 'Enter age, heart rate, systolic BP and hemoglobin to score.', missing };
  }
  const parts = {
