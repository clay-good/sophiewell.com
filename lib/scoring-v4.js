...
export function big({ baseDeficit, inr, gcs }) {
  ...
  // with the deficit, so what is entered is a lower bound: it can carry the
  // high-mortality reading, never the reassuring one.
  if (isBlank(baseDeficit) || !Number.isFinite(Number(baseDeficit))) {
    // A blank INR or GCS is absent here too: `Number('')` is 0, which would make
    // a missing GCS contribute the full 15 points (test/mcp/blank-is-absent).
    const iPart = isBlank(inr) ? NaN : Number(inr);
    const gPart = isBlank(gcs) ? NaN : Number(gcs);
    const floor = (Number.isFinite(iPart) ? 2.5 * iPart : 0) + (Number.isFinite(gPart) ? 15 - gPart : 0);
    if (floor >= 16) {
      return {
        score: null, parts: null, highMortalityRisk: true, incomplete: true,
        band: `BIG is at least ${floor.toFixed(1)} from the INR and GCS alone, which is already >=16: `
          + 'high predicted mortality per Borgman 2011. Enter the base deficit for the total.',
      };
    }
    return {
      score: null, parts: null, highMortalityRisk: null, incomplete: true,
      band: `BIG is at least ${floor.toFixed(1)} from the INR and GCS. Enter the base deficit: `
        + 'it is added to that total, so the below-threshold reading cannot stand without it.',
    };
  }
