...
export function methacholine(input = {}) {
  ...
  const metric = oneOf(METRICS, o.metric, null);
  const value = num(o.value);

  // spec-v1465: a blank metric is not PD20. The number is a dose in micrograms or a concentration
  // in mg/mL, and the same 50 reads mild as one and normal as the other.
  if (metric === null) {
    return {
      valid: false,
      message: value === null
        ? 'Choose the metric (PD20, a dose in micrograms, or PC20, a concentration in mg/mL) and enter its value.'
        : `Choose which metric ${value} is: a PD20 in micrograms and a PC20 in mg/mL are read against different cutpoints.`,
    };
  }
