...
import { r1, r2, gradeFault } from './num.js';
import { BOUNDS, boundsAdvisory } from './bounds.js';

// spec-v1040: `Number(null)` and `Number('')` are both 0, and 0 is finite, so
// these three coercions turned a value nobody entered into a measurement of
// zero. The renderer passes null for a blank field, so `stewartSidSig`'s
// "enter sodium, potassium ..." guard could never fire and the tile answered
// "SIG -122.2 mEq/L ... no excess unmeasured anion" from one lab.
const blank = (v) => v === null || v === undefined || (typeof v !== 'number' && String(v).trim() === '');
const pos = (v) => {
  const n = blank(v) ? NaN : (typeof v === 'number' ? v : Number(v));
...
export function stewartSidSig(input = {}) {
  ...
  // spec-v1178: albumin is a weak acid in the Figge-Fencl effective-SID term, so
  // the g/L figure inflates the effective SID and drives the strong-ion gap
  // negative -- "no excess unmeasured strong anions", from an albumin nobody has.
  if (alb > BOUNDS.albumin.max) {
    return { valid: false, message: `A serum albumin of ${alb} g/dL is above ~${BOUNDS.albumin.max}, beyond recorded extremes. An albumin reported in g/L is ten times the g/dL figure, so ${alb} g/L is entered as ${alb / 10}.` };
  }
  // spec-v1181: and every OTHER analyte here reaches the same gap. Guarding the
  // albumin alone left `ss-hco3` at 600 mEq/L still reading "no excess
  // unmeasured strong anions" -- one row of the probe's queue, on the tile the
  // wave before had already opened. Guard the set, not the field that was
  // reported.
  for (const [key, v] of [['sodium', na], ['potassium', k], ['chloride', cl],
    ['lactate', lactate], ['bicarbonate', hco3], ['phosphate', phos]]) {
...
// spec-v1198: `stewartSidSig` above ends on the rule -- "Guard the set, not the
// field that was reported" (spec-v1181) -- and the four gas functions beside it
// in this same file never got it. A pH of 80, a bicarbonate of 600 and a PaCO2
// of 2000 all computed, and printed things like "Base excess +1928.4 mEq/L: a
// base excess, consistent with a metabolic alkalosis".
//
// The check runs AFTER the missing-value check, never before it. `pos()` returns
// null for blank, for non-numeric AND for out-of-range alike, so folding the
// envelope into it would tell a reader to "Enter arterial pH" about the pH they
// just typed -- the failure mode `scripts/probe-envelope-unbounded.mjs` keeps its
// own section for.
function outsideEnvelope(pairs) {
  for (const [key, value] of pairs) {
    const fault = boundsAdvisory(key, value);
...
export function baseExcess(input = {}) {
  ...
  const impossible = outsideEnvelope([['pH', ph], ['bicarbonate', hco3], ['hemoglobin', hb]]);
  if (impossible) return { valid: false, message: impossible };
...
// spec-v1137: `o.chronic === true || o.chronic === 'true'` made every other value
// -- an unstated one included -- into ACUTE, and the band then printed the word:
// "Expected HCO3 26 mEq/L (acute, +1 per 10 mmHg)". That is a statement about how
// long this patient's respiratory acidosis has been going on, made by the tool,
// printed as though somebody had said it (rule 11).
//
// Both notes on these two tiles say, in as many words, "The acute-versus-chronic
// choice is yours, not inferred" -- and the refusal message lists "choose acute
// or chronic" among the things it needs, while the guard let the choice through
// missing (rule 23: when a guard's message lists what it needs, the list is a
// claim). Three statements of the same promise, none of them kept.
//
// The choice does not always decide. Where the measured bicarbonate sits within
// tolerance of BOTH the acute and the chronic expectation -- or outside both on
// the same side -- the verdict is the same either way and the tile answers,
// naming both expectations rather than one assumed one (rule 25).
const CHRONICITY_STATED = (v) => v === true || v === 'true' || v === 'chronic'
  || v === false || v === 'false' || v === 'acute';
const chronicityAsked = (what) => 'Choose acute or chronic: the two are compensated at different '
  + `rates, and on these numbers they disagree about whether ${what}.`;
export function respAcidosisCompensation(input = {}) {
  const o = obj(input);
  const paco2 = pos(o.paco2);
  const hco3 = pos(o.bicarbonate);
  const chronic = o.chronic === true || o.chronic === 'true' || o.chronic === 'chronic';
  const stated = CHRONICITY_STATED(o.chronic);
  if (paco2 === null || hco3 === null) {
    // Rule 23: the list is a claim. It named the acute/chronic choice whether or
    // not that was the thing missing, and did not require it either.
    const owed = [paco2 === null ? 'the measured PaCO2 (mmHg)' : null,
      hco3 === null ? 'the measured HCO3 (mEq/L)' : null].filter(Boolean);
    return {
      valid: false,
      message: `Enter ${owed.join(' and ')}${stated ? '' : ', and choose acute or chronic'}.`,
    };
  }
  const impossible = outsideEnvelope([['paCO2', paco2], ['bicarbonate', hco3]]);
  if (impossible) return { valid: false, message: impossible };
  const expectedFor = (isChronic) => {
    const raw = 24 + ((isChronic ? 4 : 1) * (paco2 - 40)) / 10;
    // clamp to a physiologic ceiling (compensation rarely lifts HCO3 past ~45)
    return r1(Math.min(45, Math.max(10, raw)));
  };
  const measured = r1(hco3);
  const verdictFor = (isChronic) => {
    const e = expectedFor(isChronic);
    return measured > e + 2 ? 'alkalosis' : (measured < e - 2 ? 'acidosis' : 'appropriate');
  };
  if (!stated && verdictFor(false) !== verdictFor(true)) {
    return {
      valid: false,
      expectedAcute: expectedFor(false),
      expectedChronic: expectedFor(true),
      message: `${chronicityAsked('the bicarbonate is appropriately compensated')} Acute expects `
        + `${expectedFor(false)} mEq/L and chronic expects ${expectedFor(true)} mEq/L, against a `
        + `measured ${measured} mEq/L.`,
      note: RESP_ACID_NOTE,
    };
  }
  const expected = expectedFor(chronic);
  ...
  return {
    valid: true, expected, measured, chronic, chronicityStated: stated,
    abnormal: added,
    band: stated
      ? `Expected HCO3 ${expected} mEq/L (${chronic ? 'chronic, +4 per 10 mmHg' : 'acute, +1 per 10 mmHg'}): ${cmp}.`
      : `Expected HCO3 ${expectedFor(false)} mEq/L acute or ${expectedFor(true)} mEq/L chronic: `
        + `the measured ${measured} mEq/L reads the same either way -- ${cmp.replace(/^the measured [^,]+, /, '')}.`,
    note: RESP_ACID_NOTE,
  };
}
...
export function respAlkalosisCompensation(input = {}) {
  ...
  const impossible = outsideEnvelope([['paCO2', paco2], ['bicarbonate', hco3]]);
  if (impossible) return { valid: false, message: impossible };
  const expectedFor = (isChronic) => r1(Math.max(
    isChronic ? 12 : 18,
    24 - ((isChronic ? 4 : 2) * (40 - paco2)) / 10,
  ));
  const measured = r1(hco3);
  const verdictFor = (isChronic) => {
    const e = expectedFor(isChronic);
  ...
  return {
    ...
    abnormal: added,
    band: stated
      ? `Expected HCO3 ${expected} mEq/L (${chronic ? 'chronic, -4 per 10 mmHg, floor ~12' : 'acute, -2 per 10 mmHg, floor ~18'}): ${cmp}.`
      : `Expected HCO3 ${expectedFor(false)} mEq/L acute or ${expectedFor(true)} mEq/L chronic: `
        + `the measured ${measured} mEq/L reads the same either way -- ${cmp.replace(/^the measured [^,]+, /, '')}.`,
    note: RESP_ALK_NOTE,
  };
...
export function metAlkalosisCompensation(input = {}) {
  ...
  const impossible = outsideEnvelope([['bicarbonate', hco3], ['paCO2', paco2]]);
  if (impossible) return { valid: false, message: impossible };
...
export function urineOsmolalGap(input = {}) {
  ...
  // spec-v1210: the sibling gas functions in this module have had the envelope
  // guard since spec-v1198 and this one never got it -- the row
  // `probe-unguarded-sibling` prints the module for.
  //
  // These are URINE values, so lib/bounds.js is the wrong table for every one of
  // them (spec-v1205: an envelope is a claim about a quantity in a unit AND a
  // compartment -- `BOUNDS.sodium` is serum sodium at 90-200, while a urine
  // sodium of 20 is normal). Each ceiling below is the assay's own: urine
  // osmolality tops out near the maximal concentrating ability of the kidney,
  // urine electrolytes are reported to a few hundred mmol/L, and urine urea
  // nitrogen and glucose to a few thousand mg/dL.
  //
  // Without it, a urine sodium of 99,999 produced a gap of -199,665 mOsm/kg and
  // the tool went on to reason about its sign.
  const urineFault = gradeFault([
    ['the measured urine osmolality in mOsm/kg', measured, 0, 1600],
    ['the urine sodium in mmol/L', na, 0, 400],
    ['the urine potassium in mmol/L', kx, 0, 400],
    ['the urine urea nitrogen in mg/dL', urea, 0, 5000],
    ['the urine glucose in mg/dL', glucose, 0, 20000],
  ]);
  if (urineFault) return { valid: false, message: urineFault };
  const calc = 2 * (na + kx) + urea / 2.8 + glucose / 18;
  const gap = measured - calc;
  const calcR = r1(calc);
  // spec-v1202: `+ 0` normalises negative zero. A measured osmolality equal to the
  // calculated one lands a hair below zero in floating point and rounds to `-0`,
  // which prints as "0" but is not `0` to anything comparing it.
  const gapR = r1(gap) + 0;
  const nh4 = r1(gap / 2) + 0;

  // spec-v1202: half the gap approximates urinary ammonium, and NOTHING here
  // stopped that half being NEGATIVE. A urine sodium entered as 2000 gave
  //
  ...
  // attached. And the reading is the WRONG one to reach for: every negative gap
  // lands in the narrow band, so an entry error reads as renal tubular acidosis.
  //
  // The serum envelopes in lib/bounds.js do not apply to a urine chemistry (urine
  // sodium runs well outside the serum range in health), so no ceiling is
  // invented here. What is said is only what the arithmetic already shows: the
  // calculated osmolality came out above the measured one, and a gap is a
  // difference that cannot be less than nothing.
  // On the ROUNDED gap: a measured osmolality equal to the calculated one comes
  // out a hair below zero in floating point, and a gap of nothing is a real
  // reading (the narrow band) rather than an entry error.
  if (gapR < 0) {
    return {
      valid: false,
      message: `The urine osmolal gap comes out negative (${gapR} mOsm/kg), and half a negative gap is not an `
        + 'ammonium concentration. Every negative gap falls in the narrow band, which reads as renal tubular '
        + `acidosis, so this is an entry to check rather than a result to read. Compare the measured osmolality `
        + `(${r1(measured)} mOsm/kg) with the calculated one (${calcR} mOsm/kg).`,
      calc: calcR,
      gap: gapR,
      note: UOG_NOTE,
    };
