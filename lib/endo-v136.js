...
// spec-v1229: `pos` accepts any positive number, and every reading in this file
// is either a log of one or a comparison against a cutoff -- so an impossible
// value did not look impossible from inside the formula. A fasting glucose of
// 20000 mg/dL gave "QUICKI 0.1859" and "TyG index 14.22", each printed beside
// its own reference range as if it were one more reading; on
// `metabolic-syndrome` it simply met the glucose criterion, as did a systolic BP
// of 3000 mmHg and a diastolic of 2000. Envelope and wording from BOUNDS +
// boundsAdvisory; each check runs after the function's own missing-value branch
// (spec-v1207), so a blank field is still asked for.
const envelopeFault = (rows) => {
...
export function quicki(input = {}) {
  ...
  const gFault = envelopeFault([['glucose', glucose]]);
  if (gFault) return gFault;
...
export function metabolicSyndrome(input = {}) {
  ...
  const msFault = envelopeFault([['sbp', sbp], ['dbp', dbp], ['glucose', glucose]]);
  if (msFault) return msFault;
...
export function osteoporosisPrescreen(input = {}) {
  ...
  const rangeMsg = gradeFault([['the age in years', o.age, 0, 130], ['the weight in kg', o.weight, null, Infinity]]);
  ...
  if (weightFault) return { valid: false, message: weightFault };
