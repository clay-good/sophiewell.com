...
// spec-v931: a dose is a mass, and a mass is not negative. Without this the conversion is
// arithmetically consistent and clinically meaningless -- "-100 mg hydrocortisone" converted
// cleanly to "-25 mg prednisone". Zero is refused for the same reason a zero-dose conversion
// answers nothing.
function positiveDose(v) {
  return typeof v === 'number' && Number.isFinite(v) && v > 0;
}

export function steroidEquivalent({ drug, doseMg, target, table }) {
  if (!positiveDose(doseMg)) return null;
...
export function vasopressorRateMlHr({ dose, units, weightKg, concUgPerMl }) {
  ...
  let ugPerMin;
  if (units === 'mcg/kg/min') {
    if (!(weightKg > 0)) throw new RangeError('weight kg required for mcg/kg/min');
...
export function tpnMacro({ volumeMl, dextrosePct, aminoAcidPct, lipidPctOfVolume = 0,
  ...
  if (!(volumeMl > 0)) throw new RangeError('volume ml must be positive');
...
export function beersCheck({ ageYears, medications = [], comorbidities = [] }) {
  ...
  if (!Number.isFinite(age)) throw new TypeError('beers-check: age years must be a number');
  if (age < 18 || age > 120) throw new RangeError('beers-check: age years must be 18-120');
  if (!Array.isArray(medications)) throw new TypeError('beers-check: medications must be an array');
  if (!Array.isArray(comorbidities)) throw new TypeError('beers-check: comorbidities must be an array');
  const meds = [...new Set(medications)];
  const cmb = [...new Set(comorbidities)];
  for (const m of meds) {
    if (!Object.prototype.hasOwnProperty.call(BEERS_PIM, m)) throw new TypeError(`beers-check: unknown medication category '${m}'`);
  }
  for (const c of cmb) {
    if (!Object.prototype.hasOwnProperty.call(BEERS_DISEASE, c)) throw new TypeError(`beers-check: unknown comorbidity '${c}'`);
  }
