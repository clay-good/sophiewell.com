...
import { num, r1, r2, isRealYmd, gradeFault } from './num.js';
...
export function fickCardiacOutput(input = {}) {
  ...
  // spec-v1235: the rows read above are the ones BOTH methods use, so the check
  // goes here rather than inside either branch -- the transformer that wrote the
  // other five in this file put it inside the `estimated` arm, where a measured
  // Fick never reaches it. `gradeFault` skips a blank, so the `missing` list
  // below still runs and still names every empty field at once; bounds and
  // labels are the caller's own.
  const fault = gradeFault([
    ['hemoglobin (g/dL)', o.hb, null, 25],
    ['BSA (m²)', o.bsa, null, 4],
  ]);
...
export function gorlin(input = {}) {
  ...
  const fault = gradeFault([
    ['cardiac output (L/min)', o.co, null, 20],
    ['heart rate (bpm)', o.hr, null, 300],
    ['ejection/filling period (s per beat)', o.period, null, 1],
    ['mean pressure gradient (mmHg)', o.grad, null, 200],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function lvotStrokeVolume(input = {}) {
  ...
  if (bsa === null) missing.push('BSA (m²)');
  const fault = gradeFault([
    ['LVOT diameter (cm)', o.d, null, 5],
    ['LVOT VTI (cm)', o.vti, null, 60],
    ['heart rate (bpm)', o.hr, null, 300],
    ['BSA (m²)', o.bsa, null, 4],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function matsudaIndex(input = {}) {
  ...
  const fault = gradeFault([
    ['fasting glucose (mg/dL)', o.g0, null, 1000],
    ['fasting insulin (µU/mL)', o.i0, null, 1000],
    ['mean OGTT glucose (mg/dL)', o.gMean, null, 1000],
    ['mean OGTT insulin (µU/mL)', o.iMean, null, 1000],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
// {day, inr}, and report the lines it could not read.
//
// spec-v1170: both halves of that used to be silent, and this field is the whole
// input to the tile -- a reader who typed a line meant it to count.
//
//   `2026-1-11 2.5`  the shape test wants two digits, so an ordinary way of
//                    writing a date was DROPPED. A 20-day record became 13 of 20
//                    days in range: TTR 80% -> 65%, landing exactly on the
//                    good-control threshold, with nothing on screen to say a
//                    measurement had been ignored.
//   `2026-02-30 2.5` passed the shape test, and `Date.UTC(2026, 1, 30)` is
//                    2026-03-02 rather than an error, so 20 days became 60 and
//                    TTR 80% -> 88.3%.
//
// A line the parser cannot read is a gap, not an absence (rule 1). It returns
// the bad line NUMBERS so the tile can name them instead of quietly re-scoring.
function parseSeries(text) {
  if (typeof text !== 'string') return { rows: [], bad: [] };
  const rows = [];
  const bad = [];
  ...
  for (let i = 0; i < lines.length; i += 1) {
    ...
    if (!line) continue;
    const m = line.match(/^(\d{4})-(\d{1,2})-(\d{1,2})[\s,]+([0-9]*\.?[0-9]+)$/);
    const y = m && Number(m[1]);
    const mo = m && Number(m[2]);
    const d = m && Number(m[3]);
    const inr = m && Number(m[4]);
    if (!m || !isRealYmd(y, mo, d) || !Number.isFinite(inr) || inr <= 0) {
      bad.push({ n: i + 1, text: line });
      continue;
    }
    rows.push({ day: Date.UTC(y, mo - 1, d) / DAY_MS, inr });
  }
  return { rows, bad };
}
...
export function rosendaalTtr(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  // spec-v1217: `pos()` returns null for a value off its scale and `?? 2.0` then
  // substituted the DEFAULT -- so a target INR low of 999999 was silently read as
  // 2.0 and the time-in-range was computed against a range nobody asked for. A
  // target INR of 999999 is not a target; it is a typo, and it gets said out
  // loud. `gradeFault` skips a genuinely blank field, so the documented 2.0-3.0
  // default still applies when the reader leaves it alone.
  const targetFault = gradeFault([
    ['the target INR low', o.low, null, 10],
    ['the target INR high', o.high, null, 15],
  ]);
  if (targetFault) return { valid: false, message: targetFault, band: targetFault };
  ...
  const { rows: parsed, bad } = parseSeries(o.series);
  if (bad.length) {
    const which = bad.slice(0, 3).map((b) => `line ${b.n} ("${b.text}")`).join(', ');
    return {
      valid: false,
      message: `Could not read ${bad.length === 1 ? 'one line' : `${bad.length} lines`} of the INR record — ${which}${bad.length > 3 ? ', and more' : ''}. Enter one date and one INR per line, as "YYYY-MM-DD INR", using a real calendar date. Nothing is scored until every line reads, because a skipped line changes the time in range without saying so.`,
    };
  }
  const rows = parsed.sort((a, b) => a.day - b.day);
  if (rows.length < 2) {
...
export function leanBodyWeight(input = {}) {
  ...
  const fault = gradeFault([
    ['total body weight (kg)', o.weight, null, 400],
    ['height (cm)', o.height, null, 260],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
