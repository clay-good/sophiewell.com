...
export function crusade(input = {}) {
  ...
  const fault = gradeFault([
    ['baseline hematocrit (%)', o.hct, null, 70],
    ['creatinine clearance (mL/min)', o.crcl, null, 300],
    ['heart rate (bpm)', o.hr, null, 300],
    ['systolic blood pressure (mmHg)', o.sbp, null, 320],
  ]);
...
export function scaiShock(input = {}) {
  ...
  const fault = gradeFault([
    ['systolic blood pressure (mmHg)', o.sbp, null, 320],
    ['serum lactate (mmol/L)', o.lactate, 0, 40],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
