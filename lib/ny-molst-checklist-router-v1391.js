...
// Review: the MOLST form itself, DOH-5003 (06/25), Section I "Review and Renewal" (health.ny.gov,
// read 2026-09-19 with a browser user agent): a physician, NP, or PA reviews it at least every 90
// days, and also when the patient moves to another location for care, has a major change in health
// status, or the patient or decision-maker changes their mind. A MOLST not reviewed within 90 days
// stays valid and must be followed. For a patient with I/DD, only a physician may do the Section I
// review.
//
// Pure: no DOM, no clock, no network.

export const MOLST_VERIFIED = '2026-09-19';
export const YES_NO = [
  { value: 'yes', text: 'Yes' },
  { value: 'no', text: 'No' },
];
...
export function nyMolstChecklistRouter(input = {}) {
  ...
  return {
    ...
    band: `${CL[key]}. ${why}`,
    reviewNote: `A physician, NP, or PA reviews the MOLST at least every 90 days, and also when the patient moves to another location for care, has a major change in health status, or the patient or decision-maker changes their mind. A MOLST past its 90-day review is still valid and must be followed (DOH-5003, Section I).${key === 'opwdd' ? ' For a patient with I/DD, only a physician may do this review.' : ''}`,
