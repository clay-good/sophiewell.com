...
export function calciumPhosphateProduct(input = {}) {
  ...
  // the OUTPUT unit ("Ca x PO4 57 mg2/dL2"), never the one it had assumed for the
  // inputs. Calcium and phosphate in mmol/L are 4.008 and 3.097 times smaller, so
  // a pair entered in SI with the selector missed reads about twelve times low --
  // and 55 is the threshold this tile exists to place them against.
  //
  // Said the way `salicylate-toxicity` and `urine-protein-creatinine` already say
  // it: name the assumption and give the factor to the other unit.
  const unitStated = typeof o.unit === 'string' && CAPO4_UNIT[o.unit];
  const unit = unitStated ? o.unit : 'mg-dl';
  ...
  return {
    ...
    unit,
    unitStated: !!unitStated,
    band: `Ca × PO₄ ${product} mg²/dL² — ${high ? 'above' : 'at or below'} the historical 55 caution threshold`
      + `${unitStated ? '' : ' (no unit given; the calcium and phosphate were read as mg/dL, and the same pair in mmol/L would be about 12.4 times this)'}.`,
