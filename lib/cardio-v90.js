...
export function lvhCriteria({
  sV1, rV5, rV6, sV3, rAVL, sex, sD, sV4,
} = {}) {
  ...
  //
  // spec-v1149: the criterion is SV1 + the LARGER of RV5 and RV6, so one of the
  // two is enough to form a sum -- but that sum is a FLOOR, because the lead
  // nobody measured may be the larger one. Read as a total it ruled OUT: with
  // SV1 20 and RV5 10 entered and RV6 blank, the tile said "No LVH voltage
  // criterion met by the entered amplitudes" on a sum of 30, where an RV6 of 16
  // makes the real sum 36 and the criterion met. Rule 3: an incomplete score may
  // rule in, never rule out. At or above 35 the reading stands whatever the
  // missing lead turns out to be (rule 13), so only the negative waits.
  const sokolowComplete = sv1 != null && (rv5 != null || rv6 != null);
  const sokolowPartial = sokolowComplete && (rv5 == null || rv6 == null);
  const sokolowSum = sokolowComplete ? sv1 + Math.max(rv5 || 0, rv6 || 0) : null;
  const sokolowMet = sokolowSum == null ? null
    : (sokolowSum >= 35 ? true : (sokolowPartial ? null : false));
  const sokolowMissingLead = sokolowPartial ? (rv5 == null ? 'RV5' : 'RV6') : null;
  ...
  // spec-v1457: Peguero-Lo Presti (J Am Coll Cardiol 2017) -- the deepest S wave in any lead plus
  // the S wave in V4, positive at >= 2.3 mV in women and >= 2.8 mV in men (23 and 28 mm at the
  // standard 10 mm/mV the other two criteria assume). Stated identically in open papers (for
  // example Clin Cardiol 2023, PMC10765998; Rev Cardiovasc Med 2022, PMC11262329). Both leads and
  // the sex are needed; one lead alone is asked for, never read as a sum.
  const sd = mag(sD);
  const sv4 = mag(sV4);
  const pegueroSum = sd != null && sv4 != null ? sd + sv4 : null;
  const pegueroThreshold = sexStated ? (female ? 23 : 28) : null;
  const pegueroComplete = pegueroSum != null && sexStated;
  const pegueroMet = pegueroComplete ? pegueroSum >= pegueroThreshold : null;
  const pegueroOneLead = (sd == null) !== (sv4 == null);

  if (!sokolowComplete && !cornellComplete && !pegueroComplete) {
    return {
      valid: false,
      band: 'Enter the precordial and limb-lead amplitudes (S in V1, R in V5/V6, S in V3, R in aVL, or the deepest S wave and S in V4) and the sex.',
      note: 'Sokolow-Lyon uses SV1 + max(RV5, RV6) >= 35 mm; Cornell voltage uses SV3 + RaVL > 28 mm (men) or > 20 mm (women).',
    };
  }

  // spec-v1149: a partial Sokolow sum below the threshold is not a criterion
  // that failed -- it is one still being formed, so it must not read as "no
  // criterion met" alongside a complete Cornell that also did not meet.
  const sokolowUndecided = sokolowMet === null && sokolowSum != null;
  const anyMet = sokolowMet === true || cornellMet === true || pegueroMet === true;
  const metNames = [];
  if (sokolowMet === true) metNames.push('Sokolow-Lyon');
  if (cornellMet === true) metNames.push('Cornell voltage');
  if (pegueroMet === true) metNames.push('Peguero-Lo Presti');

  let band;
  if (anyMet) {
    band = `Voltage criteria for LVH positive: ${metNames.join(' and ')}.`;
  } else if (sokolowUndecided) {
    band = `No LVH voltage criterion met by the entered amplitudes -- but ${sokolowMissingLead} is not entered, `
      + `and Sokolow-Lyon uses SV1 + the LARGER of RV5 and RV6, so ${r1(sokolowSum)} mm is a floor: `
      + `${r1(35 - sokolowSum)} mm more in ${sokolowMissingLead} would meet it.`;
  } else if (sokolowComplete || cornellComplete || pegueroComplete) {
  ...
  if (band && pegueroOneLead) {
    band += ` Enter ${sd == null ? 'the deepest S wave in any lead' : 'the S wave in V4'} as well to read Peguero-Lo Presti.`;
  } else if (band && pegueroSum != null && !sexStated) {
    band += ` The Peguero-Lo Presti sum is ${r1(pegueroSum)} mm; its threshold is sex-specific (>= 28 mm men, >= 23 mm women).`;
  }

  return {
    valid: true,
    pegueroSum: pegueroSum == null ? null : r1(pegueroSum),
    pegueroMet,
    pegueroThreshold,
    ...
    anyMet,
    band,
    // spec-v1457: with no sex entered this read "> null mm (men)"; name both thresholds instead.
    note: `Sokolow-Lyon: SV1 + max(RV5, RV6) >= 35 mm. ${sexStated
      ? `Cornell voltage: SV3 + RaVL > ${cornellThreshold} mm (${female ? 'women' : 'men'}). Peguero-Lo Presti: deepest S in any lead + SV4 >= ${pegueroThreshold} mm (${female ? 'women' : 'men'}).`
      : 'Cornell voltage: SV3 + RaVL > 28 mm (men) or > 20 mm (women). Peguero-Lo Presti: deepest S in any lead + SV4 >= 28 mm (men) or >= 23 mm (women).'} Voltage criteria are specific but not sensitive for LVH; a negative ECG does not exclude it, and the echocardiogram remains the reference standard.`,
  };
}
...
export function timiStemi({
  ...
  const a = fin(age);
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age scored its 3 points.
  const ageFault = a == null ? null : boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, total: null, mortality: null, band: ageFault, message: ageFault };
