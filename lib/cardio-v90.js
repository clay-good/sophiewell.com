...
export function timiStemi({
  ...
  const mortality = ageMissing ? null : (total > 8 ? TIMI_MORTALITY_HIGH : TIMI_MORTALITY[total]);
  ...
  const nothingEntered = ageMissing && total === 0;
  return {
    ...
    mortality,
    band: ageMissing
      ? `TIMI-STEMI at least ${total} of 14 -- enter age to read the 30-day mortality (age adds up to 3 points, so the mortality read off a blank age is an underestimate).`
