...
export function hemodynamicSuite({
  ...
  // spec-v1474: stroke work index = SVI x the pressure the ventricle works against. Studies report it
  // bare, in mmHg·mL/m², or x 0.0136 to convert to g·m/m²; both are given (the units are disputed).
  const lvswiMmhg = svi != null && ma != null && pcw != null ? svi * (ma - pcw) : null;
  const rvswiMmhg = svi != null && mpa != null && cv != null ? svi * (mpa - cv) : null;
  ...
  return {
    ...
    lvswi: lvswiMmhg == null ? null : r1(lvswiMmhg * 0.0136),
    lvswiMmhg: lvswiMmhg == null ? null : Math.round(lvswiMmhg),
    rvswi: rvswiMmhg == null ? null : r1(rvswiMmhg * 0.0136),
    rvswiMmhg: rvswiMmhg == null ? null : Math.round(rvswiMmhg),
    note: 'Normal ranges: CI 2.5-4.0 L/min/m², SV 60-100 mL, SVI 33-47 mL/m², SVR 800-1200 dynes·s·cm⁻⁵, PVR under 2 Wood units (ESC/ERS 2022). The ×80 factor converts mmHg/(L/min) to dynes·s·cm⁻⁵. Stroke work index is SVI × (MAP − PCWP) for the left ventricle and SVI × (mPAP − CVP) for the right; studies report it in mmHg·mL/m² or multiply by 0.0136 for g·m/m², and the correct units are disputed, so both are shown and no normal range is applied. Decision support, not a diagnosis of shock type; the cardiogenic-vs-distributive-vs-obstructive judgment stays with the clinician.',
  };
}
...
export function deadSpace({ paco2, expiredCo2, source } = {}) {
  ...
  // Only the arterial PaCO2: the expired CO2 is a different measurement and
  // BOUNDS names no envelope for it.
  const dsFault = envelopeFault([['paCO2', pa]]);
  if (dsFault) return dsFault;
