...
export function pfiq7(input = {}) {
  ...
  // spec-v1113: the denominator, in the reader's words. Silent when every scale
  // is complete, because there is nothing to disclose then.
  const countsText = (a) => {
    const short = [['uiq', 'UIQ-7'], ['craiq', 'CRAIQ-7'], ['popiq', 'POPIQ-7']]
      .filter(([k]) => a[k] < 7)
      .map(([k, label]) => `${label} over ${a[k]} of its 7 items`);
    return short.length ? `Scored from ${short.join(', ')}.` : '';
  };
  ...
  return {
    ...
    // spec-v1113: each scale is a MEAN over the items that were answered, which
    // is the published rule and correct -- but the denominator was computed,
    // stored in `answered`, and never shown. A mean of two items and a mean of
    // seven print the same way, so "UIQ-7 33.33 of 100" carried no sign of which
    // it was. This is spec-v1044's rule (a rating scale must say how many items
    // it scored) on a tile that already had the number to hand.
    itemCounts: countsText(answered),
    detail: `${countsText(answered) ? `${countsText(answered)} ` : ''}Each scale is the mean of its answered items multiplied by 100 divided by 3, so each runs 0 to 100; the summary adds the three, so it runs 0 to 300. Higher is more interference with daily life. The 2005 source publishes no severity cutoff, so this tile reports the scores and asserts no band; the scales are most useful compared against the same patient before and after treatment.`,
    note: PFIQ7_NOTE,
  };
}
