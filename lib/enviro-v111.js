...
export function lakeLouiseAms(input = {}) {
  ...
  // spec-v1209: each symptom is rated 0-3 and `clampInt` mapped anything above
  // that to 3 in silence, so a headache rated 9999 scored the same as the worst
  // headache the scale has -- and 12/12 "severe AMS" from four such ratings.
  // The 2018 Lake Louise scale's own range; nothing clinical decided here.
  //
  // After the missing-value branch (spec-v1207): the four-symptom sentence above
  // still answers a blank field.
  const fault = gradeFault([
    ['the headache rating', headache, 0, 3],
    ['the GI-symptom rating', gi, 0, 3],
    ['the fatigue/weakness rating', fatigue, 0, 3],
    ['the dizziness rating', dizziness, 0, 3],
  ]);
  if (fault) return { valid: false, band: fault, note: LLS_NOTE };
...
const SSS_NAMES = {
  pulmonary: 'pulmonary', cardiovascular: 'cardiovascular', local: 'local wound',
  gi: 'GI', hematologic: 'hematologic', cns: 'CNS',
};

export function snakebiteSeverity(input = {}) {
  const keys = Object.keys(SSS_MAX);
  const subs = {};
  // spec-v1105: WHICH systems were examined, as distinct from what they scored.
  //
  // The adapter has required all six since spec-v1073 -- its own summary says
  // "an unexamined system is not a system scored 0" -- so no agent could reach
  // this. The LIBRARY still read a missing sub-score as a zero, and the browser
  // goes through the library, so the sentence that rule was written to prevent
  // was printing on the page the whole time:
  //
  ...
  // sentence a complete examination with five normal systems produces. This is
  // spec-v1103's rule 18 from the third side: the guard was in the ADAPTER, and
  // an adapter is one surface too. It belongs here, where both of them look.
  const unscored = [];
  for (const k of keys) {
    const v = fin(input[k]);
    if (v == null) unscored.push(k);
    subs[k] = v == null ? 0 : clampInt(v, 0, SSS_MAX[k]);
  }
  if (unscored.length === keys.length) {
  ...
  // spec-v1209: each system has its own maximum in SSS_MAX (3 or 4) and
  // `clampInt` mapped anything higher to it without a word, so a pulmonary
  // subscore of 9999 scored the severest pulmonary envenomation the scale has.
  //
  // After the nothing-entered branch, and it SKIPS the systems that were not
  // examined, which is the distinction the note above is about.
  const subFault = gradeFault(keys.map((k) => [`the ${SSS_NAMES[k]} subscore`, fin(input[k]), 0, SSS_MAX[k]]));
  if (subFault) return { valid: false, band: subFault, note: SSS_NOTE };
  ...
  // The SSS is a sum, so a partial one is a FLOOR. The upper third rules in and
  // needs no footing (rule 13); every gentler descriptor -- and "no envenomation
  // findings scored" most of all -- is a reading an unexamined system can undo.
  // The parts list below already names the unexamined systems, so this says the
  // rule and not the roll-call -- one paragraph, not two.
  const tierText = (unscored.length && total <= 13)
    ? `a floor, not a severity reading -- scored from ${keys.length - unscored.length} of the `
      + '6 body systems, and the unexamined ones can only raise it'
    : tier;
  const parts = keys
    .map((k) => `${SSS_NAMES[k]} ${unscored.includes(k) ? 'not examined' : subs[k]}`)
    .join(', ');
  return {
    valid: true, total, subs, unscored,
    severe: total >= 14,
    abnormal: total >= 14,
    band: `SSS ${total}/20: ${tierText} (${parts}). A continuous index -- higher is more severe; Dart 1996 sets no fixed cutoff.`,
    note: SSS_NOTE,
  };
}
...
export function cauchyFrostbite(input = {}) {
  ...
  // spec-v1105: the grade is the MOST SEVERE of three findings, so a finding
  // that has not been made can only raise it -- and every grade below 4 is
  // therefore a floor. `boneScan: 'not-done'` is the reader saying so; an
  // omitted key is an agent not saying anything, and the two arrive here the
  // same way. Grade 4 is the ceiling and rules in (rule 13), so it is quiet.
  //
  // This is the ordinary case rather than the defective one -- on day 0 there
  // are no day-2 findings at all -- so it discloses rather than refusing
  // (rule 12). What it must not do is print "no amputation, no sequela" as
  // though the outstanding tests had come back.
  const outstanding = [];
  if (bone == null || input.boneScan === 'not-done') outstanding.push('the day-2 bone scan');
  if (blist == null) outstanding.push('the day-2 blister character');
  const floorNote = (grade < 4 && outstanding.length)
    ? ` Graded on what has been recorded: ${outstanding.join(' and ')} `
      + `${outstanding.length === 1 ? 'is' : 'are'} outstanding, and the grade is the most severe `
      + 'of the three findings, so this is a floor that either can only raise.'
    : '';
  return {
    valid: true, grade, prognosis: CAU_PROGNOSIS[grade], outstanding,
    abnormal: grade >= 3,
    band: `Cauchy grade ${grade} (set by ${drivers.join(' + ') || 'topography'}): ${CAU_PROGNOSIS[grade]}.${floorNote}`,
    note: CAU_NOTE,
  };
}
