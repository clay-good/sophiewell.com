...
export function ttkg({ urineK, plasmaK, urineOsm, plasmaOsm, urineNa }) {
  ...
  if (plasmaK < 3.5) {
    band = value < 2
      ? 'Hypokalemia with TTKG <2: appropriate renal K conservation (extrarenal loss)'
      : 'Hypokalemia with TTKG >2-4: renal potassium wasting';
  } else if (plasmaK > 5.0) {
    band = value < 7
...
// These are deficit ESTIMATES, not infusion rates. Over-rapid correction of
// hyponatremia (>8 mEq/L/24h) risks osmotic demyelination.
