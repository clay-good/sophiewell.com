...
export function kingsCollege({
  ph, lactate, lactateTiming,
  // spec-v1102: no `= 'no'` default here. A default parameter fires when the
  // argument is ABSENT, so it erased the difference between "nobody said" and
  // "observed absent" before any guard could see it -- the same trap spec-v930
  // hit from the other side, where a blank STRING walked past the default.
  inr, pt, creatinine, creatinineUnit, encephalopathy,
} = {}) {
  // spec-v930: a default parameter only fires when the argument is absent, so a BLANK
  // encephalopathy arrived as '' rather than 'no' -- which walked past the nothing-entered
  // guard below and produced a verdict from an empty form.
  //
  // spec-v1102: normalising a blank to 'no' keeps that guard working and makes
  // the blank indistinguishable from an observed absence of encephalopathy --
  // and 'no' is what makes the three-part limb FAIL. So an agent that omitted
  // the key turned "Meets King's College Criteria: poor prognosis, refer/list
  // for transplant" into "Does not meet". The limb already has an INCOMPLETE
  // state for a missing INR or creatinine; the encephalopathy was excluded from
  // it by the comment below, which assumed a select that always has a value.
  // That is true of the browser and false of every API caller.
  const encephProvided = !(encephalopathy === null || encephalopathy === undefined
    || String(encephalopathy).trim() === '');
  if (!encephProvided) {
    encephalopathy = 'no';
  }
  const phV = pos(ph);
  ...
  const creatinineUnitProvided = creatinineUnit === 'mg/dl' || creatinineUnit === 'umol/l';
  const lactateTimingProvided = lactateTiming === 'early' || lactateTiming === 'resuscitated';
  ...
  // spec-v1406: the INR and pH envelopes lib/bounds.js declares; an impossible value met a limb.
  const kcFault = (inrV == null ? null : boundsAdvisory('inr', inrV)) || (phV == null ? null : boundsAdvisory('pH', phV));
  if (kcFault) return { valid: false, band: kcFault, message: kcFault, note: 'The King’s College Criteria (acetaminophen pathway) read the arterial pH limb, the three-part coagulopathy/renal/encephalopathy limb, and the modified lactate limb.' };
  ...
  const creatHigh = cr == null || !creatinineUnitProvided ? null
    : (creatinineUnit === 'umol/l' ? cr > 300 : cr > 3.4);
  // The three-part limb needs all three components present. A missing one makes
  // the limb "incomplete" rather than a (false) negative.
  const threePartComplete = coag != null && cr != null && creatinineUnitProvided && encephProvided;
  const threePartMet = threePartComplete && coag && creatHigh && enceph34;

  // Modified (Bernal) lactate limb.
  const lactateThreshold = lactateTimingProvided ? (lactateTiming === 'early' ? 3.5 : 3.0) : null;
  const lactateLimb = lac != null && lactateThreshold != null ? lac > lactateThreshold : null;
  ...
  } else if ((threePartComplete === false && (inrV != null || ptV != null || cr != null || encephProvided))
      || (lac != null && !lactateTimingProvided)) {
    const wanted = [
      coag == null && 'the INR or prothrombin time',
      cr == null && 'the creatinine',
      cr != null && !creatinineUnitProvided && 'the creatinine unit',
      !encephProvided && 'the encephalopathy grade',
    ].filter(Boolean);
    const incomplete = [];
    if (threePartComplete === false && (inrV != null || ptV != null || cr != null || encephProvided)) {
      incomplete.push(`the three-part limb is incomplete (enter ${wanted.join(' and ')} to evaluate it)`);
    }
    if (lac != null && !lactateTimingProvided) {
      incomplete.push('the modified lactate limb is incomplete (enter whether the sample was early or after fluid resuscitation)');
    }
    band = `Criteria not met by the entered complete limbs; ${incomplete.join('; ')}.`;
  } else {
  ...
  return {
    ...
    coag,
    creatHigh,
    creatinineUnitProvided,
    enceph: enceph34,
    encephProvided,
    threePartComplete,
    threePartMet,
    lactateLimb,
    lactateTimingProvided,
    band,
