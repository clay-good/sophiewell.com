...
export function kingsCollege({
  ...
  inr, pt, creatinine, creatinineUnit = 'mg/dl', encephalopathy,
} = {}) {
  ...
  const threePartComplete = coag != null && creatHigh != null && encephProvided;
  ...
  if (meets) {
    ...
    const wanted = [
      ...
      creatHigh == null && 'the creatinine',
      !encephProvided && 'the encephalopathy grade',
    ].filter(Boolean);
    band = `Criteria not met by the entered limbs; the three-part limb is incomplete (enter ${wanted.join(' and ')} to evaluate it).`;
  } else {
