...
const SCOPE_OF_TABLE_NOTE = 'This is average-risk surveillance after polypectomy. It does not cover a personal or family history that puts a patient on a different schedule, or inflammatory bowel disease.';
const SCOPE_NOTE = 'This applies a published interval table to findings already recorded. It does not decide when a patient is scheduled.';

export function polypSurveillance(input = {}) {
  const o = input && typeof input === 'object' ? input : {};

  // spec-v1193: the histology was read through `oneOf(HISTOLOGY, o.histology,
  // 'none')`, and the first row of that table is "No polyps found". So a
  // histology nobody had entered read as a colonoscopy that found nothing, and
  // the tile answered:
  //
  //   Next colonoscopy in 10 years, on a normal examination.
  //
  // The worked example -- a 12 mm adenoma -- is a THREE-year interval. Dropping
  // the one field turned it into ten, and named a finding ("a normal
  // examination") that nobody had recorded. That is not a shorter answer, it is
  // a different patient.
  //
  // Rule 13: two things outrank the table, and where either holds the answer is
  // the same whatever the histology turns out to be -- so it is not asked for.
  // Piecemeal resection of a lesion 20 mm or larger is its own six-month track,
  // and an examination that did not reach the cecum, or had an inadequate
  // preparation, selects no interval at all.
  const tableApplies = !on(o.piecemealTwentyMm) && on(o.completeToCecum) && on(o.adequatePreparation);
  if (tableApplies && !known(HISTOLOGY, o.histology)) {
    return {
      valid: false,
      message: 'Choose the worst histology found. A blank is not "no polyps found" -- read that way it selects the ten-year interval and calls the examination normal, which is the longest interval in the table and the one thing nobody has recorded.',
      scopeOfTableNote: SCOPE_OF_TABLE_NOTE,
      scopeNote: SCOPE_NOTE,
      note: POLYP_NOTE,
    };
  }

  const histology = known(HISTOLOGY, o.histology) ? o.histology : 'none';
  ...
  const scopeOfTableNote = SCOPE_OF_TABLE_NOTE;

  const recordedNote = `Recorded: ${!known(HISTOLOGY, o.histology) ? 'no histology entered' : histology === 'none' ? 'no polyps' : HISTOLOGY.find((h) => h.value === histology).text.toLowerCase()}, ${count === null ? 'no adenoma count entered' : `${count} adenoma${count === 1 ? '' : 's'}`}, largest ${largest === null ? 'not entered' : `${largest} mm`}.`;
