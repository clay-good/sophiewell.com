...
// spec-v1234: the bilirubin here is one of the Paris/Toronto criteria and is
// compared against a fixed 1.0 mg/dL threshold; past it the size stops
// mattering. A bilirubin here is used as a RATIO to its own upper limit of
...
// the tile's own missing-value branch (spec-v1207). Only the bilirubin is checked: the ALP and AST
// on this tile are ratios to their own ULN and BOUNDS names neither enzyme.
...
export function udcaResponse(input = {}) {
  ...
  const biliFault = envelopeFault([['bilirubin', bilirubin]]);
