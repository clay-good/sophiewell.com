...
const fin = (v) => {
  // spec-v1042: `Number(null)` is 0 and 0 is finite -- a POP-Q point of 0 cm is a
  // real measurement (at the hymen), so a blank one arriving as zero placed an
  // unmeasured point exactly at the hymen and staged the prolapse from it. See
  // docs/spec-v1040.md; this is the same trap in the gynaecology module.
...
export function popqStaging(input = {}) {
  ...
  // spec-v1411: every point is a distance from the hymen along a vagina TVL long,
  // so none can sit farther than TVL from it -- and Aa/Ap and Ba/Bp have the ranges
  // Bump 1996 defines for them ("the range of position of point Aa relative to the
  // hymen is -3 to +3 cm"; Ba "is at -3 cm in the absence of prolapse" and rises at
  // most to the cuff). Unchecked, an Aa of 50 staged IV at "+50 cm", and a D of
  // -999999 -- the one optional point -- changed nothing on screen at all.
  const fault = gradeFault([
    ['Point Aa (cm)', vals.Aa, -3, 3], ['Point Ba (cm)', vals.Ba, -3, tvl],
    ['Point C (cm)', vals.C, -tvl, tvl], ['Point D (cm)', vals.D, -tvl, tvl],
    ['Point Ap (cm)', vals.Ap, -3, 3], ['Point Bp (cm)', vals.Bp, -3, tvl],
  ]);
