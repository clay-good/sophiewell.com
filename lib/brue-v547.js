...
// colour-change event in an infant who is AFEBRILE and WELL-APPEARING by the time they are seen. Reaching
...
// the infant needs individualised assessment. This tile does not diagnose the cause of an event, does not
// recommend or exclude any investigation, and cannot detect the concerning historical or examination
// features that criteria 6 and 7 turn on -- those are clinical judgements the tile takes as given
...
export function brue(input = {}) {
  ...
  return {
    ...
    band: lowerRisk
      ...
      : `Higher-risk, because the criteria are conjunctive and ${failed.length === 1 ? 'one criterion is' : `${failed.length} criteria are`} not met: ${failed.map((c) => c.text).join('; ')}. Higher-risk is not a diagnosis and not an admission order; it means the lower-risk pathway does not apply and the infant needs individualised assessment.`,
    note: NOTE,
  };
}
