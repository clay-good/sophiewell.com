...
export function findrisc(input = {}) {
  ...
  // spec-v1166: `o.sex === 'male' ? 'male' : 'female'` made an UNSTATED sex a
  // female one, and the waist bands are sex-specific -- 94/102 cm for men, 80/88
  // for women. A waist of 90 cm scores 0 points as a man and 4 as a woman, which
  // on this worked example is FINDRISC 9 ("slightly elevated, ~4%") against 13
  // ("moderate, ~17%"). A silent default of a known fact, in the alarming
  // direction, and the same shape spec-v1116 fixed on lvh-criteria's Cornell
  // threshold.
  //
  // The waist term is the only one that reads the sex, so the rest of the score
  // stands and it is that term that waits -- spec-v1045's "answer with the halves
  // you have". Where both bands give the same points the reading is decided
  // whatever the sex turns out to be (rule 25) and says so plainly.
  const sexStated = o.sex === 'male' || o.sex === 'female';
  const sex = o.sex === 'male' ? 'male' : 'female';
  // spec-v1406: an out-of-range value parsed to null and was asked for as if blank. Name the range instead.
  const rangeMsg = gradeFault([
    ['the age in years', o.age, null, 130],
    ['the BMI in kg/m^2', o.bmi, null, 100],
    ['the waist in cm', o.waist, null, 300],
  ]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
  if (age === null || bmi === null || waist === null) return { valid: false, note: FINDRISC_NOTE };
  const waistMale = findriscWaistPts(waist, 'male');
  const waistFemale = findriscWaistPts(waist, 'female');
  const waistDecided = sexStated || waistMale === waistFemale;
  ...
  if (!waistDecided) {
    // Re-score both ways: everything but the waist term is shared.
    const rest = score - findriscWaistPts(waist, sex);
    const lo = rest + Math.min(waistMale, waistFemale);
    const hi = rest + Math.max(waistMale, waistFemale);
    const bandOf = (v) => (v < 7 ? ['low', '≈ 1%'] : v <= 11 ? ['slightly elevated', '≈ 4%']
      : v <= 14 ? ['moderate', '≈ 17%'] : v <= 20 ? ['high', '≈ 33%'] : ['very high', '≈ 50%']);
    const [loBand, loRisk] = bandOf(lo);
    const [hiBand, hiRisk] = bandOf(hi);
    return {
      valid: true,
      score: lo,
      sexStated: false,
      waistDecided: false,
      scoreCeiling: hi,
      riskBand: loBand === hiBand ? loBand : null,
      risk: loBand === hiBand ? loRisk : null,
      abnormal: lo >= 12 ? true : (hi >= 12 ? null : false),
      bandLabel: `FINDRISC ${lo} to ${hi}`,
      band: `FINDRISC ${lo} to ${hi} of 26: the sex is not stated, and the waist bands are `
        + `sex-specific (94 / 102 cm for men, 80 / 88 for women), so a waist of ${waist} cm `
        + `scores ${waistMale} points as a man and ${waistFemale} as a woman. `
        + `10-year type-2-diabetes risk ${loRisk} to ${hiRisk} (${loBand} to ${hiBand}).`,
      note: FINDRISC_NOTE,
    };
  }
  return {
    valid: true,
    score,
    // spec-v1166: two different facts, and the first version of this conflated
    // them. `sexStated` is whether the reader said; `waistDecided` is whether it
    // matters -- true when the sex was given, and also when both bands score the
    // same points for this waist, where the reading stands either way (rule 25).
    sexStated,
    waistDecided: true,
    riskBand: band,
    risk,
...
const VAG_HISTORY_NEEDED = 'The prior vaginal-delivery history is needed: it is the strongest term in this model '
  + '(a prior VBAC adds 1.869 to the log-odds, a vaginal delivery before the cesarean 0.868), '
  + 'so leaving it out reports the lowest success probability the model can give.';
...
export function grobmanVbac(input = {}) {
  ...
  const rangeMsg = gradeFault([
    ['the age in years', o.age, null, 70],
    ['the pre-pregnancy weight in kg', o.weight, null, 400],
    ['the height in cm', o.height, null, 250],
  ]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
  if (age === null || weight === null || height === null) return { valid: false, message: 'Enter the maternal age, pre-pregnancy weight (kg), and height (cm).', note: GROBMAN_NOTE };
  // spec-v1117: this fell back to 'none' -- no prior vaginal delivery -- which
  // is not a neutral default but the strongest NEGATIVE term in the model. An
  // unstated obstetric history took a predicted VBAC success from 92.7% to
  // 66.3%, and this number is what a woman is counselled with when choosing
  // between a trial of labour and a repeat cesarean, so understating it pushes
  // toward surgery. The browser offers three options and no blank, so requiring
  // it refuses nothing a reader can reach (rule 9).
  if (!Object.prototype.hasOwnProperty.call(VAG_HISTORY, String(o.vaginalHistory))) {
    return {
      valid: false,
      band: VAG_HISTORY_NEEDED,
      message: VAG_HISTORY_NEEDED,
      note: GROBMAN_NOTE,
    };
  }
  const vagKey = String(o.vaginalHistory);
  const w = -5.952
    - 0.023 * age
    - 0.024 * weight
    + 0.056 * height
...
export function adhereHf(input = {}) {
  ...
  const fault = gradeFault([
    ['BUN (mg/dL)', o.bun, null, 400],
    ['Systolic BP (mmHg)', o.sbp, null, 300],
  ]);
  if (fault) return { valid: false, message: fault, note: ADHERE_NOTE };
  if (bun === null || sbp === null) return { valid: false, message: 'Enter the BUN (mg/dL) and systolic blood pressure (mmHg).', note: ADHERE_NOTE };
  ...
  if (bun < 43) {
    ...
    if (sbp >= 115) { group = 'intermediate'; mortality = '≈ 6.4%'; branch = 'BUN ≥ 43, SBP ≥ 115'; }
      ...
      const creatFault = gradeFault([['Creatinine (mg/dL)', o.creatinine, null, 40]]);
      if (creatFault) return { valid: false, message: creatFault, note: ADHERE_NOTE };
      if (creat === null) return { valid: false, message: 'Enter the serum creatinine (mg/dL). With a BUN of 43 or more and a systolic pressure under 115, it decides the risk group.', note: ADHERE_NOTE };
