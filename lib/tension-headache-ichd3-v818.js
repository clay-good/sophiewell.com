...
export function tensionHeadacheIchd3(input = {}) {
  ...
  const nauseaBlank = String(o.nausea == null ? '' : o.nausea).trim() === '';
  ...
  // spec-v1076: "on these entries" was said the same way whether a count fell
  // short or was never given. Every frequency test above reads `x !== null &&
  // ...`, so a blank headache-days figure made all three subtypes unmet and the
  // tile answered "No ICHD-3 tension-type headache subtype is met" -- a
  // rule-out written from a field nobody filled in. The episode count is
  // excepted: 2.3 Chronic genuinely does not ask for one, which `episodeNote`
  // already says, so it is only outstanding while an EPISODIC form is in play.
  const notEntered = [];
  if (daysPerMonth === null) notEntered.push('headache days per month');
  if (episodes === null && !chronicFreq) notEntered.push('the number of episodes');
  if (months === null && !infrequentFreq) notEntered.push('how many months the pattern has run');
  if (!duration) notEntered.push('the episode duration');
  ...
  return {
    ...
    band: anyMet
      // spec-v1467: a blank nausea is counted as none, and "no nausea" is itself a criterion, so a
      // diagnosis that rests on it says so.
      ? `ICHD-3 criteria met for ${met.join(' and ')}.${nauseaBlank ? ' No nausea severity was entered, so it was counted as none; any nausea rules out the episodic forms and moderate or severe nausea the chronic one.' : ''}`
      : notEntered.length
        ? `Not assessed: enter ${notEntered.join(', ')} — until then no tension-type subtype can be ruled out.`
