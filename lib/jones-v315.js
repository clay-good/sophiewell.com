...
// input:
//   riskPopulation: 'low' | 'modhigh' (blank: asked for only when it decides the result)
//   episode: 'initial' | 'recurrent' (blank: asked for only when it decides the result)
...
// A blank tier or episode is not the stricter choice (spec-v1461): both are evaluated, and when the
// blank one decides the result it is asked for; when it does not, the reading says it was not entered.
export function jonesCriteria(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const tierGiven = o.riskPopulation === 'low' || o.riskPopulation === 'modhigh';
  const episodeGiven = o.episode === 'initial' || o.episode === 'recurrent';
  const lows = tierGiven ? [o.riskPopulation === 'low'] : [true, false];
  const recs = episodeGiven ? [o.episode === 'recurrent'] : [false, true];
  const runs = lows.flatMap((low) => recs.map((rec) => ({ low, rec, r: evaluate(o, low, rec) })));
  const decides = (key) => runs.some((a) => runs.some((b) => a[key] !== b[key]
    && (key === 'low' ? a.rec === b.rec : a.low === b.low) && a.r.category !== b.r.category));
  const need = [];
  if (!tierGiven && decides('low')) need.push('the population risk tier (low or moderate/high)');
  if (!episodeGiven && decides('rec')) need.push('the episode (initial or recurrent)');
  if (need.length) {
    return {
      valid: false,
      message: `Choose ${need.join(' and ')}: with these manifestations it decides whether the criteria are met.`,
    };
  }
  const r = runs[0].r;
  const unstated = [];
  if (!tierGiven) unstated.push('No population risk tier was entered, so the low-risk criteria were counted; the moderate/high-risk criteria give the same result.');
  if (!episodeGiven) unstated.push('No episode was entered, so it was read as an initial episode; a recurrent episode gives the same result.');
  if (unstated.length) {
    r.band = `${r.band} ${unstated.join(' ')}`;
    if (!tierGiven) r.riskPopulation = null;
    if (!episodeGiven) r.episode = null;
  }
  return r;
}

function evaluate(o, low, recurrent) {
