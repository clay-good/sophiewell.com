...
//   - femg (Elisaf 1997, Magnes Res 10(4):315-320): FEMg (%) = (urine Mg x plasma
...
//   - stdKtv (Leypoldt 2004, Semin Dial 17(2):142-145; FHN fixed-volume form): the weekly
...
// spec-v1233: a fraction has two ends and only one of them is a quantity BOUNDS
// names -- `BOUNDS.scr` is a SERUM creatinine, and a urine creatinine runs in
// the hundreds (spec-v1205). So the plasma creatinine is checked and its urine
// partner is not. Checked after each function's own missing-value branch.
const plasmaCrFault = (v) => {
  const fault = boundsAdvisory('scr', Number(v));
  return fault ? { valid: false, message: fault, band: fault } : null;
};
// spec-v1406: the same shape for the plasma analyte and BUN envelopes; an impossible value computed.
const envFault = (key, v) => {
...
export function fepo4(input = {}) {
  ...
  const crFault = plasmaCrFault(pCr) || envFault('phosphate', pPo4);
  if (crFault) return crFault;
...
// --- 2.2 femg -----------------------------------------------------------------
const FEMG_NOTE = 'Fractional excretion of magnesium (Elisaf M, Panteli K, Theodorou J, Siamopoulos KC, Magnes Res 1997;10(4):315-320): FEMg (%) = (urine Mg x plasma creatinine) / (0.7 x plasma Mg x urine creatinine) x 100. The 0.7 corrects for the roughly 30% of plasma magnesium that is protein-bound and not filtered. In the workup of hypomagnesemia, a value above about 2% suggests renal magnesium wasting and a lower value points to an extra-renal cause. The Elisaf series is more specific than that rule of thumb: hypomagnesemia of extra-renal origin ran 0.5 to 2.7% (mean 1.4%), renal magnesium loss ran 4 to 48% (mean 15%), and healthy controls averaged 1.8%. A value between about 2.7 and 4% fell in neither observed group. It frames the excretion fraction; the management decision stays with the clinician.';
...
export function femg(input = {}) {
  ...
  const crFault = plasmaCrFault(pCr) || envFault('magnesium', pMg);
  if (crFault) return crFault;
  ...
  return {
    valid: true, fe: shown,
    abnormal: wasting,
    band: `FEMg ${shown}%: ${wasting ? 'above ~2%, consistent with renal magnesium wasting; the Elisaf series saw renal loss from 4% upward' : 'at or below ~2%, consistent with an extra-renal cause of magnesium loss'}.`,
    note: FEMG_NOTE,
  };
}
...
export function npcrPna(input = {}) {
  ...
  const bunFault = envFault('bun', post) || envFault('bun', pre);
  if (bunFault) return bunFault;
  const rise = pre - post;
  if (rise <= 0) {
