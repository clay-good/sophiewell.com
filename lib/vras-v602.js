...
const NOTE = `The Virginia Radiosurgery AVM Scale (Starke and colleagues 2013) predicts the outcome of stereotactic radiosurgery for a brain arteriovenous malformation, scoring 0 to ${VRAS_MAX}: volume under ${VOLUME_SMALL_MAX} cubic centimeters 0, ${VOLUME_SMALL_MAX} to ${VOLUME_LARGE_MIN} cubic centimeters 1, over ${VOLUME_LARGE_MIN} cubic centimeters 2, plus 1 for eloquent location and 1 for prior hemorrhage. Favorable outcome was reported in 80 percent at a score of 0 to 1, 70 percent at 2, and 45 percent at 3 to 4. The scale has five values but only three published outcome bands, so a 0 and a 1 share a figure as do a 3 and a 4, and the scale is finer than the evidence behind it. Volume is the only graded item and carries half the scale, so a large malformation with neither other feature scores 2 exactly like a tiny eloquent one that has bled. The volume item saturates above ${VOLUME_LARGE_MIN} cubic centimeters, where every malformation scores the same 2 points, while in the Pollock-Flickinger score volume is linear and unbounded, so a 5 and a 40 cubic centimeter malformation are identical here and 3.5 points apart there. The two scales share only volume, since this one uses eloquence and prior hemorrhage while the other uses age and a location tier, so they can rank patients in opposite orders and neither converts into the other. Favorable outcome is a composite of three conditions that must all hold: obliteration, no post-treatment hemorrhage, and no permanent symptomatic radiation-induced complication, so a rate against it is not the obliteration rate. This predicts the outcome of radiosurgery at a group level for a patient in whom radiosurgery is already being considered. It does not choose between radiosurgery, microsurgery, embolization and observation, and observation is a real option, since the ARUBA trial found medical management superior to intervention for unruptured malformations over its follow-up. It does not plan a dose or a target volume, does not estimate rupture risk without treatment, and a favorable score is not by itself an indication to treat.`;
...
export function vras(input = {}) {
  ...
  if (volume === null || eloquent === null || hemorrhage === null) {
    return { valid: false, message: 'Enter the AVM volume in cubic centimeters, and say whether the location is eloquent and whether there has been a prior hemorrhage. Volume is the only graded item and carries half the scale.' };
  }
