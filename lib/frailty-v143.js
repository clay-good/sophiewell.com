...
export function ves13(input = {}) {
  ...
  // spec-v1119: every lookup above falls through to its lowest level and every
  // comparison reads an unanswered select as "not limited" -- so an item nobody
  // asked scored as an item answered reassuringly, and the tile said "not
  // vulnerable" about a screen nobody had administered. The score is a
  // non-negative sum, so vulnerable (>= 3) rules in from a subset (rule 13) and
  // only the reassuring reading waits.
  const unanswered = [];
  if (!(o.age in VES_AGE)) unanswered.push('the age band');
  if (o.health === undefined || o.health === null || o.health === '') unanswered.push('self-rated health');
  const physUnanswered = VES_PHYS.filter((k) => o[k] === undefined || o[k] === null || o[k] === '');
  if (physUnanswered.length) {
    unanswered.push(`${physUnanswered.length} of the ${VES_PHYS.length} physical-function tasks`);
  }
  const vulnerable = total >= 3;
  const floored = unanswered.length > 0 && !vulnerable;
  return {
    valid: true,
    score: total,
    abnormal: vulnerable,
    ...
    domains,
    band: floored
      ? `VES-13 at least ${total}/10 on what was answered: ${unanswered.join(', ')} `
        + `${unanswered.length === 1 ? 'is' : 'are'} unanswered, and each can only add points, so `
        + 'the not-vulnerable reading cannot be given yet.'
      : `VES-13 total ${total}/10: ${vulnerable ? 'vulnerable (≥ 3) — about a four-fold two-year risk of functional decline or death' : 'not vulnerable (below the ≥ 3 threshold)'}.`,
    note: VES_NOTE,
  };
}
