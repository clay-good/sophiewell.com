...
function scaleScore(o, items, label) {
  ...
  return { sum, count, label, items, complete: count === items.length };
}
...
export function fabq(input = {}) {
  ...
  // spec-v1113: unlike the three mean-scored questionnaires fixed beside it,
  // FABQ is a SUM -- so a subscale missing an item is not a mean over fewer
  // items, it is a FLOOR. "physical activity 16 of 24" over three of its four
  // items understates by up to six, and this tile already computed `complete`
  // for both subscales and printed neither.
  //
  // Rule 13 does not exempt anything here: the 1993 source publishes no cutoff,
  // so there is no alarming band that a missing item cannot reach. Every partial
  // subscale is a floor and says so.
  const subText = (sub, max) => {
    if (sub.count === 0) return 'not answered';
    if (sub.complete) return `${sub.sum} of ${max}`;
    return `at least ${sub.sum} of ${max} over ${sub.count} of its ${sub.items.length} items`;
  };
  const paText = subText(pa, 24);
  const workText = subText(work, 42);
  const partial = [
    pa.count > 0 && !pa.complete ? 'physical activity' : null,
    work.count > 0 && !work.complete ? 'work' : null,
  ].filter(Boolean);
  const floorNote = partial.length
    ? ` The ${partial.join(' and ')} ${partial.length === 1 ? 'subscale is' : 'subscales are'} `
      + 'a sum, so an unanswered item can only raise it.'
    : '';
