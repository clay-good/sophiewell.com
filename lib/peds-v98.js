...
export function pim3({ sbp, pupilsFixed, fio2, paO2, baseExcess, mechVent, elective, recovery, riskCategory } = {}) {
  ...
  // spec-v1089: the term is right; the label was not.
  //
  // This model enters an unmeasured variable as zero -- the convention the
  // FiO2/PaO2 comment above already records -- so a base excess nobody drew
  // contributing 0 is the instrument working as designed, not a blank read as a
  // number. What the reader could not tell is WHICH zero they were looking at:
  // "|Base excess| (0)" is also what a measured base excess of 0 prints, and 0
  // is a normal base excess. Two different clinical states, one label.
  const be = fin(baseExcess);
  const beTerm = be != null ? Math.abs(be) : 0;
  const notEntered = [];
  if (be == null) notEntered.push('base excess');
  if (!(f != null && pa != null && pa > 0)) notEntered.push('FiO2 and PaO2');
  ...
  const terms = [
    ...
    { label: be == null ? '|Base excess| (not entered, so 0)' : `|Base excess| (${r2(beTerm)})`, value: r2(PIM3.absBaseExcess * beTerm) },
    { label: `SBP (${sbpClamped})`, value: r2(PIM3.sbp * sbpClamped) },
    { label: 'SBP^2 / 1000', value: r2(PIM3.sbpSq * (sbpClamped * sbpClamped / 1000)) },
    { label: fio2Term === 0 && !(f != null && pa != null && pa > 0) ? 'FiO2*100/PaO2 (not entered, so 0)' : `FiO2*100/PaO2 (${r2(fio2Term)})`, value: r2(PIM3.fio2PaO2 * fio2Term) },
  ...
  return {
    ...
    terms,
    notEntered,
    band: `PIM3 predicted probability of death: ${r2(risk)}% (logit = ${r2(x)}).${notEntered.length ? ` Computed without ${notEntered.join(' and ')}: this model enters an unmeasured variable as zero, so the figure stands, but it is not the same as ${notEntered.length > 1 ? 'those values being' : 'that value being'} measured and zero.` : ''}`,
    note: PIM3_NOTE,
  };
}
