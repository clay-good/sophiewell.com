...
export function nenWhoGrade(input = {}) {
  ...
  // spec-v1138: an unstated differentiation became 'well', and well-versus-poor is
  // not a grade on a scale -- it is which entity this is. Well differentiated is a
  // neuroendocrine TUMOUR, graded G1 to G3 on its proliferation indices; poorly
  // differentiated is a neuroendocrine CARCINOMA, high grade by definition and
  // treated differently. Defaulting to 'well' answered the classification question
  // the pathologist asks first, and there is no floor to report: NEC is not a
  // higher grade of NET, so nothing partial can be said while it is open.
  const differentiationStated = !(o.differentiation === undefined || o.differentiation === null
    || String(o.differentiation).trim() === '');
  if (!differentiationStated) {
    return {
      valid: false,
      code: 'MISSING_INPUT',
      field: 'differentiation',
      message: 'State the differentiation: well differentiated is a neuroendocrine tumor, graded G1 '
        + 'to G3 on the Ki-67 index and the mitotic count, and poorly differentiated is a '
        + 'neuroendocrine carcinoma, high grade by definition. The indices do not decide between '
        + 'them.',
      note: NEN_NOTE,
    };
  }
  const differentiation = String(o.differentiation).trim();
  ...
  // Ki-67 of 6 percent took a G1 to G2, which is the difference between
  // surveillance and treatment on most NET pathways.
  //
  // G3 needs no footing: it is the top grade for a well-differentiated NET, so
  // the missing index cannot raise it.
  const missingIndex = ki67Grade === null ? 'Ki-67 index' : (mitoticGrade === null ? 'mitotic count' : null);
  const floorNote = differentiation !== 'poor' && missingIndex && grade < 3
    ? ` The ${missingIndex} was not entered, and it can only raise this grade: the WHO grade follows whichever index is higher.`
    : '';
