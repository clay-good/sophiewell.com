...
function parseISO(d) {
  const p = ymd(d);
  if (!p) {
    const shape = /^(\d{4})-(\d{2})-(\d{2})$/.test(String(d));
    throw new RangeError(shape
      ? `${d} is not a real calendar date. Check the month and the day.`
      : 'Enter the date as YYYY-MM-DD, for example 2026-03-14.');
  }
