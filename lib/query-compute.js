...
import {
  ...
  qtc, aaGradient, pfRatio,
} from './clinical.js';
import { ibwDevine, shockIndex, maintenanceFluids, wintersFormula } from './clinical-v4.js';
import { mentzerIndex } from './clinical-v5.js';
import { eagA1c } from './clinical-v6.js';
...
// A mass for a unit-select tile, preserving the unit the user typed so the
// deep-link reproduces their entry: { unit: 'kg'|'lb', value } (value in that
// unit). Bare numbers reject (a mass without a unit is ambiguous).
function parseMassUnit(q) {
  const m = q.match(/(\d+(?:\.\d+)?)\s*(kg|kilograms?|kilos?|lbs?|pounds?)\b/);
  if (!m) return null;
  const n = Number(m[1]);
  if (!Number.isFinite(n) || n <= 0) return null;
  const isKg = /^(kg|kilo)/.test(m[2]);
  return { unit: isKg ? 'kg' : 'lb', value: round(n, 2), kg: isKg ? n : n * LB_TO_KG };
}
...
const TEMPLATES = [
  ...
  },
  {
    ...
    parse(q) {
      const m = parseMassUnit(q);
      if (!m) return null;
      let v;
      try { v = maintenanceFluids({ weightKg: m.kg }); } catch { return null; }
      return {
        tile: 'maint-fluids', label: 'Maintenance fluids', value: round(v, 1),
        text: `${round(v, 1)} mL/hr (4-2-1)`, unit: 'mL/hr',
        inputs: { 'mf-w': m.value, 'mf-w-unit': m.unit },
      };
    },
  },
  {
...
export const _testing = { parseMassKg, parseHeightM, parseBP, parseAnalyte, parseA1c, parseMassUnit, TEMPLATES };
