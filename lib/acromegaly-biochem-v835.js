...
export function acromegalyBiochem(input = {}) {
  ...
  // spec-v1459: a blank assay used to become 'conventional' -- the LENIENT cutoff -- and the note
  // then said "A conventional assay is recorded". For a nadir between the two cutoffs that turned
  // "Consistent with acromegaly" into "Discordant". Now a blank assay is read under both cutoffs:
  // where they agree the answer stands without claiming an assay; where they differ it is asked for.
  const assayRaw = String(o.assay == null ? '' : o.assay).trim().toLowerCase();
  const assayGiven = assayRaw !== '';
  const assayKey = assayRaw || 'conventional';
  const assay = ASSAYS[assayKey];
  if (!assay) return { valid: false, message: 'Assay must be conventional or ultrasensitive.' };
  const nadirInWindow = nadir !== null && nadir >= NADIR_ULTRASENSITIVE && nadir < NADIR_CONVENTIONAL;
  ...
  const readWith = (assayUsed) => {
    const suppressed = nadir !== null && nadir < assayUsed.threshold;
    const unsuppressed = nadir !== null && nadir >= assayUsed.threshold;
    ...
    if (igf1Confirmatory && typicalFeatures) {
      ...
      basis = `a raised IGF-1 with a growth hormone nadir of ${nadir} micrograms per liter, not below the ${assayUsed.threshold} threshold for ${assayUsed.text}`;
    } else if (igf1Normal && suppressed) {
      verdict = 'Acromegaly excluded';
      basis = `a normal IGF-1 with a nadir of ${nadir} micrograms per liter, below the ${assayUsed.threshold} threshold for ${assayUsed.text}`;
    } else if (igf1Normal && randomLow) {
      verdict = 'Acromegaly excluded';
      basis = `a normal IGF-1 with a random growth hormone of ${randomGh} micrograms per liter, below ${RANDOM_GH_EXCLUSION}`;
    ...
    return { verdict, basis, suppressed };
  };
  const primary = readWith(assay);
  if (!assayGiven && nadirInWindow) {
    const other = readWith(ASSAYS.ultrasensitive);
    if (other.verdict !== primary.verdict) {
      return {
        valid: false,
        message: `Choose the assay: a nadir of ${nadir} micrograms per liter falls between the two cutoffs (${NADIR_ULTRASENSITIVE} for an ultrasensitive assay, ${NADIR_CONVENTIONAL} for a conventional one), and here the assay decides the reading.`,
      };
    }
  }
  const { verdict, basis, suppressed } = primary;

  // The test that is not a test.
  const randomNote = randomGh !== null && randomGh >= RANDOM_GH_EXCLUSION
    ? `A random growth hormone of ${randomGh} micrograms per liter does not support the diagnosis. Growth hormone is secreted episodically by normal and adenomatous pituitaries alike, and the guideline recommends against single random measurements. A random level can contribute only to EXCLUSION, and only below ${RANDOM_GH_EXCLUSION} alongside a normal IGF-1.`
    : null;

  // The assay-dependent threshold.
  const assayNote = nadir !== null && nadir >= NADIR_ULTRASENSITIVE && nadir < NADIR_CONVENTIONAL
    ? `A nadir of ${nadir} micrograms per liter falls between the two assay thresholds: it is suppressed by the conventional cutoff of ${NADIR_CONVENTIONAL} and NOT suppressed by the ultrasensitive cutoff of ${NADIR_ULTRASENSITIVE}. ${!assayGiven ? 'No assay was entered; the reading here is the same under either cutoff.' : assayKey === 'conventional' ? 'A conventional assay is recorded, so it counts as suppressed here.' : 'An ultrasensitive assay is recorded, so it does not count as suppressed here.'} The assay decides this, not the number alone.`
    : null;
  ...
  return {
    ...
    igf1Raised,
    igf1Confirmatory,
    nadirThreshold: assayGiven ? assay.threshold : null,
