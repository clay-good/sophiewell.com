...
export function acromegalyBiochem(input = {}) {
  ...
  if (igf1Confirmatory && typicalFeatures) {
    ...
    basis = `a raised IGF-1 with a growth hormone nadir of ${nadir} micrograms per liter, not below the ${assay.threshold} threshold for ${assay.text}`;
  } else if (igf1Normal && suppressed) {
    verdict = 'Acromegaly excluded';
    basis = `a normal IGF-1 with a nadir of ${nadir} micrograms per liter, below the ${assay.threshold} threshold for ${assay.text}`;
  ...
  const assayNote = nadir !== null && nadir >= NADIR_ULTRASENSITIVE && nadir < NADIR_CONVENTIONAL
    ? `A nadir of ${nadir} micrograms per liter falls between the two assay thresholds: it is suppressed by the conventional cutoff of ${NADIR_CONVENTIONAL} and NOT suppressed by the ultrasensitive cutoff of ${NADIR_ULTRASENSITIVE}. ${assayKey === 'conventional' ? 'A conventional assay is recorded, so it counts as suppressed here.' : 'An ultrasensitive assay is recorded, so it does not count as suppressed here.'} The assay decides this, not the number alone.`
    : null;
