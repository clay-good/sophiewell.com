...
export function rtaType(input = {}) {
  ...
  // spec-v1464: a blank potassium is not a low one. It is the algorithm's first branch (high gives
  // type 4 whatever the urine pH), so without it no type is assigned.
  const kBlank = o.potassium === undefined || o.potassium === null || String(o.potassium).trim() === '';
  const potassium = kBlank ? null : String(o.potassium).trim();
  if (!kBlank && !Object.prototype.hasOwnProperty.call(POTASSIUM, potassium)) {
  ...
  let reason;
  if (kBlank) {
    reason = 'choose the serum potassium: a high potassium gives type 4 whatever the urine pH, so it comes first';
  } else if (potassium === 'high') {
    type = 4;
