...
export function glim(input = {}) {
  ...
  // spec-v1119: the two graded selects fall through to 'none' -- no weight loss,
  // no low BMI -- and the three checkboxes read an unanswered value as "no". The
  // diagnosis needs one phenotypic AND one etiologic criterion, so an unanswered
  // form came back "Malnutrition not diagnosed by GLIM", a diagnosis excluded
  // rather than not yet made. The checkboxes are real answers (rule 4); the two
  // selects had no empty option until this wave.
  const gradedStated = (v) => Object.prototype.hasOwnProperty.call(GLIM_GRADE, v);
  const unstated = [
    !gradedStated(o.weightLoss) && 'the weight loss',
    !gradedStated(o.lowBmi) && 'the body mass index',
  ].filter(Boolean);
  ...
  return {
    ...
    abnormal: diagnosed,
    unstated,
    // Rule 13: a diagnosis MADE stands on the criteria that were met; only the
    // exclusion depends on the ones nobody assessed -- and not even always.
    //
    // Both unstated criteria are PHENOTYPIC, and GLIM needs one of each kind.
    // The etiologic side is two checkboxes, so an unticked one is a real "no"
    // (rule 4): with neither ticked, no phenotypic criterion could complete the
    // pair and "not diagnosed" is earned however the two selects would have
    // read. The exclusion is only unsafe where the etiologic side is already
    // met and the phenotypic side is empty. An existing test named "phenotypic
    // without etiologic -> not diagnosed" is what drew the distinction.
    floorOnly: !diagnosed && unstated.length > 0 && etiologic.length >= 1 && phenotypic.length === 0,
    bandLabel: diagnosed
      ? `GLIM Stage ${stage}`
      : (unstated.length && etiologic.length >= 1 && phenotypic.length === 0
        ? 'GLIM: not yet assessable'
        : 'GLIM: not diagnosed'),
    band: diagnosed
      ? `Malnutrition diagnosed (GLIM) — Stage ${stage} (${stage === 2 ? 'severe' : 'moderate'}).`
      : (unstated.length && etiologic.length >= 1 && phenotypic.length === 0
        ? `Not yet assessable by GLIM: ${unstated.join(' and ')} `
          + `${unstated.length === 1 ? 'is' : 'are'} not stated, and either would be a phenotypic `
          + 'criterion, so malnutrition is not excluded on this.'
