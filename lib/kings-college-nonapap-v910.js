...
export function kingsCollegeNonApap(input = {}) {
  ...
  // spec-v1478: a blank cause is not "other". An unfavorable cause is one of the five factors, so a
  // blank one is not entered, like a blank age, interval or bilirubin.
  const etiologyGiven = ETIOLOGY_OPTIONS.some((e) => e.value === o.etiology);
  const etiology = etiologyGiven ? o.etiology : null;
  ...
  // spec-v1406: the INR, age and bilirubin envelopes lib/bounds.js declares; an impossible value counted.
  const envFault = [['inr', inr], ['ageYears', age], ['bilirubin', biliMgDl]]
    .map(([k, v]) => (v === null ? null : boundsAdvisory(k, v))).find(Boolean);
  if (envFault) return { valid: false, message: envFault };
  ...
  const factors = [
    ...
    },
    {
      ...
      met: etiology === null ? null : COUNTING_ETIOLOGIES.has(etiology),
      detail: etiology === null ? 'Cause not entered.' : ETIOLOGY_OPTIONS.find((e) => e.value === etiology).text + '.',
    },
    {
