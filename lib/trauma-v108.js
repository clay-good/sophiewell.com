...
export function niss({ ais1, ais2, ais3 } = {}) {
  ...
  // spec-v1042: NISS is the sum of squares of the three worst AIS, so a form with
  // fewer than three entered is a LOWER bound. That can carry "major trauma", and
  // it cannot carry the "no" -- which the tile was printing beside a score built
  // from two of the three.
  const partial = vals.length < 3;
  if (partial && !major) {
    return {
      valid: true, score, major: null, partial, force6, used: top3,
      terms: top3.map((v) => ({ label: `AIS ${v} squared`, value: v * v })),
      band: `NISS is at least ${score} from ${vals.length} of the three AIS severities. `
        + 'Enter the rest: each can only add to the total, so the major-trauma threshold cannot be ruled out yet.',
      note: NISS_NOTE,
    };
  }
  return {
    valid: true, score, major, force6, used: top3, partial,
    terms: top3.map((v) => ({ label: `AIS ${v} squared`, value: v * v })),
    band: force6
...
export function tashScore(input = {}) {
  ...
  // spec-v1007: every TASH variable adds points or leaves them alone, so a partial
  // total is a LOWER BOUND -- safe to rule a massive transfusion IN, never safe to
  // rule it out. It used to rule out: with nothing entered it answered "TASH score
  // 0: ~0.7% probability of mass transfusion", and the four unentered measurements
  // carry 18 of the 31 points. A probability that has not yet reached the 50%
  // activation threshold is withheld until they are there; the checkboxes are
  // answers, not gaps.
  const TASH_LABELS = {
    hb: 'hemoglobin', baseExcess: 'base excess', sbp: 'systolic BP', hr: 'heart rate',
  };
  const missing = Object.keys(TASH_LABELS).filter((k) => fin(input[k]) == null);
  const ps = logistic(-4.9 + 0.3 * total);
  if (missing.length && ps < 0.5) {
    return {
      valid: false, total: null, ps: null, pct: null, high: false, terms,
      band: `Enter the four TASH measurements to estimate the probability. Missing: ${missing.map((k) => TASH_LABELS[k]).join(', ')}. `
        + `The ${missing.length} unentered value${missing.length === 1 ? '' : 's'} can only add points, `
        + 'so a partial score cannot show that a massive transfusion is unlikely.',
      abnormal: false, note: TASH_NOTE,
    };
  }
  const pct = r1(ps * 100);
  const high = ps >= 0.5;
  const partial = missing.length
    ? ` Scored from ${4 - missing.length} of 4 measurements; the rest can only raise it.`
    : '';
  return {
    valid: true, total, ps, pct, high, terms, partialFrom: missing.length ? 4 - missing.length : null,
    band: `TASH score ${total}: ~${pct}% probability of mass transfusion${high ? ' (>= 50%)' : ''}.${partial}`,
...
export function rabtScore(input = {}) {
  ...
  // spec-v1007: the shock index needs both a heart rate and a systolic BP, and the
  // three flags only add points. Without the vitals the tool used to answer "RABT
  // score 0: below the massive-transfusion threshold (< 2) by the rule" while
  // reporting no shock index at all. Below the threshold, that reading waits for
  // them; at 2 or more the rule already fires and the score stands.
  const vitalsMissing = hrV == null || sbpV == null;
  if (vitalsMissing && !mt) {
    const need = [hrV == null ? 'heart rate' : null, sbpV == null ? 'systolic BP' : null].filter(Boolean);
    return {
      valid: false, total: null, mt: false, shockIndex: si, terms,
      band: `Enter ${need.join(' and ')} to compute the shock index, the fourth RABT point. `
        + 'It can only add points, so a partial total cannot show that a massive transfusion is unlikely.',
      abnormal: false, note: RABT_NOTE,
    };
  }
  return {
