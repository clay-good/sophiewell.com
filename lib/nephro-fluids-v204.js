...
import { num, r1, r2, r3, gradeFault } from './num.js';
...
// Hct_initial (Gross JB, Anesthesiology 1983;58(3):277-280).
//
// spec-v978: TWO SOURCES, NOT ONE, and the reader-facing note used to credit both
// to Gross. Gross 1983 is the dilution correction to the allowable-loss formula;
// it takes EBV as an input and publishes no per-kilogram table. The factors above
// come from the protocol sources named in this comment, and `ebv-mabl` — the same
// instrument, built separately — carries a DIFFERENT conventional table (term
// neonate 90 against 85 here, child 75 against 70). Neither can be shown to be
// the right one without a source that publishes the bands, so the divergence is
// disclosed rather than silently resolved, and neither tile credits Gross with a
// table he did not print. See docs/spec-v978.md. The average-Hct
...
const ABL_NOTE = 'Maximum Allowable Blood Loss: ABL = EBV × (Hct initial − Hct target) / Hct initial (Gross JB, Anesthesiology 1983;58(3):277-280). That paper gives the allowable-loss formula and takes the estimated blood volume as an INPUT; it does not publish a blood-volume table. The per-kilogram factors offered here — neonate 85, infant 80, child 70, adult male 75, adult female 65 mL/kg — are conventional reference values, cross-verified across several protocol sources, and they differ between references: check them against your own institution\'s table before planning on the number. An intraoperative transfusion-planning estimate, not a transfusion order — enter hematocrit or hemoglobin using the same measure for both.';
...
export function maxAllowableBloodLoss(input = {}) {
  ...
  // spec-v1404: an out-of-range value parsed to null and was asked for as if blank, so retyping it
  // gave the same sentence. Name the range instead; a blank still falls through to the ask below.
  const rangeMsg = gradeFault([
    ['the weight in kg', o.weight, null, 400],
    ['the initial hematocrit', o.hctInitial, null, 100],
    ['the target hematocrit', o.hctTarget, null, 100],
  ]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
...
export function efwClearance(input = {}) {
  ...
  const fault = gradeFault([
    ['Urine sodium (mEq/L)', o.urineNa, 0, 400],
    ['Urine potassium (mEq/L)', o.urineK, 0, 400],
    ['Plasma sodium (mEq/L)', o.plasmaNa, null, 250],
    ['Urine volume over the interval (mL)', o.urineVolume, null, 100000],
  ]);
...
// --- 2.2 Urinary-calcium assessment -----------------------------------------
// THRESHOLDS RE-FETCHED, NEVER RECALLED (spec-v97), cross-verified across
// StatPearls (Hypercalciuria, NBK448183), the Sargent infancy reference
// (Sargent JD, et al, J Pediatr 1993;123(3):393-397), and Medscape: adult spot
// UCa/Cr > 0.20 mg/mg is hypercalciuria; 24-hour urine calcium > 250 mg/day
// (women), > 300 mg/day (men), or > 4 mg/kg/day; pediatric spot 95th-percentile
// upper limits (Sargent) < 7 mo 0.86, 7-18 mo 0.60, 19 mo-6 y 0.42 mg/mg. The
// molar form is the mg/mg ratio × 2.82 (≈ mmol/mmol).
const UCA_NOTE = 'Urinary-calcium assessment (StatPearls Hypercalciuria; Sargent JD, et al, J Pediatr 1993;123(3):393-397 for the infant bands): the calciuria step in the nephrolithiasis / idiopathic-hypercalciuria workup. Adult spot UCa/Cr > 0.20 mg/mg is hypercalciuria; 24-hour calcium > 250 mg/day (women), > 300 mg/day (men), or > 4 mg/kg/day. Pediatric spot upper limits (Sargent): < 7 mo 0.86, 7–18 mo 0.60, 19 mo–6 y 0.42 mg/mg. The molar ratio ≈ mg/mg × 2.82. Decision support, not an order.';
const UCA_PEDS = { adult: 0.20, 'infant-lt7mo': 0.86, 'infant-7to18mo': 0.60, 'child-19moTo6y': 0.42 };
const UCA_PEDS_LABEL = { adult: 'adult (> 0.20)', 'infant-lt7mo': '< 7 mo (> 0.86)', 'infant-7to18mo': '7–18 mo (> 0.60)', 'child-19moTo6y': '19 mo–6 y (> 0.42)' };

export function urineCalcium(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const mode = o.mode === 'day' ? 'day' : o.mode === 'spot' ? 'spot' : null;
  if (mode === null) {
    return { valid: false, message: 'Select a mode: spot Ca/Cr ratio, or 24-hour excretion.' };
  }
  if (mode === 'spot') {
    const uCa = positive(o.urineCa, 100000);
    const uCr = positive(o.urineCr, 1000000);
    const band = Object.prototype.hasOwnProperty.call(UCA_PEDS, o.ageBand) ? o.ageBand : null;
    if (uCa === null || uCr === null || band === null) {
      return { valid: false, message: 'Spot mode: enter urine calcium and urine creatinine (same mass unit) and select the age band.' };
    }
    const ratio = r3(uCa / uCr);
    const molar = r2(ratio * 2.82);
    const threshold = UCA_PEDS[band];
    ...
    return {
      ...
      molar,
      abnormal: high,
      bandLabel: `Ca/Cr ${ratio} mg/mg`,
      band: `Spot Ca/Cr ${ratio} mg/mg (≈ ${molar} mmol/mmol) — ${high ? 'above' : 'within'} the ${UCA_PEDS_LABEL[band]} threshold${high ? ' (hypercalciuria)' : ''}.`,
      detail: `Urine Ca ${uCa} / urine Cr ${uCr} = ${ratio} mg/mg; age band ${UCA_PEDS_LABEL[band]}.`,
      note: UCA_NOTE,
    };
  }
  // 24-hour excretion mode
  const ca = positive(o.calcium24h, 100000);
  const weight = positive(o.weight, 400);
  const sex = o.sex === 'female' ? 'female' : o.sex === 'male' ? 'male' : null;
  if (ca === null || weight === null || sex === null) {
    return { valid: false, message: '24-hour mode: enter 24-hour urine calcium (mg/day), body weight (kg), and sex.' };
  }
  const perKg = r2(ca / weight);
  const sexThreshold = sex === 'male' ? 300 : 250;
  const highDay = ca > sexThreshold;
  const highKg = perKg > 4;
  const high = highDay || highKg;
  const flags = [];
  if (highDay) flags.push(`> ${sexThreshold} mg/day (${sex})`);
  if (highKg) flags.push('> 4 mg/kg/day');
  return {
    valid: true,
    score: r1(ca),
    perKg,
    abnormal: high,
    bandLabel: `${r1(ca)} mg/day`,
    band: high
      ? `24-h calcium ${r1(ca)} mg/day (${perKg} mg/kg/day) — hypercalciuria (${flags.join(', ')}).`
      : `24-h calcium ${r1(ca)} mg/day (${perKg} mg/kg/day) — within normal limits (≤ ${sexThreshold} mg/day and ≤ 4 mg/kg/day).`,
    detail: `${r1(ca)} mg/day ÷ ${weight} kg = ${perKg} mg/kg/day; ${sex} 24-h threshold ${sexThreshold} mg/day.`,
    note: UCA_NOTE,
  };
}
