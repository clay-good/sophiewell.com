...
// spec-v1201: which of the two things is wrong with an input, in one copy.
//
// The commonest parse helper in this repo -- `pos(v, max)`, `inRange(v, lo, hi)`
// and their cousins -- returns `null` for a blank field, for a non-number AND for
// a value outside the range, and every caller reads `null` as absent. So a reader
// who entered a bilirubin of 600 mg/dL was told to ENTER the bilirubin, retyped
// the same number, and got the same sentence.
//
// `scripts/probe-envelope-unbounded.mjs` keeps that dead end in a section of its
// own, apart from the tiles that answer FROM an impossible value, because it is a
// different defect: not a wrong answer, a loop with no way out.
//
// This lives here for the reason at the top of this file: the first three tiles
// fixed this way each grew their own copy, which is how `r1` and `num` came to be
// declared twice before spec-v53.
//
// Each row is `[label, rawValue, lo, hi, unit]`, in the order the caller checks
// them. `lo === null` means the caller's guard is "strictly greater than zero".
// The bounds are the caller's own; nothing clinical is decided here.
export function inputFault(spec) {
  for (const [label, raw, lo, hi, unit] of spec) {
    const where = unit ? `${label} in ${unit}` : label;
    // spec-v1415: trim BEFORE the blank test. `Number('   ')` is 0, so a whitespace-only field that
    // passed `raw === ''` was read as a measured zero.
    if (raw === null || raw === undefined || String(raw).trim() === '') return `Enter ${where}.`;
    const n = typeof raw === 'number' ? raw : Number(String(raw).trim());
    if (!Number.isFinite(n)) return `Enter ${where}.`;
    const fault = rangeFault(label, n, lo, hi, unit);
    if (fault) return fault;
  }
  return null;
}

// The sentence, once. spec-v1209 needed the range half of `inputFault` on its
// own and this is the join that keeps the two from drifting -- which is the
// mistake this whole file exists to stop making (see the note above `inputFault`,
// and spec-v1201, where `rangeFault` was recommended and `inputFault` written
// instead).
function rangeFault(label, n, lo, hi, unit) {
  const outside = lo === null ? (n <= 0 || n > hi) : (n < lo || n > hi);
  if (!outside) return null;
  const range = hi === Infinity
    ? (lo === null ? 'greater than 0' : `at least ${lo}`)
    : (lo === null ? `greater than 0 and at most ${hi}` : `between ${lo} and ${hi}`);
  return `${label.charAt(0).toUpperCase()}${label.slice(1)} must be ${range}`
    + `${unit ? ` ${unit}` : ''}. Check the value entered.`;
}

// spec-v1209: the range half alone, for a function that ALREADY has its own
// missing-value message and must keep it.
//
// `inputFault` answers "enter it" and "check it" in one pass, in field order.
// That is right for a function whose only fallback is a field list, and wrong for
// one whose fallback is a sentence worth keeping -- `mrcSumScore` says "Grade all
// 12 muscle groups (six movements, left and right) 0-5", which no per-field
// "Enter shoulderL." improves on.
//
// So this SKIPS a blank rather than reporting it, which keeps spec-v1207's rule
// intact from the other side: the caller's own missing-value branch still runs
// first, and a reader who left a field blank is asked for it rather than told the
// value they did enter is out of range.
//
// The defect it exists for is the silent clamp. `clampInt(v, 0, 5)` turns an
// impossible grade into a legitimate extreme -- twelve MRC grades of 500 scored
// 60 of 60 and read "ICU-acquired weakness is not met" -- so the impossible input
// does not merely score, it scores as NORMAL.
//
// Each row is `[label, rawValue, lo, hi]`, bounds the caller's own.
export function gradeFault(spec) {
  for (const [label, raw, lo, hi] of spec) {
    if (raw === null || raw === undefined || String(raw).trim() === '') continue;  // spec-v1415
    const n = typeof raw === 'number' ? raw : Number(String(raw).trim());
    if (!Number.isFinite(n)) continue;
    const fault = rangeFault(label, n, lo, hi, '');
    if (fault) return fault;
  }
  return null;
}

// spec-v1213: an optional input that was never entered is not a measurement of
// zero.
//
// `Number(null)`, `Number(undefined)` and `Number('')` are all `0`, and `0` is
// finite -- so the guard a library writes for an optional field,
//
//   const g = Number(girthT0Cm);
//   if (Number.isFinite(g) && ...) { /* both measurements present */ }
//
// passes for a measurement nobody took. `bristolGirth` was written exactly that
// way: an abdominal girth of 95 cm at T1 with T0 left blank read as a 95 cm rise,
// and at four hours apart that is 23.75 cm/h -- both SCCM 2013
// abdominal-compartment-syndrome banners, "escalate and consider bladder
// pressure", off one empty field.
//
// This is the read that keeps "not said" distinguishable from zero. A typed `0`
// still returns 0; blank, whitespace, null and undefined all return null. Bounds
// are deliberately not checked here -- that is `inputFault` and `gradeFault`
// above, and conflating the two is how `fin` came to mean three different things
// in `lib/` (docs/spec-v1213.md ledgers the 60 copies).
