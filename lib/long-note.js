  ...
  const scan = () => {
    ...
    try {
      for (const head of Array.prototype.slice.call(live.querySelectorAll('h2, h3'))) {
        ...
        if (short.length < 6 || full.length <= short.length) continue;
        if (!full.startsWith(short)) continue;
        // The heading has to end where a word does. `hospital-score` reads
        // "HOSPITAL 0" over "HOSPITAL 0-4: low risk", and 0 is the score while
        // 0-4 is the band -- a prefix by accident, and merging them printed
        // the band where the score had been and lost the score.
        if (!/^[\s:;,.(]/.test(full.charAt(short.length))) continue;
        ...
        // 221 characters and opened by restating its own headline. Only when
        // what is left starts a sentence on its own; a remainder beginning
        // mid-phrase would read worse than the repeat.
