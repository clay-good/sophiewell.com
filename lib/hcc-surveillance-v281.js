...
export function galadHcc(input = {}) {
  ...
  const fault = gradeFault([
    ...
    ['AFP-L3 (%)', o.afpL3, 0, 100],
    ['AFP (ng/mL)', o.afp, 0.0001, 5000000],
    ['DCP / PIVKA-II (mAU/mL)', o.dcp, 0.0001, 5000000],
  ]);
...
export function torontoHccRisk(input = {}) {
  ...
  // spec-v1237: `galadHcc` in this file got the range refusal in spec-v1236 and
  // this sibling did not, which is what `probe-unguarded-sibling` exists to
  // notice. Its own platelet cap is 3000 where BOUNDS.platelets is 2000, so the
  // shared envelope is the tighter of the two; the age cap matches. gradeFault
  // skips a blank, so the message below still runs and still names every field.
  const fault = gradeFault([
    ['Age (years)', o.age, 0, BOUNDS.ageYears.max],
    ['Platelet count (x10^9/L)', o.platelets, 0, BOUNDS.platelets.max],
  ]);
  if (fault) return { valid: false, message: fault };
