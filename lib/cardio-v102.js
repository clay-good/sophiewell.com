...
// spec-v1233: the clamp is not the guard (spec-v1224). Every continuous input in
// this file is centred or banded with `clamp(...)`, which is right for a model
// fitted on a range and silent about a value past it -- MAGGIC clamped a systolic
// BP of 3000 mmHg to 300 and reported a 1- and 3-year mortality, and CardShock
// clamped a lactate of 400 mmol/L to 40 and reported "high risk (~77%
// in-hospital mortality)". Envelope and wording from BOUNDS + boundsAdvisory,
// checked after each function's own missing-value branch (spec-v1207).
const envelopeBand = (rows, note) => {
  for (const [key, v] of rows) {
    if (v == null) continue;
    const fault = boundsAdvisory(key, Number(v));
    if (fault) return { valid: false, band: fault, message: fault, note };
  }
  return null;
};
...
export function maggic(input = {}) {
  ...
  // Only the two the probe drove, and deliberately: this tile's age field states
  // 18-120 and its BMI field 5-80, which are claims about the INSTRUMENT's
  // validated population, not about physiology. BOUNDS.ageYears (0-130) and
  // BOUNDS.bmi (5-200) are wider, so checking them here would publish a second,
  // looser range on a field that already states a tighter one -- the disagreement
  // spec-v1227/v1230/v1232 keep finding, created on purpose. The instrument's own
  // bound is the one that should win on those two, and it already holds.
  const maggicFault = envelopeBand([['sbp', s], ['scr', cr]], MAGGIC_NOTE);
  if (maggicFault) return maggicFault;
  // spec-v1406: it held only as the page's min/max hint; an agent's age of 1300 scored. Enforce the stated ranges.
  const rangeMsg = gradeFault([['the age in years', a, 18, 120], ['the BMI', b, 5, 80]]);
  if (rangeMsg) return { valid: false, band: rangeMsg, message: rangeMsg, note: MAGGIC_NOTE };
...
export function hfaPeff({ functional, morphological, biomarker } = {}) {
  // spec-v1108's ledger, drained in spec-v1112. Each domain fell through to 0,
  // which is the HFA-PEFF's "no criterion met" level, so three unassessed
  // domains answered "HFA-PEFF 0/6: HFpEF unlikely" -- a diagnosis excluded
  // without an echo, a biomarker or a functional assessment.
  //
  // spec-v1111 predicted this tile was NOT the lookup-fallback shape. It is.
  //
  // Domain points are non-negative, so the total is a FLOOR: "confirmed" (>= 5)
  // rules in from a subset (rule 13). BOTH other verdicts rule confirmation OUT
  // and so both wait -- "indeterminate" is a safe next step but it is still the
  // claim that five points were not reached.
  const unassessed = [];
  const domain = (raw, name) => {
    if (!Object.prototype.hasOwnProperty.call(HFAPEFF_DOMAIN, String(raw))) {
      unassessed.push(name);
      return 0;
    }
    return HFAPEFF_DOMAIN[String(raw)];
  };
  const fp = domain(functional, 'the functional domain');
  const mp = domain(morphological, 'the morphological domain');
  const bp = domain(biomarker, 'the biomarker domain');
  const items = [
    { label: 'Functional domain', value: fp, assessed: !unassessed.includes('the functional domain') },
    { label: 'Morphological domain', value: mp, assessed: !unassessed.includes('the morphological domain') },
    { label: 'Biomarker domain', value: bp, assessed: !unassessed.includes('the biomarker domain') },
  ];
  ...
  const floored = unassessed.length > 0 && verdict !== 'confirmed';
  return {
    valid: true,
    total,
    verdict: floored ? null : verdict,
    unassessed,
    floorOnly: floored,
    items,
    band: floored
      ? `HFA-PEFF at least ${total}/6 on what was assessed. ${unassessed.length} of the 3 domains `
        + `${unassessed.length === 1 ? 'was' : 'were'} not assessed (${unassessed.join(', ')}), and `
        + 'each can only add points, so HFpEF is neither confirmed nor excluded on this.'
      : `HFA-PEFF ${total}/6: ${verdictText[verdict]}.`,
    note: HFAPEFF_NOTE,
  };
}
...
export function cardShock(input = {}) {
  ...
  const shockFault = envelopeBand([['lactate', lac], ['eGFR', gfr]], CARDSHOCK_NOTE);
  if (shockFault) return shockFault;
  const items = [
