...
export function tpeQtRatio(input = {}) {
  ...
  const rangeMsg = gradeFault([['the Tp-e interval in ms', o.tpe, 20, 400], ['the QT interval in ms', o.qt, 200, 800]]);
...
export function spise(input = {}) {
  ...
  // spec-v1406: name the range of an entered value, rather than ask for it again.
  const rangeMsg = gradeFault([['the HDL in mg/dL', o.hdl, 5, 150], ['the triglycerides in mg/dL', o.tg, 10, 2000], ['the BMI', o.bmi, 8, 100]]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
