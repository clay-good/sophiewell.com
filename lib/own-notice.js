...
// spec-v1388 s.4: a legal-rule tile closes with one scope sentence built by
// scopeSentence() in lib/state-calendar.js -- "This states the statute as of
// [date]. It does not replace your facility's policy, its counsel, or the court."
// That sentence IS the tile's disclaimer, so the generic clinical banner goes.
export const OWN_NOTICE_LEGAL = /^This states the statute as of [A-Z][a-z]+ \d{1,2}, \d{4}\. It does not replace /;
...
export function isOwnNoticeLine(text) {
  ...
  if (OWN_NOTICE_LEGAL.test(t)) return true;
