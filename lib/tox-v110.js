...
export function digifabDosing(input = {}) {
  ...
  if (mode === 'empiric') {
    // spec-v1467: a blank setting is not an acute ingestion. The empiric dose is 10-20 vials for one
    // and 3-6 for the other, so it is asked for rather than assumed.
    if (input.timing !== 'acute' && input.timing !== 'chronic') {
      return { valid: false, band: 'Choose the empiric setting: the dose is 10-20 vials for an acute ingestion and 3-6 vials for chronic toxicity.', note: DIGIFAB_NOTE };
    }
