...
import { r2, gradeFault } from './num.js';
import { boundsAdvisory, BOUNDS } from './bounds.js';
...
export function cdaiCrohns(input = {}) {
  ...
  for (const w of [weight, standard]) { const f = boundsAdvisory('weightKg', w); if (f) return { valid: false, message: f }; }
  // spec-v1181: the last of spec-v1175's reassuring rows. The envelope and its
  // wording are BOUNDS + boundsAdvisory in lib/bounds.js; no clinical number is
  // invented here.
  // A haematocrit of 750% drove the total to CDAI -3975 -- a NEGATIVE score on a
  // 0-600 scale -- and printed it as "clinical remission".
  const hctFault = boundsAdvisory('hematocrit', hct);
  if (hctFault) return { valid: false, message: hctFault };
  // spec-v1077: the four diary items are a gap, not a zero.
  //
  // clampInt returns its LOW bound for anything non-finite, so an unfilled
  // seven-day tally arrived as 0 -- "no liquid stools all week", which is a real
  // thing a patient in remission reports and therefore indistinguishable from
  // silence. On the worked example, omitting the stool count alone took CDAI
  // from 265 "moderate disease" to 215 "mild disease": the tile de-escalated
  // Crohn's activity across a band boundary on a diary nobody had filled in.
  //
  // All four terms are non-negative with positive coefficients, so a partial
  // total is a LOWER BOUND -- it may rule in, and must not rule out
  // (docs/spec-v1006.md).
  const DIARY = [
    ['stools', 'the 7-day liquid-stool count', 1000],
    ['pain', 'the 7-day abdominal-pain sum', 21],
    ['wellbeing', 'the 7-day well-being sum', 28],
    ['complications', 'the complication count', 6],
  ];
  const notEntered = [];
  const diary = {};
  for (const [key, label, hi] of DIARY) {
    const v = numOrNull(o[key]);
    if (v === null) notEntered.push(label);
    diary[key] = v === null ? 0 : clampInt(v, 0, hi);
  }
  // spec-v1209: `clampInt` above did not only turn a blank into 0 (the bug the
  // note describes) -- it turned an IMPOSSIBLE value into the item's maximum,
  // silently. A 7-day abdominal-pain sum of 9999 scored the same 21 a week of
  // severe pain scores. Each ceiling here is arithmetic: seven days of a 0-3
  // daily grade cannot exceed 21.
  //
  // After the not-entered pass, never inside it (spec-v1207): a diary item left
  // blank is reported as a floor, not as an out-of-range value.
  const diaryFault = gradeFault([
    ...DIARY.map(([key, label, hi]) => [label, numOrNull(o[key]), 0, hi]),
    ['the abdominal-mass grade', o.abdMass === '' || o.abdMass == null ? null : o.abdMass, 0, 5],
  ]);
  if (diaryFault) return { valid: false, band: diaryFault, message: diaryFault, note: CDAI_NOTE };
  const { stools, pain, wellbeing, complications } = diary;
  ...
  const shortfall = notEntered.length
    ? ` Scored from ${DIARY.length - notEntered.length} of the 4 diary items -- ${notEntered.join(', ')} ${notEntered.length > 1 ? 'were' : 'was'} not entered, and can only raise the total, so treat this as a floor.`
    : '';
  return {
    valid: true, total,
    abnormal: high,
    diaryItemsScored: DIARY.length - notEntered.length,
    diaryItemsTotal: DIARY.length,
    notEntered,
    band: `CDAI ${total}: ${band} (< 150 remission, 150-220 mild, 221-450 moderate, > 450 severe).${shortfall}`,
    note: CDAI_NOTE,
  };
}
...
export function uceis(input = {}) {
  const o = obj(input);
  // spec-v1209: each descriptor is a fixed ordinal and `clampInt` mapped
  // anything above it to the worst grade without a word, so a vascular pattern
  // of 9999 read "obliterated". The scale's own definition, nothing clinical.
  const fault = gradeFault([
    ['the vascular-pattern grade', o.vascular, 0, 2],
    ['the bleeding grade', o.bleeding, 0, 3],
    ['the erosions/ulcers grade', o.erosions, 0, 3],
  ]);
  if (fault) return { valid: false, band: fault, message: fault, note: UCEIS_NOTE };
...
// spec-v1108's ledger, drained in spec-v1111. `sumArr` clamps every cell to
// 0-3, and an absent cell clamps to 0 -- which on the SES-CD means the segment
// was looked at and was normal. Twenty cells (four variables across five
// ileocolonic segments) therefore came back:
//
//   sesCd({})  ->  "SES-CD 0/56: endoscopic remission (0-2 remission, 3-6 mild,
//                   7-15 moderate, > 15 severe)."
//
// Endoscopic remission is the outcome Crohn's therapy is escalated or stopped
// on, reported from an endoscopy nobody described. The score is a sum of
// non-negative cells, so what was scored is a FLOOR: moderate and severe rule in
// from a subset (rule 13), and the remission and mild readings wait.
const countArr = (a) => (Array.isArray(a)
  ? a.filter((v) => v !== '' && v !== null && v !== undefined).length
  : 0);
...
export function sesCd(input = {}) {
  ...
  const cellsScored = countArr(o.ulcerSize) + countArr(o.ulceratedSurface)
    + countArr(o.affectedSurface) + countArr(o.stenosis);
  const cellsUnscored = 20 - cellsScored;
  let band; let high;
  ...
  // spec-v1114: this was `!high`, which exempted MODERATE as well as severe --
  // and moderate is not the ceiling. A partial 10 of 56 could be 16 once the
  // unscored cells are read, which is severe. Rule 13 exempts the reading that
  // nothing above it can reach; here that is only `severe`.
  const floored = cellsUnscored > 0 && band !== 'severe';
  return {
    valid: true, total, stenosisCapped: stenosisRaw > 11,
    cellsScored, floorOnly: floored,
    abnormal: high,
    band: floored
      ? `SES-CD at least ${total}/56, scored from ${cellsScored} of the 20 cells (four variables `
        + `across five segments). ${cellsUnscored} ${cellsUnscored === 1 ? 'is' : 'are'} unscored `
        + `and each can only raise it, so the endoscopic ${band} reading cannot be given yet.`
      : `SES-CD ${total}/56: endoscopic ${band} (0-2 remission, 3-6 mild, 7-15 moderate, > 15 severe).`,
    note: SESCD_NOTE,
  };
}
...
export function haps(input = {}) {
  ...
  // spec-v1206: `cdaiCrohns` above got this in spec-v1181, for a haematocrit of
  // 750% that drove CDAI to -3975 and printed it as remission. HAPS reads the
  // same two analytes and never got it: an impossible haematocrit or creatinine
  // simply failed its "normal" test and moved the answer to "not harmless".
  // Alarming rather than reassuring, and still a verdict from a number that
  // cannot be. Envelope and wording from BOUNDS + boundsAdvisory.
  for (const [key, v] of [['hematocrit', hct], ['scr', creat]]) {
...
export function ctsiBalthazar(input = {}) {
  const o = obj(input);
  // spec-v1209: the Balthazar grade is A-E and the necrosis term is 0/2/4/6, so
  // both ceilings are the scale's, not a judgment. Clamped, a grade of 9999 read
  // as grade E with over 30% necrosis and scored severe pancreatitis.
  const fault = gradeFault([
    ['the Balthazar CT grade', o.grade, 0, 4],
    ['the necrosis score', o.necrosis, 0, 6],
  ]);
  if (fault) return { valid: false, band: fault, message: fault, note: CTSI_NOTE };
...
export function modifiedMarshall(input = {}) {
  ...
  // spec-v1209: the envelope guard `probe-unguarded-sibling` prints this module
  // for -- `cdaiCrohns` and `haps` above have had it since spec-v1181/v1206.
  //
  // FiO2 IS THE TRAP (spec-v1205: an envelope is a claim about a quantity IN A
  // UNIT). `BOUNDS.fio2` is 0.21-1 as a FRACTION and this field is a PERCENT, so
  // applying it directly would refuse every legitimate value on the tile. The
  // same physical claim, stated in this field's unit, is 21-100.
  const gasFault = pao2 !== null || fio2 !== null ? gradeFault([
    ['the PaO2 in mmHg', pao2, BOUNDS.paO2.min, BOUNDS.paO2.max],
    ['the FiO2 as a percent', fio2, BOUNDS.fio2.min * 100, BOUNDS.fio2.max * 100],
  ]) : null;
  if (gasFault) return { valid: false, message: gasFault, band: gasFault, note: MARSHALL_NOTE };
  // Renal: creatinine mg/dL.
  const cr = pos(o.creatinine);
  const crFault = gradeFault([['the serum creatinine in mg/dL', cr, BOUNDS.scr.min, BOUNDS.scr.max]]);
  if (crFault) return { valid: false, message: crFault, band: crFault, note: MARSHALL_NOTE };
  ...
  // spec-v1209: the cardiovascular item is a pre-banded 0-4 selection and
  // `clampInt` read anything above 4 as the worst band, so a value of 9999
  // reported organ failure. The band table's own range.
  const cvFault = gradeFault([['the cardiovascular score', cv, 0, 4]]);
  if (cvFault) return { valid: false, message: cvFault, band: cvFault, note: MARSHALL_NOTE };
  ...
  return {
    ...
    band: failed.length > 0
      ...
      // spec-v1097: "all assessed systems below 2" was honest about WHICH
      // systems it had and silent about the direction the others can move it.
      // The score is the worst of three organ systems, so a system nobody
      // assessed can only raise it -- and "no organ failure" is the Revised
      // Atlanta line between mild and moderately severe pancreatitis.
      : `Modified Marshall: no organ failure among the systems entered (all below 2; assessed: ${detail}).${
        systems.length < 3
          ? ` ${3 - systems.length === 1 ? 'One organ system was' : `${3 - systems.length} organ systems were`} not entered, and the score is the worst of the three, so an unassessed system can only raise it.`
          : ''}`,
    note: MARSHALL_NOTE,
  };
}
