...
export function rems(input = {}) {
  ...
  const rangeMsg = gradeFault([
    ['the age in years', o.age, 0, 120], ['the mean arterial pressure in mmHg', o.map, 0, 300],
    ['the heart rate', o.hr, 0, 350], ['the respiratory rate', o.rr, 0, 120],
    ['the SpO2 in %', o.spo2, 0, 100], ['the GCS', o.gcs, 3, 15],
  ]);
...
// --- 2.4 Cardiac Arrest Risk Triage (CART) score ----------------------------
// POINT TABLE RE-FETCHED, NEVER RECALLED (spec-v97), cross-verified against the
// open-access review reproducing Churpek MM, et al, Crit Care Med
// 2012;40(7):2102-2108 Table 4 (PMC3673668) and the ≥ 57-maximum structural
// constraint: respiratory rate < 21 → 0, 21-23 → 8, 24-25 → 12, 26-29 → 15, > 29
// → 22; heart rate < 110 → 0, 110-139 → 4, > 139 → 13; diastolic BP > 49 → 0,
// 40-49 → 4, 35-39 → 6, < 35 → 13; age < 55 → 0, 55-69 → 4, > 69 → 9. Total 0-57
// (22+13+13+9). A score > 20 marks markedly elevated risk of ward cardiac arrest
// within 48 hours.
const CART_NOTE = 'Cardiac Arrest Risk Triage (Churpek MM, Yuen TC, Park SY, Gibbons R, Edelson DP, Crit Care Med 2012;40(7):2102-2108): a vital-sign early-warning model from four ward observations — respiratory rate, heart rate, diastolic BP, and age — banded to a total of 0-57. A score > 20 marks markedly elevated risk of ward cardiac arrest within 48 hours. It outperformed MEWS in its derivation for predicting in-hospital arrest. A deterioration-risk stratifier, not a rapid-response order.';

function cartRr(v) { return v < 21 ? 0 : v <= 23 ? 8 : v <= 25 ? 12 : v <= 29 ? 15 : 22; }
function cartHr(v) { return v < 110 ? 0 : v <= 139 ? 4 : 13; }
function cartDbp(v) { return v > 49 ? 0 : v >= 40 ? 4 : v >= 35 ? 6 : 13; }
function cartAge(v) { return v < 55 ? 0 : v <= 69 ? 4 : 9; }

export function cartScore(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const rr = numIn(o.rr, 0, 120);
  const hr = numIn(o.hr, 0, 350);
  const dbp = numIn(o.dbp, 0, 200);
  const age = numIn(o.age, 0, 120);
  const fault = gradeFault([
    ['Respiratory rate (breaths/min)', o.rr, 0, 120],
    ['Heart rate (bpm)', o.hr, 0, 350],
    ['Diastolic blood pressure (mmHg)', o.dbp, 0, 200],
    ['Age (years)', o.age, 0, 120],
  ]);
  if (fault) return { valid: false, message: fault };
  if ([rr, hr, dbp, age].some((v) => v === null)) {
    return { valid: false, message: 'Enter respiratory rate, heart rate, diastolic blood pressure (mmHg), and age.' };
  }
  const parts = [['Respiratory rate', cartRr(rr)], ['Heart rate', cartHr(hr)], ['Diastolic BP', cartDbp(dbp)], ['Age', cartAge(age)]];
  let total = 0;
  for (const [, pts] of parts) total += pts;
  total = num('CART', total, { min: 0, max: 57 });
  ...
  return {
    ...
    score: total,
    abnormal,
    bandLabel: `CART ${total}`,
    band: `CART ${total}/57 — ${abnormal ? 'markedly elevated risk of ward cardiac arrest within 48 h (> 20)' : 'below the > 20 high-risk threshold'}.`,
    detail: active.length ? `Contributors: ${active.join('; ')}.` : 'All variables in their reference bands — CART 0.',
    note: CART_NOTE,
  };
}
