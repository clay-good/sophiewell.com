...
// spec-v1478: a blank age group is not "one year and older". Apnea counts only in an infant, so
// with the age blank both branches are read; the tool asks when they classify differently and says
// which it used when they agree.
export function pertussisCaseDefinition(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  if (o.age === 'infant' || o.age === 'older') return pertussisAt(o);
  const older = pertussisAt({ ...o, age: 'older' });
  if (!older.valid) return older;
  const infant = pertussisAt({ ...o, age: 'infant' });
  if (infant.classification !== older.classification) {
    return { valid: false, message: `Choose the age group: ${infant.bandLabel.toLowerCase()} in an infant under one year, ${older.bandLabel.toLowerCase()} at one year and older, because apnea counts only in an infant.` };
  }
  const said = ' No age group was entered; the infant and the older branch classify it the same way.';
  return { ...older, ageStated: false, action: older.action + said, band: older.band + said };
}

function pertussisAt(o) {
