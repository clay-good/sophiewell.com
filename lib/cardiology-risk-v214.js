...
export function caapAf(input = {}) {
  ...
  const fault = gradeFault([
    ['Left-atrial diameter (cm)', o.laDiameter, 0, 20],
    ['Age (years)', o.age, 0, 130],
    ['Number of failed antiarrhythmic drugs', o.failedAad, 0, 50],
  ]);
...
export function atlas(input = {}) {
  ...
  const fault = gradeFault([
    ['Indexed LA volume (mL/m²)', o.laVolumeIndex, 0, 400],
  ]);
  if (fault) return { valid: false, message: fault };
  if (lavi === null) {
...
export function mbLater(input = {}) {
  ...
  const fault = gradeFault([
    ['AF type (0 = paroxysmal, 1 = persistent, 2 = long-standing persistent)', o.afType, 0, 2],
  ]);
  if (fault) return { valid: false, message: fault };
  if (afType === null) {
...
export function actionIcu(input = {}) {
  ...
  const fault = gradeFault([
    ['Heart rate (bpm)', o.heartRate, 0, 400],
