...
export function romeEcopd(input = {}) {
  ...
  for (const v of VARIABLES) {
    ...
    if (parsed === null) unmeasured.push(v.text.replace(/ \d.*$/, ''));
    else if (v.cutoff(parsed)) above.push(v.text);
  }
  ...
  } else if (spo2 === null && spo2Drop === null) {
    unmeasured.push('oxygen saturation');
  }
  ...
  // spec-v1006: three of the five variables above cutoff makes an exacerbation
  // moderate, so an unmeasured variable can only ever push the count UP. Counting
  // it as "not above cutoff" therefore rules OUT on evidence that is not there:
  // an empty form answered "mild COPD exacerbation - only 0 of the five variables
  // are above cutoff", and so did a form holding one normal respiratory rate.
  // Moderate and severe stand on what is measured; mild waits for all five.
  if (!severe && count < 3 && unmeasured.length) {
    return {
      valid: false,
      code: 'INCOMPLETE',
      severity: null,
      ...
      aboveCount: count,
      unmeasured,
      // The renderer's `if (!r.valid) note(o, r.message)` guard prints `message`,
      // so an incomplete grade must carry the same text under both keys.
      message: `Measure all five Rome variables to grade this exacerbation. Not yet entered: ${unmeasured.join(', ')}. `
        + `${count} of the five ${count === 1 ? 'is' : 'are'} above cutoff so far and three are needed for moderate, `
        + 'so an unmeasured variable can only raise the grade, never lower it.',
      band: `Measure all five Rome variables to grade this exacerbation. Not yet entered: ${unmeasured.join(', ')}. `
        + `${count} of the five ${count === 1 ? 'is' : 'are'} above cutoff so far and three are needed for moderate, `
        + 'so an unmeasured variable can only raise the grade, never lower it.',
      note: ROME_NOTE,
    };
  ...
  return {
    ...
    band: `Rome proposal: ${severity} COPD exacerbation — ${reason}.`
      + (unmeasured.length ? ` Graded with ${unmeasured.join(', ')} unmeasured; ${unmeasured.length === 1 ? 'it can' : 'they can'} only raise it.` : ''),
