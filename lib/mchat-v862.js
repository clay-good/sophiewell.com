...
export function mchatScore(input = {}) {
  ...
  // spec-v1108: an unanswered item scores nothing, so the total is a FLOOR and
  // can only rise. spec-v1102 gave this tile an `unansweredNote` that says so,
  // and left the HEADLINE alone -- so with all twenty unanswered, which is how
  // the page opens, the reading was:
  //
  ...
  // paragraph explaining that nobody had. Rule 14: fix the headline, not only
  // the detail -- the headline is what gets read and what gets charted.
  //
  // High risk (8 or more) rules IN and is untouched (rule 13). What cannot be
  // said from a partial instrument is `screen: 'negative'` and the words "no
  // further action", in either of the two branches that reach them.
  const incomplete = unanswered.length > 0;
  const floorNote = `An unanswered item scores nothing, so ${total} is a floor over the `
    + `${20 - unanswered.length} answered and can only rise.`;
  ...
  if (risk === 'high') {
    ...
    if (followUp === null) {
      ...
      action = `A total of ${total} is medium risk and the Follow-Up score of ${followUp} is under 2, `
        + `but this is not yet a negative screen. ${floorNote}`;
    } else {
    ...
    action = `Not yet a screen result: ${total} at-risk of the 20 items, from `
      + `${20 - unanswered.length} answered. ${floorNote} Three would be medium risk and eight `
      + 'high, so neither a low-risk reading nor a negative screen can be given yet.';
  } else {
  ...
  return {
    ...
    bandLabel: risk === 'high'
      ? 'High risk'
      : (incomplete && risk === 'low' ? 'Not yet scored' : (risk === 'medium' ? 'Medium risk' : 'Low risk')),
    band: action,
