...
export function gardnerRobertson(input = {}) {
  ...
  // spec-v1095: `|| 0` turns a measure nobody took into class 0, so the poorer
  // of the two can never be the one that is missing. The tile's own detail
  // already says serviceable hearing needs a pure tone average of 50 dB or
  // better AND a discrimination of 50 percent or better, "both rather than
  // either" -- so answering "Serviceable hearing" from one of them contradicts
  // the sentence printed underneath it. Dropping the discrimination took a
  // patient from class V (none) to class II (serviceable).
  //
  // Only the serviceable reading needs this. A non-serviceable class already
  // rests on a measure that WAS taken, and the missing one can only confirm it.
  const missingMeasure = !notTestable && pta === null ? 'pure tone average'
    : (sds === null ? 'speech discrimination score' : null);
  const partialNote = missingMeasure && serviceable
    ? ` The ${missingMeasure} was not entered. Serviceable hearing needs both measures, and the one still missing can only move this to a poorer class.`
    : '';
  ...
  return {
    ...
    band: `Gardner-Robertson class ${ROMAN[grade]} — ${NAMES[grade]}. ${serviceable ? 'Serviceable hearing.' : 'Not serviceable hearing.'}${disagree ? ` The two measures disagreed (pure tone average class ${ROMAN[ptaClass]}, discrimination class ${ROMAN[sdsClass]}); the poorer governs.` : ''}${partialNote}`,
