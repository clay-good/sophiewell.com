...
const HSCORE_NOTE = 'HScore (Fardet 2014) sums nine weighted items (max 337): immunosuppression 18; temperature 38.4-39.4 C = 33, > 39.4 = 49; organomegaly one organ 23, both 38; cytopenias 2 lineages 24, 3 lineages 34; ferritin 2000-6000 = 35, > 6000 = 50; triglyceride 1.5-4 mmol/L = 44, > 4 = 64; fibrinogen <= 2.5 g/L = 30; AST >= 30 = 19; marrow hemophagocytosis 35. An HScore >= 169 best discriminates reactive HLH (sensitivity 93%, specificity 86%). The diagnosis stays with the clinician.';
...
export function hscoreHlh({ immunosuppression, temp, organomegaly, cytopenias, ferritin, triglyceride, fibrinogen, ast, hemophagocytosis } = {}) {
  ...
  const as = fin(ast);
  // spec-v1206: `ipssrMds` and `sokalCml` in this module are guarded and this was
  // not. A temperature of 450 C scored the top band and came back as "HScore 167:
  // estimated HLH probability ~54%". Only the temperature: lib/bounds.js declares
  // no envelope for ferritin, triglyceride, fibrinogen or AST, and choosing one is
  // a clinical judgment that belongs with a source rather than a wave.
  const tempFault = boundsAdvisory('temperature', t);
  if (tempFault) return { valid: false, band: tempFault, note: HSCORE_NOTE };
  ...
  // spec-v1128: `pick` returns 0 for a key it does not hold, and on both of
  // these 0 is the least abnormal level -- no organomegaly, one cytopenic
  // lineage. Every item here scores 0 or more, so what has been entered is a
  // FLOOR and the HScore is understated by whatever was not. The tile prints a
  // probability off that score, which is the figure a reader charts.
  ...
  // spec-v1007: every HScore item adds points or leaves them alone, so a partial
  // total is a LOWER BOUND -- and the reading it produced from nothing was the
  // reassuring one: an empty form answered "HScore 0: estimated HLH probability
  // <1%". The five unentered measurements carry 212 of the 337 points, and HLH is
  // fatal untreated. Below the 169 discrimination threshold the probability waits
  // for them; at or above it the score already rules in and stands. The three
  // pickers and the two checkboxes are answers, not gaps.
  const HS_MEASUREMENTS = {
    temp: ['temperature', t], ferritin: ['ferritin', fer], triglyceride: ['triglyceride', tg],
    fibrinogen: ['fibrinogen', fib], ast: ['AST', as],
  };
  // spec-v1128: the five MEASUREMENTS have been guarded since the tile was
  // written, with the reasoning in its own refusal -- "can only add points, so a
  // partial score cannot make reactive HLH unlikely". The two graded SELECTS
  // beside them, organomegaly and the cytopenia count, were not: `pick` returns
  // 0 for a key it does not hold and 0 is the least abnormal level of each. That
  // is "one tile, two gaps, one guarded" (spec-v1101) for the sixth time, with
  // the argument for the guarded half written out above it.
  const stated = (v) => v !== undefined && v !== null && String(v).trim() !== '';
  const missing = [
    ...Object.keys(HS_MEASUREMENTS).filter((k) => HS_MEASUREMENTS[k][1] == null),
    ...(stated(organomegaly) ? [] : ['organomegaly']),
    ...(stated(cytopenias) ? [] : ['cytopenias']),
  ];
  HS_MEASUREMENTS.organomegaly = ['organomegaly', null];
  HS_MEASUREMENTS.cytopenias = ['the cytopenia count', null];
  if (missing.length && !high) {
    return {
      valid: false, total: null, probability: null, high: false, items,
      band: `Enter the HScore measurements to estimate the probability. Missing: ${missing.map((k) => HS_MEASUREMENTS[k][0]).join(', ')}. `
        + `The ${missing.length} unentered value${missing.length === 1 ? '' : 's'} can only add points, `
        + 'so a partial score cannot make reactive HLH unlikely.',
      note: HSCORE_NOTE,
    };
  }
  const partial = missing.length
    ? ` Scored from ${7 - missing.length} of 7 items; the rest can only raise it.`
    : '';
  return {
    valid: true,
    total,
    ...
    high,
    items,
    band: `HScore ${total}: estimated HLH probability ${probability} (Fardet 2014).${partial}`,
    note: HSCORE_NOTE,
  };
}
...
export function ipssrMds({ cytogenetics, blasts, hemoglobin, platelets, anc } = {}) {
  ...
  const an = fin(anc);
  // spec-v1115: the guard already ASKED for the cytogenetic risk group and did
  // not require it -- spec-v1101's "one tile, two gaps, one guarded" shape, with
  // the tile's own message naming the gap it then walked past. `pick` returns 0
  // for a group it does not hold, and 0 is `very-good`: the best cytogenetics
  // there is, and the term with the widest range in the model (0 to 4 of a
  // roughly 10-point scale). Omitting it moved a patient to "IPSS-R 3: Low risk;
  // median overall survival 5.3 years", quoting a survival figure.
  //
  // Requiring it is what the message has been promising, and it refuses nothing
  // a reader can reach: the browser renders it as a select.
  // spec-v1181: a haemoglobin of 250 g/dL -- the g/L figure for 25.0, and above
  // any survivable value -- scored the best haemoglobin band and read "IPSS-R 3:
  // Low risk", quoting a median survival. Envelope from BOUNDS.
  for (const [key, v] of [['hemoglobin', hgb], ['platelets', plt]]) {
    const fault = v == null ? null : boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault };
  }
  const cytoKnown = Object.prototype.hasOwnProperty.call(IPSSR_CYTO, String(cytogenetics));
  if (!cytoKnown || bl == null || bl < 0 || hgb == null || hgb < 0 || plt == null || plt < 0 || an == null || an < 0) {
    return {
      valid: false,
...
// spec-v1134: the burden of illness is the one MASCC item that is not a checkbox,
// and the two surfaces disagreed about an ungraded one. `pick` falls back to 0,
// which is the value of "severe"; the page's select opened on "No or mild
// symptoms (5)". Same tile, opposite assumption, and 5 points against a cut-off
// that separates outpatient oral management from admission.
//
// The other six items are checkboxes, and an unticked box is a real answer
// (rule 4) -- it says the reassuring feature is absent, which is what scores 0.
// So the burden is the only gap, it is worth 0 to 5, and the fix is the range:
// where both ends fall the same side of 21 the verdict holds and the tile answers
// (rule 25); where they straddle it, the burden is what decides and is asked for.
const MASCC_BURDEN = { 'no-mild': 5, moderate: 3, severe: 0 };
...
export function mascc({ burden, noHypotension, noCopd, solidNoFungal, noDehydration, outpatient, ageUnder60 } = {}) {
  const burdenStated = Object.prototype.hasOwnProperty.call(MASCC_BURDEN, String(burden));
  const burdenPts = pick(MASCC_BURDEN, burden);
  const items = [
  ...
  // Ungraded, the burden adds between 0 (severe) and 5 (no or mild symptoms).
  const ceiling = burdenStated ? total : total + 5;
  if (!burdenStated && !lowRisk && ceiling >= 21) {
    return {
      valid: false,
      total,
      burdenStated: false,
      band: `MASCC is ${total} without the burden of illness, which is worth 0 (severe), 3 (moderate) `
        + `or 5 (no or mild symptoms) -- so this patient scores between ${total} and ${ceiling}, and `
        + 'the 21-point line between outpatient oral management and admission runs through that range. '
        + 'Grade the burden of illness.',
      note: MASCC_NOTE,
    };
  }
  return {
    ...
    burdenStated,
    items,
    band: `MASCC ${burdenStated ? '' : 'at least '}${total}: ${lowRisk ? 'LOW risk for serious complications (>= 21) -- a candidate for outpatient/oral management per the source' : 'not low risk (< 21)'}${burdenStated ? '' : ', which holds whatever the burden of illness turns out to be'}.`,
    note: MASCC_NOTE,
  };
}
...
export function sokalCml({ age, spleen, platelets, blasts } = {}) {
  ...
  // spec-v1180: the platelet term is (plt/700)^2 INSIDE an exp(), so a count in
  // the units a US report prints -- 20,000/uL for what this field wants as 20 --
  // gives exp(153), which is 3.95e+66. `Number.isFinite(sokalRaw)` below catches
  // Infinity and not that: spec-v1012's lesson, that 1e+308 is a number that
  // exists. spec-v1179 made the browser warn; this is the same guard on the
  // surface the browser does not cover.
  if (plt > BOUNDS.platelets.max) {
    return {
      valid: false,
      band: `A platelet count of ${plt} is above ~${BOUNDS.platelets.max}, beyond recorded extremes. This field is in x10\u2079/L (the same as x10\u00B3/\u00B5L), so a lab report reading ${plt}/uL is entered as ${plt / 1000}.`,
      note: 'Sokal and ELTS are at-diagnosis CML hazard formulas in age, spleen size, platelets and blast %; the platelet count must be > 0 (the ELTS term raises it to a negative power).',
    };
  }
  // spec-v1204: spec-v1180 guarded the platelet count IN THIS FUNCTION and left
  // the age beside it. ELTS cubes the age -- `0.0025 * (age/10) ** 3` -- so an age
  // of a million came back as
  //
  //   ELTS 2500000000001.58 (high risk)
  //
  // and the Sokal line vanished from the reading entirely, because `Math.exp` of
  // that age overflows to Infinity and the `Number.isFinite` check below drops it
  // without a word. Half the answer gone, the other half absurd.
  //
  // The envelope and the sentence are lib/bounds.js's, which `ipssrMds` in this
  // same module already uses. Nothing clinical is decided here.
