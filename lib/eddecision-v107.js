...
// spec-v1134: `pick(table, key, 'h0')` falls back to the FIRST row of each table,
// and in all three tables that row is the zero-point, most reassuring level:
// history slightly suspicious, ECG normal, no known risk factors. The only guard
// was on the age, so a chest-pain form with nothing in it but a date of birth
// answered:
//
//   HEAR score 0: very low risk (<= 1) -- the troponin-free band associated with
//   ~0.4% 30-day MACE.
//
// The HEAR score exists to say who may be discharged WITHOUT a troponin. That
// reading is the one an ungraded item can undo, and each of the three is worth up
// to 2 points against a very-low-risk cut-off of 1 -- so a single ungraded item
// can move the answer across it on its own.
//
// The sum is monotone, so what is stated is a FLOOR: above the cut-off the
// verdict holds whatever the rest turn out to be and the tile answers (rule 13),
// naming the total as a floor rather than a total (spec-v1114). At or below it,
// the very-low-risk reading is withheld until the items are graded.
export function hear({ history, ecg, age, risk } = {}) {
  ...
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: HEAR_NOTE };
  const stated = (table, key) => Object.prototype.hasOwnProperty.call(table, key);
  const ungraded = [];
  if (!stated(HEAR_HISTORY, history)) ungraded.push('the history');
  if (!stated(HEAR_ECG, ecg)) ungraded.push('the ECG');
  if (!stated(HEAR_RISK, risk)) ungraded.push('the risk factors');
  ...
  const floored = ungraded.length > 0;
  if (floored && veryLow) {
    const list = ungraded.length === 1 ? ungraded[0]
      : `${ungraded.slice(0, -1).join(', ')} and ${ungraded[ungraded.length - 1]}`;
    return {
      valid: false,
      total,
      ungraded,
      band: `HEAR is ${total} so far, from the age alone; grade ${list} before reading the very-low-risk `
        + `band. Each is worth up to 2 points against a cut-off of 1, so any one of them can put this `
        + 'patient outside the band that skips the troponin.',
      note: HEAR_NOTE,
    };
  }
  return {
    valid: true, total, veryLow, terms, ungraded,
    floorOnly: floored,
    band: veryLow
      ? `HEAR score ${total}: very low risk (<= 1) -- the troponin-free band associated with ~0.4% 30-day MACE.`
      : `HEAR score ${floored ? 'at least ' : ''}${total}: not in the very-low-risk band (> 1) -- HEART scoring with troponin is indicated.`,
...
export function goFar({ age, neuroIntact, ...items } = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: GOFAR_NOTE };
  const terms = [];
  let total = 0;
