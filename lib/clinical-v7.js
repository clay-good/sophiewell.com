// spec-v61 §3 (+ calcium-replacement, spec-v64): thirteen deterministic,
// bedside-necessary nursing computations that fill confirmed medication-safety,
// electrolyte/fluid, and OB/peds gaps.
...
export function urineOutput({ volumeMl, intervalHr, weightKg }) {
  ...
  num('weightKg', weightKg, { min: 0, max: BOUNDS.weightKg.max });
...
//
// spec-v978: Gross 1983 is the MABL half only. The EBV factor is an INPUT the
// reader picks, and the conventional table here differs from the one
// `max-allowable-blood-loss` offers for the same instrument (term neonate 90
// against 85, child 75 against 70). Disclosed rather than resolved: no source
// that publishes the bands has been located. See docs/spec-v978.md.
export function ebvMabl({ weightKg, ebvFactor, startHct, minHct }) {
  ...
  return {
    ...
    note: 'A planning estimate for the transfusion-threshold conversation, not a transfusion trigger; chase the patient, the labs, and local protocol. Gross 1983 gives the allowable-loss formula and takes the estimated blood volume as an INPUT; the per-kilogram factor above is a conventional reference value and tables differ between references, so use your own institution\'s.',
  };
}
...
export function correctedPhenytoin({ measured, albumin, esrd }) {
  ...
  // spec-v1234: the albumin is the DENOMINATOR of the correction, so an
  // impossible one does not produce an impossible level -- it produces a small
  // plausible one. An albumin of 70 g/dL divided a measured 15 µg/mL down to
  // 1.06 and the tile read "Corrected level below the 10-20 µg/mL therapeutic
  // range", which reads as an instruction to give more. Envelope from BOUNDS.
  num('albumin', albumin, { min: BOUNDS.albumin.min, max: BOUNDS.albumin.max });
...
// --- 3.6b calcium-replacement — IV calcium dose, elemental Ca, gluconate<->chloride
// Calcium gluconate 10% and calcium chloride 10% are NOT interchangeable
// gram-for-gram: per gram of salt, calcium chloride carries ~273 mg (13.6 mEq)
// of elemental calcium and calcium gluconate ~93 mg (4.65 mEq) -- chloride
// delivers about 3x the elemental calcium, a recurring and dangerous bedside
// confusion (the "1 g vs 1 g" error). Given a dose of one salt this returns the
// elemental calcium delivered, the 10%-solution volume, and the equivalent dose
// of the OTHER salt that delivers the SAME elemental calcium, plus the standard
// adult dose for the chosen indication. Elemental content per USP / product
// labeling; hyperkalemia dosing per AHA ACLS 2020. A planning aid, not an order.
const CA_SALTS = {
  gluconate: { label: 'calcium gluconate 10%', elementalMgPerG: 93, other: 'chloride' },
  chloride: { label: 'calcium chloride 10%', elementalMgPerG: 273, other: 'gluconate' },
};
// Elemental calcium: 20.04 mg per mEq (atomic mass 40.08, valence 2).
const CA_MG_PER_MEQ = 20.04;
const CA_INDICATIONS = {
  hyperkalemia: 'Hyperkalemia, myocardial membrane stabilization: 1-2 g calcium gluconate (or 0.5-1 g calcium chloride) IV; onset 1-3 min, repeat if ECG changes persist (AHA ACLS 2020).',
  hypocalcemia: 'Symptomatic acute hypocalcemia: 1-2 g calcium gluconate IV over 10-20 min, then recheck ionized calcium.',
  citrate: 'Citrate toxicity / massive transfusion: titrate calcium to the ionized calcium; gluconate or chloride per venous access and local protocol.',
};

export function calciumReplacement({ product, doseGrams, indication }) {
  const salt = CA_SALTS[String(product || '').toLowerCase()];
  if (!salt) return null;
  num('doseGrams', doseGrams, { min: 0, max: 100 });
  const elementalMg = doseGrams * salt.elementalMgPerG;
  const elementalMEq = elementalMg / CA_MG_PER_MEQ;
  const volumeMl = doseGrams * 10; // 10% solution = 1 g per 10 mL
  const other = CA_SALTS[salt.other];
  const equivGrams = elementalMg / other.elementalMgPerG;
  const equivVolumeMl = equivGrams * 10;
  const indicationText = CA_INDICATIONS[String(indication || '').toLowerCase()] || null;
  return {
    product: salt.label,
    doseGrams: r2(doseGrams),
    volumeMl: r1(volumeMl),
    elementalMg: Math.round(elementalMg),
    elementalMEq: r1(elementalMEq),
    equivProduct: other.label,
    equivGrams: r2(equivGrams),
    equivVolumeMl: r1(equivVolumeMl),
    band: `${fmtNum(r2(doseGrams))} g ${salt.label} (${fmtNum(r1(volumeMl))} mL) = ${fmtInt(Math.round(elementalMg))} mg (${fmtNum(r1(elementalMEq))} mEq) elemental calcium = ${fmtNum(r2(equivGrams))} g ${other.label} (${fmtNum(r1(equivVolumeMl))} mL).`,
    indicationText,
    note: 'Planning estimate, not an order. Give slow IV push on a cardiac monitor; calcium chloride is sclerosing -- prefer a central line and watch for extravasation. Do NOT co-administer in the same line as sodium bicarbonate or phosphate (calcium precipitates). Give cautiously in digoxin toxicity. Confirm against local protocol and an independent double-check.',
  };
}
