...
export function drgPayment(input) {
  ...
  // spec-v1143: the capital standardized amount is a real part of the IPPS base
  // and defaulting it to 0 understates the DRG by roughly the capital share --
  // silently, because the note reported only the combined wage-adjusted base. A
  // default is not a defect; a SILENT default is (rule 21), so it is named in
  // the reading now rather than removed: a hospital modelling the operating
  // component alone is a legitimate call.
  const capitalStated = input.capitalBaseCents !== null && input.capitalBaseCents !== undefined && input.capitalBaseCents !== '';
  const capitalBaseCents = Math.round(num('capitalBaseCents', capitalStated ? input.capitalBaseCents : 0, { min: 0, max: 1e11 }));
  ...
  const capitalNote = capitalStated ? '' : ' The capital standardized amount was not entered, so this is the OPERATING base alone -- the full IPPS base is operating + capital, and the payment above is short by the capital share.';
  ...
  return {
    ...
    note: isTransferPriced
      ? `Transfer case: paid per diem (~$${(perDiemCents / 100).toFixed(2)}/day; first day doubled, capped at the full DRG) = $${(transferAdjustedCents / 100).toFixed(2)}, not the full $${(baseDrgCents / 100).toFixed(2)}.${addOnCents > 0 ? ` Plus $${(addOnCents / 100).toFixed(2)} entered add-ons.` : ''}${capitalNote} Estimates the operating model only; outlier/IME/DSH need the hospital's cost-report factors.`
      : `Base DRG payment = weight ${relativeWeight} x wage-adjusted base $${(wageAdjustedBaseCents / 100).toFixed(2)} = $${(baseDrgCents / 100).toFixed(2)}.${addOnCents > 0 ? ` Plus $${(addOnCents / 100).toFixed(2)} entered add-ons.` : ''}${capitalNote} Estimates the operating model only; outlier/IME/DSH/new-tech need the hospital's own factors.`,
  };
}
...
export function apcPayment(input) {
  if (!Array.isArray(input.lines)) {
    throw new TypeError('lines must be an array of { weight, status indicator } APC entries');
  }
