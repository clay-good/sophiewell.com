...
export function isthBat(input = {}) {
  ...
  // spec-v1087: an unrated domain is not a domain with no bleeding in it.
  //
  // The tile rendered all fourteen domains as number inputs PRE-FILLED with 0 --
  // the slider defect (spec-v1078) wearing the fix's clothes. A number input can
  // be blank, but only if it is rendered blank; filled with a 0 it is a form
  // already answered, and the reader would have to delete fourteen zeros to say
  // "I have not asked about any of this". So a patient nobody had interviewed
  // read "ISTH-BAT 0 -- within the normal range", and omitting a single domain
  // took the worked example from 5, "abnormal bleeding score", to 3, "within the
  // normal range" -- a bleeding-disorder screen ruling normal on a question
  // nobody put.
  //
  // Every domain only adds points, so a partial total is a floor: it may rule
  // IN, and must not rule out. Above the threshold it answers and states its
  // footing; below it, it says the score cannot yet be read as normal.
  const contributors = [];
  const unrated = [];
  let total = 0;
  for (const [key, label] of ISTHBAT_DOMAINS) {
    const raw = o[key];
    if (raw === undefined || raw === null || String(raw).trim() === '') { unrated.push(label.toLowerCase()); continue; }
    // spec-v1209: every ISTH-BAT domain is graded 0-4 and `clampInt` mapped
    // anything above that to 4, so a domain rated 9999 contributed the worst
    // bleeding severity the instrument defines. The instrument's own range.
    const domainFault = gradeFault([[`the ${label.toLowerCase()} grade`, raw, 0, 4]]);
    if (domainFault) return { valid: false, message: domainFault, band: domainFault };
    const pts = clampInt(raw, 0, 4);
    total += pts;
  ...
  return {
    ...
    unrated,
    domainsScored: ISTHBAT_DOMAINS.length - unrated.length,
    domainsTotal: ISTHBAT_DOMAINS.length,
    band: abnormal
      ? `ISTH-BAT ${total} — at or above the ≥ ${threshold} ${groupLabel} threshold: abnormal bleeding score.${unrated.length ? ` Scored from ${ISTHBAT_DOMAINS.length - unrated.length} of ${ISTHBAT_DOMAINS.length} domains; the rest can only raise it.` : ''}`
      : unrated.length
        ? `ISTH-BAT is at least ${total} from ${ISTHBAT_DOMAINS.length - unrated.length} of ${ISTHBAT_DOMAINS.length} domains — below the ≥ ${threshold} ${groupLabel} threshold so far, but each domain still unrated can only add points, so a partial score cannot yet read as within the normal range. Rate ${unrated.slice(0, 3).join(', ')}${unrated.length > 3 ? ` and ${unrated.length - 3} more` : ''}.`
...
export function figoGtn(input = {}) {
  ...
  // spec-v1406: name the range of an entered value, rather than ask for it again.
  const rangeMsg = gradeFault([
    ['the age in years', o.age, 0, 130], ['the interval from the index pregnancy in months', o.interval, 0, 2000],
    ['the pretreatment hCG in IU/L', o.hcg, 0, 1e9], ['the largest tumor size in cm', o.size, 0, 100],
    ['the number of metastases', o.mets, 0, 1000],
  ]);
