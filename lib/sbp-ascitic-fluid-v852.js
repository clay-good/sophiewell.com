...
export function sbpAsciticFluid(input = {}) {
  ...
  if (rawPmn < 0 || rawPmn > 500000) {
    return { valid: false, message: 'The neutrophil count is outside a plausible range of 0 to 500000 per cubic millimetre.' };
  }
  ...
  if (nucleated !== null && (nucleated < 0 || nucleated > 500000)) {
    return { valid: false, message: 'The total nucleated count is outside a plausible range of 0 to 500000 per cubic millimetre.' };
  }
  if (rbc !== null && (rbc < 0 || rbc > 5000000)) {
    return { valid: false, message: 'The red cell count is outside a plausible range of 0 to 5000000 per cubic millimetre.' };
  }
  ...
  const bloodNote = bloodSubtraction > 0
    ? `The tap carried blood. ${Math.round(rawPmn)} neutrophils per cubic millimetre were reported; subtracting one for every 250 red cells removes ${bloodSubtraction}, leaving ${correctedPmn}.${Math.round(rawPmn) >= THRESHOLD && !neutrocytic ? ' Uncorrected this sample crosses 250 on blood alone.' : ''}`
    : null;
  ...
  return {
    ...
    bandLabel: state,
    band: `Corrected neutrophil count ${correctedPmn} per cubic millimetre — ${state}, because ${stateDetail}.`,
    detail: 'The criterion is a corrected ascitic neutrophil count of 250 per cubic millimetre or more. The correction subtracts one neutrophil for every 250 red cells, because a bloody tap carries neutrophils in with the blood. A count at or above the line with a single organism is spontaneous bacterial peritonitis; the same count with no growth is culture-negative neutrocytic ascites and is managed the same way; a single organism below the line is bacterascites.',
    note: SBP_NOTE,
  };
}
