...
// spec-v1118: every one of these lists opened on the NORMAL value, so the four
// selects could not say "not observed" and spec-v1102's guard -- which is
// correct, and which `provided()` below implements exactly right -- never fired
// on the page. With a baseline of 140 and nothing else touched, the tile
// answered "Category I ... moderate variability, and neither late nor variable
// decelerations", which is the sentence spec-v1102 was written to prevent.
//
// That wave fixed the AGENT surface and nothing else, and no test noticed,
// because every test called the library directly. This is spec-v1078's trap:
// a guard in the right place, made unreachable by a control with no empty state
// (rule 8). The programme's own note says to re-run the control probe after any
// wave that changes what a tile asks -- spec-v1102 changed only what it answers,
// which is why it did not.
export const VARIABILITY_OPTIONS = [
  { value: '', text: 'Not observed' },
...
export function nichdFhr(input = {}) {
  ...
  return {
    ...
    bradycardia,
    tachycardia,
    // spec-v1458: report what was ENTERED. The picks above default an unentered feature to its
    // normal value for the logic (which accounts for it); echoing those defaults told an agent the
    // variability was "moderate" when nobody had read it.
    variability: provided(o.variability) ? variability : null,
    lateDecels: provided(o.lateDecels) ? lateDecels : null,
    variableDecels: provided(o.variableDecels) ? variableDecels : null,
    sinusoidal: provided(o.sinusoidal) ? sinusoidal : null,
    minimalNote,
    residualNote,
    momentNote,
    ignoredNote,
    scopeNote,
    abnormal: category !== 'I',
    bandLabel: category ? `Category ${category}` : 'Not yet categorized',
    band,
