...
export function uissRcc(input = {}) {
  ...
  // The extent is asked, not assumed: an unanswered one is not N0M0. The guard below used to read
  // `nodePositive` / `metastatic`, which neither the form nor the agent tool supplied, so a
  // node-positive patient got a localized tier and its survival figure.
  const extent = o.extent; // 'n0m0' | 'node-or-met'
  if (extent === 'node-or-met') {
  ...
  const missing = [];
  if (!validT) missing.push('the T stage');
  if (!validGrade) missing.push('the Fuhrman grade');
  if (!validEcog) missing.push('the ECOG performance status');
  if (extent !== 'n0m0') missing.push('the disease extent (the tiers apply to localized N0M0 disease only)');
  if (missing.length) {
    const list = missing.length === 1 ? missing[0] : `${missing.slice(0, -1).join(', ')} and ${missing[missing.length - 1]}`;
    return { valid: false, message: `Select ${list}.` };
  }
