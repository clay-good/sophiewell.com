...
function envelopeFault(rows) {
  for (const [key, v] of rows) {
    ...
    if (fault) return { valid: false, code: 'INVALID_INPUT', message: fault, band: fault };
  }
  return null;
}
...
export function mcmahonRhabdo(input = {}) {
  ...
  const ageRange = gradeFault([['the age in years', o.age, 0, 130]]);
  if (ageRange) return { valid: false, code: 'INVALID_INPUT', field: 'age', message: ageRange };
  ...
  const mFault = envelopeFault([['scr', creat], ['calcium', calcium], ['phosphate', phosphate], ['bicarbonate', bicarb]]);
  if (mFault) return mFault;
