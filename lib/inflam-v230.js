...
export function lmr(input = {}) {
  ...
  const fault = gradeFault([
    ['Absolute lymphocyte count (×10³/µL)', o.alc, 0.001, 500],
    ['Absolute monocyte count (×10³/µL)', o.amc, 0.001, 500],
  ]);
...
export function siri(input = {}) {
  ...
  const alc = pos(o.alc, 0.001, 500);
  const fault = gradeFault([
    ['Absolute neutrophil count (×10³/µL)', o.anc, 0.001, 500],
    ['Absolute monocyte count (×10³/µL)', o.amc, 0.001, 500],
    ['Absolute lymphocyte count (×10³/µL)', o.alc, 0.001, 500],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function piv(input = {}) {
  ...
  const fault = gradeFault([
    ['Absolute neutrophil count (×10³/µL)', o.anc, 0.001, 500],
    ['Platelet count (×10³/µL)', o.plt, 0.1, 3000],
    ['Absolute monocyte count (×10³/µL)', o.amc, 0.001, 500],
    ['Absolute lymphocyte count (×10³/µL)', o.alc, 0.001, 500],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function car(input = {}) {
  ...
  const fault = gradeFault([
    ['C-reactive protein (mg/L)', o.crp, 0, 1000],
