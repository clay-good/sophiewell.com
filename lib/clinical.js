...
export function bsaDuBois({ weightKg, heightCm }) {
  num('weightKg', weightKg, { min: 0, max: BOUNDS.weightKg.max });
  num('heightCm', heightCm, { min: 0, max: BOUNDS.heightM.max * 100 });
...
export function qtc({ qtMs, hrBpm }) {
  // spec-v1406: the qtMs and hr ceilings lib/bounds.js declares; a QT of 8000 ms computed.
  num('qtMs', qtMs, { min: 0, max: BOUNDS.qtMs.max }); num('hrBpm', hrBpm, { min: 1, max: BOUNDS.hr.max });
...
export function weightDose({ weightKg, dosePerKg }) {
  num('weightKg', weightKg, { min: 0, max: BOUNDS.weightKg.max }); num('dosePerKg', dosePerKg, { min: 0 });
...
export function concentrationToRate({ doseValue, doseUnit, weightKg, concentrationValue, concentrationUnit }) {
  ...
  let dosePerMin;
  switch (doseUnit) {
    case 'mcg/kg/min': num('weightKg', weightKg, { min: 0, max: BOUNDS.weightKg.max }); dosePerMin = doseValue * weightKg / 1000; break;
