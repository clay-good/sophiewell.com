...
// spec-v1217: `lvl` returns 0 for a value off the scale, which is the silent
// clamp spec-v1209 exists to remove. Both callers now ask `gradeFault` first, so
// an off-scale grade is refused by name instead of scored as a zero. The helper
// keeps its fallback because every caller now guards ahead of it -- and because a
// blank is still asked for upstream, by `needItems` on the page and by
// `required: true` in the adapter.
function lvl(v, hi) {
  const n = Number(v);
...
const MMT8_LABELS = {
  neck: 'the neck flexors', deltoid: 'the deltoid', biceps: 'the biceps',
  wrist: 'the wrist extensors', glutMax: 'the gluteus maximus',
  glutMed: 'the gluteus medius', quad: 'the quadriceps', ankle: 'the ankle dorsiflexors',
};
export function mmt8(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  // A grade of 99 read as 0 -- the WEAKEST the scale has -- and the total said so
  // without a word. The label is the one a reader sees, not the key (spec-v1017).
  const fault = gradeFault(MMT8_ITEMS.map((k) => [MMT8_LABELS[k], o[k], 0, 10]));
  if (fault) return { valid: false, message: fault, band: fault, note: MMT8_NOTE };
  let s = 0;
...
export function intubationDifficultyScale(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  // 99 operators scored exactly as 0 operators: `lvl` substituted the most
  // favourable value in the range and the reading stayed "slight-to-moderate
  // difficulty". N4 already refuses off-scale through `fin`; these three did not.
  const fault = gradeFault([
    ['the attempts beyond the first', o.extraAttempts, 0, 20],
    ['the operators beyond the first', o.extraOperators, 0, 10],
    ['the alternative techniques used', o.altTechniques, 0, 10],
  ]);
  if (fault) return { valid: false, message: fault, band: fault, note: IDS_NOTE };
...
export function cropIndex(input = {}) {
  ...
  const fault = gradeFault([
    ['Dynamic compliance (mL/cmH2O)', o.compliance, 1, 200],
    ['Maximal inspiratory pressure PImax (magnitude) (cmH2O)', o.pimax, 1, 200],
    ['PaO2 (mmHg)', o.pao2, 20, 700],
    ['FiO2 (fraction, 0.21-1.0)', o.fio2, 0.21, 1],
    ['PaCO2 (mmHg)', o.paco2, 10, 150],
    ['Respiratory rate (breaths/min)', o.rr, 1, 80],
  ]);
