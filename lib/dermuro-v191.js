...
export function scorten(input = {}) {
  ...
  const rangeMsg = gradeFault([
    ['the age in years', o.age, 0, 130],
    ['the heart rate in bpm', o.heartRate, null, 400],
    ['the detached BSA in %', o.bsaDetached, 0, 100],
    ['the BUN in mg/dL', o.bun, null, 400],
    ['the bicarbonate in mEq/L', o.bicarbonate, null, 80],
    ['the glucose in mg/dL', o.glucose, null, 2000],
  ]);
  ...
  // spec-v1063: each of the six measurements adds a point or leaves the total
  // alone, so a total scored from some of them is a LOWER BOUND -- safe to rule
  // in, never to rule out. With the age, heart rate and BUN blank this returned
  // SCORTEN 2 and "mortality approximately 12.1%" where the complete set gave 3
  // and 35.3%, and nothing on screen said three of the seven had not been
  // entered. The malignancy box is a real answer either way and is not counted
  // here.
  const measured = [age, hr, bsa, bun, bicarb, glucose].filter((v) => v !== null).length;
  const partial = measured < 6;
  const partialText = partial
    ? ` Scored from ${measured} of the 6 measurements; the ones still blank can only raise it, so treat this as a floor rather than as the patient's SCORTEN.`
    : '';
  return {
    valid: true,
    score,
    ...
    band: `SCORTEN ${score} — predicted in-hospital mortality ≈ ${mortality} (band ${bandName}).${partialText}`,
...
export function melanomaTStage(input = {}) {
  ...
  if (mm === null) return { valid: false, message: 'Enter the Breslow thickness in mm (greater than 0, at most 100).', note: MELANOMA_NOTE };
...
export function piRads(input = {}) {
  ...
  if (!zone) return { valid: false, message: 'Choose the zone: peripheral or transition.', note: PIRADS_NOTE };
  let category;
  let driver;
  // spec-v1126: in both zones a second sequence exists only to UPGRADE the
  // category, and an unstated one simply did not upgrade -- so the reading came
  // out at the lower category with nothing said. The upgrades are what separate
  // equivocal from likely, which is the difference between watching and biopsying.
  //
  // The guard is scoped to where the missing sequence could actually change the
  // category, which is a short list in each zone (rule 12's corollary, and the
  // narrowing spec-v1119 and spec-v1122 both had to make).
  let couldUpgrade = null;
  if (zone === 'peripheral') {
    const dwi = intIn(o.dwi, 1, 5);
    if (dwi === null) return { valid: false, message: 'Enter the DWI/ADC score, a whole number from 1 to 5.', note: PIRADS_NOTE };
    const dceStated = o.dce !== undefined && o.dce !== null && o.dce !== '';
    const dcePos = o.dce === 'positive' || truthy(o.dce);
    category = dwi;
    driver = `DWI/ADC ${dwi}`;
    if (dwi === 3 && dcePos) { category = 4; driver = 'DWI/ADC 3 upgraded by positive DCE'; }
    // DCE upgrades only a DWI of 3, so that is the only category it can move.
    else if (dwi === 3 && !dceStated) couldUpgrade = 'the dynamic contrast enhancement, which upgrades a DWI of 3 to category 4 when positive';
  } else {
    const t2w = intIn(o.t2w, 1, 5);
    if (t2w === null) return { valid: false, message: 'Enter the T2W score, a whole number from 1 to 5.', note: PIRADS_NOTE };
    const dwi = intIn(o.dwi, 1, 5);
    category = t2w;
    driver = `T2W ${t2w}`;
    ...
    // DWI upgrades a T2W of 2 (at DWI >= 4) or 3 (at DWI = 5), and nothing else.
    else if (dwi === null && (t2w === 2 || t2w === 3)) {
      couldUpgrade = t2w === 2
        ? 'the DWI/ADC score, which upgrades a T2W of 2 to category 3 when it is 4 or 5'
        : 'the DWI/ADC score, which upgrades a T2W of 3 to category 4 when it is 5';
    }
  }
  ...
  return {
    ...
    abnormal: category >= 3,
    couldUpgrade,
    bandLabel: couldUpgrade ? `PI-RADS at least ${category}` : `PI-RADS ${category}`,
    band: couldUpgrade
      ? `PI-RADS at least ${category} — ${likelihood} likelihood of clinically significant cancer `
        + `(${zone} zone, ${driver}). Not entered: ${couldUpgrade}.`
      : `PI-RADS ${category} — ${likelihood} likelihood of clinically significant cancer (${zone} zone, ${driver}).`,
...
export function guysStoneScore(input = {}) {
  ...
  if (g === null) return { valid: false, message: 'Choose the Guy\'s stone score grade, 1 to 4.', note: GUYS_NOTE };
