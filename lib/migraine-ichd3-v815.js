...
export function migraineIchd3(input = {}) {
  ...
  // spec-v1076: a count nobody entered is not a count that fell short.
  //
  // `attacks === null` and `attacks < 5` both made criterion A unmet, and both
  // printed the same sentence -- "for 1.1: at least 5 attacks" -- under the
  // verdict "Neither ICHD-3 migraine criteria set is met". So a form with the
  // attack count still blank ruled migraine OUT in the same words as a patient
  // who has genuinely had three. That is the third state spec-v1066 named: not
  // met, or not measured, and only the first of those may rule out.
  const notEntered = [];
  if (attacks === null) notEntered.push('the number of attacks');
  if (hours === null) notEntered.push('the untreated headache duration');

  const missing = [];
  if (!noAuraMet) {
    const m = [];
    if (!noAura.a) m.push(attacks === null ? `the number of attacks (1.1 asks for at least ${MIN_ATTACKS_NO_AURA})` : `at least ${MIN_ATTACKS_NO_AURA} attacks`);
    if (!noAura.b) m.push(hours === null ? `the untreated headache duration (1.1 asks for ${DURATION_MIN_HOURS} to ${DURATION_MAX_HOURS} hours)` : `attacks lasting ${DURATION_MIN_HOURS} to ${DURATION_MAX_HOURS} hours`);
  ...
  if (!withAuraMet) {
    const m = [];
    if (!withAura.a) m.push(attacks === null ? `the number of attacks (1.2 asks for at least ${MIN_ATTACKS_WITH_AURA})` : `at least ${MIN_ATTACKS_WITH_AURA} attacks`);
  ...
  return {
    ...
    band: met
      ...
      : notEntered.length
        ? `Not assessed: enter ${notEntered.join(' and ')} — until then neither migraine set can be ruled out. Also outstanding: ${missing.join('; ')}.`
