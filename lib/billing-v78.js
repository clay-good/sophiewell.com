...
// spec-v1143: the beneficiary cost-share defaulted to 0, which makes the WHOLE
// allowed the program-payment portion and the 2% cut proportionally larger --
// silently. Zero is a real case (a preventive service with no cost-share), so
// the default stays and the reading names it (rule 21).
export function sequestrationAdjust({ allowedCents, patientResponsibilityCents, seqPct = SEQUESTRATION_PCT }) {
  const patientStated = patientResponsibilityCents !== null && patientResponsibilityCents !== undefined && patientResponsibilityCents !== '';
  if (!patientStated) patientResponsibilityCents = 0;
  ...
  if (patient > allowed) {
    throw new RangeError('patient responsibility cents cannot exceed allowed cents');
  }
  ...
  return {
    allowedCents: allowed, patientResponsibilityCents: patient, patientStated,
    programPaymentCents, sequestrationCents, netPaymentCents, seqPct,
    note: patientStated ? null
      : 'The beneficiary deductible and coinsurance were not entered, so the whole allowed is treated as the program-payment portion -- the 2% cut is taken on all of it, and any cost-share on this line would make it smaller.',
  };
}
