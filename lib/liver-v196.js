...
// spec-v1226: `pos` below returns `null` for a blank field, for a non-number AND
// for a value past its own `max`, and every caller reads `null` as absent. So a
// reader who typed a bilirubin of 600 mg/dL was told *"Enter the bilirubin
// (mg/dL)"* -- retyped the same number, and got the same sentence. A refusal
// with no way out.
//
// `gradeFault` (lib/num.js, spec-v1209) SKIPS a blank, so each function's own
// `missing` list still runs and still names every empty field at once; only the
// value that is out of range is reported, by name and with the range. The labels
// are the ones the `missing` list already uses, so the two sentences name the
// same field the same way.
//
// THE BOUNDS ARE THE CALLERS' OWN, copied from the `pos` call that already
// enforced them silently. Nothing clinical is decided here and no envelope
// moves: the same inputs are accepted and rejected as before, and the rejected
// ones now say why.
...
export function abicScore(input = {}) {
  ...
  const fault = gradeFault([
    ['age (years)', o.age, null, 130],
    ['bilirubin (mg/dL)', o.bilirubin, null, 100],
    ['creatinine (mg/dL)', o.creatinine, null, 40],
    ['INR', o.inr, null, 20],
  ]);
...
export function globeScore(input = {}) {
  ...
  const fault = gradeFault([
    ['age (years)', o.age, null, 130],
    ['bilirubin (× ULN)', o.bili, null, 100],
    ['alkaline phosphatase (× ULN)', o.alp, null, 100],
    ['albumin (× LLN)', o.albumin, null, 5],
    ['platelet count (× 10⁹/L)', o.platelets, null, 2000],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function ukPbcRisk(input = {}) {
  ...
  const fault = gradeFault([
    ['alkaline phosphatase (× ULN)', o.alp, null, 100],
    ['transaminase ALT/AST (× ULN)', o.transaminase, null, 100],
    ['bilirubin (× ULN)', o.bili, null, 100],
    ['albumin (× LLN)', o.albumin, null, 5],
    ['platelets (× LLN)', o.platelets, null, 20],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
...
export function mayoPscRisk(input = {}) {
  ...
  const fault = gradeFault([
    ['age (years)', o.age, null, 130],
    ['bilirubin (mg/dL)', o.bilirubin, null, 100],
    ['albumin (g/dL)', o.albumin, null, 10],
    ['AST (U/L)', o.ast, null, 20000],
  ]);
  if (fault) return { valid: false, message: fault };
  if (missing.length) return { valid: false, message: `Enter the ${missing.join(', ')}.` };
