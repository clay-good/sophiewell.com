...
//
// spec-v59 §2.4: r2/r3/num now come from lib/num.js (single source of truth);
// the local copies are deleted. num.js's num() defaults min to -Infinity rather
// than 0, but every call site in this file passes an explicit { min }, so no
// result moves (spec-v59 §6 byte-identical requirement).

import { r2, r3, num } from './num.js';
...
// defibEnergy removed in spec-v29 wave 29-2 (Group I): "defib" was a
// 1-J/kg lookup, not a calculation.
...
// ---- NEXUS low-risk cervical-spine criteria (Hoffman JR et al. NEJM 2000) ----
// Cervical-spine imaging is NOT required when ALL FIVE low-risk criteria are
// met. Each input is true when the reassuring (low-risk) finding is present
// (no posterior midline tenderness, no intoxication, normal alertness, no focal
// neurologic deficit, no painful distracting injury). Extracted verbatim from
// the group-i renderer so the browser tile and the MCP surface share one compute;
// the returned `band` is the exact string the renderer displays. This reports the
// rule's result, not an imaging order.
export function nexusCspine({ noTenderness, noIntoxication, normalAlertness, noFocalDeficit, noDistractingInjury } = {}) {
  const met = {
    noTenderness: Boolean(noTenderness),
    noIntoxication: Boolean(noIntoxication),
    normalAlertness: Boolean(normalAlertness),
    noFocalDeficit: Boolean(noFocalDeficit),
    noDistractingInjury: Boolean(noDistractingInjury),
  };
  const criteriaMet = Object.values(met).filter(Boolean).length;
  const allLowRiskMet = criteriaMet === 5;
  return {
    criteriaMet,
    criteriaTotal: 5,
    allLowRiskMet,
    imagingRequired: !allLowRiskMet,
    met,
    band: allLowRiskMet
      ? 'NEXUS: cervical spine imaging NOT required'
      : 'NEXUS: imaging IS required (one or more criteria not met)',
  };
}
