...
export function yearsPe({ dvtSigns, hemoptysis, peMostLikely, dDimer }) {
  ...
  // spec-v1037: read as zero, a blank D-dimer is below every threshold this
  // algorithm has, so the tile answered "PE excluded: D-dimer 0 < 1000 ng/mL --
  // no CTPA needed" from a result nobody had. The whole point of YEARS is which
  // threshold the D-dimer is compared against; there is no answer without one.
  if (dDimer === null || dDimer === undefined || dDimer === ''
    || !Number.isFinite(Number(dDimer))) {
    return {
      itemCount,
      threshold,
      excluded: null,
      incomplete: true,
      band: `Enter the D-dimer: with ${itemCount} YEARS item(s) the threshold is `
        + `${threshold} ng/mL, and PE is neither excluded nor confirmed without a result to compare.`,
    };
  }
...
export function issRts({ ais1, ais2, ais3, gcs, sbp, rr }) {
  ...
  // spec-v1064: the Revised Trauma Score needs all three of its vitals, and each
  // is coded into a band -- so a blank systolic pressure was coded as 0 mmHg, the
  // bottom band, and the RTS fell from 7.84 to 4.91 beside an unchanged ISS. The
  // anatomic score stands on its own without the physiology, so the vitals are
  // optional as a SET and the RTS is withheld unless all three are present.
  const haveVitals = [gcs, sbp, rr].every((v) => v !== null && v !== undefined && v !== '');
  if (haveVitals) {
  ...
  const rts = haveVitals
    ? r2(0.9368 * rtsGcsCode(gcs) + 0.7326 * rtsSbpCode(sbp) + 0.2908 * rtsRrCode(rr))
    : null;
  return {
    iss,
    majorTrauma: iss >= 16,
