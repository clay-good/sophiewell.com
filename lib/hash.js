...
export function parseHash(raw) {
  ...
  return { route: decodeURIComponent(route), sub: decodeURIComponent(sub), state, audience, browse };
}

export function buildHash({ route = '', sub = '', state = {}, audience = 'all', browse = '' } = {}) {
  const parts = [];
