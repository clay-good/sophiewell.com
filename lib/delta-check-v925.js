...
export function deltaCheck(input = {}) {
  ...
  const band = withThreshold.length === 0
    ? `The result has ${direction} by ${safeRound(Math.abs(absolute))}${percent === null ? '' : `, which is ${safeRound(Math.abs(percent), 1)}%`}, over ${safeRound(hours, 1)} hours -- a rate of ${safeRound(Math.abs(perDay))} per 24 hours. A delta threshold was not entered, so nothing is flagged: the thresholds are local and nothing here supplies them.`
    : flagged.length
