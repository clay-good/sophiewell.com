...
// spec-v1216: `r1` is exported from `./num.js` and was declared here too.
// The local copy rounded with a bare `Math.round(n * 10) / 10`, which returns
// +/-Infinity for a float64-saturating magnitude where the canonical one returns
// `n` unchanged rather than leak an `Infinity` token into a band string
// (spec-v183). No valid result moves.
import { gradeFault, r1 } from './num.js';
...
export function sapsII(input = {}) {
  ...
  // spec-v1199: every SAPS II band saturates at its extreme, so a sodium of 2000
  // scores the same points a survivable hypernatraemia does and the total maps to
  // a predicted hospital mortality all the same. The age term already clamps to
  // 130 below -- the same instinct, applied to one variable out of twelve.
  //
  // After the required check, never inside it: a value the reader entered must
  // not come back as one of the seventeen still owed.
  for (const [key, v] of [['ageYears', a], ['hr', hr], ['sbp', bp], ['temperature', temp],
    ['sodium', na], ['potassium', k], ['bicarbonate', hco3], ['bilirubin', bili],
    ['wbc', w], ['gcs', g], ['paO2', fin(paO2)], ['bun', bn]]) { // spec-v1406: BUN joins the envelope set.
    if (v == null) continue;
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, note: SAPS_NOTE };
  }
  // PaO2/FiO2 only scores when ventilated/CPAP; otherwise 0.
  //
  // spec-v1149: when VENTILATED but the gas values were blank, the term was
  // "treated as not-measured (0)" -- and `pfPts` cannot return 0 for a
  // ventilated patient. Its three bands are 6, 9 and 11, so a blank blood gas
  // understated the total by at least six points and the mortality with it,
  // silently. The score is monotone in its items, so the total is a FLOOR: it
  // may rule in and never out (rule 3), and it says so instead of scoring the
  // level the table does not contain.
  let pfPoints = 0;
  let pfRatio = null;
  let pfMissing = false;
  if (isVent) {
  ...
  const items = [
    ...
    { label: pfMissing ? 'PaO2/FiO2 (ventilated; not entered -- scores 6 to 11)' : 'PaO2/FiO2 (if ventilated)', value: pfPoints },
  ...
  return {
    ...
    pfMissing,
    band: pfMissing
      ? `SAPS II at least ${score} points: predicted hospital mortality at least ${r1(mortality)}%. `
        + 'The patient is ventilated and the PaO2/FiO2 is not entered -- that term scores 6, 9 or 11 '
        + 'in a ventilated patient and never 0, so both figures are floors and can only rise.'
      : `SAPS II ${score} points: predicted hospital mortality ${r1(mortality)}%.`,
    note: SAPS_NOTE,
  };
}
...
export function lundBrowder({ ageBand, regions } = {}) {
  ...
  // spec-v1210: `Math.max(0, Math.min(1, raw))` below is a silent clamp, and the
  // slip it hides is the commonest one this field has: a region entered as a
  // PERCENT. "50" for half the head became 1.0 -- the whole of it -- and doubled
  // the Lund-Browder total from 3.5% to 7% without a word. %TBSA is what the
  // fluid resuscitation is calculated from (spec-v1016, the same field).
  //
  // A fraction is 0-1 by definition; nothing clinical is decided here.
  const fracFault = gradeFault(LB_REGIONS.map((r) => [`the fraction of the ${r.label.toLowerCase()} burned`, frac[r.key], 0, 1]));
  if (fracFault) {
    return { valid: false, band: `${fracFault} Each region is a FRACTION of that region burned (0-1), not a percent.`, note: LB_NOTE };
  }
  let lbTotal = 0;
  let ronTotal = 0;
  const items = [];
  ...
  // spec-v1016: an unmarked chart is not an unburned patient. With every region
  // blank the tool answered "Lund-Browder %TBSA: 0% (adult Rule of Nines
  // cross-check: 0%)", and %TBSA is what the fluid resuscitation is calculated
  // from -- a 0 there is a decision not to resuscitate, made by an empty form.
  // One region entered is enough; the rest genuinely are zero until charted.
  if (!items.length) {
    return {
      valid: false,
      band: 'Enter the fraction burned for at least one region. An unmarked chart is not a 0% burn, and the %TBSA is what the fluid resuscitation is calculated from.',
      note: LB_NOTE,
    };
  }
...
export function refeedingRisk({ bmi, weightLoss, daysNoIntake, lowElectrolytes, historyFlag } = {}) {
  ...
  // spec-v1207: sapsII above calls boundsAdvisory and this function, in the same
  // file, did not. The BMI direction is the one worth naming: a BMI of 9999
  // matches neither `< 16` nor `< 18.5`, so it REMOVES a NICE minor criterion --
  // a real patient at BMI 17 with 12% weight loss meets two minor criteria and
  // is high risk, and the same patient with the BMI mistyped reads "Not high
  // risk". The other two go the other way, each adding a major criterion from a
  // number nobody measured.
  //
  // After the missing-value branch, never inside it: a value the reader entered
  // must not come back as one of the three still owed.
  for (const [key, v] of [['bmi', b], ['percentWeightLoss', wl], ['daysNoIntake', days]]) {
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, note: REFEEDING_NOTE };
  }
  const major = [];
  const minor = [];
