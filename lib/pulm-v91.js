...
export function goldSpirometry({ fev1Pct, ratio, fev1L, fvcL } = {}) {
  ...
  if (rr == null) {
    return {
      valid: false,
      band: 'Enter the post-bronchodilator FEV1/FVC ratio (0-1), or the FEV1 and FVC in liters to compute it.',
...
export function gapIpf({ sex, age, fvcPct, dlcoPct, dlcoCannotPerform } = {}) {
  ...
  //
  // Here the default runs the OTHER way from ckdEpiCystatin's: male is the +1
  // Gender point, so an unstated sex ADDED a point and took a woman from GAP 3,
  // stage I, 16.3% three-year mortality to GAP 4, stage II, 42.1% -- an alarm
  // from nothing (rule 6). Its guard also already asked for the sex.
  ...
  if (!sexStated || a == null || fvc == null || !dlcoComplete) {
    return {
      valid: false,
  ...
  if (ageFault) return { valid: false, band: ageFault, message: ageFault };
...
export function predictedSpirometry({
  ...
  const sexStated = sex === 'male' || sex === 'm' || sex === 'female' || sex === 'f';
  //
  // GLI-2012 has a separate equation set per sex, so this default chose a
  // reference population rather than a band. The predicted FEV1 and FVC are what
  // percent-predicted is measured against, and percent-predicted stages COPD.
  //
  ...
  if (!sexStated || a == null || h == null) {
    return {
      valid: false,
      band: 'Enter age (years), height (cm) and sex to compute the GLI-2012 predicted spirometry: the equations are sex-specific, so the sex has no default.',
  ...
  // spec-v1404: the height envelope lib/bounds.js declares (in metres); a 2,500 cm height computed.
  const heightFault = boundsAdvisory('heightM', h / 100);
  if (heightFault) return { valid: false, band: heightFault, message: heightFault };
