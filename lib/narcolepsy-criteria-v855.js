...
export function narcolepsyCriteria(input = {}) {
  ...
  // was blank -- a mean latency nobody had measured could not clear 8 minutes,
  // and an unrecorded sleep-onset REM count was counted as none. So a study half
  // entered read as a study done and found negative, and the tile answered:
  //
  //   Criteria not met -- The latency findings do not meet the threshold.
  //
  // The tile already says this correctly for the hypocretin ("the hypocretin
  // above the line OR UNMEASURED"), which is the half-guarded shape: one gap
  // named, the one beside it read as a finding.
  // A sleep-onset REM period on the overnight study ADDS one; it does not stand in
  // for a latency-test count nobody recorded. With the count blank the effective
  // total is a floor, not a total, so the study is still incomplete.
  const latencyIncomplete = latencyTested && (latency === null || soremps === null);
  ...
  if (type1) {
    ...
    } else if (latencyIncomplete && !hypocretinLow) {
      // Monotone: neither half can lower the other, so an incomplete study cannot
      // yet read as a negative one.
      missing = `The latency test is only part recorded, so it is not yet a negative test: ${latency === null ? 'the mean sleep latency was not entered' : 'the number of sleep-onset REM periods was not entered'}. Both halves are needed -- a mean latency of 8 minutes or less AND two or more sleep-onset REM periods.`;
  ...
  // Only where it decides something: a diagnosis already made holds whatever the
  // rest of the study shows, and a low hypocretin settles type 1 on its own.
  const notYetAssessable = !type1 && !type2 && latencyIncomplete && !hypocretinLow && sleepiness;
  ...
  return {
    ...
    latencyIncomplete,
    // Not "Criteria not met" while the study that decides them is half recorded.
    bandLabel: type1 ? 'Type 1' : (type2 ? 'Type 2' : (notYetAssessable ? 'Not yet assessable' : 'Criteria not met')),
    band: type1 || type2
      ? `Narcolepsy ${state} — met by ${route}.`
      : `Narcolepsy criteria — ${notYetAssessable ? 'the latency test is only part recorded, so this is not yet a negative test' : state}.`,
