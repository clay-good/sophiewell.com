...
// spec-v1200: `inRange` returns null for blank, for non-numeric AND for
// out-of-range alike, and its callers read null as absent. So a reader who
// entered a creatinine of 250 mg/dL was told to ENTER the creatinine, retyped
// the same number, and got the same sentence. That dead end is the section
// `scripts/probe-envelope-unbounded.mjs` keeps separately from the tiles that
// answer from an impossible value, and LODS was five of its rows.
//
// A value that IS there and is outside the published range is a different thing
// to say, so it is said differently -- and the range is the one this tile already
// declares, so no clinical number is decided here.
function outOfRange(spec) {
  const bad = spec.filter(([, v, lo, hi]) => {
    if (v === null || v === undefined || v === '') return false;
    const n = Number(v);
    return !Number.isFinite(n) || n < lo || n > hi;
  });
  if (!bad.length) return null;
  const say = ([label, v, lo, hi]) => `${label} ${v} is outside ${lo} to ${hi}`;
  const list = bad.map(say).join('; ');
  // spec-v1203: the labels read "the sodium", so the sentence began lowercase.
  return `${list.charAt(0).toUpperCase()}${list.slice(1)}. Check the value entered -- this is not a missing measurement.`;
}
...
export function oasis(input = {}) {
  ...
  // spec-v1203: the same shape `lods` had nine functions below, in the same file
  // and with the same helper -- `inRange` returns null for blank, for non-numeric
  // AND for out-of-range alike, so a heart rate of 3000 came back as one of the
  // ten variables still owed. `scripts/probe-unguarded-sibling.mjs` printed this
  // module the moment `lods` was fixed.
  const impossible = outOfRange([
    ['the pre-ICU stay', o.preIcuHours, 0, 20000], ['the age', o.age, 0, 120],
    ['GCS', o.gcs, 3, 15], ['the heart rate', o.hr, 0, 350], ['the MAP', o.map, 0, 300],
    ['the respiratory rate', o.rr, 0, 120], ['the temperature', o.temp, 20, 45],
    ['the urine output', o.urine, 0, 40000],
  ]);
...
export function lods(input = {}) {
  ...
  // The range check comes FIRST, because a value that is present and impossible
  // must not be reported as one of the values still owed.
  const impossible = outOfRange([
    ['GCS', o.gcs, 3, 15], ['heart rate', o.hr, 0, 350], ['systolic BP', o.sbp, 0, 350],
    ['BUN', o.bun, 0, 300], ['creatinine', o.creatinine, 0, 40], ['urine output', o.urineL, 0, 40],
    ['WBC', o.wbc, 0, 500], ['platelets', o.platelets, 0, 3000], ['bilirubin', o.bilirubin, 0, 100],
    ['the PaO2/FiO2 ratio', o.pf, 0, 1000],
  ]);
  if (impossible) return { valid: false, message: impossible };
...
export function deltaGap(input = {}) {
  ...
  const impossible = outOfRange([
    ['the sodium', o.na, 80, 200], ['the chloride', o.cl, 50, 160],
    ['the bicarbonate', o.hco3, 1, 60], ['the albumin', o.albumin, 0.5, 7],
  ]);
  if (impossible) return { valid: false, message: impossible };
...
export function appsArds(input = {}) {
  ...
  const impossible = outOfRange([
    ['the age', o.age, 0, 120], ['the PaO2/FiO2 ratio', o.pf, 0, 1000],
    ['the plateau pressure', o.plateau, 0, 80],
  ]);
  if (impossible) return { valid: false, message: impossible };
