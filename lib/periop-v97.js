...
export function guptaMica({ age, asa, functional, creatinine, surgery } = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age was clamped and computed.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: MICA_NOTE };
...
export const POSPOM_MIN_AGE = 18;
const POSPOM_AGE = [
...
export function pospom({ age, comorbidities, surgery } = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age was clamped and computed.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: POSPOM_NOTE };
  // spec-v1410: POSPOM was derived on "patients aged 18 yr or older" (Le Manach Y et al,
  // Anesthesiology 2016;124:570-579). A younger age used to be clamped UP to 18 and scored, so a
  // child was read on the adult table's first band. No upper bound is stated, and the top age band
  // is open-ended, so nothing is clamped down either.
  if (a < POSPOM_MIN_AGE) {
    const msg = `POSPOM was derived in patients aged ${POSPOM_MIN_AGE} years or older; ${a} is outside that range, and the model is not extrapolated.`;
    return { valid: false, band: msg, message: msg, note: POSPOM_NOTE };
  }
  const ageClamped = a;
