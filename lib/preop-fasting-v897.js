...
export function preopFasting(input = {}) {
  ...
  // spec-v1138: `oneOf(INTAKES, o.lastIntake, 'clear-liquid')` fell back to the
  // SHORTEST interval in the table -- two hours -- so a form that did not say
  // what was last taken read "3 hours since clear liquids, against a minimum of
  // 2. The interval is met." The same three hours after breast milk is short by
  // one, after formula or a light meal short by three, after a fatty meal short
  // by five. The permissive end of an aspiration-risk table is not the place to
  // put a default, and naming it in the sentence does not make it true: it is a
  // statement about what this patient ate (rule 11).
  const intakeStated = INTAKES.some((i) => i.value === o.lastIntake);
  const intake = oneOf(INTAKES, o.lastIntake, 'clear-liquid');
  const elapsed = num(o.hoursSinceIntake);
  const shortestHours = Math.min(...INTAKES.map((i) => i.hours));
  const longestHours = Math.max(...INTAKES.map((i) => i.hours));

  if (elapsed !== null && (elapsed < 0 || elapsed > 72)) {
    return { valid: false, message: 'Enter the hours since the last intake, between 0 and 72.' };
  }

  // The intake only decides between the shortest and the longest interval. Past
  // the longest every row in the table is served; short of the shortest, none is
  // (rule 25).
  if (!intakeStated && elapsed !== null && elapsed >= shortestHours && elapsed < longestHours) {
    const reachable = INTAKES.filter((i) => elapsed >= i.hours).map((i) => i.text.split(':')[0].toLowerCase());
    const outstanding = INTAKES.filter((i) => elapsed < i.hours).map((i) => i.text.split(':')[0].toLowerCase());
    return {
      valid: false,
      hoursSinceIntake: elapsed,
      message: `Say what was last taken. At ${hrs(elapsed)} the interval is met for `
        + `${reachable.join(' and ')}, and not yet for ${outstanding.join(' or ')} -- `
        + 'the minimums run from two hours to eight, and which one applies is what was eaten.',
      minimumNote: 'These are minimum intervals, not targets. Nothing by mouth after midnight is the practice this guideline was written against: prolonged fasting is not safer, and it causes thirst, hypoglycemia, irritability and hypovolemia without reducing aspiration risk.',
      abnormal: false,
    };
  }
  ...
  const action = elapsed === null
    ? (intakeStated
      ...
      : 'Say what was last taken and how many hours ago. The minimums run from two hours for clear liquids to eight for fried or fatty food.')
    : !intakeStated
      ? (elapsed >= longestHours
        ? `${hrs(elapsed)} elapsed: every interval in the table is met, whatever was last taken -- the longest is ${hrs(longestHours)}.`
        : `${hrs(elapsed)} elapsed: no interval in the table is met, whatever was last taken -- the shortest is ${hrs(shortestHours)}.`)
    : met
  ...
  return {
    valid: true,
    lastIntake: intakeStated ? intake : null,
    intakeStated,
    ...
    // The label must not name the clear-liquid minimum when nobody said what was
    // taken -- that is the assumption this wave removed, arriving by a side door.
    bandLabel: !intakeStated
      ? (elapsed === null ? 'What was last taken?'
        : (elapsed >= longestHours ? 'Every interval met' : 'No interval met'))
      : (elapsed === null ? `${required} hour minimum` : met ? 'Interval met' : `Short by ${hrs(shortBy)}`),
    band: action,
