...
export function leipzigWilson(input = {}) {
  ...
  // spec-v1127: this tile LOOKS non-monotone -- LIVER_CU['0'] is worth -1 for a
  // normal liver copper -- and it is not, because the DEFAULT an omission lands
  // on is 'na' (0 points), not '0'. What matters for monotonicity is not whether
  // a table holds a negative but whether an OMISSION can reach it. The other six
  // items default to their 0-point level, so every unstated item can only add.
  //
  // So the score from a partial workup is a FLOOR: "diagnosis established" (>= 4)
  // rules in and needs no footing (rule 13), while "very unlikely" (<= 2) and
  // "possible" (exactly 3) are readings an unstated item can undo -- and the
  // number itself is understated in every band (spec-v1114: rule 13 exempts the
  // verdict, not the figure beneath it).
  const stated = (v) => v !== undefined && v !== null && String(v).trim() !== '';
  const unstated = [
    !stated(o.kfRings) && 'the Kayser-Fleischer rings',
    !stated(o.neurologic) && 'the neurologic symptoms or brain MRI',
    !stated(o.ceruloplasmin) && 'the serum ceruloplasmin',
    !stated(o.hemolysis) && 'the Coombs-negative hemolytic anemia',
    !stated(o.urinaryCopper) && 'the urinary copper',
    !stated(o.mutation) && 'the mutation analysis',
  ].filter(Boolean);

  const floored = unstated.length > 0;
  const owed = floored
    ? ` Scored from ${6 - unstated.length} of the 6 items; ${unstated.join(', ')} `
      + `${unstated.length === 1 ? 'was' : 'were'} not entered, and each can only add points.`
    : '';

  let bandLabel, band, abnormal;
  if (score >= 4) {
    bandLabel = 'Diagnosis established';
    band = `Leipzig score ${floored ? `at least ${score}` : score} — 4 or more: diagnosis of `
      + `Wilson disease established.${owed}`;
    abnormal = true;
  } else if (floored) {
    bandLabel = `Leipzig at least ${score}`;
    band = `Leipzig score at least ${score} — not yet a band: 4 or more establishes the diagnosis `
      + `and 3 makes it possible.${owed}`;
    abnormal = score >= 3;
  } else if (score === 3) {
