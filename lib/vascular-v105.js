...
export function abi({ rightAnkle, leftAnkle, rightBrachial, leftBrachial } = {}) {
  ...
  // invented here.
  for (const [name, v] of [['right brachial', rb], ['left brachial', lb],
    ['right ankle', pos(rightAnkle)], ['left ankle', pos(leftAnkle)]]) {
    const fault = v == null ? null : boundsAdvisory('sbp', v);
    if (fault) return { valid: false, band: `${fault} (${name})`, note: ABI_NOTE };
  }
  ...
  // spec-v1067: with only one ankle pressure entered there is no "lower of the
  // two" -- there is one index and one uncalculated leg. Saying "lower index
  // governs" over a single leg claimed a comparison that had not been made, and
  // where that one leg came back normal the tile read "normal (1.00-1.40)" for a
  // patient whose other leg nobody had measured. Peripheral artery disease is
  // frequently one-sided, so that is a rule-out the entered data cannot support.
  const missingSide = legs.find((l) => l.value == null);
  const onlyOneLeg = measured.length === 1;
  const governs = onlyOneLeg ? 'the only leg calculated' : 'lower index governs';
  const oneLegCaveat = onlyOneLeg
    ? ` The ${missingSide.side.toLowerCase()} ankle pressure was not entered, so that leg has not been calculated${governing.band.abnormal ? '' : ' and this does not exclude disease in it'}.`
    : '';
  // spec-v1100: the denominator is the HIGHER of the two brachial pressures, and
  // `Math.max(rb || 0, lb || 0)` lets a brachial nobody measured contribute 0. So
  // a single brachial can only make the divisor smaller, which can only make the
  // index LARGER -- and a larger ABI reads as less disease. Dropping one brachial
  // moved a leg from severe PAD to mild-to-moderate.
  //
  // spec-v1067 already added this caveat for a missing ANKLE; the brachial half
  // of the same divisor was left silent, so one tile guarded one of its two gaps.
  //
  // Silent at the severe end: an index at or below 0.40 cannot be talked further
  // down by a bigger divisor, so that reading is already the floor. Not silent
  // above 1.40, where an inflated index reads as non-compressible and sends the
  // reader to a toe-brachial index they may not need.
  const onlyOneBrachial = (rb == null) !== (lb == null);
  const brachialCaveat = onlyOneBrachial && governing.band.key !== 'severe PAD'
    ? ` Only the ${rb == null ? 'left' : 'right'} brachial pressure was entered. The index divides by the HIGHER of the two, so a missing one can only make this index larger than it should be, and a larger index reads as less disease.`
    : '';
  return {
    valid: true,
    higherBrachial,
    ...
    band: `${governing.side} leg ABI ${governing.value.toFixed(2)} (${governs}): ${governing.band.text}.${oneLegCaveat}${brachialCaveat}`,
...
// spec-v1107: exported so a test can assert what the disclosure above depends on
// -- that every graded coefficient is >= 0, which is what makes a partly stated
// EuroSCORE II a floor rather than an estimate that could move either way.
export const euroScore2Coefficients = ES2;
...
export function euroScore2({
  ...
  // spec-v1205: `abi` in this module has had the envelope guard since spec-v1181
  // and this never got it. The age term is linear in `max(1, age - 59)`, so an
  // implausible age drives the logit until the predicted mortality saturates:
  // an age of a million returned "predicted in-hospital mortality at least
  // 100.00%".
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, note: ES2_NOTE };
  ...
  // spec-v1107: which categoricals were STATED, as distinct from which scored 0.
  //
  // `lk` returns 0 for a key the table does not hold, and 0 is the REFERENCE
  // LEVEL of every one of these six -- NYHA I, good LV, normal PA pressure,
  // normal renal function, elective, isolated CABG. So an unstated category was
  // silently scored as the healthiest patient the model can describe, and the
  // browser's selects all open on that same first option:
  //
  //   euroScore2({ age: 85 })
  //     -> "predicted in-hospital mortality 1.01% (low predicted operative risk)"
  //
  // An 85-year-old described by nothing but their age, reported as low operative
  // risk (rule 11: six clinical characterisations asserted, none of them made).
  //
  // spec-v1102 left this open because a regression with a missing covariate
  // looked like a different shape from the threshold rules this programme had
  // been working. It is not, and the reason is a property of THIS model: every
  // coefficient in the six tables is zero or positive, so the estimate is
  // MONOTONE in them and what is stated gives a FLOOR. `euroScore2Coefficients`
  // is exported so a test can hold that property, because the whole disclosure
  // below is false the day a negative coefficient is added.
  const unstated = [];
  const addCat = (label, table, key, name) => {
    if (!Object.prototype.hasOwnProperty.call(table, key)) { unstated.push(name); return; }
    const v = table[key];
    if (v !== 0) { y += v; terms.push({ label, value: v }); }
  };
  addCat(`NYHA ${nyha}`, ES2.nyha, nyha, 'the NYHA class');
  addCat('LV function', ES2.lv, lvFunction, 'LV function');
  addCat('Pulmonary hypertension', ES2.pa, pulmonaryHypertension, 'the pulmonary artery pressure');
  addCat('Urgency', ES2.urgency, urgency, 'the urgency');
  addCat('Weight of intervention', ES2.weight, weightOfIntervention, 'the weight of intervention');
  addCat('Renal impairment', ES2.renal, renal, 'renal function');
  ...
  // Rule 13: "very high" is the top band and an unstated covariate cannot talk
  // it down, so that reading needs no footing. Every gentler one is a floor.
  const floored = unstated.length > 0 && tier !== 'very high';
  // spec-v1114: rule 13 exempts the TIER, not the number. The very-high branch
  // was left entirely alone in spec-v1107, so an unstated factor still produced
  // a specific predicted mortality -- 8.15% where the full call gives 10.66% --
  // under a tier that happened to be right. The tier stands; the percentage is a
  // floor and says so.
  const toppedOut = unstated.length > 0 && tier === 'very high';
  const band = floored
    ? `EuroSCORE II predicted in-hospital mortality at least ${pct.toFixed(2)}% on what was `
      + `stated. ${unstated.length} of the 6 graded factors ${unstated.length === 1 ? 'was' : 'were'} `
      + `not stated (${unstated.join(', ')}), and every coefficient in this model is zero or `
      + 'positive, so each can only raise it. Not yet a risk tier.'
    : (toppedOut
      ? `EuroSCORE II predicted in-hospital mortality at least ${pct.toFixed(2)}% (very high `
        + `predicted operative risk, which the unstated factors cannot lower). ${unstated.length} `
        + `of the 6 graded factors ${unstated.length === 1 ? 'was' : 'were'} not stated `
        + `(${unstated.join(', ')}), and each can only raise the figure.`
      : `EuroSCORE II predicted in-hospital mortality ${pct.toFixed(2)}% (${tier} predicted operative risk).`);
  return {
    valid: true,
    mortality: pct,
    y: r2(y),
    tier: floored ? null : tier,
    floorOnly: floored,
    unstated,
    terms,
    band,
