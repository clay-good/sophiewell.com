...
export function pfdi20(input = {}) {
  ...
  // spec-v1113: each subscale is a MEAN over its answered items, which is the
  // published rule -- but the denominator was computed, stored in `answered` and
  // never shown, so a mean of two items printed exactly like a mean of eight.
  // The subscales are different lengths, so the count comes from GROUPS.
  const short = GROUPS
    .filter((g) => answered[g.key] < g.args.length)
    .map((g) => `${g.label} over ${answered[g.key]} of its ${g.args.length} items`);
  const itemCounts = short.length ? `Scored from ${short.join(', ')}.` : '';
  ...
  return {
    ...
    itemCounts,
    detail: `${itemCounts ? `${itemCounts} ` : ''}Each subscale is the mean of its answered items multiplied by 25, so each runs 0 to 100; the summary adds the three, so it runs 0 to 300. Higher is more distress. The 2005 source publishes no severity cutoff, so this tile reports the scores and asserts no band; the scales are most useful compared against the same patient before and after treatment.`,
    note: PFDI20_NOTE,
  };
}
