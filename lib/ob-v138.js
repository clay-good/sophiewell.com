...
// without this an empty form reads as a form full of zeros and answers from nothing.
const nonNeg = (v) => {
...
export function fullPiers(input = {}) {
  ...
  // spec-v1233: the platelet term is quadratic and the logistic saturates, so a
  // count of 20,000 x10^9/L came out as a probability rather than as nonsense --
  // the fullPIERS output is a percentage risk of an adverse maternal outcome,
  // which a reader acts on.
  //
  // ONLY THE PLATELETS ARE CHECKED. fullPIERS takes creatinine in umol/L and
  // `BOUNDS.scr` is mg/dL (spec-v1205: an envelope is a claim about a quantity
  // IN A UNIT), so applying it here would refuse every legitimate value; SpO2
  // and gestational age have no envelope in the table at all.
  const pltFault = boundsAdvisory('platelets', plt);
  if (pltFault) return { valid: false, message: pltFault, band: pltFault };
...
export function miniPiers(input = {}) {
  ...
  // spec-v1233: as above -- a systolic BP of 3000 mmHg produced an adverse-outcome
  // probability. Gestational age has no envelope.
  const sbpFault = boundsAdvisory('sbp', sbp);
  if (sbpFault) return { valid: false, message: sbpFault, band: sbpFault };
  ...
  // spec-v1117: `|| 0` covered two different things -- 'lt2', which really does
  // score 0 (negative, trace or 1+), and a proteinuria nobody dipped. The
  // browser's select offers 'lt2' first and no blank, so only an API caller
  // reaches the second, and it was scored as a negative dipstick: 10% probability
  // of an adverse maternal outcome became 6.8%, across the 15% surveillance
  // threshold. Rule 9's surface split, on a regression coefficient.
  const DIP_LEVELS = ['lt2', ...Object.keys(DIP_BETA)];
  if (!DIP_LEVELS.includes(String(o.proteinuria))) {
    return {
      valid: false,
      message: 'The dipstick proteinuria is needed (negative/trace/1+, 2+, 3+ or 4+): it is a term '
        + 'in the model, and leaving it out scores it as a negative dipstick.',
    };
  }
...
export function iomGwg(input = {}) {
  // spec-v1237: `probe-unguarded-sibling` reports this beside fullPiers and
  // miniPiers, which got the envelope in spec-v1233, and it is right that this
  // one has none. The weight here is POUNDS and the height INCHES;
  // BOUNDS.weightKg is kilograms and BOUNDS.heightM is metres. Applying either
  // would refuse every legitimate value on the tile -- spec-v1205's rule, and the
  // mistake this programme has already made once on cpis-vap.
  const o = input && typeof input === 'object' ? input : {};
