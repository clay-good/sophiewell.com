...
export function mandatoryOvertimeCheck(input = {}) {
  ...
  if (sit.value === 'chronic') {
    const why = {
      ...
      NJ: 'N.J.S.A. 34:11-56a33 bars overtime used to fill vacancies from chronic short staffing.',
  ...
  if (st.value === 'NJ') {
    if (sit.value === 'procedure' || sit.value === 'disaster' || sit.value === 'declaration') {
      return { ...base, permitted: null, abnormal: false, bandLabel: 'Only as an unforeseeable emergent circumstance', band: 'New Jersey\'s section names one exception: an unforeseeable emergent circumstance, as a last resort, after reasonable efforts to staff are exhausted. This situation qualifies only if it is such a circumstance.', conditions: ['Last resort only.', 'Reasonable efforts to obtain staffing exhausted first.'] };
    }
    if (o.voluntaryTried === 'no') return { ...base, permitted: false, abnormal: true, bandLabel: 'Not yet permitted', band: 'Even in an unforeseeable emergent circumstance, overtime may be required only as a last resort after the employer has exhausted reasonable efforts to obtain staffing (N.J.S.A. 34:11-56a33).', conditions: [] };
    return { ...base, permitted: true, abnormal: false, bandLabel: 'Permitted as a last resort', band: 'An unforeseeable emergent circumstance, with reasonable staffing efforts exhausted: overtime may be required as a last resort (N.J.S.A. 34:11-56a33).', conditions: ['Not to fill vacancies from chronic short staffing.'] };
  }
