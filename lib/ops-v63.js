...
import { deadline, parseIsoStrict, addCalendarDaysUtc, fmtUtc } from './deadline.js';
import { DATED_AIC } from './partd-appeals-v1503.js';
...
// spec-v1503 backfill: every level counts from RECEIPT of the notice, and 42 CFR 405.942(a)(1),
// 405.962(a)(1), 405.1002(a)(3), 405.1102(a)(2) and 405.1136(c)(2) presume receipt "5 calendar days after
// the date of the notice, unless there is evidence to the contrary". This counted from the notice date
// itself, so every deadline it printed was 5 days early. A later receipt date can be entered with proof.
// The amounts in controversy are set per calendar year of FILING (Federal Register 2026-19016 applies the
// CY2027 amounts to requests filed on or after January 1, 2027), so the year of the deadline picks them.
export function appealDeadline({ level, decisionDate, receivedDate, now }) {
  const step = APPEAL_LEVELS[String(level)];
  if (!step) return null;
  const notice = parseIsoStrict(String(decisionDate ?? '').trim(), 'decision date');
  let receipt = addCalendarDaysUtc(notice, 5);
  let receiptPresumed = true;
  if (receivedDate !== undefined && receivedDate !== null && String(receivedDate).trim() !== '') {
    const r = parseIsoStrict(String(receivedDate).trim(), 'receipt date');
    if (r < notice) throw new RangeError('receipt date cannot come before the notice date');
    receipt = r;
    receiptPresumed = false;
  }
  const d = deadline({ anchor: fmtUtc(receipt), days: step.days, basis: 'calendar', now });
  let aicUsd = null;
  let aicEdition = null;
  if (step.aicUsd) {
    const year = Number(d.deadline.slice(0, 4));
    const row = DATED_AIC[`medicare-aic-${year}`];
    const key = String(level) === 'council' ? 'court' : 'alj';
    if (row) { aicUsd = row.values[key]; aicEdition = row.edition; }
    else aicEdition = `not yet published for ${year}`;
  }
  return {
    completedLevel: step.label,
    nextLevel: step.nextLabel,
    windowDays: step.days,
    receiptDate: fmtUtc(receipt),
    receiptPresumed,
    deadline: d.deadline,
...
export function timelyFiling({ serviceDate, payer, customLimitDays, basis = 'calendar', now }) {
  ...
  let days;
  // spec-v1220: Medicare's one calendar year is statutory, so a custom limit
  // rightly cannot override it -- but a caller who supplied one got 365 back with
  // no word, and the agent-surface label said "overrides the payer default". A
  // value that was given and not used has to be said out loud (rule 21).
  const suppliedLimit = typeof customLimitDays === 'number' && Number.isFinite(customLimitDays)
    ? customLimitDays : null;
  let unusedLimitNote = null;
  if (p === 'medicare') {
    days = 365;
    if (suppliedLimit !== null) {
      unusedLimitNote = `The filing limit of ${suppliedLimit} days entered was not used:`
        + ' the Medicare limit is one calendar year after the date of service and is set by'
        + ' 42 CFR 424.44, not by the plan.';
    }
  } else {
    if (typeof customLimitDays !== 'number' || !Number.isFinite(customLimitDays)
        || !Number.isInteger(customLimitDays) || customLimitDays <= 0) {
      throw new TypeError('custom limit days must be a positive integer for a non-Medicare payer');
    }
...
// spec-v1503 backfill: the CMS-0057-F windows cover medical items and services only -- they "do not
// apply to prior authorization decisions for drugs" -- and QHP issuers on the federal Marketplace are not
// subject to them (CMS-0057-F fact sheet). An expedited request is 72 HOURS from when it arrived, so with a
// request time the deadline is given as a date and time.
export const PA_TURNAROUND_SCOPE = 'These windows cover medical items and services, not drugs (Part D and Part B drugs have their own clocks), and do not apply to Marketplace (QHP) plans on the federal exchange. Medicare Advantage extensions and Part B drug windows: see the Medicare Advantage coverage decision clock.';
export function paTurnaround({ requestDate, type, customDays, requestTime, now }) {
  const t = String(type);
  let days;
  let windowLabel;
  // spec-v1220: `customDays` is read only on the 'custom' branch, so a window
  // supplied alongside 'standard' or 'expedited' was discarded in silence.
  const suppliedDays = typeof customDays === 'number' && Number.isFinite(customDays)
    ? customDays : null;
  const unusedWindowNote = (suppliedDays !== null && t !== 'custom')
    ? `The plan-specified window of ${suppliedDays} days entered was not used:`
      + ` the ${t} window is the one CMS-0057-F sets. Choose the plan-specified request type to use it.`
    : null;
  if (t === 'standard') {
    ...
    if (typeof customDays !== 'number' || !Number.isFinite(customDays)
      ...
      throw new TypeError('custom days must be a positive integer for a plan-specified window');
    }
    days = customDays;
  ...
  const tm = String(requestTime ?? '').trim();
  if (t === 'expedited' && tm) {
    const w = parseDateTime(`${String(requestDate).trim()}T${tm}`);
    if (w === null) throw new RangeError('request time must be HH:MM');
    const end = addHours(w, PA_TURNAROUND_CMS0057.expeditedHours).endWall;
    deadlineTime = new Date(end).toISOString().slice(0, 16);
    deadlineText = formatDeadline(w, end, 'receipt');
  }
  return {
    windowLabel,
    windowDays: days,
