...
export function renderTable(rootEl, opts) {
  ...
  // Wrap wide tables in a horizontally-scrollable, keyboard-focusable region
  // so a table with more columns than a phone is wide pans within its own box
  // instead of forcing the whole page to scroll sideways (mobile no-hscroll
  // guard, test/integration/mobile-no-hscroll.spec.js). role=region + the
  // aria-label give the focusable scroller an accessible name (WCAG 2.2).
