...
export function membranousRisk(input = {}) {
  ...
  // spec-v1063: the three high-risk add-ons are a disjunction, so "none of them"
  // is only true when all three were looked at. The urinary-marker checkbox is a
  // real answer either way, but a blank albumin or anti-PLA2R is a test nobody
  // ran -- and with the albumin blank this tile said "Moderate risk ... and none
  // of the high-risk features" for a patient whose albumin might well have been
  // below 2.5 and put them in the high-risk category.
  const addOnMissing = [];
  if (!highAddOn && albumin === null) addOnMissing.push('a serum albumin');
  if (!highAddOn && pla2r === null) addOnMissing.push('an anti-PLA2R antibody');
  const moderateText = addOnMissing.length
    ? `Moderate risk on what has been entered: a preserved eGFR with proteinuria above ${PROTEINURIA_NEPHROTIC} g/day. This is not yet a moderate-risk classification -- ${addOnMissing.join(' and ')} ${addOnMissing.length > 1 ? 'have' : 'has'} not been entered, and either would move this to high risk if abnormal.`
    : `Moderate risk: a preserved eGFR with proteinuria above ${PROTEINURIA_NEPHROTIC} g/day and none of the high-risk features.`;
