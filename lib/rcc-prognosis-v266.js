...
// key its table does not hold, and 0 is the LOWEST BAND of all six factors --
// pT1, N0, M0, under 5 cm, Fuhrman 1-2, no necrosis. So a factor nobody staged
// was scored as the most favourable pathology there is, and the detail line said
// so out loud: "all factors at their lowest band (score 0)", about six
// determinations nobody had made.
//
//   ssign({})  ->  "SSIGN 0 of 17 — low risk; ~96.8% (score 0-2) 5-year
//                   cancer-specific survival per source."
//
// Every coefficient in the six tables is >= 0, so the total is MONOTONE and what
// is staged gives a FLOOR. `ssignCoefficients` is exported so a test can hold
// that, because the disclosure below is false the day a negative one is added
// (spec-v1107, rule 19).
export const ssignCoefficients = {
  t: SSIGN_T, n: SSIGN_N, m: SSIGN_M, size: SSIGN_SIZE, grade: SSIGN_GRADE, necrosis: SSIGN_NECROSIS,
};

export function ssign(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const spec = [
    ['primary tumor', SSIGN_T, o.tStage, 'the primary tumor stage'],
    ['regional nodes', SSIGN_N, o.nStage, 'the nodal stage'],
    ['metastasis', SSIGN_M, o.mStage, 'the metastasis status'],
    ['tumor size', SSIGN_SIZE, o.size, 'the tumor size'],
    ['nuclear grade', SSIGN_GRADE, o.grade, 'the nuclear grade'],
    ['tumor necrosis', SSIGN_NECROSIS, o.necrosis, 'whether there is coagulative necrosis'],
  ];
  let total = 0;
  const fired = [];
  const unstaged = [];
  for (const [label, table, key, name] of spec) {
    if (!Object.prototype.hasOwnProperty.call(table, key)) { unstaged.push(name); continue; }
  ...
  // Rule 13: "high" is the top group and an unstaged factor cannot bring it back
  // down, so that reading needs no footing. Low and intermediate are floors, and
  // a survival figure is the last thing to quote from one.
  const floored = unstaged.length > 0 && group !== 'high';
  // spec-v1477: the group stands at "high", but the survival figure does not: it falls from ~57.7%
  // (7-9) to ~18.1% (10 and over), so a floor of 7-9 with factors unstaged must not quote the higher one.
  const highFloor = unstaged.length > 0 && group === 'high';
  const notStaged = `${unstaged.length} of the 6 factors ${unstaged.length === 1 ? 'is' : 'are'} not staged (${unstaged.join(', ')})`;
  return { valid: true, score: total, abnormal: total >= 6, unstaged,
    floorOnly: floored || highFloor,
    bandLabel: floored || highFloor ? `SSIGN at least ${total}` : `SSIGN ${total}`,
    band: floored
      ? `SSIGN at least ${total} of 17 on what was staged. ${notStaged}, and each `
        + 'can only raise the score, so no risk group or survival figure follows from this yet.'
      : highFloor
        ? `SSIGN at least ${total} of 17 — high risk, which the unstaged factors cannot lower. ${notStaged}, and each can only raise the score; `
          + 'the survival figure falls further above 9, so none is given until they are staged.'
        : `SSIGN ${total} of 17 — ${group} risk; ${survival} 5-year cancer-specific survival per source.`,
    detail: unstaged.length
      ? `Contributing: ${fired.length ? fired.join(', ') : 'nothing staged so far'}. Not staged: ${unstaged.join(', ')}.`
      : `Contributing: ${fired.length ? fired.join(', ') : 'all factors at their lowest band (score 0)'}.`,
    note: SSIGN_NOTE };
}
