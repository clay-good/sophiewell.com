...
export function ssign(input = {}) {
  ...
  return { valid: true, score: total, abnormal: total >= 6, unstaged,
    floorOnly: floored,
    bandLabel: floored ? `SSIGN at least ${total}` : `SSIGN ${total}`,
    band: floored
      ? `SSIGN at least ${total} of 17 on what was staged. ${unstaged.length} of the 6 factors `
        + `${unstaged.length === 1 ? 'is' : 'are'} not staged (${unstaged.join(', ')}), and each `
