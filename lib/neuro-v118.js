...
// spec-v1134: `input.sah === 'thin' || 'thick' ? input.sah : 'none'` mapped every
// other value -- an unstated one included -- to 'none', which grades 0 and reads
// "no subarachnoid or intraventricular hemorrhage". That is an assertion about a
// CT nobody had read, printed above a vasospasm risk, and 'none' is a real
// finding a radiologist reports rather than the absence of one.
//
// There is no floor to report here: the grade is read off the blood, so with the
// blood ungraded there is no reading to withhold PART of. It asks.
export function modifiedFisher(input = {}) {
  const sahStated = input.sah === 'thin' || input.sah === 'thick' || input.sah === 'none';
  if (!sahStated) {
    return {
      valid: false,
      band: 'Grade the cisternal subarachnoid blood on the CT -- none, thin (< 1 mm) or thick '
        + '(>= 1 mm) -- to read a modified Fisher grade. "None" is a finding on a scan somebody '
        + 'has looked at, and the grade, and the vasospasm risk under it, follow from it.',
      note: MFISHER_NOTE,
    };
  }
...
export function phases(input = {}) {
  ...
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: PHASES_NOTE };
  // spec-v1159: the same defect spec-v1141 fixed on the sibling ELAPSS score, on
  // the tile it sits beside. The refusal message above already says "then choose
  // the POPULATION AND SITE and mark the hypertension and earlier-SAH items" --
  // and both then fell back to their ZERO-POINT level, a North American cohort
  // and an ICA aneurysm. Population is worth 0 to 5 and site 0 to 4: NINE POINTS
  // between them, on a 22-point scale whose 5-year rupture risk runs 0.4% to
  // 17.8%. Rule 23 for the fifth time in this programme.
  //
  // Which artery the aneurysm sits on and which cohort the patient belongs to are
  // known facts, not optional extras, and the score is a sum: unstated, they can
  // only add. The percentage IS the verdict, so the range is disclosed rather than
  // withheld, and where both ends fall in one band the tile answers plainly.
  const popStated = PHASES_POP[input.population] != null;
  const siteStated = PHASES_SITE[input.site] != null;
  const owed = [];
  if (!popStated) owed.push('the population');
  if (!siteStated) owed.push('the site');
  const headroom = (popStated ? 0 : Math.max(...Object.values(PHASES_POP)))
    + (siteStated ? 0 : Math.max(...Object.values(PHASES_SITE)));
  ...
  const ceiling = clamp(total + headroom, 0, 22);
  const riskCeiling = phasesRisk(ceiling);
  const spansBands = owed.length > 0 && riskCeiling !== risk;
  return {
    valid: true, total, risk,
    unstated: owed, floorOnly: owed.length > 0, ceiling: owed.length ? ceiling : null,
    riskCeiling: owed.length ? riskCeiling : null,
    abnormal: total >= 12,
    band: owed.length === 0
      ? `PHASES ${total}/22: 5-year cumulative rupture risk ~${risk}.`
      : (spansBands
        ? `PHASES ${total} to ${ceiling} of 22: ${owed.join(' and ')} `
          + `${owed.length === 1 ? 'is' : 'are'} not stated, and ${owed.length === 1 ? 'it is' : 'together they are'} `
          + `worth ${headroom} points. 5-year cumulative rupture risk between ~${risk} and ~${riskCeiling}.`
        : `PHASES at least ${total}/22: 5-year cumulative rupture risk ~${risk}, which holds whatever `
          + `${owed.join(' and ')} turn${owed.length === 1 ? 's' : ''} out to be.`),
...
export function elapss(input = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = boundsAdvisory('ageYears', age);
  if (ageFault) return { valid: false, band: ageFault, message: ageFault, note: ELAPSS_NOTE };
  // spec-v1141: `ELAPSS_LOC[input.location] != null ? input.location : 'icaAcaAcom'`
  // and the same line for the population each fall back to their ZERO-point
  // level -- an ICA/ACA/ACOM aneurysm in a North American cohort. Between them
  // they are worth 12 points against risk bands 5 points wide, so an age and a
  // size alone answered "ELAPSS 14/40: aneurysm growth risk ~11.7% at 3 years,
  // ~19.3% at 5 years" when the same patient could be a 26 and ~42.7%.
  //
  // The tile's own refusal message already lists them -- "then choose the
  // earlier-SAH, location, population, and shape items" -- and then answered
  // without them (rule 23: the list is a claim).
  //
  // Which artery the aneurysm is on and which cohort the patient belongs to are
  // known facts, not optional extras, and the score is a sum: unstated, they can
  // only add. The percentage IS the verdict here, so the range is disclosed
  // rather than withheld (as euroscore2 and impede-vte do), and where both ends
  // fall in one band the tile answers plainly.
  const locStated = ELAPSS_LOC[input.location] != null;
  const popStated = ELAPSS_POP[input.population] != null;
  const owed = [];
  if (!locStated) owed.push('the location');
  if (!popStated) owed.push('the population');
  const headroom = (locStated ? 0 : Math.max(...Object.values(ELAPSS_LOC)))
    + (popStated ? 0 : Math.max(...Object.values(ELAPSS_POP)));
  ...
  const ceiling = clamp(total + headroom, 0, 40);
  const riskCeiling = elapssRisk(ceiling);
  const spansBands = owed.length > 0 && riskCeiling.three !== risk.three;
  return {
    valid: true, total, riskThree: risk.three, riskFive: risk.five,
    unstated: owed, floorOnly: owed.length > 0, ceiling: owed.length ? ceiling : null,
    // spec-v1159: the view's detail rows need the ceiling's risk too, or they
    // restate the floor as the total underneath a band that gives the range.
    riskCeilingThree: owed.length ? riskCeiling.three : null,
    riskCeilingFive: owed.length ? riskCeiling.five : null,
    abnormal: total >= 20,
    band: owed.length === 0
      ? `ELAPSS ${total}/40: aneurysm growth risk ~${risk.three} at 3 years, ~${risk.five} at 5 years.`
      : (spansBands
        ? `ELAPSS ${total} to ${ceiling} of 40: ${owed.join(' and ')} `
          + `${owed.length === 1 ? 'is' : 'are'} not stated, and ${owed.length === 1 ? 'it is' : 'together they are'} `
          + `worth ${headroom} points. Growth risk between ~${risk.three} and ~${riskCeiling.three} at 3 years, `
          + `~${risk.five} to ~${riskCeiling.five} at 5 years.`
        : `ELAPSS at least ${total}/40: aneurysm growth risk ~${risk.three} at 3 years, ~${risk.five} at `
          + `5 years, which holds whatever ${owed.join(' and ')} turn${owed.length === 1 ? 's' : ''} out to be.`),
