...
export function nyHealthCareProxyCheck(input = {}) {
  ...
  // spec-v1467: each witness rule binds only the setting 2981(2)(b)/(c) names, the same scope as
  // the questions asked above; a "no" outside that setting is not a defect.
  if (mhf && o.unaffiliated === 'no') defects.push(`no witness is unaffiliated with the facility (2981(2)(${fac.value === 'opwdd' ? 'c' : 'b'}))`);
  if (fac.value === 'omh-hospital' && o.psychWitness === 'no') defects.push('no witness is a qualified psychiatrist or psychiatric nurse practitioner (2981(2)(b))');
  if (fac.value === 'opwdd' && o.opwddClinician === 'no') defects.push('no witness is a clinician meeting the OPWDD criteria (2981(2)(c))');
