...
export function corpusDesc(row) {
  ...
  return [
    ...
    row.when,
    row.expected,
    // spec-v1185: the words a tile can ANSWER FOR -- the option values in its own
    // picklists, which its prose never says. `opioid-conversion` names the two
    // drugs in its title and thirteen more in its menus; "morphine to
    // hydromorphone" used to return NOTHING.
    row.answers,
  ].filter(Boolean).join(' ');
}
...
export function corpusOneLiner(row, max = 120) {
  if (!row || typeof row !== 'object') return '';
  // spec-v1185's `answers` is deliberately NOT in this list. It is an index
  // channel -- registry values written for agents, not sentences for a person --
  // and this repo has printed one of those to a reader before (`onevaso`). A row
  // whose only field is `answers` renders no one-liner rather than a bare list of
  // drug names where a sentence belongs. Pinned by
  // test/unit/findable-by-what-it-answers.test.js.
