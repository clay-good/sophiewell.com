...
export function albiGrade(input = {}) {
  ...
  // spec-v1178: this function converts g/dL to g/L itself, one line below. A
  // reader who enters the g/L figure off the report has it multiplied by ten
  // AGAIN -- an albumin of 40 becomes 400 g/L, which drives the score deeply
  // negative and reads as grade 1, "the best preserved liver function". The
  // ceiling is BOUNDS.albumin, whose own note says values outside it "indicate a
  // unit or entry error".
  if (albuminDl > BOUNDS.albumin.max) {
    return { valid: false, message: `A serum albumin of ${albuminDl} g/dL is above ~${BOUNDS.albumin.max}, beyond recorded extremes. An albumin reported in g/L is ten times the g/dL figure, so ${albuminDl} g/L is entered as ${albuminDl / 10}.` };
  }
  // spec-v1232: the albumin ceiling above has been here since spec-v1178 and the
  // bilirubin beside it had none -- the half-guarded shape spec-v1063 named. A
  // bilirubin of 600 mg/dL drove the log term the other way and the tile read
  // "grade 3: the poorest liver function", which is at least the alarming
  // direction, but it is still a grade read off an impossible lab.
  const biliFault = boundsAdvisory('bilirubin', biliMg);
  if (biliFault) return { valid: false, message: biliFault, band: biliFault };
...
export function meldXi(input = {}) {
  ...
  // spec-v1232: both inputs are floored at 1.0 and then logged, and a log
  // compresses -- so an impossible bilirubin does not produce an impossible
  // score, it produces a plausible one. 600 mg/dL read "MELD-XI 47" beside real
  // scores that top out around 40.
  for (const [key, v] of [['bilirubin', bili], ['scr', creat]]) {
...
export function fornsIndex(input = {}) {
  ...
  // spec-v1174: platelets are in the denominator of both scores here, so the
  // x1000 unit confusion (a US report reads 280,000/uL for what this field wants
  // as 280) drives the result toward the RULE-OUT. Ceiling from BOUNDS.platelets.
  if (plt > BOUNDS.platelets.max) return { valid: false, message: `A platelet count of ${plt} is above ~${BOUNDS.platelets.max}, beyond recorded extremes. This field is in x10^9/L (the same as x10^3/uL), so a lab report reading ${plt}/uL is entered as ${plt / 1000}.` };
  // spec-v1406: the age envelope lib/bounds.js declares; an impossible age computed.
  const ageFault = boundsAdvisory('ageYears', age);
  if (ageFault) return { valid: false, message: ageFault };
...
export function bardScore(input = {}) {
  ...
  const ast = pos(o.ast);
  const alt = pos(o.alt);
  // spec-v1237: the BMI is the only input here BOUNDS names in this field's own
  // unit -- AST and ALT have no entry in the table. Every item is a threshold
  // comparison (BMI >= 28), so past the threshold the size stops mattering.
  const bmiFault = bmi === null ? null : boundsAdvisory('bmi', bmi);
  if (bmiFault) return { valid: false, message: bmiFault, band: bmiFault };
  const counted = [];
  let total = 0;
  ...
  // spec-v1007: BARD is monotone -- BMI, the AST/ALT ratio and diabetes each add
  // points or leave them alone -- so a partial total is a LOWER BOUND. With no
  // labs at all this used to answer "BARD 0/4: 0 to 1 -- advanced fibrosis is
  // robustly ruled out", which is the whole clinical purpose of the score and the
  // one reading a missing measurement cannot support: the unentered BMI and ratio
  // carry 3 of the 4 points. Once the total reaches 2 the rule-in stands whatever
  // is missing. Diabetes is a checkbox: unchecked is an answer.
  const missing = [];
  if (bmi === null) missing.push('BMI');
  if (ast === null || alt === null) missing.push('AST and ALT (for the ratio)');
  if (missing.length && !high) {
    return {
      valid: false, total: null, ratio: null, abnormal: false,
      band: `Enter the rest of BARD to score. Missing: ${missing.join(', ')}. `
        + 'The unentered components can only add points, so a partial BARD cannot rule advanced fibrosis out.',
      counted: counted.length ? counted.join(', ') : 'no BARD points yet',
      note: BARD_NOTE,
    };
  }
  const partial = missing.length
    ? ` Scored from ${3 - missing.length} of 3 components; the rest can only raise it.`
    : '';
  return {
    valid: true, total, ratio: ratio === null ? null : r2(ratio),
    abnormal: high,
    band: `BARD ${total}/4: ${high ? '2 to 4 -- advanced fibrosis is not ruled out (odds ratio about 17)' : '0 to 1 -- advanced fibrosis is robustly ruled out'}.${partial}`,
...
export function fattyLiverIndex(input = {}) {
  ...
  // spec-v1237: as in bardScore above -- BMI is the one input with an envelope in
  // its own unit. The waist is in cm and triglycerides and GGT have no entry.
  const fliBmiFault = bmi === null ? null : boundsAdvisory('bmi', bmi);
  if (fliBmiFault) return { valid: false, message: fliBmiFault, band: fliBmiFault };
...
export function lokIndex(input = {}) {
  ...
  // spec-v1174, as in fornsIndex above.
  if (plt > BOUNDS.platelets.max) return { valid: false, message: `A platelet count of ${plt} is above ~${BOUNDS.platelets.max}, beyond recorded extremes. This field is in x10^9/L (the same as x10^3/uL), so a lab report reading ${plt}/uL is entered as ${plt / 1000}.` };
  // spec-v1406: the INR envelope lib/bounds.js declares; an impossible INR computed.
  const inrFault = boundsAdvisory('inr', inr);
  if (inrFault) return { valid: false, message: inrFault };
  const ratio = ast / alt;
