...
//
// Each of the three cost-share terms defaults to 0 because a real benefit design
// can genuinely have any ONE of them at zero -- a copay-only plan has no
// coinsurance, a met deductible has nothing remaining. A design with ALL THREE
// at zero is not a design: it is an empty form, and the answer it produced was
// "the patient owes $0.00" with the plan paying the whole allowed. That is the
// reassuring reading, and it is the one that can be wrong (rule 1 and rule 2,
// docs/product-decisions.md).
//
// So the terms stay individually optional and the SET is required: state one and
// the tool answers, state none and it says what it is missing rather than
// concluding a balance of zero from it.
const stated = (v) => v !== null && v !== undefined && v !== '';
function anyCostShareStated(input) {
  return stated(input.deductibleRemainingCents)
    || stated(input.coinsurancePct)
    || stated(input.copayCents);
}

const COST_SHARE_ASK = 'Enter the remaining deductible, the coinsurance percentage, or the copay: none of the three was entered, and a benefit design with no cost-share at all is not one -- the patient balance cannot be computed from it and is not zero.';
...
export function cobCalc(input) {
  ...
  if (primaryPaidCents > primaryAllowedCents) {
    throw new RangeError('primary paid cents cannot exceed primary allowed cents');
  }
  if (secondaryWouldPayCents > secondaryAllowedCents) {
    throw new RangeError('secondary would pay cents cannot exceed secondary allowed cents');
  }

  // spec-v1142: three of the four methods are DEFINED by what the secondary
  // would have paid as primary -- lesser-of takes the lower of it and the
  // balance, non-duplication takes it minus the primary payment, MSP takes the
  // lowest of it and two caps. Defaulting it to 0 made all three pay $0 and
  // hand the patient the whole balance the primary left, from a field nobody
  // entered. Come-out-whole does not use it and stays optional.
  if (method !== 'come-out-whole' && !stated(input.secondaryWouldPayCents)) {
    throw new RangeError(`the ${method} method is defined by what the secondary would pay as primary -- enter it; a blank one is not a benefit of zero`);
  }
...
export function allowedAmount(input) {
  ...
  const costShareStated = anyCostShareStated(input);

  // The write-off is contract arithmetic and does not depend on the benefit
  // terms, so it is still reported; the patient's share is not.
  if (!costShareStated) {
    return {
      inNetwork,
      costShareStated: false,
      billedChargeCents, allowedCents,
      contractualWriteOffCents: inNetwork ? chargeMinusAllowedCents : 0,
      deductibleAppliedCents: null,
      coinsuranceCents: null,
      copayCents: null,
      patientResponsibilityCents: null,
      payerPaymentCents: null,
      balanceBillProhibited: inNetwork && chargeMinusAllowedCents > 0,
      note: `${inNetwork
        ? `In-network: the $${(chargeMinusAllowedCents / 100).toFixed(2)} above the allowed is a contractual write-off${chargeMinusAllowedCents > 0 ? ' that may NOT be billed to the patient (balance billing is prohibited)' : ''}.`
        : `Out-of-network: there is NO contractual write-off -- the $${(chargeMinusAllowedCents / 100).toFixed(2)} above the allowed MAY be balance-billed to the patient.`} ${COST_SHARE_ASK}`,
    };
  }
...
export function nsaCostShare(input) {
  ...
  // spec-v1155: the protected cost-share is computed off the QPA and never reads
  // the billed charge -- the charge decides only the prohibited balance-bill
  // figure. Requiring it refused every call that wanted the cost-share alone.
  const chargeStated = input.billedChargeCents !== null && input.billedChargeCents !== undefined && input.billedChargeCents !== '';
  const billedChargeCents = Math.round(num('billedChargeCents', chargeStated ? input.billedChargeCents : 0, { min: 0, max: 1e11 }));
  const deductibleRemainingCents = Math.round(num('deductibleRemainingCents',
    input.deductibleRemainingCents == null ? 0 : input.deductibleRemainingCents, { min: 0, max: 1e9 }));
  const coinsurancePct = num('coinsurancePct',
    input.coinsurancePct == null ? 0 : input.coinsurancePct, { min: 0, max: 100 });
  ...
  //
  // spec-v1142: the prohibited balance bill is the NSA protection itself and
  // does not depend on the benefit terms, so it is still reported. The
  // cost-share is what the terms decide, and with none of them entered the
  // answer was "the patient owes $0.00, the plan pays the whole QPA".
  if (!anyCostShareStated(input)) {
    return {
      protected: true,
      costShareStated: false,
      serviceCategory: category,
      ...
      patientCostShareCents: null,
      planPaysCents: null,
      prohibitedBalanceBillCents: Math.max(0, billedChargeCents - qpaCents),
      note: `NSA-protected (${category === 'emergency' ? 'emergency' : 'ancillary at an in-network facility'}): balance billing is PROHIBITED -- the $${(Math.max(0, billedChargeCents - qpaCents) / 100).toFixed(2)} above the QPA may NOT be charged to the patient. ${COST_SHARE_ASK}`,
    };
  }
  ...
  return {
    ...
    chargeStated,
    note: `NSA-protected (${category === 'emergency' ? 'emergency' : 'ancillary at an in-network facility'}): the patient's cost-share is capped at the in-network amount on the QPA. Balance billing is PROHIBITED${chargeStated ? ` -- the $${(prohibitedBalanceBillCents / 100).toFixed(2)} above the QPA may NOT be charged to the patient` : '. The provider billed charge is not entered, so how much sits above the QPA is not shown -- that figure is the prohibited balance bill, not the cost-share'}.`,
  };
}
