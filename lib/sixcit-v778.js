...
export function sixcit(input = {}) {
  ...
  // spec-v1115: `step` returns 0 for a blank, and 0 on the 6CIT means the task
  // was performed correctly. The three checkbox items are real answers (rule 4);
  // these three are graded selects, and an unadministered one scored as a task
  // the patient passed. Since points are earned only for ERRORS the total is
  // monotone, so an unadministered task can only raise it -- the referral bands
  // rule in from a subset (rule 13) and "normal range" is the reading that waits.
  const unadministered = [];
  const mark = (raw, name) => {
    if (raw === '' || raw === null || raw === undefined) unadministered.push(name);
  };
  mark(o.countErrors, 'counting backward');
  mark(o.monthsErrors, 'months in reverse');
  mark(o.addressErrors, 'address recall');
  ...
  const floored = unadministered.length > 0 && total <= 7;
  ...
  return {
    ...
    parts,
    abnormal: total >= 8,
    bandLabel: floored ? `6CIT at least ${total} of 28` : `6CIT ${total} of 28`,
    band: floored
      ? `6CIT at least ${total} of 28: ${unadministered.join(', ')} `
        + `${unadministered.length === 1 ? 'was' : 'were'} not administered, and points are earned `
        + 'for errors, so each can only raise the score. Not yet the normal range.'
      : `6CIT ${total} of 28 — ${label}.`,
