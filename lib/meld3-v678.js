...
export function meld3(input = {}) {
  ...
  const bilirubinFault = inputFault([['serum bilirubin', o.bilirubin, null, 100, 'mg/dL']]);
  if (bilirubinFault) {
    return { valid: false, code: bilirubinFault.startsWith('Enter') ? 'MISSING_INPUT' : 'INVALID_INPUT', field: 'bilirubin', message: bilirubinFault, note: MELD3_NOTE };
  }
  const bili = pos(o.bilirubin, 100);
  const inrFault = inputFault([['INR', o.inr, null, 30, '']]);
  if (inrFault) {
    return { valid: false, code: inrFault.startsWith('Enter') ? 'MISSING_INPUT' : 'INVALID_INPUT', field: 'inr', message: inrFault, note: MELD3_NOTE };
  }
  const inr = pos(o.inr, 30);
  const creatinineFault = inputFault([['serum creatinine', o.creatinine, null, 40, 'mg/dL']]);
  if (creatinineFault) {
    return { valid: false, code: creatinineFault.startsWith('Enter') ? 'MISSING_INPUT' : 'INVALID_INPUT', field: 'creatinine', message: creatinineFault, note: MELD3_NOTE };
  }
  const creatRaw = pos(o.creatinine, 40);
  const sodiumFault = inputFault([['serum sodium', o.sodium, null, 200, 'mEq/L']]);
  if (sodiumFault) {
    return { valid: false, code: sodiumFault.startsWith('Enter') ? 'MISSING_INPUT' : 'INVALID_INPUT', field: 'sodium', message: sodiumFault, note: MELD3_NOTE };
  }
  const sodium = pos(o.sodium, 200);
  const albuminFault = inputFault([['serum albumin', o.albumin, null, 10, 'g/dL']]);
  if (albuminFault) {
    return { valid: false, code: albuminFault.startsWith('Enter') ? 'MISSING_INPUT' : 'INVALID_INPUT', field: 'albumin', message: albuminFault + (albuminFault.startsWith('Enter') ? '' : ' An albumin reported in g/L is ten times the g/dL figure.'), note: MELD3_NOTE };
  }
