...
export function nioshLifting(input = {}) {
  ...
  // spec-v1096: a blank lift rate takes the frequency multiplier as 1.00, which
  // is the BEST value in the published table -- the multiplier for a task with
  // no frequency penalty at all. Every other rate reduces it, so the recommended
  // weight limit is at its maximum and the lifting index at its minimum, which
  // is the wrong side of the 1.0 line the whole number exists to sit against.
  // One lift per minute took an index of 0.96, "at or below 1.0", to 1.03,
  // "above 1.0, so the task exceeds what the equation recommends".
  //
  // Silent once the index is already above 1.0: the frequency multiplier can
  // only push it further that way, so the reading is already the floor.
  const frequencyEntered = f !== null;
  const frequencyNote = !frequencyEntered && indexBand === 'within'
    ? 'The lifting frequency was not entered, so the frequency multiplier is taken as 1.00, its best value in the published table. Any real lift rate lowers it, which lowers the recommended weight limit and raises the lifting index.'
    : null;
  // spec-v1465: the same holds for the duration (once a lift rate is entered), the coupling and the
  // asymmetry angle. Each blank takes its multiplier's best value, so it is named on the same side
  // of the line.
  const blankIs = (v) => v === undefined || v === null || String(v).trim() === '';
  const bestCase = [];
  if (frequencyEntered && blankIs(o.duration)) bestCase.push('the duration was taken as one hour or less');
  if (blankIs(o.coupling)) bestCase.push('the coupling was taken as good');
  if (a === null) bestCase.push('the asymmetry angle was taken as 0 degrees');
  const assumedNote = bestCase.length && indexBand === 'within'
    ? `Not entered, so read at the most favorable value: ${bestCase.join('; ')}. Each can only lower the recommended weight limit and raise the lifting index.`
    : null;
  ...
  return {
    ...
    assumedNote,
    band: [action, frequencyNote, assumedNote].filter(Boolean).join(' '),
