...
export function crsGrade(input = {}) {
  ...
  // spec-v1120: both selects fell through to 'none' -- no hypotension, no
  // hypoxia -- and the grade is the MAX of the two, so an unstated one could
  // only ever have raised it. The tile then said so in words: "grade 1 of 4:
  // fever >=38 C WITHOUT HYPOTENSION OR HYPOXIA", and at grade 0 "no fever,
  // hypotension, or hypoxia" -- two observations asserted about a patient on
  // CAR-T, where the grade decides tocilizumab and ICU transfer (rule 11).
  //
  // Grades 3 and 4 rule in and are untouched (rule 13). Grades 0 and 1 are the
  // readings an unstated column can undo.
  const hypoStated = Object.prototype.hasOwnProperty.call(HYPOTENSION, o.hypotension);
  const hypoxStated = Object.prototype.hasOwnProperty.call(HYPOXIA, o.hypoxia);
  const unstated = [
    !hypoStated && 'the hypotension',
    !hypoxStated && 'the hypoxia',
  ].filter(Boolean);
  const hypoKey = hypoStated ? o.hypotension : 'none';
  const hypoxKey = hypoxStated ? o.hypoxia : 'none';
  ...
  const floored = unstated.length > 0 && grade <= 1;
  let band;
  if (floored) {
    band = `ASTCT CRS is at least grade ${grade} of 4 on what was stated: `
      + `${unstated.join(' and ')} ${unstated.length === 1 ? 'is' : 'are'} not stated, and the `
      + 'grade is the more severe of the two, so each can only raise it.';
  } else if (grade === 0) {
  ...
  return {
    ...
    floorOnly: floored,
    bandLabel: floored
      ? `CRS at least grade ${grade}`
      : (grade === 0 ? 'No CRS' : `CRS grade ${grade}`),
    band,
    note: NOTE,
  };
}
