...
export function renderDerivation(meta) {
  ...
  // The tile already carries a "Citation and sources" disclosure, and this one
  // sits directly above it. Both were named for provenance -- "Where does this
  // come from?" and "Citation and sources" are the same question asked twice --
  // so on the 126 tiles that have both, the reader had to open one to find out
  // it was the other they wanted. This block holds the method: the formula, who
  // it was derived on, where it stops being valid, and the arithmetic for the
  // inputs on screen. Name it that, and let the citation control own the proof.
