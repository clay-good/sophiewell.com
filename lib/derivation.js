...
//
// Reads META[id].derivation (see spec-v48.md §3) and emits a method section
// summarizing the formula, original population, validity, and the verbatim
// source quote. app.js moves it into the shared calculation disclosure. When
// `components` is present, updateDerivationSteps re-fills the
...
export function renderDerivation(meta) {
  ...
  const section = el('section', { class: 'tile-derivation', 'aria-label': 'Calculation method' });
...
// spec-v1071: a calculator that REFUSES has to take its working with it.
//
// The blank-field waves (spec-v1063 onward) gave a dozen calculators an early
// return: when a measurement they need is missing they print "Enter a heart rate
// to score" instead of a number. That return skipped this function, so the
// "show your work" panel underneath went on displaying the last complete
// calculation -- egfr answered "Enter an age to calculate" above a panel still
// reading "eGFR = ... = 64.5 mL/min/1.73 m^2", which is the very number the
// guard exists to withhold. Two surfaces on one page, disagreeing.
//
// Call this on the refusal path.
export function clearDerivationSteps(detailsEl, message) {
  if (!detailsEl) return;
  const steps = detailsEl.querySelector(`[${STEPS_DATA_ATTR}]`);
  if (!steps) return;
  clear(steps);
  steps.appendChild(el('p', { class: 'muted', text: message || 'Enter the missing values to see the calculation.' }));
}
