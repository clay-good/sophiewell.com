...
// spec-v1232: every criterion in this file is a threshold comparison -- platelet
// < 30, creatinine <= 2.26, FDP >= 25 -- and past a threshold the size of the
// number stops mattering. A platelet count of 20,000 x10^3/uL simply failed the
// "< 30" criterion and PLASMIC still returned an ADAMTS13 probability band; a
// creatinine of 250 mg/dL failed "<= 2.26" and French TTP counted the rest.
// Envelope and wording from BOUNDS + boundsAdvisory, checked after each
// function's own missing-value branch (spec-v1207) so a blank field is still
// asked for.
...
export function plasmicTtp(input = {}) {
  ...
  const fault = envelopeFault([['platelets', platelet], ['scr', creatinine], ['inr', inr]]);
...
export function frenchTtp(input = {}) {
  ...
  const fault = envelopeFault([['platelets', platelet], ['scr', creatinine]]);
  if (fault) return fault;
  const scored = [];
  let total = 0;
  if (platelet < 30) { total += 1; scored.push('platelet < 30'); }
...
export function jaamDic(input = {}) {
  ...
  const fault = envelopeFault([['platelets', platelet], ['platelets', priorPlatelet === undefined ? null : priorPlatelet]]);
  if (fault) return fault;
