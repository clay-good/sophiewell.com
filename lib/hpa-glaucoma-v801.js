...
export const HPA_NOTE = 'The Hodapp-Parrish-Anderson criteria (Hodapp E, Parrish RK, Anderson DR, Clinical Decisions in Glaucoma, Mosby 1993) stage glaucoma from the visual field alone, using four readings from one test: the mean deviation, the share of points below the 5 percent level on pattern deviation, the number of points below the 1 percent level, and what is happening within the central 5 degrees. Each reading gives its own grade and the most severe of the four is the overall grade, which is the rule that matters, because a field can look early on mean deviation and be severe on its central points. A mean deviation better than minus 6 is early, minus 6 to minus 12 moderate, and worse than minus 12 severe; up to a quarter of points below the 5 percent level is early, up to half moderate and more than half severe; under 10 points below the 1 percent level is early, 10 to 20 moderate and over 20 severe; and in the central 5 degrees a depression in both hemifields, or any point at 0 decibels, is severe. It stages a field a clinician has already reviewed for reliability and artifact; it does not read the printout, it says nothing about intraocular pressure or the optic nerve, and it sets no treatment target.';
...
export function hpaGlaucoma(input = {}) {
  ...
  // spec-v1123: this fell back to 'all-above-15' -- the mildest central finding
  // -- and the overall grade is the MOST SEVERE of the four criteria, so an
  // unstated central 5 degrees could only ever have raised it. On the tile's own
  // example, leaving that one select alone took the grade from SEVERE to EARLY:
  // two full grades, on the criterion the source treats as decisive because a
  // central defect is the one that takes reading vision.
  //
  // The three numeric criteria already omit themselves when blank, which is
  // right -- but nothing said they had. Both halves are disclosed now.
  const centralStated = !(o.central === undefined || o.central === null || o.central === '');
  const centralKey = centralStated ? String(o.central).trim() : 'all-above-15';
  if (centralStated && !Object.prototype.hasOwnProperty.call(CENTRAL, centralKey)) {
  ...
  if (centralStated) parts.push({ name: 'central 5 degrees', grade: CENTRAL[centralKey].grade, detail: CENTRAL[centralKey].text });

  const unassessed = [
    md === null && 'the mean deviation',
    pct5 === null && 'the points below the 5% level',
    count1 === null && 'the points below the 1% level',
    !centralStated && 'the central 5 degrees',
  ].filter(Boolean);
  ...
  return {
    ...
    drivers,
    unassessed,
    // Rule 13: `severe` is the top level and an unassessed criterion cannot
    // bring it down, so it needs no footing. Every gentler grade is a floor.
    floorOnly: unassessed.length > 0 && overall !== 'severe',
    abnormal: overall !== 'none',
    bandLabel: unassessed.length > 0 && overall !== 'severe'
      ? `Hodapp-Parrish-Anderson: at least ${overall}`
      : `Hodapp-Parrish-Anderson: ${overall}`,
    band: (unassessed.length > 0 && overall !== 'severe')
      ? `Hodapp-Parrish-Anderson: at least ${overall === 'none' ? 'no defect' : `${overall} defect`} `
        + `on the ${parts.length} of the 4 criteria read. ${unassessed.join(', ')} `
        + `${unassessed.length === 1 ? 'was' : 'were'} not entered, and the grade is the most `
        + 'severe of the four, so each can only raise it.'
      : (overall === 'none'
