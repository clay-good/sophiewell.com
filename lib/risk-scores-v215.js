...
export function simonBroomeFh(input = {}) {
  ...
  const ldl = pos(o.ldl, 50);
  const fault = gradeFault([
    ['Total cholesterol (mmol/L)', o.totalChol, 0, 50],
    ['LDL-C (mmol/L)', o.ldl, 0, 50],
  ]);
  if (fault) return { valid: false, message: fault };
  if (tc === null && ldl === null) {
...
export function lipi(input = {}) {
  ...
  const wbc = pos(o.wbc);
  const fault = gradeFault([
    ['Absolute neutrophil count', o.anc, 0, Infinity],
    ['Total WBC', o.wbc, 0, Infinity],
  ]);
  if (fault) return { valid: false, message: fault };
