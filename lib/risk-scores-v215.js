...
export function dlcnFh(input = {}) {
  ...
  const fault = gradeFault([
    ['Untreated LDL-C (mmol/L)', o.ldl, 0, 50],
  ]);
...
export function simonBroomeFh(input = {}) {
  ...
  // spec-v1095: the cholesterol criterion is an OR over two lipids, each tested
  // as "entered AND above threshold", so a lipid nobody measured reads as a
  // lipid below threshold. With only one entered and below the line the tile
  // said "cholesterol criterion not met" -- a statement about a measurement it
  // did not have. The other lipid at a plausible value takes the same patient
  // to definite FH, which is the difference between a cascade-screening
  // referral and none.
  else if (tc === null || ldl === null) {
    const missing = tc === null ? 'Total cholesterol' : 'LDL-C';
    tier = `cholesterol criterion not met on the ${tc === null ? 'LDL-C' : 'total cholesterol'} entered — FH not classified. `
      + `${missing} was not entered, and either lipid alone can meet the criterion, so this does not rule familial hypercholesterolemia out`;
    abnormal = false;
...
export function grimScore(input = {}) {
  ...
  const fault = gradeFault([
    ['Serum albumin (g/dL)', o.albumin, 0, 10],
    ['Neutrophil-to-lymphocyte ratio', o.nlr, 0, 1000],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function lipi(input = {}) {
  // spec-v1237: no envelope here, deliberately. The LIPI is a ratio of two
  // counts and this tile does not fix their unit -- its own message asks for
  // them "same units", whichever those are. BOUNDS.wbc is a claim about a white
  // count in x10^3/uL specifically, and applying a unit-specific envelope to a
  // field whose unit is unstated is spec-v1205's trap exactly.
  //
  // Worth knowing while reading this: the caps below are 1000, which in practice
  // admits the x10^9/L convention and refuses a count entered per uL -- so the
  // message promises a latitude the bound does not give. That mismatch is real
  // and is not this wave's to settle.
  const o = input && typeof input === 'object' ? input : {};
  const anc = pos(o.anc, 1000);
  const wbc = pos(o.wbc, 1000);
  const fault = gradeFault([
    ['Absolute neutrophil count (×10⁹/L)', o.anc, 0, 1000],
    ['Total WBC (×10⁹/L)', o.wbc, 0, 1000],
  ]);
  if (fault) return { valid: false, message: fault };
