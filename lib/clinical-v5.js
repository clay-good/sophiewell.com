...
// (hypernatremia).
//
// Citation: Adrogue HJ, Madias NE. Hyponatremia. NEJM 2000;342:1581-1589.
// (The sibling Hypernatremia paper is 342:1493-1499 and `free-water-deficit` cites that one.
// The two were transposed until spec-v941; keep them apart.)
const INFUSATE_NA = {
  '3pct-saline': 513,
  '0.9-saline': 154,
  '0.45-saline': 77,
...
export function sodiumCorrection({ weightKg, sex, age, currentNa, infusate, targetChangePer24h, acuity = 'chronic' }) {
  ...
  // spec-v62 §2 A3 / §5: reverse-solve must never silently exceed the published
  // safe-correction ceiling. When the requested target is over the ceiling, also
  // surface the rate that achieves exactly the ceiling (the max safe rate), so
  // the bedside user sees both "the rate your target implies" and "the most you
  // should run." Below the ceiling there is nothing to cap, so this stays null.
  const cappedChange = desiredSign * Math.min(targetChangePer24h, safetyCap);
  const cappedVolumeLiters = sameSign ? cappedChange / changePerLiter : null;
  const cappedRatePerHour = cappedVolumeLiters != null ? (cappedVolumeLiters * 1000) / 24 : null;
  return {
    tbwLiters: r2(tbw),
    direction,
    acuity,
    ...
    cappedRateMlPerHour: overCap && cappedRatePerHour != null ? r1(cappedRatePerHour) : null,
    safetyCap,
    overCap,
    directionMismatch,
    safetyNote: overCap
...
export function freeWaterDeficit({ weightKg, sex, age, currentNa, targetNa = 145, replaceOverHours = 48 }) {
  ...
  if (currentNa <= targetNa) {
    throw new RangeError('current na must exceed target na; free-water deficit only applies to hypernatremia');
  }
  let factor;
  const elderly = age != null && age >= 65;
  if (sex === 'M') factor = elderly ? 0.5 : 0.6;
  ...
  // spec-v62 §2 A3 / §5: when the chosen replacement schedule drops Na faster
  // than the 10 mEq/L/24h ceiling, also surface the rate that drops it at
  // exactly the ceiling (delivering the same total deficit, just over the safe
  // minimum number of hours). Na-drop is linear in the rate, so scaling the
  // rate by 10/impliedDrop hits the ceiling. Null when within the ceiling.
  const cappedRatePerHourMl = exceedsSafeDrop && impliedNaDropPer24h > 0
    ? (ratePerHourMl * 10) / impliedNaDropPer24h
    : null;
  return {
    tbwLiters: r2(tbw),
    deficitLiters: r2(deficitL),
    replacementRateMlPerHour: r1(ratePerHourMl),
    cappedReplacementRateMlPerHour: cappedRatePerHourMl != null && Number.isFinite(cappedRatePerHourMl) ? r1(cappedRatePerHourMl) : null,
...
export function ironDeficitGanzoni({ weightKg, currentHb, targetHb = 15, ironStoresMg }) {
  ...
  if (currentHb >= targetHb) {
    throw new RangeError('current hb must be below target hb; Ganzoni only applies when replacement is needed');
  }
...
export function pbwArdsnet({ heightCm, sex, mlPerKg = 6 }) {
  ...
  // Devine equation is validated above 60 inches (152.4 cm). Below that it is
  // extrapolated and a warning is surfaced. Far below 60 in the linear formula
  // goes non-physical (PBW <= 0); spec-v53 §2.2: instead of clamping to 0 and
  // showing "0 kg / 0 mL" tidal-volume targets as if they were real, return
  // null targets so the renderer shows a fallback next to the warning.
  const belowDevineFloor = heightIn < 60;
  const warning = belowDevineFloor
    ...
    : null;
  if (pbw <= 0) {
    return { pbwKg: null, vtTargetMl: null, vtRangeMl: { low: null, high: null }, warning };
  }
...
// Citation: Runyon BA, Montano AA, Akriviadis EA, et al. Ann Intern Med. 1992;117(3):215-220.
...
// 2012;2:1-138.
// spec-v1158: `anuriaHours = 0` was a DEFAULT PARAMETER, so "no anuria" and "nobody
// asked about anuria" were the same value -- rule 21, and the reason the message
// below could not tell them apart. `null` now, and the renderer passes the blank
// through instead of `Number('')`.
export function kdigoAki({ baselineCr, currentCr, riseInLast48h, uoMlPerKgPerHour, uoDurationHours, anuriaHours = null, rrtInitiated = false }) {
  ...
  // spec-v1158: two things were wrong on the urine-output side.
  //
  // 1. ANURIA >= 12 H IS STAGE 3 ON ITS OWN. KDIGO's urine-output criterion for
  //    stage 3 is "< 0.3 mL/kg/h for >= 24 h OR anuria for >= 12 h", and the
  //    anuria test sat INSIDE a gate requiring the mL/kg/h figure and its
  //    duration. So a patient with 14 hours of documented anuria and a normal
  //    creatinine read "Does not meet KDIGO AKI criteria" -- a rule-out on a
  //    finding the reader had explicitly entered and the tile then discarded.
  //    Anuria is a urine output of zero by definition; it needs no second figure.
  //
  // 2. A SUB-STAGE OF 0 WAS PRINTED FOR A URINE OUTPUT NOBODY MEASURED. With
  //    neither field given, `uoStage` stayed 0 -- indistinguishable from a normal
  //    output -- and the interpretation went on to say the patient does not meet
  //    criteria at all. That is rule 3: an incomplete assessment may rule in and
  //    never out. It is `null` now, and the interpretation says which half it saw.
  const anuriaStated = anuriaHours != null && Number.isFinite(Number(anuriaHours));
  const uoPairStated = uoMlPerKgPerHour != null && uoDurationHours != null;
  let uoStage = null;
  if (anuriaStated && Number(anuriaHours) >= 12) {
    uoStage = 3;
  } else if (uoPairStated) {
    uoStage = 0;
    if (uoMlPerKgPerHour < 0.3 && uoDurationHours >= 24) uoStage = 3;
    else if (uoMlPerKgPerHour < 0.5 && uoDurationHours >= 12) uoStage = 2;
    else if (uoMlPerKgPerHour < 0.5 && uoDurationHours >= 6) uoStage = 1;
  } else if (anuriaStated) {
    // Anuria stated and under 12 h: that alone cannot reach any UO stage, and
    // without the mL/kg/h figure the lower bands cannot be read either.
    uoStage = null;
  }
  const uoAssessed = uoStage !== null;

  const stage = Math.max(crStage, uoStage === null ? 0 : uoStage);
  let interpretation;
  if (stage > 0) {
    interpretation = `KDIGO AKI Stage ${stage}.`;
    if (!uoAssessed) interpretation += ' The urine output was not assessed, so the stage rests on the creatinine alone and can only rise.';
  } else if (uoAssessed) {
    interpretation = 'Does not meet KDIGO AKI criteria.';
  } else if (anuriaStated) {
    interpretation = `The creatinine does not meet KDIGO AKI criteria, and ${r1(Number(anuriaHours))} h of `
      + 'anuria is short of the 12 h that makes stage 3 on its own -- enter the urine output (mL/kg/h) '
      + 'with its duration for the lower bands (stage 1 is < 0.5 for 6 h). This cannot yet rule AKI out.';
  } else {
    interpretation = 'The creatinine does not meet KDIGO AKI criteria, and the urine output has not been '
      + 'assessed -- enter the urine output (mL/kg/h) with its duration, or the hours of anuria. '
      + 'Stage 3 is < 0.3 mL/kg/h for 24 h or anuria for 12 h, so this cannot yet rule AKI out.';
  }
  return {
    creatinineRatio: r2(ratio),
    creatinineStage: crStage,
    urineOutputStage: uoStage,
    urineOutputAssessed: uoAssessed,
    stage,
    interpretation,
  };
}
...
// AG = Na - (Cl + HCO3). Standard reference range 8-12 mEq/L (assumes normal
// albumin ~4.0 g/dL). Each 1 g/dL drop in albumin lowers the measured AG by
// ~2.5 mEq/L, so hypoalbuminemic patients can mask a true high-AG acidosis.
//   correctedAG = measuredAG + 2.5 * (4.0 - albumin g/dL)
// Citation: Figge J, Jabor A, Kazda A, Fencl V. Anion gap and hypoalbuminemia.
// Crit Care Med. 1998;26(11):1807-1810. Original derivation: Figge et al.,
// J Lab Clin Med 1992;120:713-719.
//
// `includePotassium=true` switches to the (Na+K) - (Cl+HCO3) form, which some
// labs report. Reference range with K is 12-16 mEq/L.
export function correctedAnionGap({ na, k = null, cl, hco3, albuminGdl, includePotassium = false }) {
  num('na', na, { min: 100, max: 200 });
  num('cl', cl, { min: 50, max: 150 });
  num('hco3', hco3, { min: 0, max: 60 });
  num('albuminGdl', albuminGdl, { min: 0.5, max: 8 });
  let measuredAg;
  if (includePotassium) {
    if (k === null || k === undefined) throw new TypeError('k required when include potassium=true');
    num('k', k, { min: 1, max: 12 });
    measuredAg = (na + k) - (cl + hco3);
  } else {
    measuredAg = na - (cl + hco3);
  }
  const correctedAg = measuredAg + 2.5 * (4.0 - albuminGdl);
  const refLow = includePotassium ? 12 : 8;
  const refHigh = includePotassium ? 16 : 12;
  let band;
  if (correctedAg > refHigh) band = `Elevated (>${refHigh}): consider HAGMA workup (MUDPILES / GOLDMARK).`;
  else if (correctedAg < refLow) band = `Low (<${refLow}): rare; consider lab error, hypoalbuminemia not yet corrected, or paraproteinemia.`;
  else band = `Within reference range ${refLow}-${refHigh}.`;
  return {
    measuredAg: r1(measuredAg),
    correctedAg: r1(correctedAg),
    referenceLow: refLow,
    referenceHigh: refHigh,
    band,
    formulaNote: includePotassium
      ? 'AG = (Na + K) - (Cl + HCO3); reference range 12-16 mEq/L assumes normal albumin ~4.0 g/dL.'
      : 'AG = Na - (Cl + HCO3); reference range 8-12 mEq/L assumes normal albumin ~4.0 g/dL.',
    citation: 'Figge J et al., Crit Care Med 1998;26:1807-10.',
  };
}

// --- T20: ABCD-squared TIA stroke risk score ----------------------------
// Five binary/categorical features summed to a 0-7 score predicting 2-day,
// 7-day, and 90-day stroke risk after TIA.
//   A: Age >= 60                                                  (+1)
//   B: BP >= 140/90 at first eval                                 (+1)
...
//   D: Diabetes                                                   (+1)
//
// Risk bands (2-day stroke risk in original derivation):
//   0-3 Low (~1.0%); 4-5 Moderate (~4.1%); 6-7 High (~8.1%).
// Citation: Johnston SC, Rothwell PM, Nguyen-Huynh MN, et al. Validation
// and refinement of scores to predict very early stroke risk after
// transient ischaemic attack. Lancet. 2007;369(9558):283-292.
export function abcd2({ age, sbp, dbp, clinicalFeatures, durationMinutes, diabetes }) {
  num('age', age, { min: 0, max: 130 });
  num('sbp', sbp, { min: 30, max: 300 });
  num('dbp', dbp, { min: 10, max: 200 });
  num('durationMinutes', durationMinutes, { min: 0, max: 1440 });
  if (typeof diabetes !== 'boolean') throw new TypeError('diabetes must be boolean');
  const allowedClinical = new Set(['weakness', 'speech', 'other']);
  if (!allowedClinical.has(clinicalFeatures)) {
    throw new RangeError('clinical features must be one of: weakness, speech, other');
  }

  const a = age >= 60 ? 1 : 0;
  const b = (sbp >= 140 || dbp >= 90) ? 1 : 0;
  const c = clinicalFeatures === 'weakness' ? 2 : (clinicalFeatures === 'speech' ? 1 : 0);
  const d1 = durationMinutes >= 60 ? 2 : (durationMinutes >= 10 ? 1 : 0);
  const d2 = diabetes ? 1 : 0;
  const total = a + b + c + d1 + d2;

  let band, twoDayRisk;
  if (total <= 3) { band = 'Low risk'; twoDayRisk = '~1.0%'; }
  else if (total <= 5) { band = 'Moderate risk'; twoDayRisk = '~4.1%'; }
  else { band = 'High risk'; twoDayRisk = '~8.1%'; }

  return {
    total,
    components: { age: a, bp: b, clinical: c, duration: d1, diabetes: d2 },
    band,
    twoDayStrokeRisk: twoDayRisk,
    interpretation: `ABCD2 ${total}/7 - ${band} (2-day stroke risk ${twoDayRisk}).`,
    citation: 'Johnston SC et al. Lancet 2007;369:283-292.',
  };
}
