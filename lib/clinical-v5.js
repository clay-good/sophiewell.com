...
export function sodiumCorrection({ weightKg, sex, age, currentNa, infusate, targetChangePer24h }) {
  ...
  if (!INFUSATE_NA.hasOwnProperty(infusate)) throw new TypeError(`unknown infusate: ${infusate}`);
  let factor;
  ...
  const volumeLiters = changePerLiter !== 0 ? targetChangePer24h / changePerLiter : null;
  ...
  const safetyCap = direction === 'hyponatremia' ? 8 : 10;
  ...
  return {
    ...
    overCap,
    safetyNote: overCap
      ? `Target change ${targetChangePer24h} mEq/L/24h exceeds the ${safetyCap} mEq/L/24h ceiling.`
      : `Within the ${safetyCap} mEq/L/24h ceiling.`,
  };
}
...
export function ironDeficitGanzoni({ weightKg, currentHb, targetHb = 15, ironStoresMg }) {
  ...
  const deficit = weightKg * (targetHb - currentHb) * 2.4 + stores;
  return {
    ironStoresMg: Math.round(stores),
    totalDeficitMg: Math.round(Math.max(0, deficit)),
  };
}
...
export function kdigoAki({ baselineCr, currentCr, riseInLast48h, uoMlPerKgPerHour, uoDurationHours, anuriaHours = 0, rrtInitiated = false }) {
  ...
  let crStage = 0;
  if (ratio >= 3 || currentCr >= 4 || rrtInitiated) crStage = 3;
  else if (ratio >= 2) crStage = 2;
  else if (ratio >= 1.5 || (riseInLast48h != null && riseInLast48h >= 0.3)) crStage = 1;
...
// Six binary risk factors. Major adverse cardiac event risk by count:
...
export function rcri(input) {
  ...
  let riskPct;
  if (count === 0) riskPct = 0.4;
  else if (count === 1) riskPct = 1.0;
  else if (count === 2) riskPct = 2.4;
  else riskPct = 5.4;
  return {
