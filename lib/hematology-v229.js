...
export function aec(input = {}) {
  ...
  const fault = gradeFault([
    ['Total WBC (×10⁹/L)', o.wbc, 0, 500],
    ['Eosinophil percentage (%)', o.eosPct, 0, 100],
  ]);
...
export function nlr(input = {}) {
  ...
  const fault = gradeFault([
    ['Absolute neutrophil count (×10⁹/L)', o.anc, 0.001, 500],
    ['Absolute lymphocyte count (×10⁹/L)', o.alc, 0.001, 500],
  ]);
  if (fault) return { valid: false, message: fault };
...
// There is no validated general cut-off: that review reports study-specific values
// from 124.63 to 272 and says cut-offs derived from a single study do not generalize.
// The bounds used here are a healthy reference interval measured in 12,160 adults
// (Lee JS, et al. Medicine 2018;97(26):e11138): mean 132.4, interval 46.8-218.0.
// Platelets and lymphocytes in the same unit (10^3/uL).
const PLR_LOW = 46.8;
const PLR_HIGH = 218;
const PLR_NOTE = 'Platelet-to-lymphocyte ratio = platelet count / absolute lymphocyte count (Gasparyan AY, et al. Ann Lab Med. 2019;39(4):345-357). An inflammatory / prothrombotic marker with no validated general cut-off — that review reports study-specific values from 124.63 to 272 and warns that a cut-off from one study does not carry over. The 46.8-218.0 band here is a healthy reference interval from 12,160 adults (Lee JS, et al. Medicine 2018;97(26):e11138), not a diagnostic threshold. A lab value, not a diagnosis or treatment order.';
export function plr(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const plt = pos(o.plt, 0.1, 3000);
  const alc = pos(o.alc, 0.001, 500);
  const fault = gradeFault([
    ['Platelet count (×10⁹/L)', o.plt, 0.1, 3000],
    ['Absolute lymphocyte count (×10⁹/L)', o.alc, 0.001, 500],
  ]);
  if (fault) return { valid: false, message: fault };
  ...
  const abnormal = score < PLR_LOW || score > PLR_HIGH;
  const tier = score > PLR_HIGH
    ? 'above the healthy reference interval (46.8-218.0)'
    : score < PLR_LOW
      ? 'below the healthy reference interval (46.8-218.0)'
      : 'within the healthy reference interval (46.8-218.0)';
