...
export const LAB_ANALYTES = {
  ...
  totalChol: {
    ...
    // spec-v53 §4.5: one-sided analyte. A LOW total cholesterol is clinically
    // benign (lower is better), so refLow:0 is a deliberate "never flag low"
    // choice, not an accidental default that suppresses an implausible reading.
    refLow: 0, refHigh: 199, oneSidedLow: true,
    source: 'NHLBI ATP III / 2018 ACC/AHA Cholesterol Guideline.',
  },
  ldl: {
    label: 'LDL Cholesterol', units: 'mg/dL',
    // spec-v53 §4.5: one-sided analyte; a low LDL is clinically benign.
    refLow: 0, refHigh: 99, oneSidedLow: true,
  ...
  triglycerides: {
    ...
    // spec-v53 §4.5: one-sided analyte; a low triglyceride level is clinically benign.
    refLow: 0, refHigh: 149, oneSidedLow: true,
    source: 'NHLBI ATP III.',
  },
...
export function interpretLab(analyteId, value, opts = {}) {
  ...
  if (!analyte) throw new TypeError(`Unknown analyte: ${analyteId}`);
