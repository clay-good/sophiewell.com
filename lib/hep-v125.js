...
export function peldScore(input = {}) {
  ...
  // spec-v1406: the INR/albumin/bilirubin envelopes lib/bounds.js declares; an impossible value computed.
  const peldFault = boundsAdvisory('inr', inr) || boundsAdvisory('albumin', albumin) || boundsAdvisory('bilirubin', bilirubin);
  if (peldFault) return { valid: false, message: peldFault };
...
export function clifCAclf(input = {}) {
  ...
  // spec-v1128: `lvl(v, 1, 3)` returns its LOW bound for a non-finite value, and
  // 1 on the CLIF-OF means an organ system graded and found to be working. Six
  // graded selects fell through to it, so an ungraded workup scored the minimum
  // 6 of 18 and the tile printed a specific CLIF-C ACLF number off it.
  //
  // Every organ contributes 1 to 3, so what has been graded is a FLOOR: the
  // score can only rise, and it is the figure -- not a band, this tile has none
  // -- that the reader takes away (spec-v1114).
  const organs = ['liver', 'kidney', 'brain', 'coag', 'circ', 'resp'];
  const ORGAN_LABEL = {
    liver: 'liver', kidney: 'kidney', brain: 'brain', coag: 'coagulation',
    circ: 'circulation', resp: 'respiration',
  };
  const ungraded = organs.filter((k) => !Number.isFinite(typeof o[k] === 'number' ? o[k] : Number(o[k])));
  let clifOF = 0;
  for (const k of organs) clifOF += lvl(o[k], 1, 3);
  const score = Math.round(10 * (0.33 * clifOF + 0.04 * age + 0.63 * Math.log(wbc) - 2));
  return {
    valid: true, clifOF, score, ungraded,
    floorOnly: ungraded.length > 0,
    abnormal: score >= 50,
    band: ungraded.length
      ? `CLIF-C ACLF at least ${score} (CLIF-OF organ-failure sub-score at least ${clifOF}/18): `
        + `${ungraded.length} of the 6 organ systems `
        + `(${ungraded.map((k) => ORGAN_LABEL[k]).join(', ')}) ${ungraded.length === 1 ? 'is' : 'are'} `
        + 'not graded, and each can only raise the score. A higher score indicates higher '
        + 'short-term mortality in acute-on-chronic liver failure.'
      : `CLIF-C ACLF ${score} (CLIF-OF organ-failure sub-score ${clifOF}/18): a higher score indicates higher short-term mortality in acute-on-chronic liver failure.`,
    note: CLIF_NOTE,
  };
}
...
export function gahs(input = {}) {
  ...
  // spec-v1406: the age and INR envelopes lib/bounds.js declares; an impossible value scored.
  const gahsFault = boundsAdvisory('ageYears', age) || boundsAdvisory('inr', inr);
  if (gahsFault) return { valid: false, message: gahsFault };
  let total = 0;
  total += age >= 50 ? 2 : 1;
  total += wbc >= 15 ? 2 : 1;
  total += urea >= 5 ? 2 : 1;
