...
export function masldCriteria(input = {}) {
  ...
  // spec-v1403: a blank sex read as female, which sets the HDL cut (50, not 40) and the MetALD
  // alcohol band (140-350 g/week, not 210-420). The page always sends a sex; an agent may not.
  const sex = String(o.sex == null ? '' : o.sex).trim().toLowerCase();
  if (sex === '') return { valid: false, message: 'Choose the sex. The HDL cut (40 mg/dL in males, 50 in females) and the MetALD alcohol band differ by sex.' };
  ...
  // spec-v1104: which of the five were ASSESSED, as distinct from which were met.
  //
  // Every criterion is a disjunction of measurements and treatment flags, and each
  // measurement was read through num(), which returns null for a blank -- so a
  // criterion nobody measured looked exactly like one that had been measured and
  // was normal. With the steatosis box ticked and nothing else entered, this
  // returned "Cryptogenic SLD -- hepatic steatosis with no cardiometabolic
  // criterion": a stated finding about five things nobody looked at (rule 11), and
  // the identical sentence a complete normal workup produces.
  //
  // An unticked treatment box is a real "no" (rule 4), so it does not assess the
  // measurement beside it: not being on an antihypertensive says nothing about the
  // blood pressure. A ticked one meets the criterion outright, so it is assessed
  // either way. And a criterion counts as assessed once ONE of its limbs was
  // observed -- a BMI without a waist circumference is what an ordinary chart looks
  // like, and rule 12 says not to break the ordinary case to catch the defective one.
  const unassessed = [];
  if (bmi === null && waist === null) unassessed.push('a body mass index or waist circumference');
  if (glucose === null && hba1c === null && !truthy(o.type2Diabetes)) unassessed.push('a fasting glucose or HbA1c');
  if (sbp === null && dbp === null && !truthy(o.antihypertensive)) unassessed.push('a blood pressure');
  if (trig === null && !truthy(o.lipidLowering)) unassessed.push('a triglyceride level');
  if (hdl === null && !truthy(o.lipidLowering)) unassessed.push('an HDL cholesterol');
  ...
  // spec-v1104: cryptogenic SLD is the one category defined by ABSENCE -- steatosis
  // with none of the five -- so it is the only reading a gap can invent. The other
  // categories all rule something IN and need no footing (rule 13): MASLD and MetALD
  // are reached by a criterion that WAS met, ALD by an alcohol intake that was
  // entered. A criterion count can only rise, so any of the unassessed five would
  // move this to MASLD.
  if (category === 'Cryptogenic SLD' && unassessed.length) {
    return {
      valid: false,
      incomplete: true,
      unassessed,
      criteriaMet: met,
      // The cuts are derived from the sex and ancestry alone, so they are correct
      // whatever is still missing -- unlike spec-v1037's smart-cop, where the
      // absent value was what CHOSE the thresholds. A reader asked for five labs
      // should be told what they will be measured against.
      bmiCut,
      waistCut,
      hdlCut,
      message: `Enter ${unassessed.join(', ')}: cryptogenic SLD means steatosis with NONE of `
        + `the five cardiometabolic criteria, and ${unassessed.length} of them `
        + `${unassessed.length === 1 ? 'has' : 'have'} not been assessed. Any one of them would `
        + 'make this MASLD.',
    };
  }
  ...
  return {
    ...
    band: category
      ? `${category} — hepatic steatosis with ${metabolic ? `${met.length} of 5 cardiometabolic criteria` : 'no cardiometabolic criterion'}.`
        // spec-v1104: reached only where a specific other cause was recorded, which
        // rules something in -- but "no cardiometabolic criterion" is still a claim
        // about five measurements, so say which of them were not taken.
        + (!metabolic && unassessed.length
          ? ` Not assessed: ${unassessed.join(', ')} — any one of them would also make this MASLD.`
          : '')
