...
export function euTirads(input = {}) {
  ...
  // spec-v1140: an unstated appearance became 'iso-hyperechoic', so an empty form
  // answered "EU-TIRADS 3 - low risk" -- a risk category for a nodule nobody had
  // described, on the tile that decides whether to put a needle in it. The
  // category IS the appearance, once the high-risk features are absent, so there
  // is nothing partial to report while it is unstated.
  //
  // A high-risk feature still rules in without it (rule 13): those override the
  // basic appearance, so category 5 stands whatever the echogenicity was.
  const appearanceStated = !(o.appearance === undefined || o.appearance === null
    || String(o.appearance).trim() === '');
  const key = appearanceStated ? String(o.appearance).trim() : 'iso-hyperechoic';
  if (appearanceStated && !Object.prototype.hasOwnProperty.call(APPEARANCE, key)) {
  ...
  if (!appearanceStated && features.length === 0) {
    return {
      valid: false,
      code: 'MISSING_INPUT',
      field: 'appearance',
      message: 'Describe the nodule to read a EU-TIRADS category: no nodule, benign, isoechoic or '
        + 'hyperechoic, or mildly hypoechoic. With none of the high-risk features present the '
        + 'category is the appearance, so there is no category to report until it is stated.',
      note: EU_TIRADS_NOTE,
    };
  }
