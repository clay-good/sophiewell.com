...
export function diabetesDiagnosis(input = {}) {
  ...
  // spec-v1196: which of the four tests nobody ran. The tile was honest about
  // having one result ("a single abnormal result") and never said what the other
  // three were -- so a reader with one A1C could not tell whether the fasting
  // glucose was normal or simply not drawn, and the same sentence carried the
  // reassuring branch too.
  const notEntered = [];
  if (a1c === null) notEntered.push('an A1C');
  if (fpg === null) notEntered.push('a fasting glucose');
  if (ogtt === null) notEntered.push('a 2-hour glucose');
  if (random === null) notEntered.push('a random glucose');
  const notEnteredList = notEntered.length === 1
    ? notEntered[0]
    : `${notEntered.slice(0, -1).join(', ')} and ${notEntered[notEntered.length - 1]}`;
  ...
  if (unequivocal) {
    ...
    basis = `${diabetesRange[0]}, a single abnormal result`
      + (notEntered.length ? `; ${notEnteredList} ${notEntered.length === 1 ? 'was' : 'were'} not entered, and the second result the Standards ask for can come from any of them` : '');
    ...
    basis = 'no value in the prediabetes or diabetes range among the tests entered'
      + (notEntered.length ? `; ${notEnteredList} ${notEntered.length === 1 ? 'was' : 'were'} not entered` : '');
  }
