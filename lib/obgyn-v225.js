...
import { fieldEntered, regionFooting } from './region-footing-v1093.js';
...
// as the chart is filled in. It said "PBAC score 65 - not in the heavy range
// (<= 100)" from a chart with three of the eight types never entered, and one
// of those types at a plausible count carried the same patient to 115: heavy
// menstrual bleeding likely. A total that can only rise must not rule out.
//
// spec-v1088 had looked at this tile and judged it correct -- "no pads counted
// yet is the normal state of a chart being filled in" -- which is true about the
// ARITHMETIC and beside the point about the SENTENCE. A blank really may mean
// none used; the tile just cannot tell that from not yet counted, and only one
// of those two readings supports the words "not in the heavy range".
//
// Ruling IN is untouched: above 100 the missing counts cannot lower it, so a
// heavy result from a partial chart is already the floor.
const PBAC_ITEMS = [
  ['lightPads', 'lightly stained pads', 1],
  ['moderatePads', 'moderately stained pads', 5],
  ['soakedPads', 'soaked pads', 20],
  ['lightTampons', 'lightly stained tampons', 1],
  ['moderateTampons', 'moderately stained tampons', 5],
  ['soakedTampons', 'soaked tampons', 10],
  ['smallClots', 'small clots', 1],
  ['largeClots', 'large clots', 5],
];

export function pbac(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  // spec-v1219: `cnt()` returns 0 for a count outside 0..1e6, and `fieldEntered`
  // returns TRUE for the same value -- so a negative count was "entered" for the
  // purposes of the footing disclosure and worth zero for the purposes of the
  // score. Two helpers disagreeing about one value, and the effect is the footing
  // never fires: the tally silently omitted an item type the reader had filled
  // in, and the score is compared against the > 100 heavy-bleeding threshold.
  //
  // This refuses exactly what `cnt` was already discarding -- no value that used
  // to score stops scoring -- and says so. `gradeFault` skips a blank, so an
  // item type left empty still reaches the footing that exists to report it.
  const fault = gradeFault(PBAC_ITEMS.map(([key, label]) => [`the ${label}`, o[key], 0, 1e6]));
  if (fault) return { valid: false, message: fault, band: fault, note: PBAC_NOTE };
  let raw = 0;
  const uncounted = [];
  for (const [key, label, weight] of PBAC_ITEMS) {
    if (!fieldEntered(o[key])) uncounted.push(label);
    raw += cnt(o[key]) * weight;
  }
  const score = Math.round(num('PBAC', raw, { min: 0, max: 1e6 }));
  const abnormal = score > 100;
  const footing = abnormal ? null : regionFooting(PBAC_ITEMS.length - uncounted.length, PBAC_ITEMS.length, uncounted, {
    unit: 'item types',
    singular: 'item type',
    zeroReads: 'exactly as a type the patient did not use does',
  });
  const verdict = abnormal
    ? 'heavy menstrual bleeding likely (> 100, ~ > 80 mL)'
    : (footing ? 'below the > 100 threshold on the item types counted so far' : 'not in the heavy range (<= 100)');
  return { valid: true, score, abnormal, footing, bandLabel: `PBAC ${score}`, band: `PBAC score ${score} — ${verdict}.`, detail: `weighted tally of pads, tampons, and clots = ${score}.`, note: PBAC_NOTE };
}
