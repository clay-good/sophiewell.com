...
const MALINAS_NOTE = 'Malinas score (Malinas Y): parity (1 = 0, 2 = 1, >= 3 = 2), duration of labor (< 3 h = 0, 3-5 h = 1, > 6 h = 2), contraction duration (< 1 min = 0, 1 min = 1, > 1 min = 2), interval between contractions (> 5 min = 0, 3-5 min = 1, < 3 min = 2), ruptured membranes (no = 0, recent = 1, > 1 h = 2). Total 0-10; < 5 transportable, >= 6 delivery likely imminent. A prehospital triage score, not a diagnosis or treatment order.';
...
export function malinasScore(input = {}) {
  ...
  return { valid: true, score, abnormal, bandLabel: `Malinas ${score}`, band: `Malinas score ${score} of 10 — ${abnormal ? 'delivery likely imminent (>= 6)' : 'transport feasible (< 6)'}.`, detail: 'Parity + labor duration + contraction duration + interval + membranes.', note: MALINAS_NOTE };
}
