...
const RANKER_RUBRIC = {
  ...
  // spec-v1186: every word of a multi-word query matched the same tile.
  //
  // The rubric rewarded ONE matching word more than TWO. A single word that
  // appears in a description collects exactPhraseInDesc (5) as well as
  // tokenInDesc (1), because a one-word query IS a phrase -- while two words that
  // both appear, but not adjacently, collect 1 + 1 and fall under the threshold
  // of 3. So "hydromorphone" found the opioid converter and "morphine
  // hydromorphone" found NOTHING, naming two drugs it converts between.
  //
  // tokenInDesc stays at 1 on purpose: scaffolding words each collect +1 across
  // hundreds of tiles, and raising it would drown the clinical terms. The signal
  // this adds is different in kind -- not "how good is one word" but "did the
  // reader's WHOLE question land here" -- and it is available only where a
  // partial match cannot claim it.
  allTokensMatched: 2,
  audienceAligned: 2,
...
// spec-v974: the exact-phrase bonus is a WHOLE-WORD containment, not a raw
// substring. Both fields are normalizePhrase output -- lowercase words separated
// by single spaces -- so padding each side and testing for the padded phrase is
// the whole test.
//
// As a raw substring it fired on the middle of unrelated words, and those
// accidental hits outscored the tile the reader was actually naming. "psi"
// matched alpha-1 antitry-PSI-n, preeclam-PSI-a and se-PSI-s; "anc" matched
// bal-ANC-e, resist-ANC-e and adv-ANC-ed; "vis" matched re-VIS-ed; "abi" matched
// prob-ABI-lity and cervical favor-ABI-lity. A buried match collected the
// name bonus AND the desc bonus for 15 points, while the Pneumonia Severity
// Index scored 13.9 for being called PSI -- so `psi` returned alpha-1
// antitrypsin deficiency first and the PSI at rank 16.
//
// This is the rule lib/synonyms.js already documents and applies ("the query is
// contained as a whole-word substring of a phrase"); the ranker had simply never
// adopted it.
...
function rankTilesAll(queryRaw, tiles, audience) {
  ...
  for (let i = 0; i < tiles.length; i += 1) {
    ...
    let idfSum = 0;
    let matchedCount = 0;
    if (phraseInField(doc.name, query)) score += RANKER_RUBRIC.exactPhraseInName;
    if (phraseInField(doc.desc, query)) score += RANKER_RUBRIC.exactPhraseInDesc;
    for (const t of tokens) {
      ...
      if (matched) { idfSum += idfNorm(vocab, t); matchedCount += 1; }
    }
    // Coverage, not weight: only a tile matching EVERY word of a two-or-more word
    // query earns this, so it can lift a complete answer over the threshold
    // without letting a partial one through.
    if (tokens.length >= 2 && matchedCount === tokens.length) score += RANKER_RUBRIC.allTokensMatched;
...
// fuzzy library.
function synonymEditFallback(queryRaw, synonyms, audience, tiles) {
  const query = normalizePhrase(queryRaw);
  ...
  //
  // This skipped a token that appeared in a synonym phrase and nothing else, so
  // a correctly spelled English word that no synonym happens to use was fair
  // game for repair. `bag` is one edit from `eag`, and every bag query -- "iv
  // bag", "bag of fluid", "how long will this bag last" -- was rewritten to
  ...
  //
  // `tokenEditFallback`, the OTHER edit-repair path, has always consulted the
  // tile vocabulary before repairing anything. The two are one rule written
  // twice, and only one of them got the guard.
  const vocab = tiles && tiles.length ? tileVocabulary(tiles) : null;
  const catalogKnows = (t) => !!vocab && (hasToken(vocab, t) || hasToken(vocab, stemToken(t)));
  ...
  for (let i = 0; i < tokens.length; i++) {
    const t = tokens[i];
    if (corpus.has(t) || catalogKnows(t)) continue;
...
// spec-v765: the words to RANK on, with the values taken out.
//
// The one-box design asks a reader to type their numbers -- "akin current
// creatinine 2.4 baseline creatinine 0.9" -- and those numbers are not name
// tokens. They rank: "2.4" and "0.9" matched COMPERA 2.0 and beat the AKIN
// tile the query literally names. Across the catalog, appending a tile's own
// example values to its own name drops the hit rate from 99.7% to 95.2%; taking
// them out again restores 99.6%.
//
// Only STANDALONE numbers go. A digit inside a name is part of the name --
// cha2ds2-vasc, 4at, phq-9 -- and stripping those would break the tiles most
// likely to be searched by their number.
export function rankableWords(query) {
  // Only a string is a query. Coercing anything else hands its own text back to
  // the caller -- rankableWords(Infinity) returned "Infinity" -- which the fuzz
  // suite treats as a leak, correctly: nothing here should invent a search term
  // out of a number that was passed by mistake.
  if (typeof query !== 'string') return '';
  return query.replace(/(^|\s)-?\d+(?:[.,]\d+)?(?=\s|$)/g, ' ').replace(/\s+/g, ' ').trim();
}
...
export function resolvePrompt(query, tiles, synonyms, audience = 'all') {
  ...
  const fallback = synonymEditFallback(q, synonyms, audience, tileList);
...
export function resolvePromptRanked(query, tiles, synonyms, audience = 'all', limit = 5) {
  ...
  const strong = ranked.length && ranked[0].score >= RANKER_RUBRIC.exactPhraseInName;
  if (!strong) {
    const fb = synonymEditFallback(q, synonyms, audience, tileList);
    if (fb) push(fb);
  }
