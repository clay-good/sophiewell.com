...
function tileVocabulary(tiles) {
  ...
  const byLen = new Map();
  const add = (t) => {
    if (!t || t.length < EDIT_MIN_TOKEN) return;
    let bucket = byLen.get(t.length);
    if (!bucket) { bucket = new Set(); byLen.set(t.length, bucket); }
    bucket.add(t);
  };
  for (const tile of tiles) {
    if (!tile || !tile.id || !tile.name) continue;
    for (const t of tokenize(tile.name)) add(t);
    for (const t of tokenize(tile.desc || '')) add(t);
    for (const t of Array.isArray(tile.tags) ? tile.tags : []) add(stemToken(normalizePhrase(t)));
    for (const s of Array.isArray(tile.specialties) ? tile.specialties : []) {
      for (const t of tokenize(s)) add(t);
    }
  }
  VOCAB_CACHE.set(tiles, byLen);
  return byLen;
}

function hasToken(byLen, t) {
...
function tokenEditFallback(queryRaw, tiles, audience) {
  const query = normalizePhrase(queryRaw);
  if (!query) return [];
  const tokens = query.split(/\s+/).filter(Boolean).map(stemToken);
  if (!tokens.length) return [];
  const byLen = tileVocabulary(tiles);
  if (!byLen.size) return [];

  let rewritten = null;
  let attempts = 0;
  for (let i = 0; i < tokens.length && attempts < EDIT_TOKEN_ATTEMPTS; i += 1) {
    const t = tokens[i];
    if (t.length < EDIT_MIN_TOKEN || QUERY_STOPWORDS.has(t) || hasToken(byLen, t)) continue;
    attempts += 1;
    for (let len = t.length - 1; len <= t.length + 1 && !rewritten; len += 1) {
    ...
    if (rewritten) break;                    // single-rewrite semantics, as pass 3a
  }
  if (!rewritten || rewritten === query) return [];
  return rankTilesAll(rewritten, tiles, audience)
...
// exact-phrase-strength hit) and the repaired reading beats it by at least
// a name-token (+3) margin. The margin is the safety valve the flat rubric
// needs without IDF: rewriting a valid rare word the vocabulary happens to
// lack ("wean" -> "mean") only ever gains common-token points and is
// rejected, while a genuine typo repair ("bradan" -> "braden") unlocks
// name/phrase-strength evidence and clears it. A query whose literal
// reading already ranks well never pays for the repair at all.
function rankWithRepair(queryRaw, tiles, audience) {
  ...
  const edited = tokenEditFallback(queryRaw, tiles, audience);
  if (edited.length && edited[0].score >= topScore + RANKER_RUBRIC.tokenInName) {
    return edited.concat(ranked);
  }
  return ranked;
}
...
export function resolvePromptRanked(query, tiles, synonyms, audience = 'all', limit = 5) {
  ...
  // Pass 2: token ranker, in score order, with the D5 typo repair allowed
  // to lead when the literal reading is weak (see rankWithRepair).
  for (const r of rankWithRepair(q, tileList, audience)) {
