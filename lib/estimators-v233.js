...
export function ageAdjustedDDimer(input = {}) {
  ...
  const rangeMsg = gradeFault([['the age in years', o.age, 0, 130], ['the D-dimer in ug/L FEU', o.ddimer, 0, 100000]]);
...
export function deurenberg(input = {}) {
  ...
  // spec-v1406: an out-of-range value parsed to null and was asked for as if blank. Name the range instead.
  const rangeMsg = gradeFault([['the BMI in kg/m^2', o.bmi, 5, 100], ['the age in years', o.age, 2, 130]]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
