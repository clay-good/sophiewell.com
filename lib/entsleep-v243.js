...
const NOAPNEA_NOTE = 'No-Apnea OSA screen (Duarte RLM, et al. 2018): neck circumference (< 37 cm = 0, 37-39.9 = 1, 40-42.9 = 3, >= 43 = 6) + age (< 35 = 0, 35-44 = 1, 45-54 = 2, >= 55 = 3). Total 0-9; the derivation used a cutoff of 3 or more to classify a patient at high risk of obstructive sleep apnea-hypopnea. It is a TWO-item model, not a four-item one -- the four-item instrument from the same group is the GOAL questionnaire, which is a different tool. A screen, not a diagnosis or treatment order.';
...
export function noApnea(input = {}) {
  ...
  const rangeMsg = gradeFault([['the neck circumference in cm', o.neck, 20, 70], ['the age in years', o.age, 0, 120]]);
  ...
  // spec-v961: the derivation is explicit -- "We used the cutoff >= 3 to classify patients at
  // high risk of having OSA" (Duarte 2018, J Clin Sleep Med 14:1097-1107, PMC6040787). This
  // read `score > 3`, so a score of exactly 3 -- the paper's own threshold -- was returned as
  // LOWER risk. An off-by-one on a screening cutoff, in the direction that misses people.
