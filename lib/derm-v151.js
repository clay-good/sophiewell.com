...
// the arithmetic and silent about the reader's problem: a region that was never
// assessed contributes exactly what a clear region contributes. `detail` lists
// only the regions that scored above 0, so "never examined" and "examined and
// clear" print identically -- as nothing at all.
//
// The score is a sum of non-negative terms, so a partial examination can only
// UNDER-state it. That makes the reassuring reading the wrong one: dropping a
// single region's area took PASI from moderate (10.2) to mild (7.8), and mild is
// the side of the line where systemic therapy is not discussed.
//
// A footing rather than a refusal, per docs/spec-v1091.md. Scoring three regions
// and leaving the fourth is a real way to use these instruments; what the reader
// is owed is the count and the direction.
...
function regionFooting(scored, total, missingLabels, unit = 'regions') {
  if (!scored || scored >= total) return null;
  const names = missingLabels.length > 1
    ? `${missingLabels.slice(0, -1).join(', ')} and ${missingLabels[missingLabels.length - 1]}`
    : missingLabels[0];
  return `Scored from ${scored} of ${total} ${unit}; ${names} ${missingLabels.length === 1 ? 'was' : 'were'} not entered. An unscored region contributes 0, exactly as a clear one does, so this total is a floor and can only rise.`;
}
