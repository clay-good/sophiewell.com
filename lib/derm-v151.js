...
import { anyEntered, fieldEntered, regionFooting } from './region-footing-v1093.js';
...
//
// `grade` and `areaGrade` both return 0 for an absent value, which is right for
// the arithmetic and silent about the reader's problem. Dropping a single
// region's area took PASI from moderate (10.2) to mild (7.8), and mild is the
// side of the line where systemic therapy is not discussed.
//
// spec-v1093 moved the sentence itself into lib/region-footing-v1093.js, once
// four more instruments turned out to share the shape.
//
// Keyed on the AREA field alone, and deliberately. The severity selects open on
// "0 - none" and are never blank on the browser, so a region would always look
// examined there and never to an agent -- the cross-surface split this programme
// keeps finding. The area is a number input that can genuinely be empty, and it
// multiplies the whole region term, so it is what decides whether a region was
// looked at.
...
export function pasi(input = {}) {
  ...
  const parts = [];
  const pasiMissing = [];
  // spec-v1125: the footing below keyed on each region's AREA alone, so with the
  // four areas entered and the twelve intensity selects left where they open --
  // on "0" -- the tile read "PASI 0/72 - mild psoriasis" with nothing graded.
  // PASI is the usual gate for systemic and biologic therapy, and mild against
  // moderate (>= 10) is which side of that gate a patient is on.
  //
  // spec-v1124 left PASI and EASI as an open question rather than changing them
  // unverified. This is the answer: both have it, in twelve and sixteen items.
  for (const r of PASI_REGIONS) {
    if (!anyEntered(o, [r.key + 'Area'])) pasiMissing.push(r.label);
    else {
      for (const [suffix, name] of [['E', 'erythema'], ['I', 'induration'], ['D', 'desquamation']]) {
        if (!fieldEntered(o[r.key + suffix])) pasiMissing.push(`${r.label} ${name}`);
      }
    }
  ...
  return {
    ...
    bandLabel: sev,
    abnormal,
    // The denominator is now every gradable slot: four areas plus their twelve
    // intensity items.
    footing: regionFooting(16 - pasiMissing.length, 16, pasiMissing, {
      unit: 'regions and intensity items', singular: 'item',
    }),
...
export function easi(input = {}) {
  ...
  const parts = [];
  const easiMissing = [];
  // spec-v1125: the same as PASI above -- the footing keyed on the area alone
  // and sixteen intensity selects opened on 0.
  for (const r of EASI_REGIONS) {
    if (!anyEntered(o, [r.key + 'Area'])) easiMissing.push(r.label);
    else {
      for (const [suffix, name] of [['E', 'erythema'], ['Ed', 'edema'], ['Ex', 'excoriation'], ['L', 'lichenification']]) {
        if (!fieldEntered(o[r.key + suffix])) easiMissing.push(`${r.label} ${name}`);
      }
    }
  ...
  return {
    ...
    abnormal,
    // Four areas plus their sixteen intensity items.
    footing: regionFooting(20 - easiMissing.length, 20, easiMissing, {
      unit: 'regions and intensity items', singular: 'item',
    }),
...
export function scorad(input = {}) {
  ...
  // spec-v1124: B is the six intensity items, worth 7B/2 -- up to 63 of the 103.
  // spec-v1093 excluded them from its footing with this reasoning, written into
  // the comment below: "Unlike the intensity items above -- selects, which open
  // on 0 and are never blank". That is the defect described accurately and
  // reasoned past, the third time this programme has found an author do it
  // (`lvh-criteria`'s "labeled male default", `essdai`'s "contributes 0, never
  // NaN"). A select that opens on 0 is rule 8, not an exemption from it.
  //
  // The six now carry a blank option, so the gap is expressible on both surfaces
  // and the same regionFooting the subjective half uses covers them too.
  let b = 0;
  const intensityMissing = [];
  for (const it of SCORAD_B) {
    if (!fieldEntered(o[it.key])) { intensityMissing.push(it.label); continue; }
    b += grade(o[it.key], 3);
  }
  ...
  // spec-v1016: A is the extent of the disease and it is a MEASUREMENT -- the
  // rule-of-nines percentage of body surface affected. Read as 0 when the field
  // is empty, it dropped the A/5 term and the tile still banded a severity: a
  // cleared extent with the intensity items entered answered "SCORAD 28/103 --
  // moderate atopic dermatitis" with "extent A 0%" printed underneath. An
  // uncharted extent is not a clear skin.
  const extentMissing = !(typeof o.extent === 'number' && Number.isFinite(o.extent));
  if (extentMissing) {
    return {
      valid: false, score: null, oscorad: null, bandLabel: null, abnormal: false,
      band: 'Enter the extent (A): the percentage of body surface affected, by the rule of nines. '
        + 'It is a fifth of the SCORAD and no severity can be read without it.',
      detail: `intensity B ${b}/18 (7B/2 = ${r1((7 * b) / 2)}), subjective C ${r1(c)}/20 so far.`,
      note: SCORAD_NOTE,
    };
  }
  // spec-v1093: the subjective half reads a blank VAS as a symptom the patient
  // denies. C is worth up to 20 of the 103, so an unasked pruritus score took
  // this tile from severe (53.5) to moderate (46) with nothing said. Unlike the
  // intensity items above -- selects, which open on 0 and are never blank -- the
  // two VAS fields are number inputs, so the gap is expressible on both surfaces.
  const subjectiveMissing = [];
  if (!fieldEntered(o.pruritus)) subjectiveMissing.push('pruritus');
  if (!fieldEntered(o.sleeplessness)) subjectiveMissing.push('sleeplessness');
  const intensityFooting = regionFooting(
    SCORAD_B.length - intensityMissing.length, SCORAD_B.length, intensityMissing,
    { unit: 'intensity items', singular: 'intensity item', zeroReads: 'exactly as a sign the examiner looked for and did not find does' },
  );
  const subjectiveFooting = regionFooting(2 - subjectiveMissing.length, 2, subjectiveMissing, {
    unit: 'subjective scores',
    singular: 'subjective score',
    zeroReads: 'exactly as a symptom the patient denies does',
  });
  ...
  return {
    ...
    score: s,
    oscorad: os,
    // Rule 13: severe is the top band and nothing unentered can bring it down.
    bandLabel: (intensityFooting || subjectiveFooting) && sev !== 'Severe' ? `At least ${sev.toLowerCase()}` : sev,
    abnormal,
    intensityMissing,
    footing: [intensityFooting, subjectiveFooting].filter(Boolean).join(' ') || null,
    band: (intensityFooting || subjectiveFooting) && sev !== 'Severe'
      ? `SCORAD at least ${s}/103 — at least ${sev.toLowerCase()} atopic dermatitis (oSCORAD at least ${os}).`
      : `SCORAD ${s}/103 — ${sev.toLowerCase()} atopic dermatitis (oSCORAD ${os}).`,
