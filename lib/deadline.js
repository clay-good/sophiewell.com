...
import { ymd, ymdFault, DATE_MIN_YEAR, DATE_MAX_YEAR } from './num.js';
...
export function parseIsoStrict(s, label = 'date') {
  ...
  // spec-v1170: this was the copy that got the calendar right, by round-tripping
  // the components back out of the Date. The rule now lives in lib/num.js so the
  // three parsers that had it wrong share this one, and the messages stay --
  // with spec-v1171's third one, for the year the round-trip had been catching
  // by accident (`Date.UTC(1, 0, 1)` is 1901).
  const fault = ymdFault(s);
  if (fault === 'shape') throw new RangeError(`${label} must match YYYY-MM-DD`);
  if (fault === 'window') throw new RangeError(`${label} must be between ${DATE_MIN_YEAR} and ${DATE_MAX_YEAR}`);
  if (fault) throw new RangeError(`${label} components do not form a real date`);
  const p = ymd(s);
...
export function deadline({ anchor, days, basis = 'calendar', now, rollForward = false }) {
  // Strings (the ops tiles' YYYY-MM-DD date inputs) go through the strict parser
  // so an impossible date (2026-13-40) is REJECTED, not silently normalized;
  // Date instances go through the lenient UTC normalizer.
  const start = typeof anchor === 'string' ? parseIsoStrict(anchor, 'anchor') : parseDate(anchor);
...
// --- spec-v1501 §5: month windows and midnight counting ----------------------

// addMonthsUtc(dt, n) -> the same day-of-month n months later, clamped to that month's last day,
// so January 31 plus one month is February 28 (or 29), never March 3. Negative n counts back.
export function addMonthsUtc(dt, n) {
  if (!Number.isInteger(n)) throw new TypeError('months must be an integer');
  const y = dt.getUTCFullYear();
  const m = dt.getUTCMonth() + n;
  const ty = y + Math.floor(m / 12);
  const tm = ((m % 12) + 12) % 12;
  const last = new Date(Date.UTC(ty, tm + 1, 0)).getUTCDate();
  return new Date(Date.UTC(ty, tm, Math.min(dt.getUTCDate(), last)));
}

// firstOfNextMonth(dt) -> the first day of the month after dt's month, including when dt is
// itself the first (Medicare general enrollment, Marketplace effective dates).
export function firstOfNextMonth(dt) {
  return new Date(Date.UTC(dt.getUTCFullYear(), dt.getUTCMonth() + 1, 1));
}

// countMidnights(admit, discharge) -> how many midnights pass while the patient is admitted:
// every calendar midnight after the admit date up to and including the discharge date. Admitted
// Monday and discharged Thursday is 3. Takes UTC-midnight dates (the date the clock reads, not
// the hour), since the qualifying-stay rules count calendar midnights.
export function countMidnights(admit, discharge) {
  const n = diffDays(discharge, admit);
  if (n === null) throw new RangeError('admit and discharge must be valid dates');
  if (n < 0) throw new RangeError('discharge is before admission');
  return n;
}
