...
export function parseIsoStrict(s, label = 'date') {
  ...
  if (!/^(\d{4})-(\d{2})-(\d{2})$/.test(s)) throw new RangeError(`${label} must match YYYY-MM-DD`);
  ...
  // three parsers that had it wrong share this one, and the message stays.
  const p = ymd(s);
  if (!p) throw new RangeError(`${label} components do not form a real date`);
