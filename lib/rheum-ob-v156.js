...
export const ESSDAI_DOMAINS = [
  ...
  { key: 'hematological', label: 'Hematological', weight: 2, levels: { No: 0, Low: 2, Moderate: 4, High: 6 } },
  { key: 'biological', label: 'Biological', weight: 1, levels: { No: 0, Low: 1, Moderate: 2 } },
];

// spec-v1109: exported so a test can hold the property the floor rests on.
export { ESSDAI_DOMAINS as essdaiDomains };

export function essdai(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  let total = 0;
  const active = [];
  // spec-v1108's ledger, drained in spec-v1109. The fallback below was 'No' --
  // the level meaning "this organ system has no activity" -- so a domain nobody
  // rated was scored as a domain examined and found quiet. Twelve of them made
  // "ESSDAI 0 — low systemic activity", with a detail line reading "No active
  // systemic domain (total 0)" about twelve assessments nobody had made.
  //
  // Every level in every domain is >= 0, so the index is MONOTONE and what has
  // been rated gives a FLOOR. `essdaiDomains` is exported so a test can hold
  // that (spec-v1107, rule 19).
  const unrated = [];
  for (const d of ESSDAI_DOMAINS) {
    const raw = o[d.key];
    // The select carries the activity-level NAME ('No'/'Low'/'Moderate'/'High').
    if (!(typeof raw === 'string' && Object.prototype.hasOwnProperty.call(d.levels, raw))) {
      unrated.push(d.label.toLowerCase());
      continue;
    }
    const contribution = d.levels[raw];
    total += contribution;
    if (contribution > 0) active.push(`${d.label} ${raw.toLowerCase()} (+${contribution})`);
  }
  ...
  // Rule 13: "high" is the top stratum and an unrated domain cannot bring it back
  // down. Low and moderate are floors.
  const floored = unrated.length > 0 && band !== 'high';
  return {
    valid: true,
    score,
    abnormal,
    unrated,
    floorOnly: floored,
    bandLabel: floored ? `ESSDAI at least ${score}` : band.replace(/^./, (m) => m.toUpperCase()),
    band: floored
      ? `ESSDAI at least ${score} on what was rated — ${unrated.length} of the 12 domains `
        + `${unrated.length === 1 ? 'is' : 'are'} unrated, and each can only raise it, so no `
        + 'activity stratum follows from this yet.'
      : `ESSDAI ${score} — ${band} systemic activity.`,
    detail: `${active.length
      ? `Active domains: ${active.join(', ')}.`
      : (unrated.length ? 'No active domain among those rated.' : 'No active systemic domain (total 0).')}`
      + `${unrated.length ? ` Unrated: ${unrated.join(', ')}.` : ''}`
      + ' Strata: low < 5, moderate 5–13, high ≥ 14.',
    note: ESSDAI_NOTE,
  };
}
