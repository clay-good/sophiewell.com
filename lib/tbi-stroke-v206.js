...
import { num, gradeFault } from './num.js';
...
export function essenStroke(input = {}) {
  ...
  const rangeMsg = gradeFault([['the age in years', o.age, 0, 120]]);
...
// --- 2.3 FUNC score (functional outcome after ICH) --------------------------
// POINT WEIGHTS RE-FETCHED, NEVER RECALLED (spec-v97), cross-verified across the
// spec draft and the MDCalc / Medscape / QxMD implementations of Rost NS, Smith
// EE, Chang Y, et al, Stroke 2008;39(8):2304-2309: ICH volume < 30 cc → 4, 30-60
// → 2, > 60 → 0; age < 70 → 2, 70-79 → 1, ≥ 80 → 0; ICH location lobar → 2, deep
// → 1, infratentorial → 0; GCS ≥ 9 → 2, ≤ 8 → 0; pre-ICH cognitive impairment
// absent → 1, present → 0. Total 0-11. Outcome anchors (firmly confirmed across
// >= 2 open sources): no patient with a FUNC score ≤ 4 reached functional
// independence (GOS ≥ 4) at 90 days, and > 80% of those with a score of 11 did;
// the intermediate per-band percentages are not reproducible from >= 2 readable
// open sources, so this tile reports the anchored endpoints and a rising
// gradient rather than fabricated middle percentages (the ADO-index precedent).
const FUNC_NOTE = 'FUNC score (Rost NS, Smith EE, Chang Y, et al, Stroke 2008;39(8):2304-2309): a point model for the likelihood of functional independence (GOS ≥ 4) at 90 days after primary intracerebral hemorrhage. ICH volume (< 30 → 4, 30-60 → 2, > 60 → 0), age (< 70 → 2, 70-79 → 1, ≥ 80 → 0), location (lobar → 2, deep → 1, infratentorial → 0), GCS (≥ 9 → 2, ≤ 8 → 0), and pre-ICH cognitive impairment (absent → 1); total 0-11. No patient with a score ≤ 4 reached functional independence at 90 days; > 80% of those with a score of 11 did, with the proportion rising across the intermediate scores. A prognostic estimate, not a goals-of-care order.';
const FUNC_LOC = { lobar: 2, deep: 1, infratentorial: 0 };

export function funcScore(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  const vol = numIn(o.ichVolume, 0, 500);
  const age = numIn(o.age, 0, 120);
  const gcs = numIn(o.gcs, 3, 15);
  const loc = Object.prototype.hasOwnProperty.call(FUNC_LOC, o.location) ? o.location : null;
  // spec-v1406: an out-of-range value parsed to null and was asked for as if blank. Name the range instead.
  const rangeMsg = gradeFault([
    ['the ICH volume in cc', o.ichVolume, 0, 500],
    ['the age in years', o.age, 0, 120],
    ['the GCS', o.gcs, 3, 15],
  ]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
  if (vol === null || age === null || gcs === null || loc === null) {
    return { valid: false, message: 'Enter ICH volume (cc), age, and GCS; select the ICH location; set pre-ICH cognitive impairment.' };
  }
  const vPts = vol < 30 ? 4 : vol <= 60 ? 2 : 0;
  const aPts = age < 70 ? 2 : age <= 79 ? 1 : 0;
  const lPts = FUNC_LOC[loc];
  const gPts = gcs >= 9 ? 2 : 0;
  const cPts = bool(o.cognitiveImpairment) ? 0 : 1;
  const total = num('FUNC', vPts + aPts + lPts + gPts + cPts, { min: 0, max: 11 });
  let tier; let abnormal = false;
  if (total <= 4) { tier = 'functional independence at 90 days ≈ 0% (very poor prognosis)'; abnormal = true; }
  else if (total <= 7) { tier = 'low probability of functional independence at 90 days'; abnormal = true; }
  else if (total <= 10) tier = 'intermediate-to-good probability of functional independence, rising with score';
  else tier = 'high probability of functional independence at 90 days (> 80%)';
  return {
    valid: true,
    score: total,
    abnormal,
    bandLabel: `FUNC ${total}`,
    band: `FUNC ${total}/11 — ${tier}.`,
    detail: `ICH volume ${vol} cc (+${vPts}); age ${age} (+${aPts}); ${loc} location (+${lPts}); GCS ${gcs} (+${gPts}); no pre-ICH cognitive impairment (+${cPts}).`,
    note: FUNC_NOTE,
  };
}
