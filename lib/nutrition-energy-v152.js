...
// spec-v1403: no function here checked an envelope, so a height of 432 cm (170 typed with the unit
// left on inches) or a weight of 800 kg computed an energy figure. The envelopes are the ones
// lib/bounds.js declares; each is checked after the function's own missing-value branch.
function bodyFault(wt, ht) {
  return boundsAdvisory('weightKg', wt) || (ht === null ? null : boundsAdvisory('heightM', ht / 100));
}
...
export function mifflinStJeor(input = {}) {
  ...
  const bodyMsg = bodyFault(wt, ht);
  if (bodyMsg) return { valid: false, message: bodyMsg };
  // spec-v1406: the ageYears envelope lib/bounds.js declares; an age of 1300 computed a negative REE.
  const ageMsg = boundsAdvisory('ageYears', age);
  if (ageMsg) return { valid: false, message: ageMsg };
...
export function katchMcArdle(input = {}) {
  ...
  if (lbm === null) {
    ...
    const bodyMsg = bodyFault(wt, null);
    if (bodyMsg) return { valid: false, message: bodyMsg };
    lbm = wt * (1 - bf / 100);
    derived = true;
  }
...
export function iretonJones(input = {}) {
  ...
  const weightMsg = bodyFault(wt, null);
  if (weightMsg) return { valid: false, message: weightMsg };
  const ventilated = o.mode !== 'spontaneous';
  // Each form's indicator is asked, not assumed. A blank sex read as male (+244 kcal/day) on the
  // ventilated form, and a blank height read as not obese (no -609) on the spontaneous form,
  // through an `obese` argument neither the page nor the agent tool supplied.
  if (ventilated && o.sex !== 'male' && o.sex !== 'female') {
    return { valid: false, message: 'Choose the sex. The ventilator-dependent form adds 244 kcal/day for a male patient.' };
  }
  const male = o.sex === 'male';
  ...
  if (ventilated) {
    ...
    if (ht === null || ht <= 0) {
      return { valid: false, message: 'Enter the height. The spontaneously breathing form subtracts 609 kcal/day for obesity (BMI > 27), and the BMI needs the height.' };
    }
    const heightMsg = bodyFault(wt, ht);
    if (heightMsg) return { valid: false, message: heightMsg };
    const bmi = wt / ((ht / 100) ** 2);
    const obese = bmi > 27;
    eee = round(num('Ireton-Jones EEE', 629 - 11 * age + 25 * wt - 609 * (obese ? 1 : 0), { min: 0, max: 20000 }));
    detail = `spontaneous: 629 − 11×${age} + 25×${wt} − 609×${obese ? 1 : 0} (BMI ${Math.round(bmi * 10) / 10}) = ${eee} kcal/day.`;
  }
  return {
    valid: true,
    score: eee,
