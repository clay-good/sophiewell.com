...
// spec-v1224: the clamp above is the opposite of a guard.
//
// Every engine in this file centers its systolic BP with `clamp(s, 60, 250)` or
// `clamp(s, 60, 300)`, which is right for the model -- the published betas are
// fitted on a range and an SBP of 260 is honestly centered at its edge. It is
// not a plausibility check, and it was doing duty as one: a systolic BP of 3000
// mmHg was clamped to the top of the fitted range and reported as an ordinary
// answer. SCORE2 said "10-year CVD risk 49.2% -- very-high category" and
// Reynolds said 89.8%, each a number a reader would act on, each computed from a
// pressure ten times the highest ever recorded.
//
// The envelope and the sentence are `BOUNDS.sbp` / `boundsAdvisory` in
// lib/bounds.js (20-300 mmHg, "values outside 20-300 are not survivable"). No
// clinical number is decided here. It runs AFTER each function's own
// missing-value branch, so a reader who left the field blank is still asked for
// it rather than told the value they did not enter is out of range
// (spec-v1207).
function sbpFault(s) {
  return boundsAdvisory('sbp', s);
}
// spec-v1406: the same for the ageYears envelope; an age the clamps pinned to the fitted range computed.
function ageFault(a) {
  return boundsAdvisory('ageYears', a);
}

// spec-v1408: the envelope is the physiologic one; a model also has the age range it was FITTED
// on, and outside it these engines clamped and answered anyway -- SCORE2 read "10-year CVD risk
// 7.6%" for a 95-year-old, from the coefficients of a 69-year-old. Only the two ranges this file
// already states from its sources are enforced: SCORE2's 40-69 and Framingham's 30-74, both named
// in their own notes. The page publishes the same two ranges on its age fields.
export const SCORE2_AGES = { min: 40, max: 69 };
export const FRAMINGHAM_AGES = { min: 30, max: 74 };
// spec-v1409: the same for three more, each range read from a source rather than from the field
// someone drew. MESA's own calculator says it is "most appropriate for patients in the 45-85 year
// age range" (mesa-nhlbi.org; derivation cohort 45-84, McClelland 2015). SCORE2-OP was derived in
// people over 65 and is "intended for use in people aged over 70" (Eur Heart J 2021;42:2455-2467),
// which the 2021 ESC prevention guideline repeats; no upper bound is stated, so none is enforced.
// Reynolds is two models on two cohorts: women 45 and older (Women's Health Study, JAMA
// 2007;297:611-619) and men 50 and older (Physicians' Health Study II, Circulation
// 2008;118:2243-2251). The sources disagree about whether the men's cohort was capped at 80, so
// only the floor each paper states is enforced.
export const MESA_AGES = { min: 45, max: 85 };
export const SCORE2OP_AGES = { min: 70, max: Infinity };
// spec-v1410: the men's paper does state a ceiling -- "men eligible for the current analysis were
// those younger than 80 at baseline" -- so the male range is 50 up to but not including 80. The
// women's paper states a floor only.
export const REYNOLDS_AGES = { female: { min: 45, max: Infinity }, male: { min: 50, below: 80 } };
function fittedAgeFault(a, range, model) {
  const max = range.below === undefined ? range.max : Infinity;
  if (a == null || (a >= range.min && a <= max && (range.below === undefined || a < range.below))) return null;
  const span = range.below !== undefined ? `ages ${range.min} to under ${range.below}`
    : range.max === Infinity ? `ages ${range.min} and over` : `ages ${range.min} to ${range.max}`;
  return `${model} was fitted on ${span}; ${a} is outside that range, and the model is not extrapolated.`;
}
...
export function score2({ age, male, smoker, sbp, totalChol, hdl, region } = {}) {
  ...
  const sFault = fittedAgeFault(a, SCORE2_AGES, 'SCORE2') || sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, message: sFault, note: SCORE2_NOTE };
...
export function score2Op({ age, male, smoker, sbp, totalChol, hdl, diabetes, region } = {}) {
  ...
  const sFault = fittedAgeFault(a, SCORE2OP_AGES, 'SCORE2-OP') || sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, message: sFault, note: SCORE2OP_NOTE };
...
export function mesaChd({ age, male, race, diabetes, smoker, totalChol, hdl, lipidMed, sbp, bpMed, familyHx, cac } = {}) {
  ...
  const sFault = fittedAgeFault(a, MESA_AGES, 'The MESA risk score') || sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, message: sFault, note: MESA_NOTE };
...
export function framinghamCvd({ age, male, totalChol, hdl, sbp, bpTreated, smoker, diabetes } = {}) {
  ...
  const sFault = fittedAgeFault(a, FRAMINGHAM_AGES, 'The Framingham general-CVD model') || sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, message: sFault, note: FRAM_NOTE };
  const b = FRAM[sex];
...
export function reynoldsRisk({ age, male, sbp, smoker, totalChol, hdl, hsCrp, familyHx, diabetic, hba1c } = {}) {
  ...
  const rsex = onFlag(male) ? 'male' : 'female';
  const sFault = fittedAgeFault(a, REYNOLDS_AGES[rsex], `The Reynolds Risk Score for ${rsex === 'male' ? 'men' : 'women'}`) || sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, message: sFault, note: REYNOLDS_NOTE };
