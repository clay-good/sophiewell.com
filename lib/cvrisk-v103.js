...
export const REYNOLDS_AGES = { female: { min: 45, max: Infinity }, male: { min: 50, max: Infinity } };
function fittedAgeFault(a, range, model) {
  if (a == null || a >= range.min && a <= range.max) return null;
  const span = range.max === Infinity ? `ages ${range.min} and over` : `ages ${range.min} to ${range.max}`;
