...
function fittedAgeFault(a, range, model) {
  ...
  return `${model} was fitted on ages ${range.min} to ${range.max}; ${a} is outside that range, and the model is not extrapolated.`;
}
...
export function score2Op({ age, male, smoker, sbp, totalChol, hdl, diabetes, region } = {}) {
  ...
  const sFault = sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, note: SCORE2OP_NOTE };
...
export function mesaChd({ age, male, race, diabetes, smoker, totalChol, hdl, lipidMed, sbp, bpMed, familyHx, cac } = {}) {
  ...
  const sFault = sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, note: MESA_NOTE };
...
export function reynoldsRisk({ age, male, sbp, smoker, totalChol, hdl, hsCrp, familyHx, diabetic, hba1c } = {}) {
  ...
  const sFault = sbpFault(s) || ageFault(a);
  if (sFault) return { valid: false, band: sFault, note: REYNOLDS_NOTE };
