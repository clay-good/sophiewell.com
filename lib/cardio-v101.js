...
// `qtc` corrected-interval calculator. None duplicates a live tile.
//
...
export function cha2ds2Va({ age, chf, hypertension, diabetes, stroke, vascular } = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; the clamp below scored an impossible age.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, note: CHA2DS2VA_NOTE };
...
export function chads65({ age, chf, hypertension, diabetes, stroke, vascularDisease } = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; the clamp below scored an impossible age.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, note: CHADS65_NOTE };
  const clampedAge = Math.max(0, Math.min(130, a));
...
export function atriaStroke(input = {}) {
  ...
  // spec-v1406: the ageYears envelope lib/bounds.js declares; the clamp below scored an impossible age.
  const ageFault = boundsAdvisory('ageYears', a);
  if (ageFault) return { valid: false, band: ageFault, note: ATRIA_NOTE };
  const clampedAge = Math.max(0, Math.min(130, a));
