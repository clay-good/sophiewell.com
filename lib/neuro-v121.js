...
export function mgfa(input = {}) {
  ...
  // spec-v1129: `=== 'b' ? 'b' : 'a'` made every value that is not 'b' -- an
  // unstated subtype included -- into 'a', limb/axial-predominant. The subtype
  // is not a shade: 'b' is the oropharyngeal and respiratory form, which is the
  // one that threatens the airway, and classes II, III and IV all carry it.
  //
  // The severity is already `required`, so Class I and Class V, which have no
  // subtype, are unaffected. Only the three classes that HAVE a subtype wait for
  // it -- rule 25, the guard scoped to the readings the input can move.
  const subStated = input && typeof input === 'object'
    && (input.subtype === 'a' || input.subtype === 'b');
  ...
  const needsSubtype = sev === 'severe' || sev === 'moderate' || sev === 'mild';
  const subtypeOwed = needsSubtype && !subStated;
  const roman = { severe: 'IV', moderate: 'III', mild: 'II' };
  const weakness = { severe: 'severe', moderate: 'moderate', mild: 'mild' };
  ...
  if (sev === 'intubation') { cls = 'V'; high = true; classText = 'Class V -- intubation, with or without mechanical ventilation'; }
    ...
    cls = roman[sev];
    high = sev === 'severe';
    classText = `Class ${cls}, subtype not stated -- ${weakness[sev]} generalized weakness. `
      + 'The subtype is needed: a is limb/axial-predominant and b is '
      + 'oropharyngeal/respiratory-predominant, and it is not a default';
  ...
  // spec-v1135: `scored` counted items whose value was ABOVE ZERO, so the
  // parenthetical "(all items at 0)" printed both when all eight items had been
  // graded normal and when none had been graded at all. Those are different
  // statements about a myasthenic patient -- normal swallowing and breathing is a
  // finding, and the absence of one is not -- and the tile made them the same
  // sentence.
  //
  // The MG-ADL is a sum of non-negative items, so a partial total is a floor.
  // What it needs is the count of items that were actually graded (spec-v1113:
  // disclosing in data is not disclosing, so the count goes in the sentence).
  let adlTotal = 0;
  let adlGraded = 0;
  const adlUngraded = [];
  for (const [key, label] of MGFA_ADL_ITEMS) {
    const raw = input && typeof input === 'object' ? input[key] : undefined;
    if (raw !== undefined && raw !== '' && raw !== null) {
      adlTotal += lvl(raw, 3);
      adlGraded += 1;
    } else {
      adlUngraded.push(label);
    }
  }
  adlTotal = clamp(adlTotal, 0, 24);
  const adlComplete = adlGraded === MGFA_ADL_ITEMS.length;
  const adlText = adlComplete
    ? `MG-ADL ${adlTotal}/24`
    : (adlGraded === 0
      ? 'MG-ADL not scored: none of its eight items has been rated'
      : `MG-ADL at least ${adlTotal}/24 from the ${adlGraded} of 8 items rated `
        + `(${adlUngraded.length <= 3 ? `${adlUngraded.join(', ')} not rated` : `the other ${adlUngraded.length} not rated`}`
        + ', each worth up to 3)');

  return {
    valid: true, cls, adlTotal, subtypeOwed,
    adlGraded, adlUngraded, adlComplete,
    abnormal: high,
    band: `MGFA ${classText}; ${adlText}.`,
    note: MGFA_NOTE,
  };
}
