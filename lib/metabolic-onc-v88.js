...
// status; DKA is graded mild/moderate/severe on the worst of the pH, bicarbonate
// and mental-status rows of that table (spec-v1069). This
...
export function dkaHhs({
  ...
  // spec-v1228: every ADA criterion here is a threshold comparison, and past a
  // threshold the size of the number stops mattering -- which is exactly where
  // an entry error hides. A glucose of 20000 mg/dL read "Mixed DKA/HHS picture
  // ... with hyperosmolality"; a pH of 80 and a bicarbonate of 600 each read
  // "Criteria for a complete DKA or HHS classification are not met", which is
  // the reassuring direction. The check runs after the missing-value branch
  // above (spec-v1207) and skips a value that was not entered, so a blank field
  // is still asked for rather than called out of range.
  for (const [key, v] of [['glucose', g], ['pH', phV], ['bicarbonate', hco3], ['sodium', na], ['chloride', cl]]) {
    if (v == null) continue;
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, note: 'Envelope from lib/bounds.js; the value was not classified.' };
  }
  ...
  // DKA severity (ADA table): take the more severe of the rows when they
  // disagree.
  //
  // spec-v1069: mental status is one of those rows, and it was being collected
  // and thrown away. Both surfaces ask for it -- the browser with a three-option
  // picker, the agent surface with the same enum -- and the value reached this
  // function as an argument that nothing read, so "Stupor / coma" graded a
  // patient exactly as "Alert" did. The severity table runs Alert (mild),
  // Alert/drowsy (moderate), Stupor/coma (severe).
  //
  // Alert appears in the mild AND the moderate row, so it cannot raise the grade
  // and cannot lower one the chemistry has already earned: it scores 0 and the
  // max() leaves it alone. Only drowsy and stupor/coma carry a grade of their
  // own, which is the conservative half of the table and the only half that is
  // unambiguous.
  const phGrade = phV < 7.00 ? 3 : (phV < 7.25 ? 2 : (phV <= 7.30 ? 1 : 0));
  const hco3Grade = hco3 < 10 ? 3 : (hco3 < 15 ? 2 : (hco3 <= 18 ? 1 : 0));
  const mentalKey = String(mental == null ? '' : mental).trim().toLowerCase();
  const mentalGrade = mentalKey === 'stupor' ? 3 : (mentalKey === 'drowsy' ? 2 : 0);
  const sev = Math.max(phGrade, hco3Grade, mentalGrade);
  ...
  } else if (hhsCore && !ketosisKnown) {
    // spec-v1099: MINIMAL KETOSIS IS ONE OF THE HHS CRITERIA, and this branch
    // asserted it from a ketone measurement nobody took. The branch below
    // already refuses for exactly this reason when no classification is
    // reached; the HHS path did not, so one tile guarded one of its two exits.
    //
    // It matters more here than as a wording slip: the alternative to HHS on
    // these numbers is a MIXED DKA/HHS picture, which the branch above names
    // and which is managed differently. Ketones are what separate them.
    classification = 'none';
    band = 'Marked hyperglycemia and hyperosmolality are present, but HHS also requires minimal ketosis and no beta-hydroxybutyrate or urine ketone grade was entered. '
      + 'Enter one: with significant ketosis these same values are a mixed DKA/HHS picture, which is managed differently.';
  } else if (hhsCore) {
    classification = 'HHS';
  ...
  return {
    ...
    note: 'Effective serum osmolality = 2 × Na + glucose/18; anion gap = Na − Cl − HCO3. Grading cutoffs are the ADA table (mild pH 7.25–7.30 / HCO3 15–18; moderate pH 7.00–7.24 / HCO3 10–14; severe pH < 7.00 / HCO3 < 10), and the grade is the worst row that applies. Mental status is a row of that table too: drowsy reads as at least moderate and stupor or coma as severe, whatever the chemistry shows. Alert sits in both the mild and the moderate row, so it never lowers a grade the chemistry has earned. Classification only; the insulin, fluid, and potassium orders stay with the clinician and protocol.',
  };
}
...
export function calvertCarboplatin({ targetAuc, gfr, capGfr = true } = {}) {
  // spec-v1237: no envelope on the GFR, deliberately. Calvert takes an ABSOLUTE
  // GFR in mL/min and BOUNDS.eGFR is an INDEXED estimate in mL/min/1.73m^2 --
  // the same-looking number in a different quantity, which is spec-v1205's rule
  // at its narrowest. `capGfr` already applies the Calvert-Chatelut ceiling the
  // dose calculation needs.
  ...
  // spec-v1406: the cap above is for a plausible GFR; a GFR of 2000 is an entry
  // error, not a value to cap. Ceiling from BOUNDS.eGFR, worded in this field's mL/min.
  if (gfrV > BOUNDS.eGFR.max) {
    const msg = `A GFR of ${gfrV} mL/min is above ~${BOUNDS.eGFR.max}, beyond a plausible filtration rate; verify the value entered.`;
    return { valid: false, band: msg, message: msg, note: 'The Calvert dose needs a target AUC (typically 4–6 mg/mL·min) and a glomerular filtration rate (mL/min).' };
  }
...
export function tlsCairoBishop({
  ...
  // spec-v1228: as for dkaHhs above -- Cairo-Bishop is four threshold
  // comparisons plus an end-organ criterion, so a potassium of 100 mmol/L met
  // the potassium criterion and read "Clinical tumor lysis syndrome (grade II)",
  // and a creatinine of 250 mg/dL supplied the end-organ half. Blanks are
  // skipped: every lab here is optional by design, and the function already
  // says which ones it read.
  for (const [key, v] of [['potassium', k], ['calcium', ca], ['phosphate', phos],
    ['scr', fin(creatinine)], ['scr', fin(creatinineUln)], ['albumin', fin(albumin)]]) {
    if (v == null) continue;
    const fault = boundsAdvisory(key, v);
    if (fault) return { valid: false, band: fault, note: 'Envelope from lib/bounds.js; the value was not classified.' };
  }
  ...
  // spec-v1163: the BAND already says "Not entered: potassium, phosphate" when a
  // lab is missing, and the per-criterion rows underneath it said "not met" for
  // the same lab -- one screen contradicting itself, and a "not met" for a lab
  // nobody ran is a fabricated observation (rule 11). `null` distinguishes the
  // three states the reader needs: met, not met, and not measured. A criterion
  // reached through the 25%-rise baseline counts as assessed.
  const assessed = {
    ua: ua != null || uaBase,
    k: k != null || kBase,
    phos: phos != null || phosBase,
    ca: correctedCa != null || caBase,
  };
  const tri = (met, seen) => (seen ? met : null);
  ...
  if (clinicalTls) {
    ...
    // spec-v1066: this tile invites partial entry ("enter the ones available"),
    // so "not met" has to say how much was looked at. Two of the four criteria
    // are all it takes, and a lab nobody ran can still be one of them -- so a
    // count taken over three values must not read as a count over four.
    const unrun = [];
    if (ua == null) unrun.push('uric acid');
    if (k == null) unrun.push('potassium');
    if (phos == null) unrun.push('phosphate');
    if (ca == null) unrun.push('calcium');
    band = unrun.length
      ? `Laboratory TLS is not met by what has been entered (${metCount} of ${4 - unrun.length} criteria assessed; 2 are required). Not entered: ${unrun.join(', ')} -- any of these can meet a criterion, so this does not rule laboratory TLS out.`
      : `Criteria for laboratory TLS are not met (${metCount} of the 4 metabolic criteria present; 2 are required).`;
  }
  ...
  return {
    ...
    usedBaseline,
    criteria: {
      uaMet: tri(uaMet, assessed.ua),
      kMet: tri(kMet, assessed.k),
      phosMet: tri(phosMet, assessed.phos),
      caMet: tri(caMet, assessed.ca),
      renalCriterion,
    },
    // The booleans the arithmetic used, kept separate from what the reader is
    // shown, so nothing downstream has to re-derive "met" from a tri-state.
    criteriaMet: { uaMet, kMet, phosMet, caMet, renalCriterion },
