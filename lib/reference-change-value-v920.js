...
// A blank field and an absent field mean the same thing. Number('') is 0, so without this an
// empty form reads as a form full of zeros and the tile answers with confidence from nothing.
// spec-v930.
function num(v) {
...
export function referenceChangeValue(input = {}) {
  ...
  // spec-v1478: the level is named, and a blank one is said to be the conventional two-sided 95%.
  const probabilityGiven = Boolean(Z[String(o.probability)]);
  const probability = probabilityGiven ? String(o.probability) : 'two-95';
  const level = probability.endsWith('99') ? '99%' : '95%';
  ...
  const bandLabel = !comparable
    ? `Reference change value ${rcvText}${(previous === null) !== (current === null) ? ', no comparison made' : ''}`
    : exceeded
  ...
  // own, with no sign that the comparison the reader asked for had been dropped.
  // The half that IS there is the evidence that they wanted one.
  const halfPair = !comparable && (previous === null) !== (current === null);
  const missingHalf = previous === null ? 'the previous result' : 'the current result';
  const dividedByZero = !comparable && previous === 0 && current !== null;
  ...
  const band = !comparable
    ? `A difference has to exceed ${rcvText} to be distinguishable from analytical and biological variation, at this probability.`
      + (halfPair
        ? ` No comparison is made here: ${missingHalf} was not entered, and both are needed to measure a difference against that threshold.`
        : dividedByZero
          ? ' No comparison is made here: the previous result is zero, and a percentage change from zero is not defined.'
          : '')
    : exceeded
  ...
  const sidedNote = (probabilityGiven ? '' : 'No probability was entered, so the conventional two-sided 95% is used. ')
    + (probability.startsWith('one')
      ? `This is the one-sided factor at ${level}, for a change watched for in one direction only. If a change in either direction would count, the two-sided factor is the right one.`
      : `This is the two-sided factor at ${level}, for a change in either direction. If only a rise, or only a fall, is being watched for, the one-sided factor is the right one and this makes the test harder to pass than the question asked for.`);
