...
export function amslerKrumeich(input = {}) {
  ...
  // entered reads as a parameter that did not advance it. Refraction is
  // genuinely optional -- it is "not measurable" at stage 4, which is why it was
  // made optional -- but that reasoning only holds once the stage IS 4. Below
  // that, a refraction nobody recorded took a stage 3 cornea to stage 2:
  // moderate rather than severe keratoconus, and the two are managed
  // differently.
  const refractionEntered = Number.isFinite(refraction) && refraction >= 0;
  const floorNote = !refractionEntered && stage < 4
    ? ' The refraction was not entered. The stage is the most advanced parameter, so it can only raise this.'
    : '';
  ...
  return {
    ...
    band: `Amsler-Krumeich stage ${stage} — ${STAGE_LABEL[stage]}.${floorNote}`,
