...
function ca(o) {
  ...
  const s = o.setting;
  const nonrefill7 = o.days <= 7 && o.refills === 'no';
  const exemptOnRefills = o.days <= 7 && (s === 'procedure' || s === 'no-access' || (s === 'ed' && o.buprenorphine !== 'yes'));
  // A blank buprenorphine answer is not a no: in the emergency department it alone decides the
  // exemption, unless the supply is already a nonrefillable seven days or less.
  if (s === 'ed' && !nonrefill7 && o.buprenorphine !== 'yes' && o.buprenorphine !== 'no') return { verdict: 'unanswered', why: 'Say whether the drug is buprenorphine. California exempts buprenorphine prescribed or furnished in a general acute care hospital emergency department.' };
  if (exemptOnRefills && o.refills !== 'yes' && o.refills !== 'no') return { verdict: 'unanswered', why: 'Say whether refills are ordered. The California exemption for this setting needs a nonrefillable supply of no more than seven days.' };
