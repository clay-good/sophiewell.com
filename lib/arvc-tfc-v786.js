...
export function arvcTfc(input = {}) {
  ...
  let minors = 0;
  const met = [];
  // spec-v1108's ledger, drained in spec-v1111. A category nobody assessed was
  // read as 'none' -- assessed and negative -- so six of them answered "ARVC
  // Task Force 0 points (0 major, 0 minor) — criteria not met", which is the
  // reading that closes a cardiomyopathy workup and releases an athlete.
  //
  // Points are non-negative, so what has been assessed is a FLOOR: definite (>=
  // 4) rules in from a subset (rule 13) and every gentler tier waits.
  const unassessed = [];
  for (const c of CATEGORIES) {
    if (o[c.arg] === undefined || o[c.arg] === null || o[c.arg] === '') {
      unassessed.push(`category ${c.roman}`);
      continue;
    }
    const raw = String(o[c.arg]).trim();
  ...
  const floored = unassessed.length > 0 && tier !== 'definite';
  ...
  return {
    ...
    met,
    abnormal: points >= 4,
    bandLabel: floored ? `ARVC Task Force at least ${points} points` : `ARVC Task Force ${points} points`,
    band: floored
      ? `ARVC Task Force at least ${points} points (${majors} major, ${minors} minor) on what was `
        + `assessed. ${unassessed.length} of the 6 categories `
        + `${unassessed.length === 1 ? 'is' : 'are'} not assessed (${unassessed.join(', ')}), and `
        + 'each can only add points, so no tier follows from this yet.'
      : `ARVC Task Force ${points} points (${majors} major, ${minors} minor) — ${label}.`,
