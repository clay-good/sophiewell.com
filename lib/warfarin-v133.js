...
export function warfarinIwpc(input = {}) {
  ...
  // spec-v1406: an out-of-range age/height/weight was asked for again; name the range.
  const rangeMsg = gradeFault([['the age in years', o.age, 0, 130], ['the height (cm)', o.height, null, Infinity], ['the weight (kg)', o.weight, null, Infinity]]);
  ...
  const bodyFault = boundsAdvisory('weightKg', weight) || boundsAdvisory('heightM', height / 100);
  if (bodyFault) return { valid: false, message: bodyFault };
...
export function warfarinGage(input = {}) {
  ...
  // spec-v1406: an out-of-range age/height/weight/target INR was asked for again; name the range.
  const rangeMsg = gradeFault([['the age in years', o.age, 0, 130], ['the height (cm)', o.height, null, Infinity], ['the weight (kg)', o.weight, null, Infinity], ['the target INR', o.targetInr, null, 10]]);
  if (rangeMsg) return { valid: false, message: rangeMsg };
  if (age === null || age < 0 || age > 130 || height === null || weight === null
...
export function warfarinInit10mg(input = {}) {
  ...
  // spec-v1406: the INR envelope lib/bounds.js declares; an INR of 200 dosed.
  const inr3Fault = boundsAdvisory('inr', inr3);
  if (inr3Fault) return { valid: false, message: inr3Fault };
  ...
  const inr5Fault = boundsAdvisory('inr', inr5);
  if (inr5Fault) return { valid: false, message: inr5Fault };
