...
// spec-v59 §2.7: sanitize a user-supplied value before interpolating it into a
// generated-document string. An empty/absent value or a non-finite number
// becomes the labeled fallback rather than leaking "undefined"/"Infinity"/"NaN"
// into the printed form. A real string (the reachable input) is unchanged.
  ...
  return {
    ...
    sections: [
      { heading: 'Header', paragraphs: [
        ...
        `From: ${txt(fromProvider, '[provider / facility releasing records]')}`,
        `To: ${txt(toRecipient, '[third party receiving records]')}`,
      ] },
      { heading: 'Patient', items: [
        `Name: ${txt(patient, '[blank]')}`,
        `DOB: ${txt(dob, '[blank]')}`,
      ...
      { heading: 'Records requested', items: [
        `Date range: ${txt(dateRange, '[from / to]')}`,
        `Records: ${txt(recordsRequested, '[clinic notes / labs / imaging / discharge summary]')}`,
        `Delivery: ${txt(deliveryMethod, '[mail / secure email / portal / fax]')}`,
      ] },
...
export function buildWalletCard({ patientName, allergies = [], conditions = [], medications = [],
  ...
  return {
    ...
    sections: [
      { heading: 'Patient', items: [`Name: ${txt(patientName, '[blank]')}`, `Emergency contact: ${txt(emergencyContact, '[blank]')}`] },
      ...
      { heading: 'Care team', items: [
        `Primary care: ${txt(primaryProvider, '[blank]')}`,
        `Pharmacy: ${txt(pharmacy, '[blank]')}`,
      ] },
    ],
  };
}
