...
export function lrinec(input = {}) {
  ...
  // spec-v1006: every LRINEC component adds zero or more, so a partial total is a
  // LOWER BOUND. That makes an incomplete score safe to rule IN and never safe to
  // rule OUT -- and this used to rule out: with no labs at all it answered
  // "LRINEC 0: low risk of necrotizing fasciitis", and with CRP alone it answered
  // "LRINEC 4: low risk". The missing five can carry nine more points. Necrotising
  // fasciitis is a surgical emergency and a falsely reassuring score is the worst
  // direction to fail in, so a total that has not yet reached the suspicion
  // threshold is withheld until every component is present.
  const LABELS = {
    crp: 'CRP', wbc: 'WBC', hemoglobin: 'hemoglobin',
    sodium: 'sodium', creatinine: 'creatinine', glucose: 'glucose',
  };
  const missing = Object.keys(LABELS).filter((k) => fin(input[k]) == null);
  if (missing.length && total < 6) {
    return {
      valid: false, total: null, riskBand: null, terms,
      band: `Enter all six LRINEC labs to score. Missing: ${missing.map((k) => LABELS[k]).join(', ')}. `
        + `The ${missing.length} unentered value${missing.length === 1 ? '' : 's'} can only add points, `
        + 'so a partial total cannot rule necrotizing fasciitis out.',
      abnormal: false,
      note: LRINEC_NOTE,
    };
  }
  ...
  const partial = missing.length
    ? ` Scored from ${6 - missing.length} of 6 components; the rest can only raise it.`
    : '';
  return {
    valid: true, total, riskBand: band, terms, partialFrom: missing.length ? 6 - missing.length : null,
    band: `LRINEC ${total}: ${band} risk of necrotizing fasciitis (${prob}).${total >= 6 ? ' A score >= 6 should raise suspicion.' : ''}${partial}`,
    abnormal: total >= 6,
    note: LRINEC_NOTE,
  };
}
...
export function alt70(input = {}) {
  ...
  // spec-v1007: the same lower-bound rule as lrinec() above. Leukocytosis,
  // tachycardia and age add points or leave them alone, so a partial ALT-70 is a
  // floor -- and this one used to rule out: with nothing entered it answered
  // "ALT-70 0: cellulitis unlikely (>= 83% likelihood of pseudocellulitis)",
  // which is an argument for stopping antibiotics. The four unentered points can
  // carry the score into the likely band, so a total that has not yet reached it
  // is withheld until the three measurements are there. Asymmetry is a checkbox:
  // unchecked is an answer, not a gap.
  const ALT70_LABELS = { wbc: 'white blood cell count', hr: 'heart rate', age: 'age' };
  const missing = Object.keys(ALT70_LABELS).filter((k) => fin(input[k]) == null);
  if (missing.length && total < 5) {
    return {
      valid: false, total: null, riskBand: null, terms,
      band: `Enter the three ALT-70 measurements to score. Missing: ${missing.map((k) => ALT70_LABELS[k]).join(', ')}. `
        + `The ${missing.length} unentered value${missing.length === 1 ? '' : 's'} can only add points, `
        + 'so a partial score cannot place this in the pseudocellulitis band.',
      abnormal: false,
      note: ALT70_NOTE,
    };
  }
  ...
  const partial = missing.length
    ? ` Scored from ${3 - missing.length} of 3 measurements; the rest can only raise it.`
    : '';
  return {
    valid: true, total, riskBand: band, terms, partialFrom: missing.length ? 3 - missing.length : null,
    band: `ALT-70 ${total}: ${band}${detail}.${partial}`,
    abnormal: total <= 2,
    note: ALT70_NOTE,
  };
}
