...
export function vasi(input = {}) {
  ...
  for (const region of VASI_REGIONS) {
    ...
    // spec-v1093: an unrecorded area reads as 0 hand units, exactly as a region
    ...
    // Lower-cased because these labels are title-case and the footing reads them mid-sentence.
    if (!fieldEntered(o[`${region.key}Area`])) missingRegions.push(region.text.charAt(0).toLowerCase() + region.text.slice(1));
  ...
  return {
    ...
    footing: regionFooting(VASI_REGIONS.length - missingRegions.length, VASI_REGIONS.length, missingRegions),
