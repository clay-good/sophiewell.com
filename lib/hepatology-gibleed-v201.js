...
import { num, r1, r2, r3, inputFault } from './num.js';
...
export function glasgowBlatchford(input = {}) {
  ...
  if (ureaRaw === null || hb === null || sbp === null || sex === null) {
    return {
      valid: false,
      message: inputFault([
        ['the blood urea', o.urea, 0, unit === 'mgdl' ? 560 : 200, unit === 'mgdl' ? 'mg/dL' : 'mmol/L'],
        ['the hemoglobin', o.hb, 1, 25, 'g/dL'],
        ['the systolic BP', o.sbp, 20, 300, 'mmHg'],
      ]) || 'Enter blood urea (mmol/L) or BUN (mg/dL), hemoglobin (g/dL), systolic BP (mmHg), and select sex; then set the clinical flags.',
    };
  }
...
export function clifcAd(input = {}) {
  ...
  if (age === null || creat === null || inr === null || wbc === null || na === null) {
    return {
      valid: false,
      message: inputFault([
        ['the age', o.age, 0, 120, 'years'],
        ['the creatinine', o.creatinine, null, 40, 'mg/dL'],
        ['the INR', o.inr, null, 30, ''],
        ['the white cell count', o.wbc, null, 500, '10^9/L'],
        ['the sodium', o.sodium, 100, 180, 'mmol/L'],
      ]) || 'Enter age (years), creatinine (mg/dL), INR, WBC (×10⁹/L), and sodium (mmol/L) — all positive.',
    };
  }
...
export function hepamet(input = {}) {
  ...
  if (age === null || sex === null || ast === null || alb === null || plt === null) {
    return {
      ...
      message: inputFault([
        ['the age', o.age, 0, 120, 'years'],
        ['the AST', o.ast, 0, 20000, 'IU/L'],
        ['the albumin', o.albumin, 0.5, 7, 'g/dL'],
        ['the platelet count', o.platelets, 0, 3000, 'x10^9/L'],
      ]) || 'Enter age (years), sex, AST (IU/L), albumin (g/dL), and platelets (×10⁹/L); set diabetes; enter HOMA-IR unless diabetic.',
    };
  }
...
export function clip(input = {}) {
  ...
  if (child === null || morph === null || afp === null) {
    return {
      valid: false,
      message: inputFault([['the AFP', o.afp, 0, 5000000, 'ng/mL']])
        || 'Select Child-Pugh stage (A/B/C) and tumor morphology, enter AFP (ng/mL), and set portal-vein thrombosis.',
    };
  }
...
export function agile3plus(input = {}) {
  ...
  if (lsm === null || ast === null || alt === null || plt === null || age === null || sex === null) {
    return {
      valid: false,
      message: inputFault([
        ['the liver stiffness measurement', o.lsm, null, 100, 'kPa'],
        ['the AST', o.ast, null, 20000, 'IU/L'],
        ['the ALT', o.alt, null, 20000, 'IU/L'],
        ['the platelet count', o.platelets, null, 3000, 'x10^9/L'],
        ['the age', o.age, 0, 120, 'years'],
      ]) || 'Enter LSM (kPa), AST, ALT, platelets (×10⁹/L), age, and sex — all positive; set diabetes.',
    };
  }
  // spec-v59 output-safety: a vanishingly small AST would overflow alt/ast to
  // Infinity and leak a non-finite token into logit and the detail string.
  // Saturate the ratio at a large finite bound (well past any physiologic value)
  // so the logistic stays finite; normal ratios (< ~1e3) are untouched.
  const altAstRatio = Math.min(alt / ast, 1e6);
  const logit = -3.92368
