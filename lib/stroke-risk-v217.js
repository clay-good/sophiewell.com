...
export function sitsSich(input = {}) {
  ...
  const fault = gradeFault([
    ['Admission NIHSS', o.nihss, 0, 42],
    ['Blood glucose (mg/dL)', o.glucose, 0, 2000],
    ['Systolic BP (mmHg)', o.sbp, 0, 400],
    ['Weight (kg)', o.weight, 0, 700],
    ['Age (years)', o.age, 0, 130],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function vasograde(input = {}) {
  // spec-v1237: no envelope here, deliberately, and none is missing. Both inputs
  // are GRADED SCALES -- modified Fisher 0-4 and WFNS 1-5 -- and the reads below
  // already bound each to its own scale. A scale's own range is the envelope;
  // BOUNDS names neither.
  const o = input && typeof input === 'object' ? input : {};
