...
export function goldAbe(input = {}) {
  ...
  // PRINTED as "0 moderate exacerbations, none hospitalized in the past year" --
  // a history asserted rather than taken. Group E is reached by >= 2 moderate
  // OR any hospitalization, so the missing history can only move the group TO
  // E, never away from it: the low-risk reading is the one it can be wrong
  // about, and A and B are exactly the groups that do not get an inhaled
  // corticosteroid on the GOLD 2023 pathway.
  let moderate = toNum(o.moderateExacerbations);
  const exacEntered = moderate !== null;
  ...
  const exacPhrase = !exacEntered && !hospitalized
    ? 'the exacerbation history was not entered'
    : (hospitalized
  ...
  const exacFooting = !exacEntered && !hospitalized
    ? ' The number of moderate exacerbations in the past 12 months was not entered, and it can only raise this: two or more puts the patient in group E whatever the symptom burden.'
    : '';
  ...
  if (highExacRisk) {
    ...
    group = 'B';
    band = `Group B — more symptoms (${symptomDetail}) with low future-exacerbation risk (${exacPhrase}).${exacFooting}`;
  } else {
    group = 'A';
    band = `Group A — less symptoms (${symptomDetail}) with low future-exacerbation risk (${exacPhrase}).${exacFooting}`;
  }
  ...
  return {
    ...
    moderateExacerbations: exacEntered ? moderate : null,
    exacerbationHistoryEntered: exacEntered,
