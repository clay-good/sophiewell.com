...
// spec-v1466: a blank procedure is not a drug-eluting stent. The minimums differ twelvefold, so the
// interval is read against all three; a blank is asked for unless every procedure gives one answer.
export function pciSurgeryTiming(input = {}) {
  const o = input && typeof input === 'object' ? input : {};
  if (PROCEDURES.some((p) => p.value === o.procedure)) return evaluate(o, o.procedure);

  const runs = PROCEDURES.map((p) => evaluate(o, p.value));
  if (runs.some((r) => !r.valid)) return runs.find((r) => !r.valid);
  const first = runs[0];
  const minimums = 'a 180-day minimum after a drug-eluting stent, 30 days after a bare-metal stent, and 14 days after balloon angioplasty';
  if (first.status === 'no-interval') {
    return { valid: false, message: `Choose the procedure and enter the days since it: the delay before elective noncardiac surgery is ${minimums}.` };
  }
  if (runs.some((r) => r.status !== first.status)) {
    return { valid: false, message: `Choose the procedure: at ${first.daysSince} days the answer differs between them (${minimums}).` };
  }
  const generic = first.status === 'urgent'
    ? 'Urgent or emergency surgery is not delayed by these intervals. They govern elective procedures; a time-sensitive operation after a coronary intervention is a different decision, made with cardiology.'
    : `${first.daysSince} days after the coronary intervention, at or beyond the minimum for elective noncardiac surgery whichever procedure it was (${minimums}).`;
  const note = ' No procedure was entered; every procedure gives this same answer.';
  return { ...first, procedure: null, minDays: null, considerDays: null, shortBy: null, action: generic + note, band: generic + note };
}

function evaluate(o, which) {
