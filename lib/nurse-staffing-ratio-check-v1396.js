...
//
//   22 CCR 71215.1 (CDPH emergency rulemaking DPH-19-001E, text dated May 7, 2026; OAL file
//     2026-0522-01EFP; effective June 1, 2026; read 2026-09-19 on cdph.ca.gov). Acute psychiatric
//     hospitals not run by the Department of State Hospitals ((r)). (h) at least one licensed nurse
//     (RN, LVN, or psychiatric technician, (b)) per 6 adult patients and per 5 patients under 18.
//     (i) LVNs and psychiatric technicians no more than 50% of licensed nurses on the unit. (j) only
//     nurses giving direct care count. (k) the ratio is the most patients assigned at any one time,
//     with no averaging. (l) a charge nurse or manager counts only while giving direct care.
//     (d)(3) an RN on duty and awake at all times. An emergency rule lasts no more than 180 days
//     unless readopted (Finding of Emergency).
//
// New Jersey and Texas are not offered.
//
...
export const CA_UNITS = [
  ...
  { value: 'aph-adult', text: 'Acute psychiatric hospital, adult patients', ratio: 6, cite: '22 CCR 71215.1(h)(1)', aph: true },
  { value: 'aph-minor', text: 'Acute psychiatric hospital, patients under 18', ratio: 5, cite: '22 CCR 71215.1(h)(2)', aph: true },
];

const APH_NOTE = 'Acute psychiatric hospitals follow 22 CCR 71215.1, a CDPH emergency rule in effect since June 1, 2026. An emergency rule lasts no more than 180 days unless it is readopted or made permanent, so check that it is still in force. It does not apply to Department of State Hospitals facilities.';
...
export function nurseStaffingRatioCheck(input = {}) {
  ...
  if (unit.aph) notes.push('Licensed vocational nurses and psychiatric technicians together may be no more than half of the licensed nurses counted (71215.1(i)). A registered nurse must be on duty and awake at all times (71215.1(d)(3)).');
  if (unit.aph) notes.push(APH_NOTE);
  ...
  const refText = o.state === 'NY' ? '10 NYCRR 405.22(5)' : unit.cite || `22 CCR 70217${unit.ref}`;
  return {
    valid: true,
    required,
    counted,
    ...
    notes,
    averagingNote: o.state === 'CA' ? `Check each unit and each moment separately: ${unit.aph ? '71215.1' : '70217'} forbids averaging across a shift or over time, and counts a charge nurse only while giving direct care.` : null,
