...
// spec-v1226: `pos` and `real` below return `null` for a blank field, for a
// non-number AND for a value outside their own bounds, and every caller reads
// `null` as absent. So a reader who typed a hematocrit of 750% was told
// *"Enter hematocrit (%) and hemoglobin (g/dL), both greater than 0"* -- retyped
// the same number, and got the same sentence. A refusal with no way out.
//
// `gradeFault` (lib/num.js, spec-v1209) is the half that tells the two apart: it
// SKIPS a blank, so each function's own carefully-worded missing-value message
// still runs and still names every field at once, and it reports only the value
// that is out of range, by name and with the range.
//
// THE BOUNDS BELOW ARE THE CALLERS' OWN, copied from the `pos`/`real` call that
// already enforced them silently. Nothing clinical is decided in this wave and
// no envelope moves; the same inputs are accepted and rejected as before, and
// the rejected ones now say why.
...
export function watsonTbw(input = {}) {
  ...
  const fault = gradeFault([
    ...
    ['the height in cm', o.height, null, 300],
    ['the weight in kg', o.weight, null, 700],
  ]);
...
export function salazarCorcoran(input = {}) {
  ...
  const fault = gradeFault([
    ...
    ['the weight in kg', o.weight, null, 700],
    ['the height in cm', o.height, null, 300],
    ['the serum creatinine in mg/dL', o.creatinine, null, 50],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function epvs(input = {}) {
  ...
  const fault = gradeFault([
    ['the hematocrit in %', o.hematocrit, null, 100],
    ['the hemoglobin in g/dL', o.hemoglobin, null, 30],
  ]);
  if (fault) return { valid: false, message: fault };
  if (hct === null || hb === null) {
...
export function furosemideStressTest(input = {}) {
  ...
  const fault = gradeFault([
    ['the weight in kg', o.weight, null, 700],
    ['the 2-hour urine output in mL', o.urineOutput2h, 0, 100000],
  ]);
  if (fault) return { valid: false, message: fault };
  if (wt === null || uop === null) {
...
export function feBicarbonate(input = {}) {
  ...
  const fault = gradeFault([
    ['the urine bicarbonate in mEq/L', o.urineHco3, null, 10000],
    ['the plasma bicarbonate in mEq/L', o.plasmaHco3, null, 100],
    ['the plasma creatinine in mg/dL', o.plasmaCr, null, 50],
    ['the urine creatinine in mg/dL', o.urineCr, null, 10000],
  ]);
  if (fault) return { valid: false, message: fault };
...
export function correctedPotassiumPh(input = {}) {
  ...
  const fault = gradeFault([
    ['the measured serum potassium in mEq/L', o.potassium, null, 20],
    ['the arterial pH', o.ph, 6.5, 8],
  ]);
  if (fault) return { valid: false, message: fault };
  if (k === null || ph === null) {
