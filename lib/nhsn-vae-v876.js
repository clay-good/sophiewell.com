...
export function nhsnVae(input = {}) {
  ...
  if (tier === 'none') {
    ...
    if (!oxygenationQualifies) {
      // spec-v1098: "the rise is unknown" said that a rise could not be
      // computed and not WHICH measurement was absent. Oxygenation qualifies on
      // the FiO2 route OR the PEEP route, so an unentered pair can only create
      // an event, never remove one -- and "no ventilator-associated event" is a
      // surveillance denominator, a number that gets reported.
      const absent = [
        baseFio2 === null && 'the baseline daily minimum FiO2',
        eventFio2 === null && 'the event daily minimum FiO2',
        basePeep === null && 'the baseline daily minimum PEEP',
        eventPeep === null && 'the event daily minimum PEEP',
      ].filter(Boolean);
      missing.push(fio2Rise === null && peepRise === null
        ? 'no daily minimum settings are entered'
        : `the rise is ${fio2Rise === null ? 'not computable' : `${fio2Rise} points of FiO2`} and ${peepRise === null ? 'not computable' : `${peepRise} cmH2O of PEEP`}, against thresholds of ${FIO2_RISE_POINTS} and ${PEEP_RISE_CMH2O}`
          + (absent.length ? `, and ${absent.join(', ')} ${absent.length === 1 ? 'was' : 'were'} not entered, so either route can still qualify` : ''));
    }
