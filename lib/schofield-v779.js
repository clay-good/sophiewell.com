...
export function schofield(input = {}) {
  ...
  if (weightFault) return { valid: false, code: 'OUT_OF_RANGE', field: 'weight', message: weightFault, note: SCHOFIELD_NOTE };
  // spec-v1406: name the range of an entered value, rather than ask for it again.
  const ageRange = gradeFault([['the age in years', o.age, 0, 120]]);
  if (ageRange) return { valid: false, code: 'OUT_OF_RANGE', field: 'age', message: ageRange, note: SCHOFIELD_NOTE };
