...
export function hfEfClassification(input = {}) {
  ...
  //
  // spec-v1101: this fired for HFmrEF only, and HFimpEF needs the current
  // measurement merely to be ABOVE 40 -- which is the whole preserved range too.
  // So a baseline of 30 rising to 55 is HFimpEF, and with the baseline left out
  // the tile called it HFpEF and said nothing, while the identical gap one band
  // down was already spelled out. The file's own header says a tool classifying
  // from one ejection fraction "will silently call these patients HFmrEF"; it
  // silently called them HFpEF as well.
  //
  // The note is worded from `category` rather than naming one, so a later band
  // cannot fall out of it the way this one did.
  const singleMeasurementNote = ((category === 'HFmrEF' || category === 'HFpEF') && baseline === null)
    ? `With no baseline measurement this is ${category}. The same ${current} percent in a patient whose baseline was at or below ${HFREF_MAX} percent, and who rose at least ${IMPROVEMENT_POINTS} points from it, would be HFimpEF - a different category and a different conversation - so a single ejection fraction cannot settle this.`
    : null;
