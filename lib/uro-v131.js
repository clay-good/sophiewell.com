...
// spec-v1155: `Number(null)` and `Number('')` are both 0, and 0 is finite and
// >= 0 -- so this returned ZERO for a value nobody entered, and every
// `x === null` guard downstream stopped firing. `capraScore` has exactly such a
// guard, written for exactly this, and it could never fire: a blank
// positive-core percentage scored the FAVOURABLE level (under 34%, 0 points) and
// the tile answered "CAPRA 3 of 10 -- intermediate risk" as if the biopsy had
// been counted.
//
// The identical helper in lib/ob-v138.js was fixed at spec-v930 with the guard
// below, and this copy was not: one rule written twice, and the sibling with the
// fix is the proof of what the fix should be.
const nonNeg = (v) => {
...
const intIn = (v, lo, hi) => {
  // spec-v1155: the same blank hole as nonNeg above. Its only caller (`lvl`) is
  // saved by `allowed.includes(n)` on lists that start at 1, so nothing was wrong
  // -- guarded here so the next caller does not inherit the hole.
  if (v === null || v === undefined || (typeof v !== 'number' && String(v).trim() === '')) return null;
  const n = typeof v === 'number' ? v : Number(v);
