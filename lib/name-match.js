...
// Medicare's parts are named by one letter -- "Part B Late Enrollment Penalty" and "Part D Late
// Enrollment Penalty" differ ONLY in it -- and every word rule below drops single letters, so the two
// names reduced to the same words and list order decided (spec-v1507). Joining the letter to its word
// ("part-d") keeps it, on both the name and the query side.
export const joinParts = (s) => String(s || '').toLowerCase().replace(/\bpart ([a-e])\b/g, 'part-$1');

export function nameWords(name) {
  return (joinParts(name).match(/[a-z0-9-]+/g) || [])
...
export function findNamed(query, index, counts, byId, opts = {}) {
  ...
  const words = new Set((joinParts(query).match(/[a-z0-9-]+/g) || [])
    .filter((w) => w.length >= 4));
  const seen = new Set();
  let best = null;
  let bestScore = 0;
...
/**
 * Does this query name the calculator IN FULL -- every distinctive word of its
 * name present?
 *
 * Naming a calculator outright is a different act from describing what you
 * want, and only the first should outrank a ranker. Looser rules override
 * curated routes: one rare word put "Therapy Units" over CHA2DS2-VASc for
 * "antithrombotic therapy not recommended", and two matched words put
 * Cockcroft-Gault over the synonym-routed eGFR for "creatinine clearance".
 *
 * There used to be a further guard: a name reducing to fewer than two
 * DISTINCTIVE words was never "named in full", because "TIMI Risk Index"
 * reduces to `timi` alone and `timi` is shared with two siblings. That guard
 * was protecting against the looser rules above, and the every-token test
 * below already does the protecting: naming `timi` alone does not name "TIMI
 * Risk Index" in full, because `risk` and `index` are missing. What the guard
 * actually cost was the case this function exists for -- a reader who typed
 * "TIMI Risk Index", the whole name, got "TIMI Risk Score (UA / NSTEMI)"
 * first on both surfaces, and so did anyone who typed "Carpenter-Coustan GDM
 * Criteria". Removing it leaves every curated route the comment named exactly
 * where it was ("antithrombotic therapy not recommended" -> chads,
 * "creatinine clearance" -> egfr, "wells" -> wells-pe, "timi" -> timi) and
 * takes the exposed tiles that could not be found by their own name from two
 * to zero.
 *
 * Shared so the website and the MCP server rank an exact name the same way --
 * they disagreed until spec-v772, where find_calculator put "KDIGO CKD Staging"
 * first and the search bar put kdigo-aki first for the same words.
 */
export function namesInFull(query, name) {
  // EVERY token of the name, not just the distinctive ones. Two sibling names
  // can reduce to the same distinctive set -- "KDIGO CKD Staging (G×A risk)" and
  // "KDIGO AKI Staging" both come down to {kdigo, staging} -- and then this
  // cannot tell them apart and list order decides, which is the bug. What
  // separates them is the word the query did NOT say: `aki`.
  const tokens = (joinParts(name).match(/[a-z0-9-]+/g) || []).filter((w) => w.length >= 3);
  if (!tokens.length) return false;
  const q = ` ${joinParts(query)} `;
  return tokens.every((w) => new RegExp(`\\b${w.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}\\b`).test(q));
}
