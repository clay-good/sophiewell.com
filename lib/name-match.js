 ...
 *
 * A name with fewer than two distinctive words is never "named in full" here:
 * "TIMI Risk Index" reduces to `timi` alone, which is shared with two siblings
 * and cannot carry a promotion on its own.
 *
...
export function namesInFull(query, name) {
  if (nameWords(name).length < 2) return false;
