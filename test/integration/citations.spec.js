...
test('320px: a long-DOI tile renders its inline citation and the references block does not scroll horizontally', async ({ page }) => {
  ...
  // The citation lives with the method inside the collapsed "How this is
  // calculated" <details>: the proof is one click away rather than in the
  // reader's face.
  ...
  await expect(proof.locator('summary')).toHaveText('How this is calculated');
  await expect(proof.locator('.tile-derivation')).toHaveCount(1);
  await expect(page.getByText('Citation and sources', { exact: true })).toHaveCount(0);
