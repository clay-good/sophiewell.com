...
test('methacholine: the PD20 bands, read strictly', () => {
  assert.equal(m({ metric: 'pd20', value: 401 }).grade, 'normal');
  assert.equal(m({ metric: 'pd20', value: 400 }).grade, 'borderline');
  assert.equal(m({ metric: 'pd20', value: 100 }).grade, 'borderline');
  assert.equal(m({ metric: 'pd20', value: 99 }).grade, 'mild');
  assert.equal(m({ metric: 'pd20', value: 25 }).grade, 'mild');
  assert.equal(m({ metric: 'pd20', value: 24 }).grade, 'moderate-severe');
});
...
test('methacholine: the metric decides the cutpoints, and the tile says which it used', () => {
  ...
  assert.equal(m({ metric: 'pd20', value: 10 }).grade, 'moderate-severe');
  ...
  assert.match(m({ metric: 'pd20', value: 10 }).metricNote, /a delivered dose is comparable between laboratories/);
  assert.match(m({ metric: 'pd20', value: 10 }).band, /^PD20 10 micrograms/);
  assert.match(m({ metric: 'pc20', value: 10 }).band, /^PC20 10 mg\/mL/);
});

test('methacholine: the negative test is the informative one', () => {
  assert.match(m({ metric: 'pd20', value: 800 }).negativeNote, /negative predictive value/);
  assert.equal(m({ metric: 'pd20', value: 800 }).abnormal, false);
  assert.equal(m({ metric: 'pd20', value: 800 }).positiveNote, null);
  // Borderline is not flagged abnormal, and still carries the positive caveat.
  assert.equal(m({ metric: 'pd20', value: 200 }).abnormal, false);
  assert.match(m({ metric: 'pd20', value: 200 }).positiveNote, /does not diagnose asthma on its own/);
});

test('methacholine: an unwithheld medication is called out on a normal result', () => {
  assert.match(m({ metric: 'pd20', value: 800 }).withholdNote, /falsely negative test is usually a drug that was not withheld/);
  assert.match(m({ metric: 'pd20', value: 800, medicationsWithheld: true }).withholdNote, /recorded as withheld/);
  // On an abnormal result the point is general rather than an indictment of that result.
  assert.match(m({ metric: 'pd20', value: 10 }).withholdNote, /hard to read/);
});

test('methacholine: safety and scope are stated, not implied', () => {
  for (const value of [800, 10]) {
    assert.match(m({ metric: 'pd20', value }).safetyNote, /decided before the test, not read from the result/);
    assert.match(m({ metric: 'pd20', value }).scopeNote, /does not diagnose asthma/);
  }
});

test('methacholine: a missing or out-of-range value is refused, and the limit follows the metric', () => {
  assert.equal(m({}).valid, false);
  assert.equal(m({ metric: 'pd20', value: 0 }).valid, false);
  assert.equal(m({ metric: 'pd20', value: 10001 }).valid, false);
  assert.equal(m({ metric: 'pc20', value: 201 }).valid, false);
  assert.equal(m({ metric: 'pc20', value: 150 }).valid, true);
  assert.equal(m({ metric: 'made-up', value: 50 }).valid, false);
});

test('methacholine (spec-v1465): a blank metric is asked for, never read as PD20', () => {
  const r = m({ value: 50 });
  assert.equal(r.valid, false);
  assert.match(r.message, /^Choose which metric 50 is/);
  assert.match(m({}).message, /^Choose the metric/);
  assert.equal(m({ metric: 'pd20', value: 50 }).grade, 'mild');
  assert.equal(m({ metric: 'pc20', value: 50 }).grade, 'normal');
});
