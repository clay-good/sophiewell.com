...
test('uiss-rcc: localized low tier (T1, grade 1-2, ECOG 0)', () => {
  const r = uissRcc({ tStage: 't1', grade: '1', ecog: 'ecog0', extent: 'n0m0' });
  assert.equal(r.valid, true);
  assert.equal(r.tier, 'low');
  assert.equal(r.abnormal, false);
  assert.ok(r.detail.includes('~92%'));
  // A grade-2 ECOG-0 T1 is still low; ECOG >= 1 pushes it to intermediate.
  assert.equal(uissRcc({ tStage: 't1', grade: '2', ecog: 'ecog0', extent: 'n0m0' }).tier, 'low');
  assert.equal(uissRcc({ tStage: 't1', grade: '2', ecog: 'ecog1plus', extent: 'n0m0' }).tier, 'intermediate');
});

test('uiss-rcc: localized high tier (T4, or T3 grade 2-4 with ECOG >= 1)', () => {
  assert.equal(uissRcc({ tStage: 't4', grade: '1', ecog: 'ecog0', extent: 'n0m0' }).tier, 'high');
  assert.equal(uissRcc({ tStage: 't3', grade: '3', ecog: 'ecog1plus', extent: 'n0m0' }).tier, 'high');
  const r = uissRcc({ tStage: 't4', grade: '2', ecog: 'ecog0', extent: 'n0m0' });
...
test('uiss-rcc: intermediate covers the middle combinations', () => {
  assert.equal(uissRcc({ tStage: 't1', grade: '3', ecog: 'ecog0', extent: 'n0m0' }).tier, 'intermediate'); // T1 grade 3
  assert.equal(uissRcc({ tStage: 't2', grade: '1', ecog: 'ecog0', extent: 'n0m0' }).tier, 'intermediate'); // T2
  assert.equal(uissRcc({ tStage: 't3', grade: '3', ecog: 'ecog0', extent: 'n0m0' }).tier, 'intermediate'); // T3 G3 but ECOG 0
  assert.equal(uissRcc({ tStage: 't3', grade: '1', ecog: 'ecog1plus', extent: 'n0m0' }).tier, 'intermediate'); // T3 G1
});

test('uiss-rcc: node-positive / metastatic disease is out of scope', () => {
  assert.equal(uissRcc({ tStage: 't1', grade: '1', ecog: 'ecog0', extent: 'node-or-met' }).valid, false);
  // An unanswered extent is asked, never read as N0M0.
  const blank = uissRcc({ tStage: 't1', grade: '1', ecog: 'ecog0' });
  assert.equal(blank.valid, false);
  assert.match(blank.message, /extent/);
  assert.equal(blank.tier, undefined);
});
