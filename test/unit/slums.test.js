...
test('education-adjusted bands: a score of 20 is dementia (HS) but MNCD (< HS)', () => {
  ...
  const base = { story: '8', recall: '5', clock: '4', money: '3' };
...
test('high-school boundary: 27 is normal, 26 is mild neurocognitive disorder', () => {
  ...
  const at27 = slums({ education: 'hs', story: '8', recall: '5', clock: '4', figures: '2', digits: '2', money: '3', animals: '3' });
  assert.equal(at27.total, 27);
  assert.equal(at27.band, 'normal');
  // Drop animals from 3 to 2 -> 26 -> mild neurocognitive disorder.
  const at26 = slums({ education: 'hs', story: '8', recall: '5', clock: '4', figures: '2', digits: '2', money: '3', animals: '2' });
...
test('absent items count as 0 (all-blank scores 0, dementia)', () => {
  const r = slums({ education: 'hs' });
  assert.equal(r.total, 0);
  assert.equal(r.band, 'dementia');
