# Change: IV thrombolysis eligibility decision aid for acute ischemic stroke (AHA/ASA 2026)

> **GUIDELINE CURRENCY (updated 2026-07-11):** the governing guideline is now the **2026**
> AHA/ASA acute ischemic stroke guideline (published 2026-01-26, *Stroke*,
> doi:10.1161/STR.0000000000000513), which **replaces the 2018 guidelines and the 2019 update**.
> The 2026 update endorses **either tenecteplase (0.25 mg/kg, max 25 mg) or alteplase
> (0.9 mg/kg, max 90 mg)** in the 4.5 h window — the tile is agent-neutral ("IV thrombolysis"),
> not alteplase-only. **Do NOT build against the 2019 criteria** below without re-verifying every
> criterion against the 2026 guideline: the eligibility/exclusion tables may have changed. This
> is a HIGH-STAKES tile; stale criteria are a safety defect, not a nit.
...
The AHA/ASA 2026 acute ischemic stroke guideline publishes the time-window rules (≤ 3 h and the
3–4.5 h extended window) and the inclusion/exclusion criteria — reproducible, citable,
checklist-shaped content the catalog is built from, in the same decision-rule genre as the
shipped `perc`, `wells-pe`, and `canadian-ct-head` tiles.
...
- **One new clinical tile, `tpa-eligibility` (group G):** inputs = time from last-known-well
  ...
  **Output = a first-class eligibility verdict:** "no absolute contraindication met in the
  ≤ 3 h window — the patient meets the AHA/ASA 2026 checklist" / "excluded: [criteria]" /
...
thrombolysis not recommended per the extended-window relative-exclusion list"), the same shape as the shipped
...
- **Docs-only proposal** (propose-first, per the spec-v292 / v279 / v264–v266 precedent). Every
  ...
  2026 guideline and the thrombolytic package inserts at build (spec-v97 discipline); the sketch is
