...
<p align="center">
  <strong>1137 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v277 close the catalog is 1137
...
reviewable spec at a time to **1137** deterministic calculators
...
content-addressed). One build emits **1137 HTML pages** plus the supporting
assets:

| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1137 | `scripts/build-tool-pages.mjs` |
...
| Sitemap URLs (`sitemap.xml`) | 1137 | `scripts/build-sitemap.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (1137) is identical across all 12 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 1137 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1137 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1137 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1137

With 1137 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1137-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1137 tiles:
...
Adapting the catalog is incremental. Coverage now stands at **1034 clinical
calculators across 174 `lib` modules** (of 1137 catalog tiles), built module by <!-- catalog-truth:historical (174 is the count of lib modules adapted, not a catalog tile count) -->
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1137 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1137/1137 tiles** carry a committed per-tile audit log
