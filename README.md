...
<p align="center">
  <strong>863 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v201 close the catalog is 863
...
reviewable spec at a time to **863** deterministic calculators
...
### Deep Subspecialty Quantitation program (spec-v199–v203, +17 so far → 863)
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 863 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (863 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (863 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 863

With 863 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 863-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 863 tiles:
...
calculators across 94 `lib` modules** (of 863 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 863 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 863/863 tiles** carry a committed per-tile audit log
