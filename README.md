...
<p align="center">
  <strong>372 deterministic healthcare calculators tuned to the nurse on shift.</strong><br>
...
and the v29 catalog ledger. At v87 close the catalog is 372
...
reviewable spec at a time to **372** deterministic calculators
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (372 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (372 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 372

With 372 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 372-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 372 tiles:
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 372 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 372/372 tiles** carry a committed per-tile audit log
