...
<p align="center">
  <strong>1136 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v276 close the catalog is 1136
...
reviewable spec at a time to **1136** deterministic calculators
...
content-addressed). One build emits **1136 HTML pages** plus the supporting
assets:

| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1136 | `scripts/build-tool-pages.mjs` |
...
| Sitemap URLs (`sitemap.xml`) | 1136 | `scripts/build-sitemap.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (1136) is identical across all 12 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 1136 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1136 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1136 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1136

With 1136 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1136-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1136 tiles:
...
calculators across 173 `lib` modules** (of 1136 catalog tiles), built module by <!-- catalog-truth:historical (173 is the count of lib modules adapted, not a catalog tile count) -->
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1136 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1136/1136 tiles** carry a committed per-tile audit log
