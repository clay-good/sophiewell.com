...
<p align="center">
  <strong>1070 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v247 close the catalog is 1070
...
reviewable spec at a time to **1070** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1070 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1070 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1070 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1070

With 1070 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1070-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1070 tiles:
...
calculators across 94 `lib` modules** (of 1070 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1070 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1070/1070 tiles** carry a committed per-tile audit log
