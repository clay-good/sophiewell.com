...
<p align="center">
  <strong>1128 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v268 close the catalog is 1128
...
reviewable spec at a time to **1128** deterministic calculators
...
content-addressed). One build emits **1128 HTML pages** plus the supporting
assets:

| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1128 | `scripts/build-tool-pages.mjs` |
...
| Sitemap URLs (`sitemap.xml`) | 1128 | `scripts/build-sitemap.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (1128) is identical across all 12 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 1128 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1128 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1128 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1128

With 1128 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1128-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1128 tiles:
...
calculators across 173 `lib` modules** (of 1128 catalog tiles), built module by <!-- catalog-truth:historical (173 is the count of lib modules adapted, not a catalog tile count) -->
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1128 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1128/1128 tiles** carry a committed per-tile audit log
