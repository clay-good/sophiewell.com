...
UnitedHealthcare, or Anthem request carry a discharge plan — **not** clinical
...
```
 ...
 └──────┬───────┘     anthem | commercial | unknown
        ▼
 ┌──────────────┐   lib/pa/rules.js → 195 rules, each a pure check(bundle).
...
The three commercial overlays (§4.5.7 Aetna, §4.5.8 UnitedHealthcare, §4.5.9
Anthem) are each keyed to a single named payer and ship 20 rules apiece. They
are deliberately structurally parallel — same families, same severities — so
a packet linted under any one payer is auditable against the others. The
payer-specific routing names differ where each payer actually differs
(Aetna's CPB / NME; UHC's Provider Portal / Optum; Anthem's Availity ICR /
Carelon / Blue Distinction Centers):

| Rules     | Aetna (`-AETNA-`) / UnitedHealthcare (`-UHC-`) / Anthem (`-ANTHEM-`)     |
|-----------|-------------------------------------------------------------------------|
...
passes on any other packet, so the 135 non-commercial rules and the three
20-rule commercial overlays coexist without false positives — a Medicare FFS
packet never trips an Anthem rule, and vice versa. Each rule's
...
ten committed golden reports so any rule, extractor, or classifier change
...
never an edit to an existing rule, so the 195-rule set grows without
...
- [docs/spec-v52.md](docs/spec-v52.md) — the `pa-lint` prior-auth packet
  linter: pipeline, the 195-rule ruleset, payer overlays (Aetna +
  UnitedHealthcare + Anthem), and the byte-determinism / golden-fixture guarantee
