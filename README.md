...
<p align="center">
  <strong>991 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v227 close the catalog is 991
...
reviewable spec at a time to **991** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 991 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (991 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (991 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 991

With 991 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 991-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 991 tiles:
...
calculators across 94 `lib` modules** (of 991 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 991 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 991/991 tiles** carry a committed per-tile audit log
