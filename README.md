...
<p align="center">
  <strong>1144 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v281 close the catalog is 1144
...
reviewable spec at a time to **1144** deterministic calculators
...
content-addressed). One build emits **1144 HTML pages** plus the supporting
assets:

| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1144 | `scripts/build-tool-pages.mjs` |
...
| Sitemap URLs (`sitemap.xml`) | 1144 | `scripts/build-sitemap.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (1144) is identical across all 12 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 1144 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1144 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1144 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1144

With 1144 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1144-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1144 tiles:
...
Adapting the catalog is incremental. Coverage now stands at **1047 clinical
calculators across 186 `lib` modules** (of 1144 catalog tiles), built module by <!-- catalog-truth:historical (186 is the count of lib modules adapted, not a catalog tile count) -->
...
| nineteenth (quantitative thyroid / beta-cell) | `endo-quant-v197` (SPINA-GT, SPINA-GD, Jostel TSH index, HOMA-B, oral disposition index) | 5 |
| twentieth (cross-subspecialty prognosis) | `subspecialty-v198` (CNS-IPI, ISTH-BAT, VIRSTA, SeLECT, WHO/FIGO GTN) | 5 |
| twenty-first (myeloid-neoplasm / transplant prognosis) | `myeloid-prognosis-v199` (MIPSS70, GIPSS, MYSEC-PM, Sorror HCT-CI) | 4 |
| twenty-second (critical-care severity / acid-base) | `critcare-severity-v200` (OASIS, LODS, delta gap / delta ratio, APPS) | 4 |
| twenty-third (hepatology / GI bleed) | `hepatology-gibleed-v201` (Glasgow-Blatchford, CLIF-C AD, Hepamet, CLIP, Agile 3+) | 5 |
| twenty-fourth (CPET-anchored HF prognosis) | `cvrisk-engines-v202` (MECKI) | 1 |
| twenty-fifth (perioperative / TIA risk) | `periop-frailty-v203` (DASI, ABCD3-I, SORT) | 3 |
| twenty-sixth (nephrology / fluid & electrolyte) | `nephro-fluids-v204` (CCCR, ABL, EFWC, TmP/GFR, urinary calcium) | 5 |
| twenty-seventh (pulmonary / COPD / sleep) | `pulm-copd-v205` (CAT, LENT, ADO, DOSE, SACS) | 5 |
| twenty-eighth (TBI / stroke prognosis) | `tbi-stroke-v206` (Essen, Rotterdam CT, Marshall CT, FUNC) | 4 |
| twenty-ninth (resuscitation / early warning) | `resus-trauma-v207` (TOR rules, REMS, CART) | 3 |
| thirtieth (nutrition / maternal-fetal) | `nutrition-maternal-v208` (ponderal index, sFlt-1/PlGF, GLIM, SGA) | 4 |
| thirty-first (cardiology risk engines) | `cardiology-risk-v209` (HCM Risk-SCD, CHARGE-AF) | 2 |
| thirty-second (acute-stroke prognosis) | `stroke-prognosis-v210` (SPAN-100) | 1 |
| thirty-third (hematology-oncology risk) | `heme-onc-risk-v211` (EUTOS, IMPROVEDD, COMPASS-CAT, ELN 2022 AML) | 4 |
| thirty-fourth (hepatology fibrosis / portal HTN) | `hep-fibrosis-portal-v212` (King's Score, Baveno VII) | 2 |
| thirty-fifth (acute injury / ED decision) | `acute-injury-v213` (HEART Pathway, Ottawa HF, Light's criteria, Baux, revised Baux) | 5 |
| thirty-sixth (cardiology risk scores) | `cardiology-risk-v214` (APPLE, CAAP-AF, ATLAS, HATCH, MB-LATER, C-ACS, ACTION ICU) | 7 |
| thirty-seventh (lipid / device / onco-VTE risk) | `risk-scores-v215` (DLCN, Simon Broome, PADIT, GRIm, LIPI, ONKOTEV, PROTECHT) | 7 |
| thirty-eighth (hematology prognosis) | `heme-prognostic-v216` (WPSS, MDACC CLL, PIT, PRIMA-PI, Durie-Salmon, LDT, Talcott) | 7 |
...
| forty-third (pulmonary risk) | `pulmonary-risk-v221` (simplified Geneva, SCAP, CORB, RESP, ILD-GAP, du Bois IPF, pneumothorax volume) | 7 |
| forty-fourth (rheumatology classification) | `rheum-classification-v222` (IIM 2017, PMR 2012, Bohan-Peter, SSc 2013, mRSS, Sjogren 2016, ESSPRI) | 7 |
| forty-fifth (dermatology) | `dermatology-v223` (UAS7, HiSCR, Hurley, POEM, ALDEN, PEST, Glasgow 7-point) | 7 |
| forty-sixth (neurology) | `neurology-v224` (ID Migraine, ONLS, END-IT, Engel, ILAE, Salzburg NCSE, DHI) | 7 |
| forty-seventh (obstetrics / gynecology) | `obgyn-v225` (Nugent, Amsel, Ferriman-Gallwey, PBAC, Thompson HIE, MRS, Kupperman) | 7 |
| forty-eighth (nephrology / fluid & electrolyte) | `nephrology-v226` (Watson TBW, Salazar-Corcoran, ePVS, furosemide stress test, FE-bicarbonate, corrected potassium) | 6 |
...
| sixty-third (falls-risk + neuro assessment) | `scoring-v4` (Braden, Morse Fall Scale, Hendrich II, CAM, ICH Score, Hunt-Hess + WFNS, modified NIHSS, FOUR Score) | 8 |
| sixty-fourth (pediatric / ICU pain, sedation, withdrawal) | `scoring-v4` (FLACC, PAINAD, NIPS, N-PASS, CRIES, POSS, COMFORT-B, WAT-1, SBS, SOS) | 10 |
| sixty-fifth (prehospital stroke scales, ADLs, C-SSRS) | `scoring-v4` (CPSS, LAMS, RACE, ROSIER, GUSS, Barthel, Lawton IADL, Katz ADL, C-SSRS) | 9 |
| sixty-sixth (pulmonary / CAP severity) | `scoring-v4` (HACOR, Berlin ARDS, Murray LIS, SMART-COP, CRB-65, ATS/IDSA severe CAP) | 6 |
| sixty-seventh (nutrition risk + Ottawa rules) | `scoring-v4` (NUTRIC, mNUTRIC, NRS-2002, MUST, Ottawa Ankle, Ottawa SAH) | 6 |
...
| seventy-second (pediatric / adult severity scores) | `scoring-v6` (PELOD-2, pSOFA, Burch-Wartofsky, ARISCAT, APACHE II, Braden Q) | 6 |
| seventy-third (remaining group-v5 scores) | `clinical-v5` (RCRI, PEWS, ABCD2) | 3 |
| seventy-fourth (deterministic ICU workflow) | `scoring-v4` (LIPS, MTP ratio tracker, Bristol stool scale) | 3 |
| seventy-fifth (ID / critical-care criteria, **new module**) | `idcrit-v99` (modified Duke endocarditis, Pitt Bacteremia, SAPS II, refeeding risk) | 4 |
| seventy-sixth (screening / decision instruments, **new module**) | `scoring-v5` (PHQ-2/GAD-2, AUDIT, DAST-10, GDS-15, Ottawa Knee, NEXUS Chest, SF + Canadian syncope, EDACS, YEARS, FeverPAIN, STONE, ISS+RTS, SIPA) | 14 |
| seventy-seventh (cardiovascular risk engines) | `scoring-v4` (ASCVD Pooled Cohort Equations, AHA PREVENT 2023) | 2 |
| seventy-eighth (burn / airway / drug-dose recipes) | `field` (burn fluid, pediatric ETT, naloxone dosing, pediatric weight-based dose) | 4 |
...
| eighty-third (ventilator SBT readiness + ARDSnet PEEP) | `scoring-v4` (Boles 2007 SBT readiness + ARDSnet PEEP/FiO2 look-up) | 1 |
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1144 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1144/1144 tiles** carry a committed per-tile audit log
