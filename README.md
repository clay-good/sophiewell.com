...
<p align="center">
  <strong>662 deterministic healthcare calculators tuned to the nurse on shift.</strong><br>
...
and the v29 catalog ledger. At v146 close the catalog is 662
...
reviewable spec at a time to **662** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 662 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (662 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (662 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 662

With 662 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 662-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 662 tiles:
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 662 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 662/662 tiles** carry a committed per-tile audit log
