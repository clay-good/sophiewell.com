...
<p align="center">
  <strong>1140 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v279 close the catalog is 1140
...
reviewable spec at a time to **1140** deterministic calculators
...
content-addressed). One build emits **1140 HTML pages** plus the supporting
assets:

| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1140 | `scripts/build-tool-pages.mjs` |
...
| Sitemap URLs (`sitemap.xml`) | 1140 | `scripts/build-sitemap.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (1140) is identical across all 12 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 1140 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1140 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1140 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1140

With 1140 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1140-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1140 tiles:
...
calculators across 184 `lib` modules** (of 1140 catalog tiles), built module by <!-- catalog-truth:historical (184 is the count of lib modules adapted, not a catalog tile count) -->
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1140 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1140/1140 tiles** carry a committed per-tile audit log
