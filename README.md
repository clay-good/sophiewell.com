...
<p align="center">
  <strong>876 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v201 close the catalog is 876
...
reviewable spec at a time to **876** deterministic calculators
...
### Deep Subspecialty Quantitation program (spec-v199–v203, +17 so far → 876)
...
```
  ...
  $876/day (LRD).            duplication / MSP — never      charge. In-network →       billing PROHIBITED.
...
→ **$876** + **$174** (the second discounted **50%**), a packaged status-N line **$0**,
total **$1,044**.
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 876 | `scripts/build-tool-pages.mjs` |
...
`npm run test` adds the 5,876-test unit suite, the a11y check, and dataset
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (876 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (876 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 876

With 876 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 876-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 876 tiles:
...
calculators across 94 `lib` modules** (of 876 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 876 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 876/876 tiles** carry a committed per-tile audit log
