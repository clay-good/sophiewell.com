...
<p align="center">
  <strong>882 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v201 close the catalog is 882
...
reviewable spec at a time to **882** deterministic calculators
...
### Deep Subspecialty Quantitation program (spec-v199–v203, +17 so far → 882)
...
```
  ...
  $882/day (LRD).            duplication / MSP — never      charge. In-network →       billing PROHIBITED.
...
→ **$882** + **$174** (the second discounted **50%**), a packaged status-N line **$0**,
total **$1,044**.
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 882 | `scripts/build-tool-pages.mjs` |
...
`npm run test` adds the 5,882-test unit suite, the a11y check, and dataset
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (882 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (882 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 882

With 882 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 882-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 882 tiles:
...
```
 ...
 ┌──────────────┐   lib/pa/rules.js → 882 rules, each a pure check(bundle).
...
never an edit to an existing rule, so the 882-rule set grows without
...
calculators across 94 `lib` modules** (of 882 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 882 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 882/882 tiles** carry a committed per-tile audit log
...
- [docs/spec-v52.md](docs/spec-v52.md) — the `pa-lint` prior-auth packet
  linter: pipeline, the 882-rule ruleset, payer overlays (Aetna +
