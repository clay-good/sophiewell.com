...
<p align="center">
  <strong>877 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v201 close the catalog is 877
...
reviewable spec at a time to **877** deterministic calculators
...
### Deep Subspecialty Quantitation program (spec-v199–v203, +17 so far → 877)
...
```
  ...
  $877/day (LRD).            duplication / MSP — never      charge. In-network →       billing PROHIBITED.
...
→ **$877** + **$174** (the second discounted **50%**), a packaged status-N line **$0**,
total **$1,044**.
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 877 | `scripts/build-tool-pages.mjs` |
...
`npm run test` adds the 5,877-test unit suite, the a11y check, and dataset
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (877 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (877 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 877

With 877 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 877-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 877 tiles:
...
```
 ...
 ┌──────────────┐   lib/pa/rules.js → 877 rules, each a pure check(bundle).
...
never an edit to an existing rule, so the 877-rule set grows without
...
calculators across 94 `lib` modules** (of 877 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 877 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 877/877 tiles** carry a committed per-tile audit log
...
- [docs/spec-v52.md](docs/spec-v52.md) — the `pa-lint` prior-auth packet
  linter: pipeline, the 877-rule ruleset, payer overlays (Aetna +
