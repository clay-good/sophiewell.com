...
<p align="center">
  <strong>437 deterministic healthcare calculators tuned to the nurse on shift.</strong><br>
...
and the v29 catalog ledger. At v101 close the catalog is 437
...
reviewable spec at a time to **437** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 437 | `scripts/build-tool-pages.mjs` |
...
| `check-catalog-truth.mjs` | the catalog count (437) is identical across all 13 surfaces; no orphan/removed-tile ids |
...
scroll sweep over every SPA route **and** every one of the 437 pre-rendered
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (437 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (437 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 437

With 437 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 437-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 437 tiles:
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 437 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 437/437 tiles** carry a committed per-tile audit log
