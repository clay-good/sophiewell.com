...
<p align="center">
  <strong>1090 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v252 close the catalog is 1090
...
reviewable spec at a time to **1090** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1090 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1090 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1090 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1090

With 1090 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1090-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1090 tiles:
...
calculators across 94 `lib` modules** (of 1090 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1090 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1090/1090 tiles** carry a committed per-tile audit log
