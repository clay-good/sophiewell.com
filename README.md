...
<p align="center">
  <strong>1007 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v231 close the catalog is 1007
...
reviewable spec at a time to **1007** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1007 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1007 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1007 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1007

With 1007 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1007-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1007 tiles:
...
calculators across 94 `lib` modules** (of 1007 catalog tiles), built module by
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 1007 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1007/1007 tiles** carry a committed per-tile audit log
