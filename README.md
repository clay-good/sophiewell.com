...
<p align="center">
  <strong>1109 deterministic healthcare calculators that run entirely in your browser.</strong><br>
...
and the v29 catalog ledger. At v257 close the catalog is 1109
...
reviewable spec at a time to **1109** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 1109 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (1109 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (1109 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 1109

With 1109 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 1109-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 1109 tiles:
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 1109/1109 tiles** carry a committed per-tile audit log
