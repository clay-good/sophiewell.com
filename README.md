...
<p align="center">
  <strong>635 deterministic healthcare calculators tuned to the nurse on shift.</strong><br>
...
and the v29 catalog ledger. At v141 close the catalog is 635
...
reviewable spec at a time to **635** deterministic calculators
...
| Output | Count | Source |
|--------|------:|--------|
| Pre-rendered tool pages (`dist/tools/<id>/`) | 635 | `scripts/build-tool-pages.mjs` |
...
app.js              router, hero-search wiring, view wiring, the UTILITIES catalog
                    (635 tiles — the single source of truth; zero runtime deps)
...
dist/               build output (635 tool pages, OG cards, sitemap, SBOM)
```

### Discovery: how a query finds the right tool among 635

With 635 tiles, search quality *is* the product — a tool you cannot find does
...
across the full 635-tile catalog, pinning the last three unpinned "current
...
`check-citations.mjs` lint gate (in the `npm run lint` chain) over all 635 tiles:
...
| `npm run test:e2e`       | Build `dist/`, then run Playwright integration tests against real browsers — incl. a full-catalog 320px no-horizontal-scroll sweep over both the SPA routes and the 635 pre-rendered static tool pages, the hub/topic/commitments pages, and the citation-wrap pin |
...
- [docs/spec-v11.md](docs/spec-v11.md) — correctness-floor spec:
  ...
  — 635/635 tiles** carry a committed per-tile audit log
