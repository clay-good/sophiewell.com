...
export default [
  {
    ...
    fields: S.SPADI_ITEMS.map((item) => ({
      // spec-v1075: the form is a 0-10 select, so the integers are the whole of
      // it. Undeclared, the surface took 3.7 and reported "SPADI 43.7".
      dom: `spadi-${item.key}`, arg: item.key, kind: 'number', unit: 'points', required: true,
      values: Array.from({ length: S.ITEM_MAX + 1 }, (_, i) => String(i)),
