...
| `describe_calculator` | `{ id }` | The full contract: `inputSchema` (JSON Schema), a worked `example`, `citation` + `citationUrl` + `citationAccessed` (plus `citationUrls`, one labelled link per paper, when the citation names more than one), the source interpretation bands, `related` calculator ids, and the clinical-posture disclaimer. |
