...
export const TOPICS = {
  ...
  'billing-and-coding': {
    ...
    description: 'Sophie\'s billing and coding calculators: the MPFS reimbursement engine (RVU x GPCI x CF allowed amount, multiple-procedure and bilateral reductions, assistant/co-surgeon pay, 2% sequestration), the claim-edit decision engines (NCCI procedure-to-procedure bypass, MUE units adjudication, 59-vs-X{EPSU} modifier selection, global-surgery period math, modifier ordering), the E/M and time-based coding engines (MDM leveling across every setting, critical-care time, split/shared and FS modifier, prolonged services, therapy 8-minute rule, anesthesia units), the drug and infusion engines (HCPCS billing units, JW/JZ wastage, the 96360-96379 infusion hierarchy), the patient-responsibility engines (Medicare Part A/B/SNF cost-share, coordination of benefits / Medicare Secondary Payer, contractual write-off vs patient balance, No Surprises Act QPA cost-share), the claim-integrity validators (NPI Luhn check digit, MBI position grammar, ICD-10-CM structure & specificity, 835/EOB remittance balancing) and the facility pricers (IPPS DRG, OPPS APC), the time-based E/M code selector, NDC 10/11 converter, and HIPAA breach-notification clock.',
    lede: 'spec-v29 removed the static code-reference lookups (ICD-10, HCPCS, CPT, NDC, POS, modifiers, CARC/RARC, MS-DRG, APC, RxNorm, NUBC) and the form-locator / EOB-glossary references - those belong in your EHR or the upstream CMS / FDA / NUBC / X12 release. What remains are the calculators that compute a deterministic output: the spec-v78 MPFS reimbursement engine (what Medicare actually pays a line, after every reduction), the spec-v79 claim-edit decision engines (will this line deny, and which modifier unlocks it), the spec-v80 E/M and time-based coding engines (how the visit codes in every setting, plus critical-care, prolonged-service, therapy, and anesthesia units), the spec-v81 drug and infusion engines (how many billing units a dose is, when JW/JZ applies, and which infusion is the initial code), the spec-v82 patient-responsibility engines (what the patient actually owes - Medicare cost-share, coordination of benefits, the contractual write-off, and the No Surprises Act cap), the spec-v83 claim-integrity validators (a bad NPI/MBI/ICD-10 code or an out-of-balance 835 caught before the clearinghouse rejects it) and facility pricers (the IPPS DRG and OPPS APC institutional payment the professional fee schedule does not compute), the time-based E/M selector, the NDC 10/11 converter, and the HIPAA breach-notification clock.',
    tiles: [
  ...
  'patient-literacy': {
    ...
    title: 'Patient & Insurance Literacy - Appeal, HIPAA, Wallet Card · Sophie Well',
    ...
    lede: 'spec-v29 narrowed patient-literacy to the generators that assemble a tailored document from user inputs. The static decoders / eligibility infographics were removed (bill / EOB / MSN decoders, NSA eligibility, IDR, ABN, insurance card, COBRA timeline, Medicare enrollment, ACA SEP, EOB jargon glossary). What remains: the appeal letter, the HIPAA Right of Access request, the lab result interpreter, and the patient medication wallet card.',
    tiles: [
