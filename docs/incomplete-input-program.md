...
| `scripts/probe-unguarded-sibling.mjs` | within one library module, does an exported function call a measurement guard that a sibling reading the same kind of number does not? A module is written by one hand at one sitting ([spec-v1202](spec-v1202.md)). Static, so it runs in a second. |
