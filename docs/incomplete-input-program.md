...
| `scripts/probe-unguarded-sibling.mjs` | within one library module, does an exported function call a measurement guard that a sibling reading the same kind of number does not? A module is written by one hand at one sitting ([spec-v1202](spec-v1202.md)). Static, so it runs in a second. Drained to zero at [spec-v1210](spec-v1210.md), **refilled to 14 modules / 22 functions by the envelope waves of spec-v1224–v1236, and triaged back to 11 / 15 at [spec-v1237](spec-v1237.md)** — where it also learned that a direct `BOUNDS.<key>.max` comparison is a guard, having reported three functions that refuse by that route as unguarded. Every remaining row is a function whose numbers the table does not name in that field's unit; four carry the reason in the code. Reach unchanged — 20 modules call a watched guard and 17 are comparable. Its body extraction was wrong until that last wave: it read each function as everything up to the next `export function`, so trailing constants and helpers convicted the function above. |
| `scripts/probe-static-exemption.mjs` | which rows does the asking/disclosing vocabulary exempt on text that did **not change** when the field was dropped? Static prose cannot be a statement about a gap that did not exist when it was written ([spec-v1193](spec-v1193.md)). Ran 17 rows down to 2 over spec-v1193 to [spec-v1196](spec-v1196.md), then to 0 in [spec-v1257](spec-v1257.md) by making the two remaining dynamic refusals actionable. |
...
| `scripts/probe-refusal-unrendered.mjs` | can the compute function return `valid: false`, and would the page **show** it — does the renderer branch at all, and does every refusal carry the key that branch prints? A wrong-key or reachable no-branch row is actionable. Found the `cdai-crohns` regression at [spec-v1212](spec-v1212.md). [Spec-v1249](spec-v1249.md) taught it the alternate partial-score branch (`r.total == null` plus rendering `r.band`), removing 5 handled rows. [Spec-v1260](spec-v1260.md) adds refusal branches to 2 renderers whose non-inline controls could not be proven unreachable and classifies the remaining 179 as populated-select-only states, leaving **0 actionable rows** while retaining the excluded rows in JSON. |
...
| `scripts/probe-impossible-reads-as-absent.mjs` | the other half: does an impossible value produce **exactly the answer you get by omitting the field**? Catches the SUBSTITUTED case the probe above is blind to, without guessing the constant — substitution shapes fall back to whatever a missing field gets. Only asks it where the omission is accepted, which narrows the numeric fields to a few hundred. Splits rows by whether the reading SAYS it dropped the value (`abi` does). **11 silent** after [spec-v1219](spec-v1219.md) taught it the third way a tile discloses — saying nothing and **enumerating less** (`pk-suite` drops three quantities from its band; `modified-marshall` shortens its "assessed:" list). Rows where the reading is IDENTICAL to the normal one are marked separately: nothing moved on screen at all. Three corrections: blank-example rows give no signal, `ASKING` is a refusing vocabulary and not a disclosing one, and the movement rule must compare sentences because word-subtraction destroys the phrase. |
| `scripts/probe-unsupplied-input.mjs` | does a library **read an argument that neither the page nor the agent tool can supply**? The branch behind it decides the answer for everyone who could not say it: `uiss-rcc` gave node-positive disease a localized tier, `ireton-jones` read every blank height as not obese ([spec-v1403](spec-v1403.md)). **2 rows, both accepted alternatives** (a BMI's weight and height; a Spetzler-Martin grade for its class). Reach printed: 1,346 of 1,763 compute functions read directly. |
| `test/integration/one-blank-field-probe.spec.js` | the browser half of the one-field-short question, and it had drifted from the gate it shadows: matched against the WHOLE reading rather than what it ADDED, and carried `ASKING` without `DISCLOSING`, so a tile that says what it is missing (`glasgow-imrie`: "not assessed … 7 of 8 items assessed") was reported as recomputing in silence. Both taught, shard 0 goes 20 rows to 2 ([spec-v1221](spec-v1221.md)). The two left are `addedText` having no sentence boundary to split on when a reading runs values together — a limitation the GATE shares. Excluded from CI unless `RUN_PROBES=1`. |
...
- ~~**A value past a ceiling `lib/bounds.js` already declares still produces an
  ...
  The probe's current reach is 708 fields mapped to one of 36 envelopes; 664 are
  testable against a worked example inside that envelope, none
  is mis-mapped, and 44 carry no usable example. **Zero is a statement about
  those 664**, not about the catalog, and it was a statement about 367 until
  ...
  [spec-v1406](spec-v1406.md) mapped the other twelve envelopes (age, INR, BMI,
  eGFR, BUN, QT, and the electrolytes), fixed 97 fields, and made all three
  sections a unit-test gate with a reach floor.
...
- **A refusal with no way out, in 55 fields.** The probe's second section is a
  ...
  [spec-v1235](spec-v1235.md) the file spec-v1227's own filter skipped, and
  [spec-v1236](spec-v1236.md) the twenty-five whose missing-value message is one
  sentence rather than a `missing` list — labelled from the field registry each
  tile already publishes to agents. **89 rows → 11.**

  What is left: a function with more than one missing-value branch (a mechanical
  pass cannot tell which one guards which read), an arg with no registry label
  (`vasograde`'s `modifiedFisher`), callers whose bound is a named constant
  rather than a literal, and eleven rows whose tiles take a different shape
  again.
