...
| `scripts/probe-impossible-reads-as-absent.mjs` | the other half: does an impossible value produce **exactly the answer you get by omitting the field**? Catches the SUBSTITUTED case the probe above is blind to, without guessing the constant — substitution shapes fall back to whatever a missing field gets. Only asks it where the omission is accepted, which narrows the numeric fields to a few hundred. Splits rows by whether the reading SAYS it dropped the value (`abi` does). **38 silent, untriaged** — see [spec-v1218](spec-v1218.md) for the counts. Three corrections: blank-example rows give no signal, `ASKING` is a refusing vocabulary and not a disclosing one, and the movement rule must compare sentences because word-subtraction destroys the phrase. |
