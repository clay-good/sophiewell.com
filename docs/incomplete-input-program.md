...
| `scripts/probe-unguarded-sibling.mjs` | within one library module, does an exported function call a measurement guard that a sibling reading the same kind of number does not? A module is written by one hand at one sitting ([spec-v1202](spec-v1202.md)). Static, so it runs in a second. Drained from 13 modules / 41 functions to 6 / 6 over spec-v1202 to [spec-v1207](spec-v1207.md); the residue needs a `BOUNDS` entry that does not exist yet. |
