...
opened in wave 52-21 (§4.5.21) as the fifteenth.

#### 4.5.21 Commercial payer overlays — Blue Cross Blue Shield of Tennessee (wave 52-21)

The fifteenth named commercial-payer overlay, and the tenth "Blues plans
by state" overlay after HCSC, Highmark, Florida Blue, BCBSM, Blue Shield
of California, Independence Blue Cross, CareFirst, Blue Cross NC, and
Horizon. **Blue Cross Blue Shield of Tennessee** (BCBST) is the dominant
Blue Cross Blue Shield licensee in **Tennessee** and one of the largest
independent licensees not already routed to the Anthem/Elevance, HCSC,
Highmark, Florida Blue, BCBSM, Blue Shield of California, IBX, CareFirst,
Blue Cross NC, or Horizon buckets. Like the fourteen before it, BCBST is
keyed to its own payer bucket (`'bcbst'`, detected by `lib/pa/payer.js`
and placed before the generic `'commercial'` fall-through, after
`'horizon'`). The bucket matches only definitively-Tennessee anchors —
the plan name and the `bcbst` acronym — so generic `blue cross` / `blue
shield` and other Blues licensees stay in the commercial fall-through,
and an explicit "Medicare Advantage" string still wins the MA bucket
earlier. Each commercial rule self-gates on `bundle.payer === 'bcbst'`
and returns a vacuous pass on every other packet.

Scope discipline is identical to §4.5.7–§4.5.20: the rules check the
**procedural completeness** of a BCBST prior-authorization packet
against the plan's *own published* submission requirements — not
clinical coverage criteria, which are the reviewer's judgement and the
applicable Medical Policy's job. Every rule is anchored to a public BCBST
provider authorizations URL tracked in the staleness ledger (§8.3, source
`bcbst-precert`) and re-verified on the §4.5.6 cadence.

The set mirrors the earlier commercial families so the fifteen overlays
stay structurally parallel and auditable side by side; BCBST-specific
routing names appear where the plan actually uses them — the **Availity
Essentials / BlueAccess** provider portal for submission, the
...
52-21 ships the full planned set of 20 (`R-PA-BCBST-NNN`), structurally
parallel to the Horizon set (§4.5.20).

`R-PA-BCBST-004` mirrors core `R-PA-053` and the Aetna / UHC / Anthem /
Cigna / Humana / HCSC / Highmark / Florida Blue / BCBSM / Blue Shield of
California / Independence Blue Cross / CareFirst / Blue Cross NC / Horizon
-004 rules: it ships without a bundled prior-authorization list and
vacuously passes with a pointer until a later wave bundles the list. With
fifteen commercial overlays shipped — the five largest commercial / MA
...
— Blue Cross Blue Shield of Alabama opened in wave 52-23 (§4.5.23) as the
seventeenth.

#### 4.5.23 Commercial payer overlays — Blue Cross Blue Shield of Alabama (wave 52-23)

The seventeenth named commercial-payer overlay, and the twelfth "Blues
plans by state" overlay after HCSC, Highmark, Florida Blue, BCBSM, Blue
Shield of California, Independence Blue Cross, CareFirst, Blue Cross NC,
Horizon, BCBST, and BCBSMA. **Blue Cross Blue Shield of Alabama**
(BCBSAL) is the dominant Blue Cross Blue Shield licensee in **Alabama**
...
buckets. Like the sixteen before it, BCBSAL is keyed to its own payer
bucket (`'bcbsal'`, detected by `lib/pa/payer.js` and placed before the
generic `'commercial'` fall-through, after `'bcbsma'`). The bucket
matches only definitively-Alabama anchors — the plan name and the
`bcbsal` acronym — so generic `blue cross` / `blue shield` and other
Blues licensees stay in the commercial fall-through, and an explicit
"Medicare Advantage" string still wins the MA bucket earlier. Each
commercial rule self-gates on `bundle.payer === 'bcbsal'` and returns a
vacuous pass on every other packet.

Scope discipline is identical to §4.5.7–§4.5.22: the rules check the
**procedural completeness** of a BCBSAL prior-authorization packet
against the plan's *own published* submission requirements — not
clinical coverage criteria, which are the reviewer's judgement and the
applicable Medical Policy's job. Every rule is anchored to a public
BCBSAL provider URL tracked in the staleness ledger (§8.3, source
`bcbsal-precert`) and re-verified on the §4.5.6 cadence.

The set mirrors the earlier commercial families so the seventeen overlays
stay structurally parallel and auditable side by side; BCBSAL-specific
routing names appear where the plan actually uses them — the
**ProviderAccess portal / Availity** for submission, the
**advanced-imaging utilization-management program** for advanced imaging
and genetic / molecular testing, the plan's **pharmacy management** for
pharmacy / step therapy, **behavioral health** for behavioral health, and
the **Blue Distinction Centers for Transplant** for transplant. Wave
52-23 ships the full planned set of 20 (`R-PA-BCBSAL-NNN`), structurally
parallel to the BCBSMA set (§4.5.22).

`R-PA-BCBSAL-004` mirrors core `R-PA-053` and the Aetna / UHC / Anthem /
Cigna / Humana / HCSC / Highmark / Florida Blue / BCBSM / Blue Shield of
California / Independence Blue Cross / CareFirst / Blue Cross NC / Horizon
/ BCBS Tennessee / BCBS Massachusetts -004 rules: it ships without a
bundled prior-authorization list and vacuously passes with a pointer
until a later wave bundles the list. With seventeen commercial overlays
shipped — the five largest commercial / MA plans plus the twelve largest
independent Blues licensees (HCSC, Highmark, Florida Blue, BCBSM, Blue
Shield of California, Independence Blue Cross, CareFirst, Blue Cross NC,
Horizon, BCBST, BCBSMA, BCBSAL) — Blue Cross Blue Shield of South
Carolina opened in wave 52-24 (§4.5.24) as the eighteenth.

#### 4.5.24 Commercial payer overlays — Blue Cross Blue Shield of South Carolina (wave 52-24)

The eighteenth named commercial-payer overlay, and the thirteenth "Blues
plans by state" overlay after HCSC, Highmark, Florida Blue, BCBSM, Blue
Shield of California, Independence Blue Cross, CareFirst, Blue Cross NC,
Horizon, BCBST, BCBSMA, and BCBSAL. **Blue Cross Blue Shield of South
Carolina** (BCBSSC) is the dominant Blue Cross Blue Shield licensee in
**South Carolina** — and the operator of a large federal-program
administration business — and one of the largest independent licensees
not already routed to an earlier bucket. Like the seventeen before it,
BCBSSC is keyed to its own payer bucket (`'bcbssc'`, detected by
`lib/pa/payer.js` and placed before the generic `'commercial'`
fall-through, after `'bcbsal'`). The bucket matches only
definitively-South-Carolina anchors — the plan name and the `bcbssc`
acronym, which carries no substring collision with the earlier `bcbsm`
(Michigan) bucket — so generic `blue cross` / `blue shield` and other
Blues licensees stay in the commercial fall-through, and an explicit
"Medicare Advantage" string still wins the MA bucket earlier. Each
commercial rule self-gates on `bundle.payer === 'bcbssc'` and returns a
vacuous pass on every other packet.

Scope discipline is identical to §4.5.7–§4.5.23: the rules check the
**procedural completeness** of a BCBSSC prior-authorization packet
against the plan's *own published* submission requirements — not
clinical coverage criteria, which are the reviewer's judgement and the
applicable Medical Policy's job. Every rule is anchored to a public
BCBSSC provider URL tracked in the staleness ledger (§8.3, source
`bcbssc-precert`) and re-verified on the §4.5.6 cadence.

The set mirrors the earlier commercial families so the eighteen overlays
stay structurally parallel and auditable side by side; BCBSSC-specific
routing names appear where the plan actually uses them — the **My
Insurance Manager portal / Availity** for submission, the
**advanced-imaging utilization-management program** for advanced imaging
and genetic / molecular testing, the plan's **pharmacy management** for
pharmacy / step therapy, **behavioral health** for behavioral health, and
the **Blue Distinction Centers for Transplant** for transplant. Wave
52-24 ships the full planned set of 20 (`R-PA-BCBSSC-NNN`), structurally
parallel to the BCBSAL set (§4.5.23).

`R-PA-BCBSSC-004` mirrors core `R-PA-053` and the prior commercial -004
rules: it ships without a bundled prior-authorization list and vacuously
passes with a pointer until a later wave bundles the list. With eighteen
...
KC) — Blue Cross and Blue Shield of Minnesota opened in wave 52-27
(§4.5.27) as the twenty-first.

#### 4.5.27 Commercial payer overlays — Blue Cross and Blue Shield of Minnesota (wave 52-27)

The twenty-first named commercial-payer overlay, and the sixteenth "Blues
plans by state" overlay. **Blue Cross and Blue Shield of Minnesota**
(BCBSMN) is the dominant Blue Cross Blue Shield licensee in **Minnesota**
and one of the largest independent licensees not already routed to an
earlier bucket. Like the twenty before it, BCBSMN is keyed to its own
payer bucket (`'bcbsmn'`, detected by `lib/pa/payer.js` and placed before
the generic `'commercial'` fall-through, after `'bluekc'`). The bucket
matches only the spelled-out plan name and the `blue cross of minnesota`
short form — the bare `bcbsmn` acronym is **deliberately not an anchor**,
because `bcbsm` (the earlier Michigan bucket, §4.5.15) is a substring of
`bcbsmn` and a bare-acronym packet would otherwise route to Michigan; the
...
Each commercial rule self-gates on `bundle.payer === 'bcbsmn'` and returns
a vacuous pass on every other packet.

Scope discipline is identical to §4.5.7–§4.5.26: the rules check the
**procedural completeness** of a BCBSMN prior-authorization packet
against the plan's *own published* submission requirements — not clinical
coverage criteria, which are the reviewer's judgement and the applicable
Medical Policy's job. Every rule is anchored to a public BCBSMN provider
URL tracked in the staleness ledger (§8.3, source `bcbsmn-precert`) and
re-verified on the §4.5.6 cadence.

The set mirrors the earlier commercial families so the twenty-one overlays
stay structurally parallel and auditable side by side; BCBSMN-specific
routing names appear where the plan actually uses them — the **Availity /
provider portal** for submission, the **advanced-imaging
utilization-management program** for advanced imaging and genetic /
molecular testing, the plan's **pharmacy management** for pharmacy / step
therapy, **behavioral health** for behavioral health, and the **Blue
Distinction Centers for Transplant** for transplant. Wave 52-27 ships the
full planned set of 20 (`R-PA-BCBSMN-NNN`), structurally parallel to the
Blue KC set (§4.5.26).

`R-PA-BCBSMN-004` mirrors core `R-PA-053` and the prior commercial -004
rules: it ships without a bundled prior-authorization list and vacuously
passes with a pointer until a later wave bundles the list. With
twenty-one commercial overlays shipped — the five largest commercial / MA
plans plus the sixteen largest independent Blues licensees (HCSC,
...
Arkansas BCBS, Blue KC, BCBSMN) — Blue Cross and Blue Shield of Louisiana
opened in wave 52-28 (§4.5.28) as the twenty-second.

#### 4.5.28 Commercial payer overlays — Blue Cross and Blue Shield of Louisiana (wave 52-28)

The twenty-second named commercial-payer overlay, and the seventeenth
"Blues plans by state" overlay. **Blue Cross and Blue Shield of
Louisiana** (BCBSLA) is the dominant Blue Cross Blue Shield licensee in
**Louisiana** and one of the largest independent licensees not already
routed to an earlier bucket. Like the twenty-one before it, BCBSLA is
keyed to its own payer bucket (`'bcbsla'`, detected by `lib/pa/payer.js`
and placed before the generic `'commercial'` fall-through, after
`'bcbsmn'`). The bucket matches only definitively-Louisiana anchors — the
plan name and the `bcbsla` acronym, which carries no substring collision
with the earlier `bcbsal` (Alabama) or `bcbsm` (Michigan) buckets — so
generic `blue cross` / `blue shield` and other Blues licensees stay in the
commercial fall-through, and an explicit "Medicare Advantage" string still
wins the MA bucket earlier. Each commercial rule self-gates on
`bundle.payer === 'bcbsla'` and returns a vacuous pass on every other
packet.

Scope discipline is identical to §4.5.7–§4.5.27: the rules check the
**procedural completeness** of a BCBSLA prior-authorization packet
against the plan's *own published* submission requirements — not clinical
coverage criteria, which are the reviewer's judgement and the applicable
Medical Policy's job. Every rule is anchored to a public BCBSLA provider
URL tracked in the staleness ledger (§8.3, source `bcbsla-precert`) and
re-verified on the §4.5.6 cadence.

The set mirrors the earlier commercial families so the twenty-two overlays
stay structurally parallel and auditable side by side; BCBSLA-specific
routing names appear where the plan actually uses them — the **iLinkBlue /
Availity** provider portal for submission, the **advanced-imaging
utilization-management program** for advanced imaging and genetic /
molecular testing, the plan's **pharmacy management** for pharmacy / step
therapy, **behavioral health** for behavioral health, and the **Blue
Distinction Centers for Transplant** for transplant. Wave 52-28 ships the
full planned set of 20 (`R-PA-BCBSLA-NNN`), structurally parallel to the
BCBSMN set (§4.5.27).

`R-PA-BCBSLA-004` mirrors core `R-PA-053` and the prior commercial -004
rules: it ships without a bundled prior-authorization list and vacuously
passes with a pointer until a later wave bundles the list. With
twenty-two commercial overlays shipped — the five largest commercial / MA
...
#### 4.5.42 Per-state Medicaid overlays — New Jersey Medicaid (wave 52-42)
...
the brand **NJ FamilyCare** (Medicaid + CHIP); the fiscal-agent provider portal
is **NJMMIS** (the New Jersey Medicaid Management Information System). It is
detected by a per-state bucket (`'medicaid-nj'`, anchors `new jersey medicaid` /
`nj medicaid` / `nj familycare` / `njmmis`) placed before the generic
`'medicaid'` bucket, and composes with the §4.5.4 Medicaid core through
`isMedicaid(bundle.payer)`. It is deliberately distinct from the same-state
commercial Blues licensee `'horizon'` (Horizon Blue Cross Blue Shield of New
Jersey, §4.5.20): a New Jersey Medicaid packet and a Horizon packet route to
different overlays (unit-tested). An explicit "Medicare Advantage" string still
wins the MA bucket earlier.

The 20 New Jersey rules (`R-PA-MCNJ-NNN`) mirror the established families with
the two Medicaid reframings (transplant → Medicaid-designated transplant center;
appeal → state fair hearing) and NJMMIS routing. Each rule self-gates on
`bundle.payer === 'medicaid-nj'` and returns a vacuous pass on every other
packet. Every rule is anchored to a public New Jersey Medicaid provider URL
tracked in the staleness ledger (§8.3, source `nj-medicaid-precert`).
`R-PA-MCNJ-004` mirrors core `R-PA-053`: it ships without a bundled
prior-authorization list and vacuously passes with a pointer until a later wave
bundles the list. With twelve state Medicaid programs shipped (California, New
York, Texas, Florida, Ohio, Illinois, Washington, Georgia, North Carolina,
Pennsylvania, Michigan, New Jersey), the ruleset is **835 rules**; the remaining
...
### Wave 52-21 — Blue Cross Blue Shield of Tennessee commercial overlay (2026-06)

- The 20 BCBST rules (§4.5.21), the `R-PA-BCBST-NNN` family, anchored to
  ...
  `bcbst-precert`).
- A `'bcbst'` payer bucket in `lib/pa/payer.js`, placed before the
  generic `'commercial'` fall-through and after `'horizon'`. It matches
  only definitively-Tennessee anchors (the plan name and the `bcbst`
  acronym); generic Blues and other licensees stay in the commercial
  fall-through, and an explicit "Medicare Advantage" string still wins
  the MA bucket earlier.
- Catalog count unchanged (255 tiles; BCBST adds rules, not a tile).
  Ruleset rises 415 → 435. Brings the named commercial / MA overlays to
  fifteen (Aetna + UnitedHealthcare + Anthem + Cigna + Humana + HCSC +
  Highmark + Florida Blue + BCBSM + Blue Shield of California +
...
### Wave 52-23 — Blue Cross Blue Shield of Alabama commercial overlay (2026-06)

- The 20 BCBSAL rules (§4.5.23), the `R-PA-BCBSAL-NNN` family, anchored
  to the plan's public provider pages, Medical Policies, and
  utilization-management / pharmacy program requirements (ledger source
  `bcbsal-precert`).
- A `'bcbsal'` payer bucket in `lib/pa/payer.js`, placed before the
  generic `'commercial'` fall-through and after `'bcbsma'`. It matches
  only definitively-Alabama anchors (the plan name and the `bcbsal`
  acronym); generic Blues and other licensees stay in the commercial
  fall-through, and an explicit "Medicare Advantage" string still wins
  the MA bucket earlier.
- Catalog count unchanged (255 tiles; BCBSAL adds rules, not a tile).
  Ruleset rises 455 → 475. Brings the named commercial / MA overlays to
  seventeen (Aetna + UnitedHealthcare + Anthem + Cigna + Humana + HCSC +
  Highmark + Florida Blue + BCBSM + Blue Shield of California +
  Independence Blue Cross + CareFirst + Blue Cross NC + Horizon + BCBS
  Tennessee + BCBS Massachusetts + BCBS Alabama), the twelve largest
  independent Blues licensees now all covered.

### Wave 52-24 — Blue Cross Blue Shield of South Carolina commercial overlay (2026-06)

- The 20 BCBSSC rules (§4.5.24), the `R-PA-BCBSSC-NNN` family, anchored
  to the plan's public provider pages, Medical Policies, and
  utilization-management / pharmacy program requirements (ledger source
  `bcbssc-precert`).
- A `'bcbssc'` payer bucket in `lib/pa/payer.js`, placed before the
  generic `'commercial'` fall-through and after `'bcbsal'`. It matches
  only definitively-South-Carolina anchors (the plan name and the
  `bcbssc` acronym, which has no substring collision with the Michigan
  `bcbsm` bucket); generic Blues and other licensees stay in the
  commercial fall-through, and an explicit "Medicare Advantage" string
  still wins the MA bucket earlier.
- Catalog count unchanged (255 tiles; BCBSSC adds rules, not a tile).
  Ruleset rises 475 → 495. Brings the named commercial / MA overlays to
...
### Wave 52-27 — Blue Cross and Blue Shield of Minnesota commercial overlay (2026-06)

- The 20 BCBSMN rules (§4.5.27), the `R-PA-BCBSMN-NNN` family, anchored
  to the plan's public provider pages, Medical Policies, and
  utilization-management / pharmacy program requirements (ledger source
  `bcbsmn-precert`).
- A `'bcbsmn'` payer bucket in `lib/pa/payer.js`, placed before the
  generic `'commercial'` fall-through and after `'bluekc'`. It matches
  only the spelled-out plan name and the `blue cross of minnesota` short
  form; the bare `bcbsmn` acronym is omitted because the earlier Michigan
  `bcbsm` anchor is a substring of it. Generic Blues and other licensees
  stay in the commercial fall-through, and an explicit "Medicare
  Advantage" string still wins the MA bucket earlier.
- Catalog count unchanged (255 tiles; BCBSMN adds rules, not a tile).
  Ruleset rises 535 → 555. Brings the named commercial / MA overlays to
  twenty-one, the sixteen largest independent Blues licensees now all
  covered.

### Wave 52-28 — Blue Cross and Blue Shield of Louisiana commercial overlay (2026-06)

- The 20 BCBSLA rules (§4.5.28), the `R-PA-BCBSLA-NNN` family, anchored
  to the plan's public provider pages, Medical Policies, and
  utilization-management / pharmacy program requirements (ledger source
  `bcbsla-precert`).
- A `'bcbsla'` payer bucket in `lib/pa/payer.js`, placed before the
  generic `'commercial'` fall-through and after `'bcbsmn'`. It matches
  only definitively-Louisiana anchors (the plan name and the `bcbsla`
  acronym, which has no substring collision with the `bcbsal` Alabama or
  `bcbsm` Michigan buckets); generic Blues and other licensees stay in the
  commercial fall-through, and an explicit "Medicare Advantage" string
  still wins the MA bucket earlier.
- Catalog count unchanged (255 tiles; BCBSLA adds rules, not a tile).
  Ruleset rises 555 → 575. Brings the named commercial / MA overlays to
...
### Wave 52-42 — New Jersey Medicaid overlay (2026-06)

- Twelfth per-state Medicaid overlay (§4.5.42), thirty-fifth named-payer
  overlay overall. New per-state bucket `'medicaid-nj'` (anchors `new jersey
  medicaid` / `nj medicaid` / `nj familycare` / `njmmis`), placed before the
  generic `'medicaid'` bucket and composing with the §4.5.4 core via
  `isMedicaid`.
- 20 rules `R-PA-MCNJ-001..020`, each self-gating on `bundle.payer ===
  'medicaid-nj'`, anchored to the DMAHS provider page and the new
  staleness-ledger source `nj-medicaid-precert`. NJMMIS routing; transplant /
  appeal reframed for Medicaid. Disjoint from the same-state Blues bucket
  `'horizon'` (§4.5.20; unit-tested).
- Golden fixture `nj-medicaid-precert` added (42 fixtures total).
- Catalog count unchanged (255 tiles). Ruleset rises 815 → 835. Brings
...
- 2026-06-05 — wave 52-42 (§4.5.42 New Jersey Medicaid — the twelfth per-state
  ...
  familycare` / `njmmis`), placed before the generic `'medicaid'` bucket and
  composing with the §4.5.4 core via `isMedicaid`. Disjoint from the same-state
  Blues bucket `'horizon'` (§4.5.20; unit-tested). 20 rules `R-PA-MCNJ-001..020`
  self-gating on `bundle.payer === 'medicaid-nj'`, anchored to the DMAHS provider
  page (NJ FamilyCare / NJMMIS) and the new staleness-ledger source
  `nj-medicaid-precert`. Golden fixture `nj-medicaid-precert` added (42 fixtures).
  Ruleset 815 → 835. Catalog unchanged (255). View wave banner advanced to 52-42.
...
- 2026-06-05 — wave 52-28 (§4.5.28 Blue Cross and Blue Shield of Louisiana
  ...
  state" overlay). Opens a `'bcbsla'` payer bucket in `lib/pa/payer.js` (placed
  after `'bcbsmn'` and before the generic `'commercial'` fall-through). BCBSLA is
  the dominant Blues licensee in Louisiana; the bucket matches only
  definitively-Louisiana anchors (the plan name and the `bcbsla` acronym, which
  has no substring collision with the `bcbsal` Alabama or `bcbsm` Michigan
  buckets), so generic `blue cross` / `blue shield` and other licensees stay in
  the commercial fall-through, and an explicit "Medicare Advantage" string still
  wins the MA bucket earlier. The 20 rules mirror the prior twenty-one commercial
  families so the twenty-two overlays stay structurally parallel, with
  BCBSLA-specific routing names where the plan uses them (the iLinkBlue / Availity
  submission channel, the advanced-imaging utilization-management program,
  ...
  'bcbsla'` and vacuously passes on every other packet. New ledger source
  `bcbsla-precert` anchored to the plan's public provider page (all twenty rules
  map to it by prefix). Coverage is now 575 rules shipped (was 555), 527
  source-anchored (was 507), 37 sources (was 36), 0 orphans, 0 gaps. The golden
  fixtures re-seed deterministically (a new `bcbsla-precert` fixture exercises the
  on-bucket path — 009 flag, 003 info; the other twenty-eight gain +20
  vacuous-pass findings each). Tests: +9 engine assertions (count 575, the
  off-bucket loop, and fire/pass checks) and +1 classify assertion. Catalog count
  unchanged (255). View wave banner advanced to 52-28.
- 2026-06-05 — wave 52-27 (§4.5.27 Blue Cross and Blue Shield of Minnesota
  commercial overlay, the full 20-rule `R-PA-BCBSMN-NNN` family — the twenty-first
  named commercial overlay and the sixteenth "Blues plans by state" overlay).
  Opens a `'bcbsmn'` payer bucket in `lib/pa/payer.js` (placed after `'bluekc'`
  and before the generic `'commercial'` fall-through). BCBSMN is the dominant
  Blues licensee in Minnesota; the bucket matches only the spelled-out plan name
  and the `blue cross of minnesota` short form. The bare `bcbsmn` acronym is
  deliberately omitted because the earlier Michigan bucket's `bcbsm` anchor
  (§4.5.15) is a substring of `bcbsmn`, so a bare-acronym packet would route to
  Michigan first; the spelled-out name has no such collision (a classify test
  asserts Michigan still routes to `bcbsm`). Generic `blue cross` / `blue shield`
  and other licensees stay in the commercial fall-through, and an explicit
  "Medicare Advantage" string still wins the MA bucket earlier. The 20 rules
  mirror the prior twenty commercial families so the twenty-one overlays stay
  structurally parallel, with BCBSMN-specific routing names where the plan uses
  them (the Availity / provider portal submission channel, the advanced-imaging
  ...
  self-gates on `bundle.payer === 'bcbsmn'` and vacuously passes on every other
  packet. New ledger source `bcbsmn-precert` anchored to the plan's public
  provider page (all twenty rules map to it by prefix). Coverage is now 555 rules
  shipped (was 535), 507 source-anchored (was 487), 36 sources (was 35), 0
  orphans, 0 gaps. The golden fixtures re-seed deterministically (a new
  `bcbsmn-precert` fixture exercises the on-bucket path — 009 flag, 003 info; the
  other twenty-seven gain +20 vacuous-pass findings each). Tests: +9 engine
  assertions (count 555, the off-bucket loop, and fire/pass checks) and +1
...
- 2026-06-05 — wave 52-24 (§4.5.24 Blue Cross Blue Shield of South Carolina
  ...
  Opens a `'bcbssc'` payer bucket in `lib/pa/payer.js` (placed after `'bcbsal'`
  and before the generic `'commercial'` fall-through). BCBSSC is the dominant
  Blues licensee in South Carolina; the bucket matches only
  definitively-South-Carolina anchors (the plan name and the `bcbssc` acronym,
  which has no substring collision with the Michigan `bcbsm` bucket), so generic
  ...
  bucket earlier. The 20 rules mirror the prior seventeen commercial families so
  the eighteen overlays stay structurally parallel, with BCBSSC-specific routing
  names where the plan uses them (the My Insurance Manager / Availity submission
  channel, the advanced-imaging utilization-management program, pharmacy
  management for step therapy, behavioral health, and the Blue Distinction
  Centers for Transplant). Each self-gates on `bundle.payer === 'bcbssc'` and
  vacuously passes on every other packet. New ledger source `bcbssc-precert`
  anchored to the plan's public provider page (all twenty rules map to it by
  prefix). Coverage is now 495 rules shipped (was 475), 447 source-anchored (was
  427), 33 sources (was 32), 0 orphans, 0 gaps. The golden fixtures re-seed
  deterministically (a new `bcbssc-precert` fixture exercises the on-bucket path —
  009 flag, 003 info; the other twenty-four gain +20 vacuous-pass findings each).
  Tests: +9 engine assertions (count 495, the off-bucket loop, and fire/pass
  checks) and +1 classify assertion. Catalog count unchanged (255). View wave
  banner advanced to 52-24.
- 2026-06-05 — wave 52-23 (§4.5.23 Blue Cross Blue Shield of Alabama commercial
  overlay, the full 20-rule `R-PA-BCBSAL-NNN` family — the seventeenth named
  commercial overlay after Aetna, UnitedHealthcare, Anthem, Cigna, Humana, HCSC,
  Highmark, Florida Blue, BCBSM, Blue Shield of California, Independence Blue
  Cross, CareFirst, Blue Cross NC, Horizon, BCBST, and BCBSMA, and the twelfth
  "Blues plans by state" overlay). Opens a `'bcbsal'` payer bucket in
  `lib/pa/payer.js` (placed after `'bcbsma'` and before the generic `'commercial'`
  fall-through). BCBSAL is the dominant Blues licensee in Alabama; the bucket
  matches only definitively-Alabama anchors (the plan name and the `bcbsal`
  acronym), so generic `blue cross` / `blue shield` and other licensees stay in
  the commercial fall-through, and an explicit "Medicare Advantage" string still
  wins the MA bucket earlier. The 20 rules mirror the prior sixteen commercial
  families so the seventeen overlays stay structurally parallel, with
  ...
  Distinction Centers for Transplant). Each self-gates on `bundle.payer ===
  'bcbsal'` and vacuously passes on every other packet. New ledger source
  `bcbsal-precert` anchored to the plan's public provider page (all twenty rules
  map to it by prefix). Coverage is now 475 rules shipped (was 455), 427
  source-anchored (was 407), 32 sources (was 31), 0 orphans, 0 gaps. The golden
  fixtures re-seed deterministically (a new `bcbsal-precert` fixture exercises the
  on-bucket path — 009 flag, 003 info; the other twenty-three gain +20
  vacuous-pass findings each). Tests: +9 engine assertions (count 475, the
  off-bucket loop, and fire/pass checks) and +1 classify assertion (BCBSAL →
  `bcbsal`; generic Blues → `commercial`; BCBSAL Medicare Advantage → the MA
  bucket). Catalog count unchanged (255). View wave banner advanced to 52-23.
...
- 2026-06-05 — wave 52-21 (§4.5.21 Blue Cross Blue Shield of Tennessee commercial
  ...
  commercial overlay after Aetna, UnitedHealthcare, Anthem, Cigna, Humana, HCSC,
  Highmark, Florida Blue, BCBSM, Blue Shield of California, Independence Blue
  Cross, CareFirst, Blue Cross NC, and Horizon, and the tenth "Blues plans by
  state" overlay). Opens a `'bcbst'` payer bucket in `lib/pa/payer.js` (placed
  after `'horizon'` and before the generic `'commercial'` fall-through). BCBST is
  the dominant Blues licensee in Tennessee; the bucket matches only
  ...
  fall-through, and an explicit "Medicare Advantage" string still wins the MA
  bucket earlier. The 20 rules mirror the prior fourteen commercial families so
  the fifteen overlays stay structurally parallel, with BCBST-specific routing
  names where the plan uses them (the Availity Essentials / BlueAccess submission
  channel, the advanced-imaging utilization-management program, pharmacy
  management for step therapy, behavioral health, and the Blue Distinction
  Centers for Transplant). Each self-gates on `bundle.payer === 'bcbst'` and
  vacuously passes on every other packet. New ledger source `bcbst-precert`
  anchored to the plan's public provider authorizations page (all twenty rules
  map to it by prefix). Coverage is now 435 rules shipped (was 415), 387
  source-anchored (was 367), 30 sources (was 29), 0 orphans, 0 gaps. The golden
  fixtures re-seed deterministically (a new `bcbst-precert` fixture exercises the
  on-bucket path — 009 flag, 003 info; the other twenty-one gain +20
  vacuous-pass findings each). Tests: +9 engine assertions (count 435, the
  off-bucket loop, and fire/pass checks) and +1 classify assertion (BCBST →
  `bcbst`; generic Blues → `commercial`; BCBST Medicare Advantage → the MA
  bucket). Catalog count unchanged (255). View wave banner advanced to 52-21.
